...
!  Parameters:
!
!    Input, real ( kind = rk ) X, Y, define the evaluation point X/Y.
!
!    Input, real ( kind = rk ) A, B, the parameters.
!
!    Input, integer N, the highest degree to compute.
!
!    Output, real ( kind = rk ) POLS(N+1), the polynomial values.
!
  implicit none
...
!
!    Input, real ( kind = rk ) Y, the parameter.
!
...
!
!    Input, real ( kind = rk ) XYZ(3), the evaluation point.
!
!    Output, real ( kind = rk ) FVALS(NVALS), the polynomial values.
!
  implicit none
  ...
  real ( kind = rk ) f2s(degree+1,degree+1)
  real ( kind = rk ) f3s(degree+1,degree+1)
  real ( kind = rk ) fvals(((degree+1)*(degree+2)*(degree+3))/6)
  integer i
  integer j
  integer k
  integer m
...
!
!    Output, real ( kind = rk ) R8VEC_SUM, the sum of the entries.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) R(3), the coordinates of a point in the
!    reference tetrahedron.
!
!    Output, real ( kind = rk ) U(3), the coordinates of the point in
...
!  Discussion:
!
!    The reference tetrahedron has its centroid at (0,0,0).
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!  Parameters:
!
!    Output, real ( kind = rk ) V1(3), V2(3), V3(3), V4(3), the vertices.
!
  implicit none
