...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: element_quality
  real ( kind = rk ), allocatable, dimension ( :, : ) :: element_quality_table
  ...
  real ( kind = rk ) value_mean
  real ( kind = rk ) value_min
  real ( kind = rk ) value_var
...
!    that make up the tetrahedrons.
!
!    Output, real ( kind = rk ) TETRA_QUALITY(TETRA_NUM), the quality
...
!
  ...
  integer tetra
  integer tetra_node(tetra_order,tetra_num)
  real ( kind = rk ) tetra_quality(tetra_num)
  real ( kind = rk ) tetra_xyz(dim_num,4)
...
!
  ...
  real ( kind = rk ) r
  real ( kind = rk ) tetra(1:dim_num,4)
