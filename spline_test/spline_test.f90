...
!
  ...
  integer i
  integer left
  real ( kind = rk ) xdata(ndata)
  real ( kind = rk ) xval
  real ( kind = rk ) ydata(dim_num,ndata)
  real ( kind = rk ) yval(dim_num)
  real ( kind = rk ) zdata(ndata)
  real ( kind = rk ) zval(dim_num)
...
!
  ...
  integer order
  integer test
  real ( kind = rk ), dimension(n,test_num) :: x
...
!
  ...
  integer jhi
  character mark
  integer, parameter :: nsample = 4
  real (  kind = rk ), dimension ( ndata ) :: tdata = (/ &
  ...
  do i = 0, ndata
    ...
    do j = 0, jhi
      ...
      tval = ( real ( nsample - j, kind = rk ) * tlo   &
             + real (           j, kind = rk ) * thi ) &
             / real ( nsample,     kind = rk )
...
!
  ...
  real ( kind = rk ) tlo
  real ( kind = rk ) tval
  real (  kind = rk ), dimension (ndata) ::  ydata = (/ &
...
!
  ...
  real ( kind = rk ) t
  real (  kind = rk ), dimension (0:n) :: xcon = (/ 0.0D+00, 0.75D+00, 1.0D+00 /)
  real ( kind = rk ) xval
  real (  kind = rk ), dimension (0:n) :: ycon = (/ 1.0D+00, 0.0D+00,  1.0D+00 /)
  real ( kind = rk ) yval
...
!
  ...
  integer :: nsample = 21
  real ( kind = rk ) x
  real (  kind = rk ), dimension ( 0 : n ) :: y = (/ 1.0D+00, 0.0D+00, 1.0D+00 /)
...
!
  ...
  integer, parameter :: ntab = 21

  real ( kind = rk ) b(1:maxdeg)
  real ( kind = rk ) c(0:maxdeg)
  real ( kind = rk ) d(2:maxdeg)
  real ( kind = rk ) eps
  real ( kind = rk ) error
  integer i
  integer ierror
  ...
  do i = 1, ntab
    xtab(i) = ( real ( ntab - i,     kind = rk ) * ( -1.0D+00 )   &
    ...
    do i = 1, ntab
      ...
      do j = 0, jhi
        ...
        if ( i < ntab ) then
          ...
          xval = ( real ( 3 - j, kind = rk ) * xtab(i)     &
                 + real (     j, kind = rk ) * xtab(i+1) ) &
                 / real ( 3,     kind = rk )
        ...
        ytrue = real ( int ( exp ( xval ) * 100.0D+00 + 0.5D+00 ), kind = rk ) &
          / 100.0D+00
...
!
  ...
  integer, parameter :: point_num = 21
  ...
  do i = 1, point_num
    ...
    f(i) = real ( int ( exp ( x(i) ) * 100.0D+00 + 0.5D+00 ), kind = rk ) &
      / 100.0D+00
  end do
...
!
  ...
  do i = 1, ndata
    tdata(i) = real ( i - 1, kind = rk )
    ydata(i) = sin ( 2.0D+00 * pi * tdata(i) / real ( ndata - 1, kind = rk ) )
  end do
...
!
  ...
  integer, parameter :: dim_num = 1
  integer, parameter :: interval_num = 3

  real ( kind = rk ) data_val(dim_num,3*interval_num+1)
  real ( kind = rk ) dxdt
  integer interval
  integer j
  character mark
  integer point
  integer point_num
  real ( kind = rk ), allocatable, dimension ( : ) :: point_t
  real ( kind = rk ), allocatable, dimension ( :, : ) :: point_val
  real ( kind = rk ) t
  real ( kind = rk ) t_max
  real ( kind = rk ) t_min
  real ( kind = rk ) x
  real ( kind = rk ), parameter :: x_max = 2.0D+00 * 3.141592653589793D+00
  real ( kind = rk ), parameter :: x_min = 0.0D+00
  real ( kind = rk ) xdata(0:interval_num)
  real ( kind = rk ) ydata(0:interval_num)
  real ( kind = rk ) ypdata(0:interval_num)
...
!
  do interval = 0, interval_num
    ...
    x = ( real ( interval_num - interval, kind = rk ) * x_min   &
        + real (                interval, kind = rk ) * x_max ) &
        / real ( interval_num,            kind = rk )
...
!
  dxdt = ( x_max - x_min ) / real ( interval_num, kind = rk )
  j = 0
  ...
  t_min = 0.0D+00
  t_max = real ( interval_num, kind = rk )

  do point = 1, point_num

    t = ( real ( point_num - point,     kind = rk ) * t_min   &
        + real (             point - 1, kind = rk ) * t_max ) &
        / real ( point_num         - 1, kind = rk )
  ...
  do point = 1, point_num
    ...
    x = ( real ( point_num - point,     kind = rk ) * x_min   &
        + real (             point - 1, kind = rk ) * x_max ) &
        / real ( point_num         - 1, kind = rk )
...
!
  ...
  real ( kind = rk ), parameter :: x_max = 2.0D+00 * 3.141592653589793D+00
  real ( kind = rk ), parameter :: x_min = 0.0D+00
  real ( kind = rk ) xdata(0:3*interval_num)
  real ( kind = rk ) ydata(0:3*interval_num)
  real ( kind = rk ) y0
  real ( kind = rk ) y1
  real ( kind = rk ) y2
  real ( kind = rk ) y3
...
!
  do interval = 0, 3 * interval_num
    ...
    x = ( real ( 3 * interval_num - interval, kind = rk ) * x_min   &
        + real (                    interval, kind = rk ) * x_max ) &
        / real ( 3 * interval_num,            kind = rk )
...
!
  ...
  real ( kind = rk ) frunge
  real ( kind = rk ) fprunge
  real ( kind = rk ) fpprunge
  integer i
  integer ibcbeg
  integer ibcend
  integer j
    ...
    do i = 0, n
      ...
      do j = 1, jhi
        ...
        if ( i == 0 ) then
          ...
          tval = ( real ( jhi - j + 1, kind = rk ) * t(i)     &
                 + real (       j - 1, kind = rk ) * t(i+1) ) &
                 / real ( jhi,         kind = rk )
        else
          if ( j == 1 ) then
            tval = t(n)
...
!
  ...
  do i = 1, n
    t(i) = real ( i - 1, kind = rk ) / real ( n - 1, kind = rk )
    y(i) =  fcube ( t(i) )
  end do
...
!
  do i = 1, ndata
    tdata(i) = ( real ( ndata - i,     kind = rk ) *   0.0D+00          &
               + real (         i - 1, kind = rk ) * ( 0.5D+00 * pi ) ) &
               / real ( ndata     - 1, kind = rk )
    ydata(i) = sin ( tdata(i) )
...
!
  ...
  do i = 1, ndata
    ...
    do j = 0, jhi
      ...
      tval = real ( 3 * ( i - 1 ) + j, kind = rk ) * ( 0.5D+00 * pi ) &
           / real ( 3 * ( ndata - 1 ), kind = rk )
...
!
  ...
  real (  kind = rk ), dimension ( n ) :: t = (/ 2.0D+00, 4.5D+00, 7.5D+00 /)
  real (  kind = rk ), dimension ( n ) :: y = (/ 3.0D+00, 3.75D+00, 5.5D+00 /)
