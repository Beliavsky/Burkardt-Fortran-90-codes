...
!
!    Output, real ( kind = rk ) A(N,N), the R8PO matrix.
!
  implicit none
...
!    N must be positive.
!
!    Input, real ( kind = rk ) A(N,N), B(N,N), the R8PO factor matrices.
!
!    Output, real ( kind = rk ) C(N,N), the R8PO product matrix.
!
  implicit none
...
!    Input, integer N, the order of the matrix.
!
!    Input, real ( kind = rk ) R(N,N), the Cholesky factor from R8PO_FA.
!
