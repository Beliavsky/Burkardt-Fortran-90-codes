...
!  Parameters:
!
!    Input, integer DIM_NUM, the dimension of the sequence to 
!    be generated.
!
...
!    to be handled. 
!
!    Local, integer DIM_MAX, the maximum dimension that will 
!    be used.
!
!    Local, integer MAXE; we need DIM_MAX irreducible polynomials 
!    over Z2.  MAXE is the highest degree among these.
!
!    Local, integer MAXV, the maximum possible index used in V.
!
!    Local, integer NBITS, the number of bits (not counting 
...
!
!    Input, integer PX(-1:MAXDEG), the appropriate irreducible 
...
!
!    Input/output, integer B(-1:MAXDEG).  On input, B is the 
...
!    to be handled. 
!
!    Local, integer NONZER, shows where the user must put an 
...
!    be used.
!
!    Local, integer NBITS, the number of bits (not counting the 
!    sign) in a fixed-point integer.
!
!    Local, real ( kind = rk ) RECIP, the multiplier which changes the
...
!
!    Local, integer NEXTQ(DIM_MAX), the numerators of the next item
...
!
  ...
  integer r
  real ( kind = rk ), parameter :: recip = 2.0D+00**(-nbits)
  integer seed
...
!  quasi-random vector.
!
  quasi(1:dim_save) = real ( nextq(1:dim_save), kind = rk ) * recip
!
