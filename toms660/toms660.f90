...
!  Parameters:
!
!    Input, real ( kind = rk ) PX, PY, the (X,Y) coordinates of the point P
!    whose nearest unmarked neighbor is to be found.
!
!    Input, real ( kind = rk ) X(N), Y(N), the coordinates of the nodes at which
...
!
!    Input, real ( kind = rk ) XMIN, YMIN, DX, DY, the minimum nodal X, Y
...
!
!    Output, real ( kind = rk ) DSQ, if NP is nonzero, then DSQ is the 
...
!        S = SQRT ( 1 - C*C ).
!
!    Output, real ( kind = rk ) C, S, the components of the Givens
...
!  Parameters:
!
!    Input, real ( kind = rk ) PX, PY, the coordinates of the point at which the
...
!
!    Input, real ( kind = rk ) RSQ(N), the squared radii which enter into the
!    weights defining the interpolant Q.  Computed by QSHEP2.
!
!    Input, real ( kind = rk ) A(5,N), the coefficients for the nodal functions 
!    defining the interpolant Q.  Computed by QSHEP2.
!
!    Output, real ( kind = rk ) Q, QX, QY, the value of the interpolant, and
...
!    data has been supplied.
!
!    Input, real ( kind = rk ) F(N), the data values.
!
...
!    negatives ).
!
!    Output, real ( kind = rk ) XMIN, YMIN, DX, DY, the minimum nodal X, Y
!    coordinates, and the X, Y dimensions of a cell.
!
!    Output, real ( kind = rk ) RMAX, the square root of the largest element
!    in RSQ, the maximum radius of influence.
!
!    Output, real ( kind = rk ) RSQ(N), the squared radii which enter into 
!    the weights defining the interpolant Q.
!
!    Output, real ( kind = rk ) A(5,N), the coefficients for the nodal functions 
...
!  Parameters:
!
!    Input, real ( kind = rk ) PX, PY, the (X,Y) coordinates of the point P at
...
!    defining the interpolant Q.  Computed by QSHEP2.
!
!    Output, real ( kind = rk ) QS2VAL, the interpolated function value
!    at (PX,PY).
!
  implicit none
...
!
!    Input, real ( kind = rk ) C, S, the cosine and sine entries of the Givens
!    rotation matrix.  These may be determined by subroutine GIVENS.
!
!    Input/output, real ( kind = rk ) X(N), Y(N), the rotated vectors. 
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) XK, YK, FK, the coordinates and value of the data
!    at data node K.
!
!    Input, real ( kind = rk ) XI, YI, FI, the coorindates and value of the data
!    at data node I.
!
!    Input, real ( kind = rk ) S1, S2, reciprocals of the scale factors.
!
!    Input, real ( kind = rk ) R, the maximum radius of influence about node K.
!
!    Output, real ( kind = rk ) ROW(6), a row of the augmented regression matrix.
!
  implicit none
...
!
!    Input, real ( kind = rk ) X(N), Y(N), the coordinates of the data nodes.
!
...
!      LNEXT(5) = 5.
!
!    Output, real ( kind = rk ) XMIN, YMIN, the X, Y coordinates of the lower
!    left corner of the rectangle defined by the data nodes.  The upper right 
!    corner is ( XMAX, YMAX ), where
!      XMAX = XMIN + NR * DX,
!      YMAX = YMIN + NR * DY.
!
!    Output, real ( kind = rk ) DX, DY, the X and Y dimensions of the 
!    individual cells.
!      DX = ( XMAX - XMIN ) / NR
!      DY = ( YMAX - YMIN ) / NR,
...
!
  dx = ( xmax - xmin ) / real ( nr, kind = rk )
  dy = ( ymax - ymin ) / real ( nr, kind = rk )
!
!  Test for zero area.
!
