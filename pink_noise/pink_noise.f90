...
!
!    Input/output, integer Q, a counter which is decremented 
!    on every call.
...
!    samples.
!
!    Input, real ( kind = rk ) X(0:N-1), the signal samples.
!
!    Input, integer M, the maximum lag to consider.
!    0 <= M < N.
!
!    Output, real ( kind = rk ) R(0:M), the sample correlations.
!
  implicit none
  ...
  xbar = sum ( x(0:n-1) ) / real ( n, kind = rk )
...
!    samples.
!
!    Input, real ( kind = rk ) X(0:N-1), Y(0:N-1), the signal samples.
!
!    Input, integer M, the maximum lag to consider.
!    0 <= M < N.
!
...
!
  ...
  xbar = sum ( x(0:n-1) ) / real ( n, kind = rk )
  ybar = sum ( y(0:n-1) ) / real ( n, kind = rk )
...
!
!    Input/output, real ( kind = rk ) U(B), the signals to combine.  It is 
!    expected that each of the initial values of U will be drawn from a 
!    distribution with zero mean.
!
!    Input/output, integer Q(B), a set of counters that determine 
!    when each entry of U is to be updated.
!
!    Output, real ( kind = rk ) RAN1F, the value.
!
  implicit none
...
!  Parameters:
!
!    Input, integer D, the hold period.  D must be at least 1.
!
!    Input/output, real ( kind = rk ) U, a value to be held until Q has 
!    decremented to 0, when Q will be reset to D, and U will be randomly reset.
!
!    Input/output, integer Q, a counter which is decremented by 1 
!    on each call until reaching 0.
!
!    Output, real ( kind = rk ) RANH, the input value of U.
!
  implicit none
...
!
!    Input/output, integer Q, the value to be wrapped.
!
  implicit none
