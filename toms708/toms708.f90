...
!  Author:
!
!    Original Fortran77 version by Alfred Morris.
!
!  Reference:
!
...
!*****************************************************************************80
!
!! apser() yields the incomplete beta ratio i(sub(1-x))(b,a) using a series.
!
!  Discussion:
!
!    The algorithm is used for a <= min(eps,eps*b), b*x <= 1, and x <= 0.5. 
!    It is used when a is very small. 
...
!
  ...
  real c
  real eps
  real, parameter :: g = 0.577215664901533E+00
  real j
  real psi
  real s
  real t
...
!     e1 = 2**(-3/2)
  ...
  do n = 2, num, 2
    ...
    hn = h2 * hn
    a0(n) = 2.0 * r0 * ( 1.0 + h * hn ) / real ( n + 2 )
    np1 = n + 1
    s = s + hn
    a0(np1) = 2.0 * r1 * s / real ( n + 3 )
...
!  Discussion:
!
!    Here ln(gamma(a)) = (a - 0.5)*ln(a) - a + 0.5*ln(2*pi) + del(a).
...
!  Local:
!
!    real E, the value of Log ( 2 * PI ) / 2.
!
  implicit none
...
!*****************************************************************************80
!
!! bpser() uses the power series expansion for evaluating ix(a,b).
!
!  Discussion:
!
!    It is assumed that b <= 1 or b*x <= 0.7.  eps is the tolerance used.
!
!  Author:
!
!    Original Fortran77 version by Alfred Morris.
...
!
   ...
   50 z = a* log ( x ) - u
      ...
      if (apb .gt. 1.0) go to 51
         t = 1.0 + gam1(apb)
         go to 52
   51 u = real ( dble(a0) + dble(b0) - 1.d0 )
      t = (1.0 + gam1(u))/apb
...
!  Parameters:
!
!    Input, real A, B, the parameters of the function.  A and B
...
!
   ...
   70 z = z - u
      ...
      if (apb .gt. 1.0) go to 71
         t = 1.0 + gam1(apb)
         go to 72
   71 u = real ( dble(a0) + dble(b0) - 1.d0 )
      t = (1.0 + gam1(u))/apb
...
!*****************************************************************************80
!
!! r4_erf() evaluates the real error function.
!
!  Discussion:
!
!    This function was originally named "erf(x)", but this conflicts with
...
!
  ...
  if ( ax <= 0.5 ) then
    ...
    bot = ((((((q(1)*ax + q(2))*ax + q(3))*ax + q(4))*ax + q(5))*ax &
                        + q(6))*ax + q(7))*ax + q(8)
    r4_erf = 0.5 + (0.5 - exp(-x*x)*top/bot)
    if ( x < 0.0 ) then
      r4_erf = -r4_erf
    end if
    ...
    top = (((r(1)*t + r(2))*t + r(3))*t + r(4))*t + r(5)
    bot = (((s(1)*t + s(2))*t + s(3))*t + s(4))*t + 1.0
    r4_erf = (c - top/(x2*bot)) / ax
    r4_erf = 0.5 + (0.5 - exp(-x2)*r4_erf)
...
!
  ...
  real w
      ...
      if ( t == 0.0 ) then
        ...
        top = (((((p(7)*t + p(6))*t + p(5))*t + p(4))*t + p(3))*t &
                        + p(2))*t + p(1)
...
!  Discussion:
!
!    The range is for 1 <= a <= 2  and  1 <= b <= 2
!
!  Author:
!
!    Original Fortran77 version by Alfred Morris.
...
!*****************************************************************************80
!
!! ipmpar() provides the integer machine constants.
!
!  Discussion:
!
...
!  Local:
!
!    real ( kind = rk ) DX0, zero of PSI.
!
!    real PIOV4 = pi/4.
!
!    real XMAX1, the smallest positive floating point 
...
!
  ...
  real aug
  real den
  real ( kind = rk ), parameter :: dx0 = 1.461632144968362341262659542325721325d0
  integer i
  integer ipmpar
  integer m
  integer n
