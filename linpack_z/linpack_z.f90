...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) Z, the number whose norm is desired.
!
!    Output, real ( kind = rk8 ) ZABS1, the L1 norm of Z.
!
  implicit none
...
!    Input, complex ( kind = ck8 ) Z, the number whose norm is desired.
!
!    Output, real ( kind = rk8 ) ZABS2, the L2 norm of Z.
!
  implicit none
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) A(LDA,P).  On input, A contains
...
!
!    Workspace, complex ( kind = ck8 ) WORK(P).
!
!    Input/output, integer IPVT(P).  IPVT is not referenced if 
...
!
!    Input, integer JOB, specifies whether column pivoting is to
!    be done.
...
!
    if ( pl <= k .and. k < pu ) then
      do l = k+1, pu
        if ( maxdia < real ( a(l,l), kind = rk8 ) ) then
          maxdia = real ( a(l,l), kind = rk8 )
          maxl = l
        end if
      end do
    end if
...
!
    if ( k /= maxl ) then
      ...
      a(maxl,maxl) = a(k,k)
      a(k,k) = cmplx ( maxdia, 0.0D+00, kind = ck8 )
...
!
    work(k) = cmplx ( sqrt ( real ( a(k,k), kind = rk8 ) ), 0.0D+00, kind = ck8 )
    a(k,k) = work(k)
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) R(LDR,P); on input, the upper
...
!    Input, integer P, the order of the matrix.
!
!    Input, complex ( kind = ck8 ) X(P), the row vector that is to
!    be removed from R.
!
!    Input/output, complex ( kind = ck8 ) Z(LDZ,NZ); on input, an array of NZ
...
!
!    Input, complex ( kind = ck8 ) Y(NZ), the scalars for the downdating
!    of the vectors Z.
!
!    Input/output, real ( kind = rk8 ) RHO(NZ).  On input, the norms of
!    the residual vectors that are to be downdated.  On output,
!    the downdated norms.
!
!    Output, real ( kind = rk8 ) C(P), the cosines of the transforming rotations.
!
!    Output, complex ( kind = ck8 ) S(P), the sines of the transforming rotations.
!
!    Output, integer INFO:
...
!
  ...
  complex ( kind = ck8 ) zdotc
  complex ( kind = ck8 ) zeta
!
...
!
  do ii = 1, p
    ...
    a = alpha / scale
    b = s(i) / scale
    norm = sqrt ( a**2 + ( real ( b, kind = rk8 ) )**2 + ( aimag ( b ) )**2 )
    c(i) = a / norm
    s(i) = conjg ( b ) / norm
    alpha = scale * norm
  end do
...
!
  do j = 1, p
    xx = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
    do ii = 1, j
      i = j - ii + 1
      t = c(i) * xx + s(i) * r(i,j)
...
!
!    Input, integer LDR, the leading dimension of R, which is 
!    at least P.
!
!    Input, integer P, the order of the matrix.
!
...
!
    ...
    do jj = k, l - 1
      j = l - 1 - jj + k
      r(1:j,j+1) = r(1:j,j)
      r(j+1,j+1) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
    end do
...
!
    ...
    do i = k+1, l
      r(i,l) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
    end do
!
!  Reduction loop.
!
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) R(LDR,P), the upper triangular matrix
...
!    Input, integer P, the order of the matrix.
!
!    Input, complex ( kind = ck8 ) X(P), the row to be added to R.
!
!    Input/output, complex ( kind = ck8 ) Z(LDZ,NZ), NZ P-vectors to
!    be updated with R.
!
...
!
!    Input, complex ( kind = ck8 ) Y(NZ), the scalars for updating the vectors Z.
!
!    Input/output, real ( kind = rk8 ) RHO(NZ); on input, the norms of
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) ABD(LDA,N), on input, contains the
...
!
!    Input, integer ML, the number of diagonals below the 
!    main diagonal.  0 <= ML < N.
!
!    Input, integer MU, the number of diagonals above the main 
!    diagonal.  0 <= MU < N.
!    More efficient if ML <= MU.
!
!    Output, integer IPVT(N), the pivot indices.
!
!    Output, real ( kind = rk8 ) RCOND, an estimate of the reciprocal condition
...
!
  ...
  complex ( kind = ck8 ) abd(lda,n)
  real ( kind = rk8 ) anorm
  real ( kind = rk8 ) dzasum
  complex ( kind = ck8 ) ek
  integer info
  integer ipvt(n)
  integer is
  ...
  complex ( kind = ck8 ) t
  complex ( kind = ck8 ) wk
  complex ( kind = ck8 ) wkm
  real ( kind = rk8 ) ynorm
  complex ( kind = ck8 ) z(n)
  real ( kind = rk8 ) zabs1
  complex ( kind = ck8 ) zdotc
  complex ( kind = ck8 ) zsign1
!
!  Compute 1-norm of A.
!
  anorm = 0.0D+00
...
!
  ek = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 )

  do j = 1, n
    z(j) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
  end do
  ...
  do k = 1, n
    ...
    if ( zabs1 ( abd(m,k) ) < zabs1 ( ek - z(k) ) ) then
      ...
      call zdscal ( n, s, z, 1 )
      ek = cmplx ( s, 0.0D+00, kind = ck8 ) * ek
    end if
    ...
    if ( zabs1 ( abd(m,k) ) /= 0.0D+00 ) then
      ...
      wk = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 )
      wkm = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 )
    end if
...
!
  do k = n, 1, -1
    ...
    if ( zabs1 ( abd(m,k) ) /= 0.0D+00 ) then
      ...
      z(k) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 )
    end if
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) ABD(LDA,N), the output from ZGBCO or ZGBFA.
!
!    Input, integer LDA, the leading dimension of A.
!
!    Input, integer N, the order of the matrix.
...
!    main diagonal.
!
!    Input, integer MU, the number of diagonals above the 
!    main diagonal.
!
!    Input, integer IPVT(N), the pivot vector from ZGBCO or ZGBFA.
!
!    Output, complex ( kind = ck8 ) DET(2), determinant of original matrix.
...
!
  ...
  do i = 1, n
    ...
    do while ( zabs1 ( det(1) ) < 1.0D+00 )
      det(1) = det(1) * cmplx ( 10.0D+00, 0.0D+00, kind = ck8 )
...
!
  ...
  do jz = j0, j1
    i0 = m + 1 - jz
    do i = i0, ml
      abd(i,jz) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
    end do
  end do
...
!
    jz = jz + 1
    if ( jz <= n ) then
      abd(1:ml,jz) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
    end if
!
!  Find L = pivot index.
!
...
!  Compute multipliers.
!
    t = - cmplx ( 1.0D+00, 0.0D+00, kind = ck8 ) / abd(m,k)
...
!    Input, integer IPVT(N), the pivot vector from ZGBCO or ZGBFA.
!
!    Input/output, complex ( kind = ck8 ) B(N).  On input, the right hand side.
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 )A(LDA,N), on input, the matrix to be
...
!    Output, integer IPVT(N), the pivot indices.
!
!    Output, real ( kind = rk8 ) RCOND, an estimate of the reciprocal
...
!
  ek = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 )
  z(1:n) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) A(LDA,N); on input, the factor
...
!
!    Output, complex ( kind = ck8 ) DET(2), the determinant of the original
...
!
  if ( mod ( job, 10 ) /= 0 ) then
    ...
    do k = 1, n
      ...
      a(k,k) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 ) / a(k,k)
      t = -a(k,k)
      call zscal ( k-1, t, a(1,k), 1 )

      do j = k+1, n
        t = a(k,j)
        a(k,j) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
...
!
    do k = n-1, 1, -1
      ...
      do i = k+1, n
        work(i) = a(i,k)
        a(i,k) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
      end do
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) A(LDA,N), the factored matrix information,
...
!    Input, integer N, the order of the matrix.
!
!    Input/output, complex ( kind = ck8 ) C(N); on input, the subdiagonal of the
!    tridiagonal matrix in entries C(2:N).  On output, C has been overwritten.
!
!    Input/output, complex ( kind = ck8 ) D(N); on input, the diagonal of the
!    tridiagonal matrix.  On output, D has been overwritten.
!
!    Input/output, complex ( kind = ck8 ) E(N); on input, the superdiagonal of
...
!
  ...
  if ( 1 <= n-1 ) then
    ...
    d(1) = e(1)
    e(1) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
    e(n) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
    ...
    do k = 1, n-1
      ...
      c(k+1) = d(k+1) + t * d(k)
      d(k+1) = e(k+1) + t * e(k)
      e(k+1) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
      b(k+1) = b(k+1) + t * b(k)
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) A(LDA,N); on input, the hermitian
...
!    Output, integer IPVT(N), the pivot indices.
!
!    Output, real ( kind = rk8 ) RCOND, an estimate of the reciprocal condition of
...
!
  ...
  complex ( kind = ck8 ) a(lda,n)
  complex ( kind = ck8 ) ak
  complex ( kind = ck8 ) akm1
  real ( kind = rk8 ) anorm
  complex ( kind = ck8 ) bk
  complex ( kind = ck8 ) bkm1
  complex ( kind = ck8 ) denom
  real ( kind = rk8 ) dzasum
  complex ( kind = ck8 ) ek
  integer i
  integer info
...
!
  do j = 1, n
    ...
    z(j) = cmplx ( dzasum ( j, a(1,j), 1 ), 0.0D+00, kind = ck8 )

    do i = 1, j - 1
      z(i) = cmplx ( real ( z(i), kind = rk8 ) + zabs1 ( a(i,j) ), &
        0.0D+00, kind = ck8 )
    end do
  ...
  anorm = 0.0D+00
  do j = 1, n
    anorm = max ( anorm, real ( z(j), kind = rk8 ) )
  end do
!
!  Factor.
!
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) A(LDA,N); on input, the factored matrix
...
!
!    Output, real ( kind = rk8 ) DET(2), the determinant of the original matrix.
...
!
        if ( t == 0.0D+00 ) then
          t = abs ( a(k,k+1) )
          d = ( d / t ) * real ( a(k+1,k+1), kind = rk8 ) - t
        else
          d = t
          t = 0.0D+00
        end if
...
!  1 by 1
!
        a(k,k) = cmplx ( 1.0D+00 / real ( a(k,k), kind = rk8 ), &
          0.0D+00, kind = ck8 )
        ...
        if ( 1 <= km1 ) then
          ...
          a(k,k) = a(k,k) + cmplx ( &
            real ( zdotc ( km1, work, 1, a(1,k), 1 ), kind = rk8 ), &
            0.0D+00, kind = ck8 )
...
!
        t = abs ( a(k,k+1) )
        ak = real ( a(k,k), kind = rk8 ) / t
        akp1 = real ( a(k+1,k+1), kind = rk8 ) / t
        akkp1 = a(k,k+1) / t
        d = t * ( ak * akp1 - 1.0D+00 )
        a(k,k) = cmplx ( akp1 / d, 0.0D+00, kind = ck8 )
        a(k+1,k+1) = cmplx ( ak / d, 0.0D+00, kind = ck8 )
        a(k,k+1) = -akkp1 / d
        ...
        if ( 1 <= km1 ) then
          ...
          a(k+1,k+1) = a(k+1,k+1) + cmplx ( &
            real ( zdotc ( km1, work, 1, a(1,k+1), 1 ), kind = rk8 ), &
            0.0D+00, kind = ck8 )
...
!
  ...
  complex ( kind = ck8 ) a(lda,n)
  real ( kind = rk8 ) absakk
  complex ( kind = ck8 ) ak
  complex ( kind = ck8 ) akm1
  real ( kind = rk8 ) alpha
  complex ( kind = ck8 ) bk
  complex ( kind = ck8 ) bkm1
  real ( kind = rk8 ) colmax
  complex ( kind = ck8 ) denom
  integer imax
  integer info
  integer ipvt(n)
  ...
  integer km2
  integer kstep
  complex ( kind = ck8 ) mulk
  complex ( kind = ck8 ) mulkm1
  real ( kind = rk8 ) rowmax
  logical swap
  complex ( kind = ck8 ) t
  real ( kind = rk8 ) zabs1
!
...
!
      do jj = 1, km1
        ...
        a(j,j) = cmplx ( real ( a(j,j), kind = rk8 ), 0.0D+00, kind = ck8 )
        a(j,k) = mulk
      end do
!
!  Set the pivot array.
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) A(LDA,N), the output from ZHIFA.
!
!    Input, integer LDA, the leading dimension of A.
!
!    Input, integer N, the order of the matrix.
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) AP(N*(N+1)/2); on input, the packed
...
!
  ...
  do j = 1, n
    ...
    z(j) = cmplx ( dzasum ( j, ap(j1), 1 ), 0.0D+00, kind = ck8 )
    ij = j1
    j1 = j1 + j

    do i = 1, j-1
      z(i) = cmplx ( real ( z(i), kind = rk8 ) + zabs1 ( ap(ij) ), &
      0.0D+00, kind = ck8 )
      ij = ij + 1
    end do
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) AP(N*(N+1)/2); on input, the factored
...
!    For example, JOB = 111 gives all three.
!
!    Output, real ( kind = rk8 ) DET(2), if requested, the determinant of
!    the original matrix.  Determinant = DET(1) * 10.0**DET(2) with
!    1.0 <= abs ( DET(1) ) < 10.0 or DET(1) = 0.0.
!
!    Output, integer INERT(3), if requested, the inertia of
!    the original matrix.
!    INERT(1) = number of positive eigenvalues.
!    INERT(2) = number of negative eigenvalues.
!    INERT(3) = number of zero eigenvalues.
...
!
        if ( t == 0.0D+00 ) then
          ...
          kkp1 = ikp1 + k
          t = abs ( ap(kkp1) )
          d = ( d / t ) * real ( ap(kkp1+1), kind = rk8 ) - t
        else
          d = t
          t = 0.0D+00
        end if
...
!
      if ( 0 <= ipvt(k) ) then
        ...
        ap(kk) = cmplx ( 1.0D+00 / real ( ap(kk), kind = rk8 ), &
          0.0D+00, kind = ck8 )
        ...
        if ( 1 <= km1 ) then
          ...
          ap(kk) = ap(kk) + cmplx ( real ( &
            zdotc ( km1, work, 1, ap(ik+1), 1), kind = rk8 ), &
            0.0D+00, kind = ck8 )
...
!
        ...
        t = abs ( ap(kkp1) )
        ak = real ( ap(kk), kind = rk8 ) / t
        akp1 = real ( ap(kkp1+1), kind = rk8 ) / t
        akkp1 = ap(kkp1) / t
        d = t * ( ak * akp1 - 1.0D+00 )
        ap(kk) = cmplx ( akp1 / d, 0.0D+00, kind = ck8 )
        ap(kkp1+1) = cmplx ( ak / d, 0.0D+00, kind = ck8 )
        ap(kkp1) = -akkp1 / d
        ...
        if ( 1 <= km1 ) then
          ...
          ap(kkp1+1) = ap(kkp1+1) + cmplx ( real ( zdotc ( km1, work, 1, &
            ap(ikp1+1), 1 ), kind = rk8 ), 0.0D+00, kind = ck8 )
          ...
          ap(kk) = ap(kk) &
            + cmplx ( &
            real ( zdotc ( km1, work, 1, ap(ik+1), 1 ), kind = rk8 ), &
            0.0D+00, kind = ck8 )
...
!
      ...
      do jj = 1, km1
        ...
        ijj = ij + j
        ap(ijj) = cmplx ( real ( ap(ijj), kind = rk8 ), 0.0D+00, kind = ck8 )
        ap(jk) = mulk
        ij = ij - ( j - 1 )
      end do
!
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) AP(N*(N+1)/2), the output from ZHPFA.
!
!    Input, integer N, the order of the matrix.
!
!    Input, integer IPVT(N), the pivot vector from ZHPFA.
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) ABD(LDA,N); on input, the matrix to be
...
!    Input, integer N, the order of the matrix.
!
!    Input, integer M, the number of diagonals above the 
!    main diagonal.  0 <= M < N.
!
!    Output, real ( kind = rk8 ) RCOND, an estimate of the reciprocal condition
!    of the matrix.  For the system A*X = B, relative perturbations in A and B
...
!
  do j = 1, n
    ...
    l = min ( j, m + 1 )
    mu = max ( m + 2 - j, 1 )
    z(j) = cmplx ( dzasum ( l, abd(mu,j), 1 ), 0.0D+00, kind = ck8 )
    k = j - l

    do i = mu, m
      k = k + 1
      z(k) = cmplx ( real ( z(k), kind = rk8 ) + zabs1 ( abd(i,j) ), &
        0.0D+00, kind = ck8 )
    end do
...
!
  ...
  do k = 1, n
    ...
    if ( real ( abd(m+1,k), kind = rk8 ) < zabs1 ( ek - z(k) ) ) then
      s = real ( abd(m+1,k), kind = rk8 ) / zabs1 ( ek - z(k) )
      call zdscal ( n, s, z, 1 )
      ek = cmplx ( s, 0.0D+00, kind = ck8 ) * ek
    end if
...
!
  do k = n, 1, -1
    ...
    if ( real ( abd(m+1,k), kind = rk8 ) < zabs1 ( z(k) ) ) then
      s = real ( abd(m+1,k), kind = rk8 ) / zabs1 ( z(k) )
      call zdscal ( n, s, z, 1 )
    end if
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) ABD(LDA,N), the output from ZPBCO or ZPBFA.
!
...
!    main diagonal.
!
!    Output, real ( kind = rk8 ) DET(2), the determinant of the original
!    matrix in the form
...
!
  ...
  do i = 1, n
    ...
    det(1) = det(1) * real ( abd(m+1,i), kind = rk8 )**2
...
!
  ...
  do j = 1, n
    ...
    do k = mu, m
      ...
      t = t / abd(m+1,jk)
      abd(k,j) = t
      s = s + real ( t * conjg ( t ), kind = rk8 )
      ik = ik - 1
      jk = jk + 1
    end do

    s = real ( abd(m+1,j), kind = rk8 ) - s
    ...
    abd(m+1,j) = cmplx ( sqrt ( s ), 0.0D+00, kind = ck8 )
...
!
  ...
  do k = 1, n
    ...
    if ( real ( a(k,k), kind = rk8 ) < zabs1 ( ek - z(k) ) ) then
      s = real ( a(k,k), kind = rk8 ) / zabs1 ( ek - z(k) )
      call zdscal ( n, s, z, 1 )
      ek = cmplx ( s, 0.0D+00, kind = ck8 ) * ek
    end if
...
!
  do k = n, 1, -1
    ...
    if ( real ( a(k,k), kind = rk8 ) < zabs1 ( z(k) ) ) then
      s = real ( a(k,k), kind = rk8 ) / zabs1 ( z(k) )
      call zdscal ( n, s, z, 1 )
    end if
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) A(LDA,N); on input, the output A from
...
!    10, determinant only.
!
!    Output, real ( kind = rk8 ) DET(2), if requested, the determinant of A or of
...
!
  if ( ( job / 10 ) /= 0 ) then
    ...
    do i = 1, n
      ...
      det(1) = det(1) * real ( a(i,i), kind = rk8 )**2
...
!
  ...
  do j = 1, n
    ...
    a(j,j) = cmplx ( sqrt ( s ), 0.0D+00, kind = ck8 )
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) A(LDA,N), the output from ZPOCO or ZPOFA.
!
!    Input, integer LDA, the leading dimension of A.
!
!    Input, integer N, the order of the matrix.
...
!
  ...
  anorm = maxval ( real ( z(1:n), kind = rk8 ) )
!
!  Factor.
!
...
!
  ...
  do k = 1, n
    ...
    if ( real ( ap(kk), kind = rk8 ) < zabs1 ( ek - z(k) ) ) then
      s = real ( ap(kk), kind = rk8 ) / zabs1 ( ek - z(k) )
      call zdscal ( n, s, z, 1 )
      ek = cmplx ( s, 0.0D+00, kind = ck8 ) * ek
    end if
...
!
  do k = n, 1, -1
    ...
    if ( real ( ap(kk), kind = rk8 ) < zabs1 ( z(k) ) ) then
      s = real ( ap(kk), kind = rk8 ) / zabs1 ( z(k) )
      call zdscal ( n, s, z, 1 )
    end if
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) A((N*(N+1))/2); on input, the output
...
!    10, determinant only.
!
!    Output, real ( kind = rk8 ) DET(2), the determinant of original matrix
...
!
  if ( ( job / 10 ) /= 0 ) then
    ...
    do i = 1, n
      ...
      ii = ii + i
      det(1) = det(1) * real ( ap(ii), kind = rk8 )**2
...
!
  if ( mod ( job, 10 ) /= 0 ) then
    ...
    do k = 1, n
      ...
      k1 = kk + 1
      kk = kk + k
      ap(kk) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 ) / ap(kk)
      t = -ap(kk)
      ...
      do j = kp1, n
        t = ap(kj)
        ap(kj) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
...
!
  ...
  do j = 1, n
    ...
    ap(jj) = cmplx ( sqrt ( s ), 0.0D+00, kind = ck8 )
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) AP(N*(N+1)/2), the output from ZPPCO or ZPPFA.
!
!    Input, integer N, the order of the matrix.
!
!    Input/output, complex ( kind = ck8 ) B(N).  On input, the right hand side.
...
!
!    Input/output, complex ( kind = ck8 ) E(N).  On input, the superdiagonal
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) X(LDX,P); on input, the matrix whose
...
!
!    Output, complex ( kind = ck8 ) QRAUX(P), further information required to
!    recover the unitary part of the decomposition.
!
!    Input/output, integer IPVT(P); on input, integers that control
...
!
  ...
  integer lup
  integer maxj
  real ( kind = rk8 ) maxnrm
  logical negj
  complex ( kind = ck8 ) nrmxl
  integer pl
  integer pu
  ...
  real ( kind = rk8 ) tt
  complex ( kind = ck8 ) work(p)
  complex ( kind = ck8 ) x(ldx,p)
  real ( kind = rk8 ) zabs1
  complex ( kind = ck8 ) zdotc
  complex ( kind = ck8 ) zsign2
...
!
  do j = pl, pu
    qraux(j) = cmplx ( dznrm2 ( n, x(1,j), 1 ), 0.0D+00, kind = ck8 )
    work(j) = qraux(j)
  end do
!
...
!
    if ( pl <= l .and. l < pu ) then
      ...
      do j = l, pu
        if ( maxnrm < real ( qraux(j), kind = rk8 ) ) then
          maxnrm = real ( qraux(j), kind = rk8 )
          maxj = j
        end if
      end do
    ...
    qraux(l) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )

    if ( l /= n ) then
!
!  Compute the Householder transformation for column L.
!
      nrmxl = cmplx ( dznrm2 ( n-l+1, x(l,l), 1 ), 0.0D+00, kind = ck8 )
      ...
      if ( zabs1 ( nrmxl ) /= 0.0D+00 ) then
        ...
        call zscal ( n-l+1, cmplx ( 1.0D+00, 0.0D+00, kind = ck8 ) &
          / nrmxl, x(l,l), 1 )
        x(l,l) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 ) + x(l,l)
!
...
!
        ...
        do j = l+1, p
          ...
          tt = 1.0D+00 - ( abs ( x(l,j) ) / real ( qraux(j), kind = rk8 ) )**2
          tt = max ( tt, 0.0D+00 )
          t = cmplx ( tt, 0.0D+00, kind = ck8 )
          tt = 1.0D+00 &
            + 0.05D+00 * tt * ( real ( qraux(j), kind = rk8 ) &
            / real ( work(j), kind = rk8 ) )**2

          if ( tt /= 1.0D+00 ) then
            qraux(j) = qraux(j) * sqrt ( t )
          else
            qraux(j) = cmplx ( dznrm2 ( n-l, x(l+1,j), 1 ), 0.0D+00, kind = ck8 )
            work(j) = qraux(j)
          end if
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) X(LDX,P), the output of ZQRDC.
!
...
!
!    Input, integer K, the number of columns of the matrix XK.
!    K must not be greater than min ( N, P), where P is the same as in the
!    calling sequence to ZQRDC.
!
!    Input, complex ( kind = ck8 ) QRAUX(P), the auxiliary output from ZQRDC.
!
!    Input, complex ( kind = ck8 ) Y(N), a vector that is to be manipulated
!    by ZQRSL.
!
!    Output, complex ( kind = ck8 ) QY(N), contains Q*Y, if it has been requested.
!
!    Output, complex ( kind = ck8 ) QTY(N), contains hermitian(Q)*Y, if it has
!    been requested.  Here hermitian(Q) is the conjugate transpose
!    of the matrix Q.
!
!    Output, complex ( kind = ck8 ) B(K), the solution of the least squares
!    problem
...
!
!    Output, complex ( kind = ck8 ) RSD(N), the least squares residual Y - XK*B,
...
!
!    Output, complex ( kind = ck8 ) XB(N), the least squares approximation XK*N,
...
!
  if ( ju == 0 ) then
    ...
    if ( cr ) then
      rsd(1) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
    end if
...
!
  ...
  if ( cxb ) then
    xb(k+1:n) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
  end if

  if ( cr ) then
    rsd(1:k) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
  end if
!
!  Compute B.
!
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) A(LDA,N); on input, the symmetric
!    matrix to be factored.  On output, a block diagonal matrix and the
!    multipliers which were used to obtain it.  The factorization can be
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) A(LDA,N); on input, the output from
...
!
!    Output, complex ( kind = ck8 ) DET(2), if requested, the determinant of the
...
!
        ...
        akp1 = a(k+1,k+1) / t
        akkp1 = a(k,k+1) / t
        d = t * ( ak * akp1 - cmplx ( 1.0D+00, 0.0D+00, kind = ck8 ) )
        a(k,k) = akp1 / d
        a(k+1,k+1) = ak / d
        a(k,k+1) = -akkp1 / d
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) Z1, Z2, the arguments.
!
!    Output, complex ( kind = ck8 ) ZSIGN1,  a complex value, with the 
...
!
  ...
  if ( zabs1 ( z2 ) == 0.0D+00 ) then
    zsign1 = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
  else
...
!    Input, complex ( kind = ck8 ) Z1, Z2, the arguments.
!
!    Output, complex ( kind = ck8 ) CSIGN2,  a complex value, with the 
!    magnitude of Z1, and the argument of Z2.
!
  implicit none
  ...
  if ( zabs2 ( z2 ) == 0.0D+00 ) then
    zsign2 = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
  else
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) A(LDA,N), the output from ZSICO or ZSIFA.
!
!    Input, integer LDA, the leading dimension of A.
!
!    Input, integer N, the order of the matrix.
!
!    Input, integer IPVT(N), the pivot vector from ZSICO or ZSIFA.
!
!    Input/output, complex ( kind = ck8 ) B(N).  On input, the right hand side.
!    On output, the solution.
!
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) AP(N*(N+1)/2); on input, the matrix
...
!    Input, integer N, the order of the matrix.
!
!    Input, integer IPVT(N), the pivot vector from ZSPFA.
!
!    Workspace, complex WORK(N).
!
!    Input, integer JOB, has the decimal expansion AB where
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) AP(N*(N+1)/2), the output from ZSPFA.
!
!    Input, integer N, the order of the matrix.
!
!    Input, integer IPVT(N), the pivot vector from ZSPFA.
...
!
!    Output, complex ( kind = ck8 ) S(MM), where MM = max ( N + 1, P ), the first
!    min ( N, P ) entries of S contain the singular values of X arranged
!    in descending order of magnitude.
!
!    Output, complex ( kind = ck8 ) E(MM), where MM = max ( N + 1, P ).
!    Ordinarily contains zeros on output.
!    However, see the discussion of INFO for exceptions.
!
!    Output, complex ( kind = ck8 ) U(LDU,K).  If JOBA == 1 then K == N; if
...
!
!    Output, complex ( kind = ck8 ) V(LDV,P), if requested, the matrix of right
...
!
!    Output, integer INFO.  The singular values and their
...
!
  ...
  real ( kind = rk8 ) t1
  real ( kind = rk8 ) test
  complex ( kind = ck8 ) u(ldu,*)
  complex ( kind = ck8 ) v(ldv,p)
  logical wantu
  logical wantv
  complex ( kind = ck8 ) work(n)
  complex ( kind = ck8 ) x(ldx,p)
...
!
    if ( l <= nct ) then
      ...
      s(l) = cmplx ( dznrm2 ( n-l+1, x(l,l), 1 ), 0.0D+00, kind = ck8 )
...
!
      e(l) = cmplx ( dznrm2 ( p-l, e(lp1), 1 ), 0.0D+00, kind = ck8 )
      ...
      if ( zabs1 ( e(l) ) /= 0.0D+00 ) then
        ...
        e(lp1) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 ) + e(lp1)
...
!
      if ( lp1 <= n .and. zabs1 ( e(l) ) /= 0.0D+00 ) then
        ...
        work(lp1:n) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
...
!
  ...
  if ( n < m ) then
    s(m) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
  end if
  ...
  e(m) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
!
...
!
  if ( wantu ) then
    ...
    do j = nctp1, ncu
      u(1:n,j) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
      u(j,j) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 )
    end do
    ...
    do ll = 1, nct
      ...
      if ( zabs1 ( s(l) ) /= 0.0D+00 ) then
        ...
        call zscal ( n-l+1, cmplx ( -1.0D+00, 0.0D+00, kind = ck8 ), u(l,l), 1 )
        u(l,l) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 ) + u(l,l)
        u(1:l-1,l) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )

      else

        u(1:n,l) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
        u(l,l) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 )
...
!
  if ( wantv ) then
    ...
    do ll = 1, p
      ...
      v(1:p,l) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
      v(l,l) = cmplx ( 1.0D+00, 0.0D+00, kind = ck8 )
...
!
  do i = 1, m
    ...
    if ( zabs1 ( s(i) ) /= 0.0D+00 ) then
      ...
      t = cmplx ( abs ( s(i) ), 0.0D+00, kind = ck8 )
      r = s(i) / t
      s(i) = t
    ...
    if ( zabs1 ( e(i) ) /= 0.0D+00 ) then
      ...
      t = cmplx ( abs ( e(i) ), 0.0D+00, kind = ck8 )
      r = t / e(i)
      e(i) = t
      s(i+1) = s(i+1) * r
...
!
    do ll = 1, m
      ...
      if ( ztest == test ) then
        e(l) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
        exit
      end if
    ...
    if ( l == m - 1 ) then
      ...
      do lls = lp1, mp1
        ...
        if ( ztest == test) then
          s(ls) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
          exit
        end if
...
!
    if ( kase == 1 ) then
      ...
      mm1 = m - 1
      f = real ( e(m-1), kind = rk8 )
      e(m-1) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )

      do kk = l, mm1

        k = mm1 - kk + l
        t1 = real ( s(k), kind = rk8 )
        call drotg ( t1, f, cs, sn )
        s(k) = cmplx ( t1, 0.0D+00, kind = ck8 )

        if ( k /= l ) then
          f = -sn * real ( e(k-1), kind = rk8 )
          e(k-1) = cs * e(k-1)
        end if
...
!
      ...
      f = real ( e(l-1), kind = rk8 )
      e(l-1) = cmplx ( 0.0D+00, 0.0D+00, kind = ck8 )
...
!
      ...
      sm = real ( s(m), kind = rk8 ) / scale
      smm1 = real ( s(m-1), kind = rk8 ) / scale
      emm1 = real ( e(m-1), kind = rk8 ) / scale
      sl = real ( s(l), kind = rk8 ) / scale
      el = real ( e(l), kind = rk8 ) / scale
...
!
      ...
      do k = l, mm1
        ...
        if ( k /= l ) then
          e(k-1) = cmplx ( f, 0.0D+00, kind = ck8 )
        end if

        f = cs * real ( s(k), kind = rk8 ) + sn * real ( e(k), kind = rk8 )
        e(k) = cs * e(k) - sn * s(k)
        g = sn * real ( s(k+1), kind = rk8 )
        s(k+1) = cs * s(k+1)
        ...
        call drotg ( f, g, cs, sn )
        s(k) = cmplx ( f, 0.0D+00, kind = ck8 )
        f = cs * real ( e(k), kind = rk8 ) + sn * real ( s(k+1), kind = rk8 )
        s(k+1) = -sn * e(k) + cs * s(k+1)
        g = sn * real ( e(k+1), kind = rk8 )
        e(k+1) = cs * e(k+1)
      ...
      e(m-1) = cmplx ( f, 0.0D+00, kind = ck8 )
      iter = iter + 1
!
!  Convergence.
!
...
!
      if ( real ( s(l), kind = rk8 ) < 0.0D+00 ) then
        s(l) = -s(l)
        if ( wantv ) then
          call zscal ( p, cmplx ( -1.0D+00, 0.0D+00, kind = ck8 ), v(1,l), 1 )
        end if
      end if
!
...
!
      do while ( l /= mm )
        ...
        if ( real ( s(l+1), kind = rk8 ) <= real ( s(l), kind = rk8 ) ) then
          exit
        end if
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) T(LDT,N), the triangular matrix.  The zero
...
!      norm(A*Z) = RCOND * norm(A) * norm(Z).
!
!    Input, integer JOB, indicates if matrix is upper or lower
!    triangular.
!    0, lower triangular.
...
!  Parameters:
!
!    Input/output, complex ( kind = ck8 ) T(LDT,N), the triangular matrix.
...
!  Parameters:
!
!    Input, complex ( kind = ck8 ) T(LDT,N), the matrix of the system.  The zero
!    elements of the matrix are not referenced, and the corresponding
!    elements of the array can be used to store other information.
!
!    Input, integer LDT, the leading dimension of T.
...
!    On output, the solution.
!
!    Input, integer JOB, specifies what kind of system is to
!    be solved.
