...
!
  ...
  integer, parameter :: nnzmax = 100

  complex cval(nnzmax)
  real ( kind = rk ) dval(nnzmax)
  integer nnz
  character ( len = 7 ) field
  character ( len = 14 ) id
  integer ihi
...
!
  ...
  complex, dimension ( 0 ) :: cval
  real (  kind = rk ), allocatable, dimension ( :, : ) :: dval
  character ( len = 7 ) :: field = 'double'
  character ( len = 14 ) :: id = '%%MatrixMarket'
  integer, dimension ( 0 ) :: indx
  integer ios
  integer, dimension ( 0 ) :: ival
  integer, dimension ( 0 ) :: jndx
  integer n
  integer ncol
  integer nnz
  integer nrow
...
!
!    Input, integer N, the order of the matrix, as determined
!    by NX and NY.
!
...
!
  ...
  real ( kind = rk ) a(n,n)
  real ( kind = rk ), save, dimension ( 8, 8 ) :: em =  reshape ( (/ &
...
!    Input, integer NX, NY, values which determine the size of A.
!
!    Output, integer N, the order of the matrix, as determined
!    by NX and NY.
!
  implicit none
