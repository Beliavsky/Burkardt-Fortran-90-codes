...
!  Parameters:
!
!    Input, real ( kind = rk ) P1(2), P2(2), P3(2), three points that define
...
!  Parameters:
!
!    Input, real ( kind = rk ) P1(2), P2(2), P3(2), points defining the angle.
!
!    Input, real ( kind = rk ) P4(2), a point defining the half angle.
...
!
!    Output, real ( kind = rk ) ATAN4, an angle between 0 and 2 * PI, whose
...
!  Parameters:
!
!    Input, real ( kind = rk ) BOX(DIM_NUM,2), the lower left and upper right
!    corners of the box.
!
...
!
!    Input, real ( kind = rk ) C(2), the center of the circle.
!
!    Input, real ( kind = rk ) THETA1, THETA2, the angles defining the arc,
!    in radians.  Normally, THETA1 < THETA2.
!
!    Input, real ( kind = rk ) X(2), the point to be checked.
!
!    Output, real ( kind = rk ) NEAREST(2), a point on the circular arc which is
!    nearest to the point.
!
!    Output, real ( kind = rk ) DIST, the distance to the nearest point.
!
  implicit none
...
!
!    A circle in 2D satisfies the equation:
!
!      ( X - C(1) ) * ( X - C(1) ) + ( Y - C(2) ) * ( Y - C(2) ) = R * R
!
!  Licensing:
...
!    Input, real ( kind = rk ) X(2), the point to be checked.
!
!    Output, real ( kind = rk ) NEAREST(2), the nearest point on the circle.
!
!    Output, real ( kind = rk ) DIST, the distance of the point to the circle.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) ANGLE, the angle, in degrees.
!
!    Output, real ( kind = rk ) COS_DEG, the cosine of the angle.
!
  implicit none
  ...
  real ( kind = rk ) angle
  real ( kind = rk ) cos_deg
  real ( kind = rk ), parameter :: degrees_to_radians &
...
!
!    Input, real ( kind = rk ) VALUE, the function value at ARG, as
...
!    corners of the box.
!
!    Input, real ( kind = rk ) CENTER(DIM_NUM), the center of the grid.
!    This point must be inside the unit square.
!
!    Input, real ( kind = rk ) ANGLE, the angle, in degrees, of the grid.
!    Normally, 0 <= ANGLE <= 180, but any value is allowed.
!
!    Input, real ( kind = rk ) H, the spacing between neighboring
...
!
!    Output, real ( kind = rk ) R(DIM_NUM,N), the grid points.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) V(2,6), the vertics, in counterclockwise order.
!
!    Input, real ( kind = rk ) P(2), the point to be tested.
!
...
!
!    Input, real ( kind = rk ) H, the maximum size of a segment of the boundary.
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: boundary
  ...
  integer :: x_ps_max_user
  integer :: x_ps_min = 36
  integer :: x_ps_min_clip = 18
  integer :: x_ps_min_user
...
!
  x_ps_min_user = ( ( x_ps_max + x_ps_min ) &
    - int ( real ( x_ps_max - x_ps_min, kind = rk ) * x_scale / scale ) ) / 2
  x_ps_max_user = ( ( x_ps_max + x_ps_min ) &
    + int ( real ( x_ps_max - x_ps_min, kind = rk ) * x_scale / scale ) ) / 2
  y_ps_min_user = ( ( y_ps_max + y_ps_min ) &
    - int ( real ( y_ps_max - y_ps_min, kind = rk ) * y_scale / scale ) ) / 2
  ...
  do segment = 1, segment_num
    ...
    if ( show_nodes ) then
      ...
      do j = 1, segment_length
        ...
        x_ps = int ( &
          ( ( x_max - boundary(1,j)         ) * real ( x_ps_min, kind = rk )   &
          + (         boundary(1,j) - x_min ) * real ( x_ps_max, kind = rk ) ) &
          / ( x_max                 - x_min ) )

        y_ps = int ( &
          ( ( y_max - boundary(2,j)         ) * real ( y_ps_min, kind = rk )   &
          + (         boundary(2,j) - y_min ) * real ( y_ps_max, kind = rk ) ) &
          / ( y_max                 - y_min ) )
...
!
!    Input, real ( kind = rk ) POINT(DIM_NUM,N), the coordinates
!    of the points.
!
!    Output, real ( kind = rk ) BOUNDARY(DIM_NUM,N), points on the boundary
...
!
  ...
  real ( kind = rk ), dimension ( dim_num, n ) :: boundary
...
!    Input, integer N, the number of points.
!
!    Input/output, real ( kind = rk ) POINT(DIM_NUM,N), the coordinates
...
!
!    Output, real ( kind = rk ) SEGMENT(M,SEGMENT_LENGTH), the
...
!
  ...
  real ( kind = rk ) segment(m,segment_length)
  integer test
...
!    Input, integer TEST, the index of the test problem
!
!    Input, integer SEGMENT_INDEX, the index of one of the
!    boundary segments.
!
!    Input, real ( kind = rk ) H, the suggested spacing between points.
!
!    Output, integer SEGMENT_LENGTH, the number of points
!    in the segment.
!
  implicit none
...
!    Input, integer M, the spatial dimension.
!
!    Output, real ( kind = rk ) LO(M), HI(M), the lower and
...
!    of the points.
!
!    Output, real ( kind = rk ) DENSITY(N), the mesh density
!    at each point.
!
  implicit none
  ...
  real ( kind = rk ) density(n)
  real ( kind = rk ) point(m,n)
  integer test
...
!    Input, integer TEST, the index of the test problem
!
!    Input, real ( kind = rk ) ELEMENT_SIZE, a typical element size.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) element_size
  integer test
...
!
!    Output, real ( kind = rk ) FIXED(M,FIXED_NUM), the
...
!
  ...
  real ( kind = rk ) fixed(m,fixed_num)
  integer test
...
!    Input, integer M, the spatial dimension.
!
!    Input, real ( kind = rk ) H, the hexagonal spacing.
!
...
!
  ...
  integer n2
  real ( kind = rk ) point(m,n)
  real ( kind = rk ), allocatable, dimension ( :, : ) :: point2
  integer test
!
!  Get the box limits.
!
...
!    Input, integer M, the spatial dimension.
!
!    Output, real ( kind = rk ) HOLE_POINT(M), a point in the hole
!
  implicit none
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: boundary
  integer, parameter :: circle_size = 4
  character ( len = * ) eps_file_name
  integer eps_unit
  real ( kind = rk ) h
  real ( kind = rk ), allocatable, dimension ( : ) :: hi
...
!
  ...
  do j = 1, n
    ...
    x_ps = int ( &
      ( ( x_max - points(1,j)         ) * real ( x_ps_min, kind = rk )   &
      + (         points(1,j) - x_min ) * real ( x_ps_max, kind = rk ) ) &
      / ( x_max               - x_min ) )

    y_ps = int ( &
      ( ( y_max - points(2,j)         ) * real ( y_ps_min, kind = rk )   &
      + (         points(2,j) - y_min ) * real ( y_ps_max, kind = rk ) ) &
      / ( y_max               - y_min ) )
...
!
  ...
  integer hole_index
  integer hole_num
  real ( kind = rk ), allocatable, dimension ( :, : ) :: hole
  integer j
  real ( kind = rk ) lo(dim_num)
  integer next
  integer node_num
  real ( kind = rk ) scale
  real ( kind = rk ), allocatable, dimension ( :, : ) :: segment
  integer segment_index
  integer segment_length
  integer test
!
...
subroutine p00_sample ( test, m, n, point )
...
!    Input, integer N, the number of points.
!
!    Output, real ( kind = rk ) POINT(M,N), the coordinates
!    of the points.
!
  implicit none
  ...
  if ( test == 1 ) then
    call p01_sample ( m, n, point )
  else if ( test == 2 ) then
    call p02_sample ( m, n, point )
  else if ( test == 3 ) then
    call p03_sample ( m, n, point )
  else if ( test == 4 ) then
    call p04_sample ( m, n, point )
  else if ( test == 5 ) then
    call p05_sample ( m, n, point )
  else if ( test == 6 ) then
    call p06_sample ( m, n, point )
  else if ( test == 7 ) then
    call p07_sample ( m, n, point )
  else if ( test == 8 ) then
    call p08_sample ( m, n, point )
  else if ( test == 9 ) then
    call p09_sample ( m, n, point )
  else if ( test == 10 ) then
    call p10_sample ( m, n, point )
  else if ( test == 11 ) then
    call p11_sample ( m, n, point )
  else if ( test == 12 ) then
    call p12_sample ( m, n, point )
  else if ( test == 13 ) then
    call p13_sample ( m, n, point )
  else if ( test == 14 ) then
    call p14_sample ( m, n, point )
  else if ( test == 15 ) then
    call p15_sample ( m, n, point )
  else
    write ( *, '(a)' ) ' '
...
subroutine p00_sample_h1 ( test, m, n, h, point )
...
!
  ...
  if ( test == 1 ) then
    call p01_sample_h1 ( m, n, h, point )
  else if ( test == 2 ) then
    call p02_sample_h1 ( m, n, h, point )
  else if ( test == 3 ) then
    call p03_sample_h1 ( m, n, h, point )
  else if ( test == 4 ) then
    call p04_sample_h1 ( m, n, h, point )
  else if ( test == 5 ) then
   call p05_sample_h1 ( m, n, h, point )
  else if ( test == 6 ) then
    call p06_sample_h1 ( m, n, h, point )
  else if ( test == 7 ) then
    call p07_sample_h1 ( m, n, h, point )
  else if ( test == 8 ) then
    call p08_sample_h1 ( m, n, h, point )
  else if ( test == 9 ) then
    call p09_sample_h1 ( m, n, h, point )
  else if ( test == 10 ) then
    call p10_sample_h1 ( m, n, h, point )
  else if ( test == 11 ) then
    call p11_sample_h1 ( m, n, h, point )
  else if ( test == 12 ) then
    call p12_sample_h1 ( m, n, h, point )
  else if ( test == 13 ) then
    call p13_sample_h1 ( m, n, h, point )
  else if ( test == 14 ) then
    call p14_sample_h1 ( m, n, h, point )
  else if ( test == 15 ) then
    call p15_sample_h1 ( m, n, h, point )
  else
    write ( *, '(a)' ) ' '
...
!    of the points.
!
!    Output, real ( kind = rk ) BOUNDARY(M,N), points on the boundary
!    that are nearest to each point.
!
  implicit none
  ...
  real ( kind = rk ), dimension ( m, n ) :: boundary
  real ( kind = rk ), dimension ( 2 ) :: center =  (/ &
    0.0D+00, 0.0D+00 /)
  integer j
  real ( kind = rk ), dimension ( m, n ) :: point
  real ( kind = rk ) :: r
  real ( kind = rk ), parameter :: r1 = 1.0D+00
...
!    Input, integer N, the number of points.
!
!    Input/output, real ( kind = rk ) POINT(M,N), the coordinates
...
!
  ...
  real ( kind = rk ), dimension ( 2 ), parameter :: center =  &
    (/ 0.0D+00, 0.0D+00 /)
  logical inside(n)
  integer j
  real ( kind = rk ), dimension ( m, n ) :: point
...
!
  ...
  real ( kind = rk ) angle
  real ( kind = rk ), dimension ( 2 ) :: center = (/ 0.0D+00, 0.0D+00 /)
  integer i
  real ( kind = rk ), parameter :: pi = 3.141592653589793D+00
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  integer segment_index
...
!  Parameters:
!
!    Output, integer BOUNDARY_SEGMENT_NUM, the number of
!    boundary segments.
!
  implicit none
...
!    Input, integer M, the spatial dimension.
!
!    Output, real ( kind = rk ) LO(M), HI(M), coordinates of the
...
!    Input, integer FIXED_NUM, the number of fixed points.
!
!    Output, real ( kind = rk ) FIXED(M,FIXED_NUM), the coordinates
!    of the fixed points.
!
  implicit none
...
!
  ...
  integer boundary_segment_num
  real ( kind = rk ), dimension ( m ) :: center = (/ 0.0D+00, 0.0D+00 /)
  integer fixed_num
  integer hole_num
  real ( kind = rk ), parameter :: r1 = 1.0D+00
...
subroutine p01_sample ( m, n, point )
...
subroutine p01_sample_h1 ( m, n, h, point )
...
!
  ...
  real ( kind = rk ) r
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.4D+00
...
!    of the points.
!
!    Output, real ( kind = rk ) DENSITY(N), the mesh density at
!    each point.
!
  implicit none
...
subroutine p02_sample ( m, n, point )
...
subroutine p02_sample_h1 ( m, n, h, point )
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) POINT(M,N), the points.
!
!    Output, real ( kind = rk ) BOUNDARY(M,N), points on the boundary
!    that are nearest to each point.
!
  ...
  real ( kind = rk ), dimension ( 2 ), parameter :: center =  &
    (/ 0.0D+00, 0.0D+00 /)
  real ( kind = rk ) dist_box
  real ( kind = rk ) dist_circle
  integer j
  real ( kind = rk ), dimension ( m, n ) :: point
  real ( kind = rk ) :: r
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.4D+00
  real ( kind = rk ) x
  real ( kind = rk ), parameter :: x1 = -1.0D+00
  real ( kind = rk ), parameter :: x2 = +1.0D+00
  real ( kind = rk ) y
  real ( kind = rk ), parameter :: y1 = -1.0D+00
...
!
  ...
  if ( segment_index == 1 ) then
    ...
    n1 = nint ( real ( segment_length - 1, kind = rk ) &
              / real ( 4, kind = rk ) )
    n2 = nint ( real ( 2 * ( segment_length - 1 ), kind = rk ) &
              / real ( 4, kind = rk ) ) - n1
    n3 = nint ( real ( 3 * ( segment_length - 1 ), kind = rk ) &
              / real ( 4, kind = rk ) ) - n1 - n2
    ...
    do i = 1, n1
      j = j + 1
      segment(1:2,j) = ( real ( n1 - i + 1, kind = rk ) * s(1:2,1)   &
                       + real (      i - 1, kind = rk ) * s(1:2,2) ) &
                       / real ( n1,         kind = rk )
    end do

    do i = 1, n2
      j = j + 1
      segment(1:2,j) = ( real ( n2 - i + 1, kind = rk ) * s(1:2,2)   &
                       + real (      i - 1, kind = rk ) * s(1:2,3) ) &
                       / real ( n2,         kind = rk )
    end do

    do i = 1, n3
      j = j + 1
      segment(1:2,j) = ( real ( n3 - i + 1, kind = rk ) * s(1:2,3)   &
                       + real (      i - 1, kind = rk ) * s(1:2,4) ) &
                       / real ( n3,         kind = rk )
    end do

    do i = 1, n4
      j = j + 1
      segment(1:2,j) = ( real ( n4 - i + 1, kind = rk ) * s(1:2,4)   &
                       + real (      i - 1, kind = rk ) * s(1:2,1) ) &
                       / real ( n4,         kind = rk )
    end do
...
!    Input, real ( kind = rk ) H, the suggested spacing between points.
!
!    Output, integer SEGMENT_LENGTH, the number of points in
!    the segment.
!
  implicit none
...
subroutine p03_sample ( m, n, point )
...
!
    call r8mat_uniform_01 ( m, sample_num, sample )
!
...
subroutine p03_sample_h1 ( m, n, h, point )
...
!
  ...
  real ( kind = rk ), parameter :: x1 = -1.0D+00
  real ( kind = rk ), parameter :: x2 = +1.0D+00
  real ( kind = rk ), parameter :: xc =  0.0D+00
  real ( kind = rk ), parameter :: y1 = -1.0D+00
  real ( kind = rk ), parameter :: y2 = +1.0D+00
  real ( kind = rk ), parameter :: yc = 0.0D+00
...
!
  ...
  real ( kind = rk ) pn(m)
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.5D+00
  real ( kind = rk ) t
  logical, save :: v_set = .false.
  real ( kind = rk ), save, dimension(2,6) :: v1
  real ( kind = rk ), save, dimension(2,6) :: v2

  if ( .not. v_set ) then

    do i = 0, 5
      angle = real ( 2 * ( i - 1 ), kind = rk ) * pi / 6.0D+00
...
!    Input, integer M, the spatial dimension.
!
!    Input, integer SEGMENT_LENGTH, the number of points in
!    the segment.
!
!    Output, real ( kind = rk ) SEGMENT(M,SEGMENT_LENGTH), the
!    points that make up the boundary segment.
!
  ...
  if ( segment_index == 1 ) then
    ...
    n2 = nint ( real ( 2 * ( segment_length - 1 ), kind = rk ) &
              / real ( 6, kind = rk ) ) - n1
    n3 = nint ( real ( 3 * ( segment_length - 1 ), kind = rk ) &
              / real ( 6, kind = rk ) ) - n1 - n2
    n4 = nint ( real ( 4 * ( segment_length - 1 ), kind = rk ) &
              / real ( 6, kind = rk ) ) - n1 - n2 - n3
    n5 = nint ( real ( 5 * ( segment_length - 1 ), kind = rk ) &
              / real ( 6, kind = rk ) ) - n1 - n2 - n3 - n4
    n6 = segment_length - 1 - n1 - n2 - n3 - n4 - n5

    do j = 1, 6
      angle = real ( 2 * ( j - 1 ), kind = rk ) * pi / 6.0D+00
      s(1,j) = r1 * cos ( angle )
      s(2,j) = r1 * sin ( angle )
    end do
    ...
    do i = 1, n4
      j = j + 1
      segment(1:2,j) = ( real ( n4 - i + 1, kind = rk ) * s(1:2,4)   &
                       + real (      i - 1, kind = rk ) * s(1:2,5) ) &
                       / real ( n4,         kind = rk )
    end do

    do i = 1, n5
      j = j + 1
      segment(1:2,j) = ( real ( n5 - i + 1, kind = rk ) * s(1:2,5)   &
                       + real (      i - 1, kind = rk ) * s(1:2,6) ) &
                       / real ( n5,         kind = rk )
    end do

    do i = 1, n6
      j = j + 1
      segment(1:2,j) = ( real ( n6 - i + 1, kind = rk ) * s(1:2,6)   &
                       + real (      i - 1, kind = rk ) * s(1:2,1) ) &
                       / real ( n6,         kind = rk )
    end do
    ...
    do j = 1, 6
      angle = real ( 2 * ( 6 - j ) + 1, kind = rk ) * pi / 6.0D+00
      s(1,j) = r2 * cos ( angle )
      s(2,j) = r2 * sin ( angle )
    end do
...
subroutine p04_sample ( m, n, point )
...
!    Input, integer N, the number of points.
!
!    Output, real ( kind = rk ) POINT(M,N), the coordinates of the points.
!
  implicit none
...
subroutine p04_sample_h1 ( m, n, h, point )
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) H, the enlargement value.
!
!    Output, real ( kind = rk ) POINT(M,N), the coordinates of the points.
!
  implicit none
...
!
  ...
  real ( kind = rk ), dimension ( m, n ) :: boundary
  real ( kind = rk ), dimension ( 2 ) :: center1 = (/  0.0D+00, 0.0D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: center2 = (/ -0.4D+00, 0.0D+00 /)
  real ( kind = rk ) dist
  real ( kind = rk ) dist_min
  integer j
  real ( kind = rk ) norm
  real ( kind = rk ), dimension ( m ) :: p1
  real ( kind = rk ), dimension ( m ) :: p2
  real ( kind = rk ), dimension ( m ) :: pn
  real ( kind = rk ), dimension ( m, n ) :: point
  real ( kind = rk ), dimension ( m ) :: q1
  real ( kind = rk ), dimension ( m ) :: q2
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.55D+00
  real ( kind = rk ) t
...
!
  ...
  real ( kind = rk ), dimension ( m ) :: boundary
  real ( kind = rk ), dimension ( 2 ) :: center1 = (/  0.0D+00, 0.0D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: center2 = (/ -0.4D+00, 0.0D+00 /)
  real ( kind = rk ) dist
  real ( kind = rk ) dist_min
...
!
    n4 = nint ( real ( segment_length - 1, kind = rk ) &
            / ( 1.0D+00 + r2 + 0.90D+00 / pi )  )

    n1 = nint ( 0.05D+00 * real ( n4, kind = rk ) / pi )
    n2 = nint (       r2 * real ( n4, kind = rk )            )
    n3 = nint ( 0.85D+00 * real ( n4, kind = rk ) / pi )
...
!
    do i = 1, n1
      j = j + 1
      segment(1:2,j) = &
        ( real ( n1 - i + 1, kind = rk ) * (/ center1(1) - r1, center1(2) /)   &
        + real (      i - 1, kind = rk ) * (/ center2(1) - r2, center2(2) /) ) &
        / real ( n1,         kind = rk )
    end do
!
...
!
    do i = 1, n3
      j = j + 1
      segment(1:2,j) = &
        ( real ( n3 - i + 1, kind = rk ) * (/ center2(1) + r2, center2(2) /)   &
        + real (      i - 1, kind = rk ) * (/ center1(1) + r1, center1(1) /) ) &
        / real ( n3,         kind = rk )
    end do
!
...
!
  ...
  integer n
  real ( kind = rk ), parameter :: pi = 3.141592653589793D+00
  real ( kind = rk ), parameter :: r1 = 1.00D+00
  real ( kind = rk ), parameter :: r2 = 0.55D+00
  integer segment_index
  integer segment_length
...
!
  ...
  integer boundary_segment_num
  real ( kind = rk ), dimension ( m ) :: center1 = (/  0.0D+00, 0.0D+00 /)
  real ( kind = rk ), dimension ( m ) :: center2 = (/ -0.4D+00, 0.0D+00 /)
  integer fixed_num
  integer hole_num
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.55D+00
...
subroutine p05_sample ( m, n, point )
...
subroutine p05_sample_h1 ( m, n, h, point )
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) H, the enlargement.
!
!    Output, real ( kind = rk ) POINT(M,N), the coordinates
!    of the points.
!
...
!
  ...
  do j = 1, segment_length
    ...
    angle = ( 2.0D+00 * pi * real (              j - 1, kind = rk ) ) &
                           / real ( segment_length - 1, kind = rk )
    x = r * cos ( angle )
    y = r * sin ( angle )
...
subroutine p06_sample ( m, n, point )
...
subroutine p06_sample_h1 ( m, n, h, point )
...
subroutine p07_sample ( m, n, point )
...
subroutine p07_sample_h1 ( m, n, h, point )
...
!
  ...
  real ( kind = rk ), dimension ( m, n ) :: boundary
  real ( kind = rk ), dimension ( 2 ) :: center1 = (/ 0.0D+00, 0.0D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: center2 = (/ 0.6D+00, 0.0D+00 /)
  real ( kind = rk ) dist
  real ( kind = rk ) dist_min
  integer j
  integer k
  ...
  real ( kind = rk ), dimension ( m, n ) :: point
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.1D+00
  real ( kind = rk ) t
  real ( kind = rk ), parameter :: theta1_max =  pi / 12.0D+00
  real ( kind = rk ), parameter :: theta1_min = -pi / 12.0D+00
  real ( kind = rk ) theta2_max
  real ( kind = rk ) theta2_min
  real ( kind = rk ), dimension ( 2, 4 ) :: v1 = reshape ( (/ &
    0.995435619D+00, -0.095435619D+00,   &
    0.9D+00,          0.0D+00,           &
    0.96592581D+00,   0.25881904D+00,    &
    0.0D+00,          0.0D+00 /), (/ 2, 4 /) )
  real ( kind = rk ), dimension ( 2, 4 ) :: v2 = reshape ( (/ &
    0.9D+00,          0.0D+00,         &
    0.995435619D+00,  0.095435619D+00, &
    0.0D+00,          0.0D+00,         &
...
!
    ...
    n2 = nint ( ( theta1_max - theta2_max ) * real ( n1, kind = rk ) )
    n3 = nint (    a * sqrt ( 2.0D+00 ) * real ( n1, kind = rk ) )
    ...
    do i = 1, n1
      j = j + 1
      segment(1:2,j) = ( real ( n1 - i + 1, kind = rk ) * s(1:2)   &
                       + real (      i - 1, kind = rk ) * t(1:2) ) &
                       / real ( n1,         kind = rk )
    end do

    do i = 1, n2

      theta = (   real ( n2 - i + 1, kind = rk ) * theta1_min   &
                + real (      i - 1, kind = rk ) * theta2_min ) &
              /   real ( n2,         kind = rk )
    ...
    do i = 1, n3
      j = j + 1
      segment(1:2,j) = ( real ( n3 - i + 1, kind = rk ) * s(1:2)   &
                       + real (      i - 1, kind = rk ) * t(1:2) ) &
                       / real ( n3,         kind = rk )
    end do
    ...
    do i = 1, n4
      j = j + 1
      segment(1:2,j) = ( real ( n4 - i + 1, kind = rk ) * s(1:2)   &
                       + real (      i - 1, kind = rk ) * t(1:2) ) &
                       / real ( n4,         kind = rk )
    end do
    ...
    do i = 1, n5
      ...
      theta = (   real ( n5 - i + 1, kind = rk ) * theta2_max   &
              +   real (      i - 1, kind = rk ) * theta1_max ) &
              /   real ( n5,         kind = rk )
    ...
    do i = 1, n6
      j = j + 1
      segment(1:2,j) = ( real ( n6 - i + 1, kind = rk ) * s(1:2)   &
                       + real (      i - 1, kind = rk ) * t(1:2) ) &
                       / real ( n6,         kind = rk )
    end do
...
!
    ...
    do j = 1, segment_length
      theta = real ( segment_length - j, kind = rk ) * 2.0D+00 * pi &
            / real ( segment_length - 1, kind = rk )
...
!
  ...
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.1D+00
  real ( kind = rk ) :: theta1_max =  pi / 12.0D+00
  real ( kind = rk ) :: theta1_min = -pi / 12.0D+00
...
!
  ...
  integer boundary_segment_num
  real ( kind = rk ), dimension ( m ) :: center1 = (/ 0.0D+00, 0.0D+00 /)
  real ( kind = rk ), dimension ( m ) :: center2 = (/ 0.6D+00, 0.0D+00 /)
  integer fixed_num
  integer hole_num
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.1D+00
...
!
  ...
  real ( kind = rk ), parameter :: r1 = 1.0D+00
  real ( kind = rk ), parameter :: r2 = 0.1D+00
  real ( kind = rk ), dimension ( 2 ) :: p1 = (/ &
   0.995435619D+00, -0.095435619D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: p2 = (/ &
    0.9000000D+00,  0.0000000D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: p3 = (/ &
   0.995435619D+00, +0.095435619D+00 /)
  real ( kind = rk ) :: theta1_max =  pi / 12.0D+00
  real ( kind = rk ) :: theta1_min = -pi / 12.0D+00
...
subroutine p08_sample ( m, n, point )
...
!
    ...
    if ( 10.0D+00 + 0.95D+00 * real ( have + reject, kind = rk ) &
       < real ( reject, kind = rk ) ) then
      write ( *, '(a)' ) ' '
...
subroutine p08_sample_h1 ( m, n, h, point )
...
!
  ...
  real ( kind = rk ) b_coef
  integer, parameter :: batch = 1000
  real ( kind = rk ), dimension ( m ) :: c
  real ( kind = rk ) c_coef
  real ( kind = rk ), dimension ( 2 ) :: center1 = (/ 0.0D+00, 0.0D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: center2 = (/ 0.6D+00, 0.0D+00 /)
  logical circle_imp_contains_point_2d
  logical circle_sector_contains_point_2d
  real ( kind = rk ), dimension ( m ) :: d
  real ( kind = rk ), dimension ( m ) :: e
  real ( kind = rk ), dimension ( m ) :: f
  real ( kind = rk ) h
  integer have
  real ( kind = rk ) hi(m)
  logical indent
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: sample
  integer sample_num
  real ( kind = rk ), dimension ( 2, 3 ) :: t
  real ( kind = rk ) :: theta1_max =  pi / 12.0D+00
  real ( kind = rk ) :: theta1_maxh
  real ( kind = rk ) :: theta1_min = -pi / 12.0D+00
  real ( kind = rk ) :: theta1_minh
...
!
  ...
  real ( kind = rk ) angle
  real ( kind = rk ), dimension ( m, n ) :: boundary
  real ( kind = rk ), dimension ( 2 ) :: center1 = (/ 0.50D+00, 0.50D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: center2 = (/ 0.25D+00, 0.75D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: center3 = (/ 0.60D+00, 0.40D+00 /)
  real ( kind = rk ) dist
  real ( kind = rk ) dist_min
  integer i
  integer j
  ...
  real ( kind = rk ), dimension ( m, n ) :: point
  real ( kind = rk ) pn(m)
  real ( kind = rk ) :: r1 = 0.5D+00
  real ( kind = rk ) :: r2 = 0.1D+00
  real ( kind = rk ) :: r3 = 0.1D+00
  real ( kind = rk ) t
  real ( kind = rk ), dimension(m,4) :: v1
  real ( kind = rk ), dimension(m,6) :: v2
  real ( kind = rk ), dimension(m,6) :: v3
  ...
  do j = 1, 6
    angle = real ( ( j - 1 ) * 2, kind = rk ) * pi / 6.0D+00
...
!
  ...
  if ( segment_index == 1 ) then
    ...
    do i = 1, n1
      j = j + 1
      segment(1:2,j) = ( real ( n1 - i + 1, kind = rk ) * v1(1:2,1)   &
                       + real (      i - 1, kind = rk ) * v1(1:2,2) ) &
                       / real ( n1,         kind = rk )
    end do

    do i = 1, n2
      j = j + 1
      segment(1:2,j) = ( real ( n2 - i + 1, kind = rk ) * v1(1:2,2)   &
                       + real (      i - 1, kind = rk ) * v1(1:2,3) ) &
                       / real ( n2,         kind = rk )
    end do

    do i = 1, n3
      j = j + 1
      segment(1:2,j) = ( real ( n3 - i + 1, kind = rk ) * v1(1:2,3)   &
                       + real (      i - 1, kind = rk ) * v1(1:2,4) ) &
                       / real ( n3,         kind = rk )
    end do

    do i = 1, n4
      j = j + 1
      segment(1:2,j) = ( real ( n4 - i + 1, kind = rk ) * v1(1:2,4)   &
                       + real (      i - 1, kind = rk ) * v1(1:2,1) ) &
                       / real ( n4,         kind = rk )
    end do
    ...
    do i = 1, n1
      j = j + 1
      segment(1:2,j) = ( real ( n1 - i + 1, kind = rk ) * v2(1:2,1)   &
                       + real (      i - 1, kind = rk ) * v2(1:2,2) ) &
                       / real ( n1,         kind = rk )
    end do

    do i = 1, n2
      j = j + 1
      segment(1:2,j) = ( real ( n2 - i + 1, kind = rk ) * v2(1:2,2)   &
                       + real (      i - 1, kind = rk ) * v2(1:2,3) ) &
                       / real ( n2,         kind = rk )
    end do

    do i = 1, n3
      j = j + 1
      segment(1:2,j) = ( real ( n3 - i + 1, kind = rk ) * v2(1:2,3)   &
                       + real (      i - 1, kind = rk ) * v2(1:2,4) ) &
                       / real ( n3,         kind = rk )
    end do

    do i = 1, n4
      j = j + 1
      segment(1:2,j) = ( real ( n4 - i + 1, kind = rk ) * v2(1:2,4)   &
                       + real (      i - 1, kind = rk ) * v2(1:2,5) ) &
                       / real ( n4,         kind = rk )
    end do

    do i = 1, n5
      j = j + 1
      segment(1:2,j) = ( real ( n5 - i + 1, kind = rk ) * v2(1:2,5)   &
                       + real (      i - 1, kind = rk ) * v2(1:2,6) ) &
                       / real ( n5,         kind = rk )
    end do

    do i = 1, n6
      j = j + 1
      segment(1:2,j) = ( real ( n6 - i + 1, kind = rk ) * v2(1:2,6)   &
                       + real (      i - 1, kind = rk ) * v2(1:2,1) ) &
                       / real ( n6,         kind = rk )
    end do
    ...
    do i = 1, n1
      j = j + 1
      segment(1:2,j) = ( real ( n1 - i + 1, kind = rk ) * v3(1:2,1)   &
                       + real (      i - 1, kind = rk ) * v3(1:2,2) ) &
                       / real ( n1,         kind = rk )
    end do

    do i = 1, n2
      j = j + 1
      segment(1:2,j) = ( real ( n2 - i + 1, kind = rk ) * v3(1:2,2)   &
                       + real (      i - 1, kind = rk ) * v3(1:2,3) ) &
                       / real ( n2,         kind = rk )
    end do

    do i = 1, n3
      j = j + 1
      segment(1:2,j) = ( real ( n3 - i + 1, kind = rk ) * v3(1:2,3)   &
                       + real (      i - 1, kind = rk ) * v3(1:2,4) ) &
                       / real ( n3,         kind = rk )
    end do

    do i = 1, n4
      j = j + 1
      segment(1:2,j) = ( real ( n4 - i + 1, kind = rk ) * v3(1:2,4)   &
                       + real (      i - 1, kind = rk ) * v3(1:2,5) ) &
                       / real ( n4,         kind = rk )
    end do

    do i = 1, n5
      j = j + 1
      segment(1:2,j) = ( real ( n5 - i + 1, kind = rk ) * v3(1:2,5)   &
                       + real (      i - 1, kind = rk ) * v3(1:2,6) ) &
                       / real ( n5,         kind = rk )
    end do

    do i = 1, n6
      j = j + 1
      segment(1:2,j) = ( real ( n6 - i + 1, kind = rk ) * v3(1:2,6)   &
                       + real (      i - 1, kind = rk ) * v3(1:2,1) ) &
                       / real ( n6,         kind = rk )
    end do
...
!
  ...
  real ( kind = rk ), dimension ( 2 ) :: center2 = (/ 0.25D+00, 0.75D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: center3 = (/ 0.60D+00, 0.40D+00 /)
  real ( kind = rk ), dimension(m,fixed_num) :: fixed
  integer j
  real ( kind = rk ), parameter :: pi = 3.141592653589793D+00
  real ( kind = rk ) :: r1 = 0.5D+00
  real ( kind = rk ) :: r2 = 0.1D+00
...
!
  ...
  integer boundary_segment_num
  real ( kind = rk ), dimension ( m ) :: center1 = (/ 0.50D+00, 0.50D+00 /)
  real ( kind = rk ), dimension ( m ) :: center2 = (/ 0.25D+00, 0.75D+00 /)
  real ( kind = rk ), dimension ( m ) :: center3 = (/ 0.60D+00, 0.40D+00 /)
  integer fixed_num
  integer hole_num
  real ( kind = rk ) :: r1 = 0.5D+00
  real ( kind = rk ) :: r2 = 0.1D+00
...
subroutine p09_sample ( m, n, point )
...
!
  ...
  real ( kind = rk ) :: r2 = 0.1D+00
  real ( kind = rk ) :: r3 = 0.1D+00
  real ( kind = rk ), dimension ( 2, 4 ) :: v1
  real ( kind = rk ), dimension ( 2, 6 ) :: v2
  real ( kind = rk ), dimension ( 2, 6 ) :: v3
  real ( kind = rk ) y(m)
  ...
  do j = 1, n
    ...
    do
      ...
      call random_number ( harvest = y(1:m) )
...
subroutine p09_sample_h1 ( m, n, h, point )
...
!
  ...
  do j = 1, n
    ...
    do
      ...
      call random_number ( harvest = y(1:m) )
      do i = 1, m
        y(i) = center1(i) + ( 2.0D+00 * y(i) - 1.0D+00 ) * r1h
      end do
...
!  Parameters:
!
!    Input, integer SEGMENT_INDEX, the index of the 
!    boundary segment.
!
!    Input, integer M, the spatial dimension.
!
...
subroutine p10_sample ( m, n, point )
...
!  Generate a batch of points in [0,1]x[0,1].
!
  call r8mat_uniform_01 ( m, n, point )
!
!  Remap the points to the box [X1,X2] x [Y1,Y2].
!
...
subroutine p10_sample_h1 ( m, n, h, point )
...
!
  ...
  integer j
  real ( kind = rk ), dimension ( m, n ) :: point
  real ( kind = rk ), dimension(2,4) :: q1 = reshape ( (/ &
    0.00D+00, 0.00D+00, &
    1.00D+00, 0.00D+00, &
    0.75D+00, 0.25D+00, &
    0.25D+00, 0.25D+00 /), (/ 2, 4 /) )
  real ( kind = rk ), dimension(2,4) :: q2 = reshape ( (/ &
    0.00D+00, 0.00D+00, &
    0.25D+00, 0.25D+00, &
    0.25D+00, 0.75D+00, &
    0.00D+00, 1.00D+00 /), (/ 2, 4 /) )
  real ( kind = rk ), dimension(2,4) :: q3 = reshape ( (/ &
    0.50D+00, 0.50D+00, &
    0.50D+00, 0.25D+00, &
    0.75D+00, 0.25D+00, &
    1.00D+00, 0.50D+00 /), (/ 2, 4 /) )
  real ( kind = rk ), dimension(2,4) :: q4 = reshape ( (/ &
    0.50D+00, 0.50D+00, &
    0.25D+00, 0.50D+00, &
    0.25D+00, 0.25D+00, &
    0.50D+00, 0.25D+00 /), (/ 2, 4 /) )
  real ( kind = rk ), dimension(2,4) :: q5 = reshape ( (/ &
    0.50D+00, 0.50D+00, &
    0.50D+00, 1.00D+00, &
    0.25D+00, 0.75D+00, &
  ...
  logical triangle_contains_point_2d
  real ( kind = rk ), parameter :: x1 =  0.0D+00
  real ( kind = rk ), parameter :: x2 =  0.5D+00
  real ( kind = rk ), parameter :: x3 = +1.0D+00
  real ( kind = rk ), parameter :: y1 =  0.0D+00
  real ( kind = rk ), parameter :: y2 =  0.5D+00
  real ( kind = rk ), parameter :: y3 = +1.0D+00
...
subroutine p11_sample ( m, n, point )
...
subroutine p11_sample_h1 ( m, n, h, point )
...
!
  ...
  integer j
  real ( kind = rk ), dimension ( m, n ) :: point
  real ( kind = rk ), dimension ( m, 4 ) :: q1
  real ( kind = rk ), dimension ( m, 4 ) :: q2
  real ( kind = rk ), dimension ( m, 4 ) :: q3
  real ( kind = rk ), dimension ( m, 4 ) :: q4
  real ( kind = rk ), dimension ( m, 4 ) :: q5
  real ( kind = rk ), dimension ( m, 4 ) :: q6
  real ( kind = rk ), dimension ( m, 4 ) :: q7
  real ( kind = rk ), dimension ( m, 4 ) :: q8
  real ( kind = rk ), dimension ( m, 4 ) :: q9
  real ( kind = rk ), dimension ( m, 4 ) :: q10
  real ( kind = rk ), dimension ( m, 4 ) :: q11
  real ( kind = rk ), dimension ( m, 4 ) :: q12
  real ( kind = rk ), dimension ( m, 4 ) :: q13
  logical quad_contains_point_2d
  real ( kind = rk ), dimension ( m, 3 ) :: t1
  real ( kind = rk ), dimension ( m, 3 ) :: t2
  real ( kind = rk ), dimension ( m, 3 ) :: t3
  real ( kind = rk ), dimension ( m, 3 ) :: t4
  real ( kind = rk ), dimension ( m, 3 ) :: t5
  real ( kind = rk ), dimension ( m, 3 ) :: t6
  real ( kind = rk ), dimension ( m, 3 ) :: t7
  logical triangle_contains_point_2d
  real ( kind = rk ), parameter :: x1 = 0.000D+00
  real ( kind = rk ), parameter :: x2 = 0.500D+00
  real ( kind = rk ), parameter :: x2p5 = 0.5625D+00
  real ( kind = rk ), parameter :: x3 = 0.625D+00
  real ( kind = rk ), parameter :: x3p5 = 0.8125D+00
  real ( kind = rk ), parameter :: x4 = 1.000D+00
  real ( kind = rk ), parameter :: y1 = 0.000D+00
  real ( kind = rk ), parameter :: y2 = 0.25D+00
  real ( kind = rk ), parameter :: y2p5 = 0.31250D+00
  real ( kind = rk ), parameter :: y3 = 0.375D+00
  real ( kind = rk ), parameter :: y4 = 1.000D+00
...
!
  ...
  real ( kind = rk ), parameter :: x4 = 1.000D+00
  real ( kind = rk ), parameter :: y1 = 0.000D+00
  real ( kind = rk ), parameter :: y2 = 0.125D+00
  real ( kind = rk ), parameter :: y2p5 = 0.250D+00
  real ( kind = rk ), parameter :: y3 = 0.375D+00
  real ( kind = rk ), parameter :: y4 = 1.000D+00
...
!
  ...
  if ( segment_index == 1 ) then
    ...
    n01 = nint ( 0.500D+00 * real ( segment_length - 1, kind = rk ) &
              / 5.75D+00 )

    n02 = nint ( ( 0.500D+00 + 0.250D+00) &
              * real ( segment_length - 1, kind = rk ) &
              / real ( 5.75D+00, kind = rk ) ) - n01

    n03 = nint ( ( 0.500D+00 + 0.250D+00 + 0.125D+00 ) &
              * real ( segment_length - 1, kind = rk ) &
              / real ( 5.75D+00, kind = rk ) ) - n01 - n02

    n04 = nint ( ( 0.500D+00 + 0.250D+00 + 0.125D+00 + 0.250D+00 ) &
              * real ( segment_length - 1, kind = rk ) &
              / real ( 5.75D+00, kind = rk ) ) - n01 - n02 - n03

    n05 = nint ( ( 0.500D+00 + 0.250D+00 + 0.125D+00 + 0.250D+00 &
                 + 0.375D+00 ) &
              * real ( segment_length - 1, kind = rk ) &
              / real ( 5.75D+00, kind = rk ) ) - n01 - n02 - n03 - n04

    n06 = nint ( ( 0.500D+00 + 0.250D+00 + 0.125D+00 + 0.250D+00 &
                 + 0.375D+00 + 1.000D+00 ) &
              * real ( segment_length - 1, kind = rk ) &
              / real ( 5.75D+00, kind = rk ) ) - n01 - n02 - n03 - n04 - n05
    ...
    do i = 1, n01
      j = j + 1
      segment(1:2,j) = ( real ( n01 - i + 1, kind = rk ) * s(1:2,1)   &
                       + real (       i - 1, kind = rk ) * s(1:2,2) ) &
                       / real ( n01,         kind = rk )
    end do
    ...
    do i = 1, n02
      j = j + 1
      segment(1:2,j) = ( real ( n02 - i + 1, kind = rk ) * s(1:2,2)   &
                       + real (       i - 1, kind = rk ) * s(1:2,3) ) &
                       / real ( n02,         kind = rk )
    end do
    ...
    do i = 1, n03
      j = j + 1
      segment(1:2,j) = ( real ( n03 - i + 1, kind = rk ) * s(1:2,3)   &
                       + real (       i - 1, kind = rk ) * s(1:2,4) ) &
                       / real ( n03,         kind = rk )
    end do
    ...
    do i = 1, n04
      j = j + 1
      segment(1:2,j) = ( real ( n04 - i + 1, kind = rk ) * s(1:2,4)   &
                       + real (       i - 1, kind = rk ) * s(1:2,5) ) &
                       / real ( n04,         kind = rk )
    end do
    ...
    do i = 1, n05
      j = j + 1
      segment(1:2,j) = ( real ( n05 - i + 1, kind = rk ) * s(1:2,5)   &
                       + real (       i - 1, kind = rk ) * s(1:2,6) ) &
                       / real ( n05,         kind = rk )
    end do
    ...
    do i = 1, n06
      j = j + 1
      segment(1:2,j) = ( real ( n06 - i + 1, kind = rk ) * s(1:2,6)   &
                       + real (       i - 1, kind = rk ) * s(1:2,7) ) &
                       / real ( n06,         kind = rk )
    end do

    do i = 1, n07
      j = j + 1
      segment(1:2,j) = ( real ( n07 - i + 1, kind = rk ) * s(1:2,7)   &
                       + real (       i - 1, kind = rk ) * s(1:2,8) ) &
                       / real ( n07,         kind = rk )
    end do

    do i = 1, n08
      j = j + 1
      segment(1:2,j) = ( real ( n08 - i + 1, kind = rk ) * s(1:2,8)   &
                       + real (       i - 1, kind = rk ) * s(1:2,9) ) &
                       / real ( n08,         kind = rk )
    end do

    do i = 1, n09
      j = j + 1
      segment(1:2,j) = ( real ( n09 - i + 1, kind = rk ) * s(1:2,9)   &
                       + real (       i - 1, kind = rk ) * s(1:2,10) ) &
                       / real ( n09,         kind = rk )
    end do

    do i = 1, n10
      j = j + 1
      segment(1:2,j) = ( real ( n10 - i + 1, kind = rk ) * s(1:2,10)   &
                       + real (       i - 1, kind = rk ) * s(1:2,11) ) &
                       / real ( n10,         kind = rk )
    end do

    do i = 1, n11
      j = j + 1
      segment(1:2,j) = ( real ( n11 - i + 1, kind = rk ) * s(1:2,11)   &
                       + real (       i - 1, kind = rk ) * s(1:2,12) ) &
                       / real ( n11,         kind = rk )
    end do

    do i = 1, n12
      j = j + 1
      segment(1:2,j) = ( real ( n12 - i + 1, kind = rk ) * s(1:2,12)   &
                       + real (       i - 1, kind = rk ) * s(1:2,1) ) &
                       / real ( n12,         kind = rk )
    end do
...
subroutine p12_sample ( m, n, point )
...
!
  ...
  real ( kind = rk ) prob(n)
  real ( kind = rk ) point(m,n)
  real ( kind = rk ) :: x1 = 0.000D+00
  real ( kind = rk ) :: x2 = 0.500D+00
  real ( kind = rk ) :: x3 = 0.625D+00
  real ( kind = rk ) :: x4 = 1.000D+00
  real ( kind = rk ) :: y1 = 0.000D+00
  real ( kind = rk ) :: y2 = 0.250D+00
  real ( kind = rk ) :: y3 = 0.375D+00
  real ( kind = rk ) :: y4 = 1.000D+00
...
subroutine p12_sample_h1 ( m, n, h, point )
...
!
  ...
  integer n08
  real ( kind = rk ), parameter :: pi = 3.141592653589793D+00
  real ( kind = rk ), dimension ( 2, 8 ) :: s = reshape ( (/ &
...
!
  do i = 1, n01
    j = j + 1
    angle = real ( n01 - i + 1, kind = rk ) * pi / real ( n01, kind = rk )
...
!
  do i = 1, n02
    j = j + 1
    segment(1:2,j) = ( real ( n02 - i + 1, kind = rk ) * s(1:2,3)   &
                     + real (       i - 1, kind = rk ) * s(1:2,4) ) &
                     / real ( n02,         kind = rk )
  end do
!
...
!
  ...
  do i = 1, n03
    j = j + 1
    angle = real ( i - 1, kind = rk ) * angle2 / real ( n03, kind = rk )
...
!
  do i = 1, n04
    j = j + 1
    segment(1:2,j) = ( real ( n04 - i + 1, kind = rk ) * s(1:2,5)   &
                     + real (       i - 1, kind = rk ) * s(1:2,6) ) &
                       / real ( n04,         kind = rk )
  end do
!
...
!
  do i = 1, n05
    j = j + 1
    segment(1:2,j) = ( real ( n05 - i + 1, kind = rk ) * s(1:2,6)   &
                     + real (       i - 1, kind = rk ) * s(1:2,7) ) &
                     / real ( n05,         kind = rk )
  end do
!
...
!
  do i = 1, n06
    j = j + 1
    segment(1:2,j) = ( real ( n06 - i + 1, kind = rk ) * s(1:2,7)   &
                     + real (       i - 1, kind = rk ) * s(1:2,8) ) &
                     / real ( n06,         kind = rk )
  end do
!
...
!
  ...
  do i = 1, n07
    j = j + 1
    angle = ( real ( n07 - i + 1, kind = rk ) * angle1 + &
              real (       i - 1, kind = rk ) * angle2 ) &
            / real ( n07,         kind = rk )
    segment(1,j) = 50.0D+00 + 40.0D+00 * cos ( angle )
    segment(2,j) =  0.0D+00 + 40.0D+00 * sin ( angle )
  end do
...
!
  do i = 1, n08
    j = j + 1
    segment(1:2,j) = ( real ( n08 - i + 1, kind = rk ) * s(1:2,1)   &
                     + real (       i - 1, kind = rk ) * s(1:2,2) ) &
                     / real ( n08,         kind = rk )
  end do
...
subroutine p13_sample ( m, n, point )
...
!
  ...
  real ( kind = rk ) point(m,n)
  integer reject
  real ( kind = rk ), dimension(3) :: width = (/ &
...
subroutine p13_sample_h1 ( m, n, h, point )
...
!
  ...
  integer m
  integer n
  integer, parameter :: n1 = 90
  integer, parameter :: n2 = 11

  real ( kind = rk ) boundary(m,n)
  real ( kind = rk ) dist1
  real ( kind = rk ) dist2
  integer j
  real ( kind = rk ) pn1(m)
...
subroutine p14_sample ( m, n, point )
...
!
  ...
  real ( kind = rk ), dimension ( 2, 2 ) :: box = reshape ( (/ &
    100.0D+00,  145.0D+00, &
  ...
  do j = 1, n
    ...
    do
      ...
      call p14_inside ( m, 1, x, inside(1) )
...
subroutine p14_sample_h1 ( m, n, h, point )
...
!
  ...
  real ( kind = rk ), dimension ( 2, n2 ) :: v2 = reshape ( (/ &
    ...
    255.73801D+00, 249.16872D+00, &
    241.72690D+00, 256.73448D+00 /), (/ 2, n2 /) )
  real ( kind = rk ), dimension ( 2, n1 ) :: w1
  real ( kind = rk ), dimension ( 2, n2 ) :: w2
  real ( kind = rk ) x(m)
!
...
!
  ...
  if ( segment_index == 1 ) then
    ...
    n1 = nint ( 10.0D+00 * real ( sl_extra, kind = rk ) &
              / real ( 36, kind = rk ) )

    n2 = nint (  1.0D+00 * real ( sl_extra, kind = rk ) &
              / real ( 36, kind = rk ) )
    n2 = max ( n2, 0 )

    n3 = nint (  6.0D+00 * real ( sl_extra, kind = rk ) &
                / real ( 36, kind = rk ) )
    n3 = max ( n3, 0 )

    n4 = nint (  1.0D+00 * real ( sl_extra, kind = rk ) &
                / real ( 36, kind = rk ) )
    n4 = max ( n4, 0 )

    n5 = nint ( 10.0D+00 * real ( sl_extra, kind = rk ) &
                / real ( 36, kind = rk ) )
    n5 = max ( n5, 0 )

    n6 = nint (  1.0D+00 * real ( sl_extra, kind = rk ) &
                / real ( 36, kind = rk ) )
    n6 = max ( n6, 0 )

    n7 = nint (  6.0D+00 * real ( sl_extra, kind = rk ) &
                / real ( 36, kind = rk ) )
    n7 = max ( n7, 0 )
    ...
    do i = 1, n7
      j = j + 1
      segment(1:2,j) = ( real ( n7 - i + 1, kind = rk ) * s(1:2,7)   &
                       + real (      i - 1, kind = rk ) * s(1:2,8) ) &
                       / real ( n7,         kind = rk )
    end do
    ...
    do i = 1, n8
      j = j + 1
      segment(1:2,j) = ( real ( n8 - i + 1, kind = rk ) * s(1:2,8)   &
                       + real (      i - 1, kind = rk ) * s(1:2,1) ) &
                       / real ( n8,         kind = rk )
    end do
...
subroutine p15_sample ( m, n, point )
...
subroutine p15_sample_h1 ( m, n, h, point )
...
!
!    Input, real ( kind = rk ) SEGMENT(2,NODE_NUM), the nodes.
!
!    Input, integer EDGE_NUM, the number of edges.
!
!    Input, integer EDGE_NODES(2,EDGE_NUM), the nodes that form 
!    each edge.
!
!    Input, integer HOLE_NUM, the number of holes in the mesh.
!
!    Input, real ( kind = rk ) HOLE_POINT(2,HOLE_NUM), a point in each hole.
!
  implicit none
  ...
  integer edge
  integer edge_nodes(2,edge_num)
  character ( len = * ) file_name
  integer hole
  real ( kind = rk ) hole_point(2,hole_num)
  integer ios
  integer node
  integer poly_unit
  integer, parameter :: region_num = 0
  real ( kind = rk ) segment(2,node_num)
...
!
!    Input, real ( kind = rk ) H, the expansion amount.
!
!    Output, real ( kind = rk ) W(2,N), the "expanded" coordinates.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) V(2,N), the polygon vertices.
!
!    Input, real ( kind = rk ) P(2), the point whose nearest polygon point
!    is to be determined.
!
...
!
  ...
  do i = 1,  nt
    ...
    st = ( real ( nt - i,     kind = rk ) * 0.0D+00 + &
           real (      i - 1, kind = rk ) * sk(nk+1) ) &
         / real ( nt     - 1, kind = rk )
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the number to be divided.
!
!    Input, real ( kind = rk ) Y, the number that divides X.
!
!    Output, real ( kind = rk ) R8_MODP, the nonnegative remainder
...
!  Parameters:
!
!    Input, real ( kind = rk ) P1(2), P2(2), the two endpoints of the line
...
!
!    Output, real ( kind = rk ) T, the relative position of the point (XN,YN)
...
!    Input, real ( kind = rk ) ANGLE, the angle, in degrees.
!
!    Output, real ( kind = rk ) SIN_DEG, the sine of the angle.
!
  implicit none
