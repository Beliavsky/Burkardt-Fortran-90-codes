...
!*****************************************************************************80
!
!! lagrange_interp_nd_test() tests lagrange_interp_nd().
!
!  Licensing:
!
...
!
  ...
  call random_number ( harvest = xi(1:m,1:ni) )
...
!
    ...
    e = r8vec_norm_affine ( ni, zi, ze ) / real ( ni, kind = rk )
...
!    Input, real ( kind = rk ) X(M,N), the points.
!
!    Output, real ( kind = rk ) Z(N,1), the value of the function at each point.
!
  implicit none
