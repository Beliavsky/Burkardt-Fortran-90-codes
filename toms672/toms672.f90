...
!  Input:
!
!    integer N, the number of preassigned nodes.
!
!    real ( kind = rk ) PNODES(N), the preassigned nodes.
!
!    integer IWORK(N), working array.
!
!    real ( kind = rk ) WORK(N,N+1).
!
!    integer LDW, the number of elements in the 
!    leading dimension of WORK declared in the calling program.
!
!    external RECUR, the user-supplied function which defines the
!    orthogonal polynomials.
!
!  Output:
!
!    real ( kind = rk ) T(0:N), the coefficients of the polynomial whose
...
!
  ...
  real ( kind = rk ) p0
  real ( kind = rk ) p1
  real ( kind = rk ) pnodes(n)
  external recur
  real ( kind = rk ) t(0:n)
  real ( kind = rk ) work(0:ldw,0:*)
  real ( kind = rk ) x
...
!  Parameters:
!
!    Input, integer N, the degree of input polynomial POLIN.
!
!    Input, real ( kind = rk ) POLIN(0:N), coefficients of the polynomial whose
!    quadratic factor is to be found, i.e.
!      POLIN = SUM(I = 0 to N) POLIN(I) * P(I,X)
!
!    Output, real ( kind = rk ) POLOUT(0:N-2), coefficients of the deflated
!    polynomial of degree N-2 with the quadratic factor removed, i.e.
!      POLOUT = SUM(I = 0 to N-2) POLOUT(I) * P(I,X)
!
!    Input/output, real ( kind = rk ) A0, A1, A2, on input, the estimated
...
!
!    Input, integer IDIGIT, the node convergence parameter, an
...
!
!    Output, real ( kind = rk ) ERRVAL, the mean value of the correction to
...
!
  ...
  integer ifail
  integer iter
  real ( kind = rk ) polin(0:n)
  real ( kind = rk ) polout(0:n-2)
  external recur
  real ( kind = rk ) scale
  real ( kind = rk ) tol
...
!  Input:
!
!    integer N, the number of nodes in the quadrature rule.
!    1 <= N.
!
!    real ( kind = rk ) PNODES(N), the nodes.
!
!    real ( kind = rk ) WT(N), the weights.
!
!    integer K, the index of the orthogonal polynomial whose
!    weighted square is to be integrated.  0 <= K.
!
!    real ( kind = rk ) H0, the integral of the orthogonality weight
!    function over the interval of integration.  Note that
!    P(0,X) is arbitrarily taken to be 1.0.
!
!    external RECUR (), the function which defines the
!    orthogonal polynomials.  See EXTEND for a full description.
!
!  Output:
!
!    real ( kind = rk ) TEST, the approximate value of the test integral
!    normalized to unity.  Thus, abs(TEST-1) gives a measure of the
!    quality of the calculated rule.
!
!    integer IERR, error flag.
!    * 0, OK.
!    * 1, Rule quality unsatisfactory.
!    * 2, Invalid values for input arguments.
!
  implicit none
  ...
  external recur
  real ( kind = rk ) test
  real ( kind = rk ), parameter :: tol = 0.0000001D+00
  real ( kind = rk ) wt(n)
  real ( kind = rk ) x
...
!
  ...
  test = test * cj / p
!
!  Test for rule quality.
!
  if ( tol < abs ( test - 1.0D+00 ) ) then
    ierr = 1
  end if
...
!  Parameters:
!
!    Input, integer N, the highest polynomial degree.  Note that
!    after the initial call with J = 0 the value of N in this argument is
!    ignored.
!
!    Input, integer J, the current product of P(J,X) with P(N,X)
...
!
!    Output, real ( kind = rk ) COEFF(2*J+1), the coefficients of the expansion.
!
!    Workspace, real ( kind = rk ) WORK(2*J+1,2).
!    The contents of this work area must not be altered between
!    calls by the calling program.
!
!    Input, integer LW, the leading dimension of WORK.
!
...
!  Parameters:
!
!    Input/output, integer N; on input, the number of preassigned
...
!
!    Input/ouput, integer M0; on input, the lower limit to the
...
!
!    Input/output, real ( kind = rk ) T(0:max(N-M0,M)); on input, the
...
!
!    Input, real ( kind = rk ) PNODES(N+M), on input, the preassigned nodes.
...
!
!    Input, real ( kind = rk ) H0, the integral of the orthogonality weight
!    function over the interval of integration. Zero moment integral.
!
!    Input, integer NEXP, the largest negative decimal exponent
...
!
!    Input, integer IDIGIT, the node convergence parameter,
...
!    value is 5.
!
!    Output, real ( kind = rk ) WT(N+M), the quadrature weights for
!    the extended rule associated with the nodes in PNODES.
!
!    Output, integer NODES, the number of extended nodes found.
!    Normally equals M, but see IFAIL.
!
!    Output, real ( kind = rk ) QR(M), the real parts of the extended nodes.
!
!    Output, real ( kind = rk ) QI(M), the imaginary parts of the extended
!    nodes (1,..,NODES).  Hopefully these values are zero!
!
!    Output, real ( kind = rk ) ERR(M), a measure of the relative error in the
...
!
!    Output, real ( kind = rk ) EXT(M+1), the coefficients of the polynomial
!    whose roots are the  extended nodes (QRS(*),QIS(*)) and expressed as:
!      EXT = SUM (0 <= I <= M) EXT(I) * P(I,X).
!
!    Output, integer IWORK(max(M,N)), node convergence flags.
...
!
!    Workspace, real ( kind = rk ) WORKA(max(M+1,N+1),max(M+1,N+1)).
!    If SYMMET = TRUE, the dimensions can be reduced to max(M/2+1,N) by
!    max(M/2+1,N+1).
!
!    Input, integer LDA, the leading dimension of WORKA.
!
!    Input, real ( kind = rk ) WORKB(2*M+1,3).
!
!    Input, integer LDB, the leading dimension of WORKB.
!
!    Output, integer IFLAG  = 0, No error detected
...
!
  ...
  real ( kind = rk ) test
  real ( kind = rk ), parameter :: tol = 0.0000001D+00
  real ( kind = rk ) worka(0:lda-1,0:*)
  real ( kind = rk ) workb(0:ldb-1,3)
  real ( kind = rk ) wt(n+m)
!
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) T(0:N); on input, the coefficients TI of
...
!
!    Input/output, integer M0, the lower limit to the expansion of
!    T.  On output, this is updated to correspond with the output value of T.
!
!    Input/output, integer N, the upper limit to expansion of T.
...
!      produced.
!
!    Output, real ( kind = rk ) EXT(0:M), the coefficients of the polynomial
!    whose roots are the  new extended nodes and expressed as:
!      EXT = SUM (I = 0 to M) EXT(I) * P(I,X)
!
!    Workspace, integer IWORK(M).
!
!    Workspace, real ( kind = rk ) WORKA(M+1,max(M+1,N+1)).
!    If SYMMET = TRUE, the dimension can be reduced to
!    M/2+1 by max(M/2+1,N/2+1).
!
!    Input, integer LDA, the number of elements in the leading
!    dimension of WORKA declared in the calling program.
!
!    Input, real ( kind = rk ) WORKB(2*M+1,3).
!
!    Input, integer LDB, the number of elements in the leading
...
!  Parameters:
!
!    Input, real ( kind = rk ) GAMMA(0:N), the polynomial from which the factor
!    is to be removed and expressed as:
!      GAMMA = SUM (0 <= I <= N) GAMMA(I) * P(I,X)
!
!    Output, real ( kind = rk ) DELTA(0:N-1), the quotient polynomial expressed
!    as:
!      DELTA = SUM (0 <= I <= N-1) DELTA(I) * P(I,X)
!
!    Input, integer N, the degree of GAMMA.
!
!    Input, real ( kind = rk ) XNODE, the node to be removed.
!
!    Input, external RECUR ( ), the function which defines the orthogonal
!    polynomials.  See EXTEND for a full description.
!
!    Output, real ( kind = rk ) R, the remainder from division.
!
!    Output, real ( kind = rk ) DR, the derivative of R with respect to XNODE.
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(0:N), the polynomial whose roots define the
!    nodes of the quadrature rule and expressed as:
!      T = SUM (0 <= I <= N) T(I) * P(I,X)
!
!    Input, integer N, the degree of the expansion of T.
!
!    Input/output, real ( kind = rk ) XNODE, on input, a rough estimate for
...
!    to round-off, in the computed relative error.  A typical value is 5.
!
!    Output, real ( kind = rk ) DELTA(0:N-1), the coefficients of the deflated
!    polynomial.
!
!    Output, real ( kind = rk ) ERRVAL, the value of the correction.  May be used
...
!    Input, integer N, the degree of GAMMA.
!
!    Input, real ( kind = rk ) GAMMA(0:N), a polynomial to be divided by
!    a quadratic factor.
!
!    Output, real ( kind = rk ) DELTA(0:N-2), the quotient polynomial of
!    degree N-2.
!
!    Input, external RECUR(), the function which defines the
!    orthogonal polynomials.  See EXTEND for a full description.
!
!    Input, real ( kind = rk ) ALPHA, BETA, the coefficients of the quadratic
!    factor.
!
!    Output, real ( kind = rk ) A, B, the remainder coefficients.
!
!    Output, real ( kind = rk ) AA, the partial of A with respect to ALPHA.
!
!    Output, real ( kind = rk ) AB, the partial of A with respect to BETA.
!
!    Output, real ( kind = rk ) BA, the partial of B with respect to ALPHA.
!
!    Output, real ( kind = rk ) BB, the partial of B with respect to BETA.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) A0, A1, A2, the coefficients of the quadratic
!    factor.
!
!    Output, real ( kind = rk ) ZREAL1, ZIMAG1, the real and imaginary parts
!    of root 1.
!
!    Output, real ( kind = rk ) ZREAL2, ZIMAG2, the real and Imaginary parts
!    of root 2.
!
!    Input, external RECUR ( ), the function which defines the orthogonal
!    polynomials.  See EXTEND for full description.
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) A(N), the array to be sorted.
!
!    Input, integer N, the number of elements to be sorted
!
!    Input, integer IFLAG, determines the sort direction.
...
!
!    Input, integer M, the upper limit to expansion EXT
!    (polynomial degree).
!
!    Input, logical SYMMET
...
!    value is 5.
!
!    Output, real ( kind = rk ) QR(M), the real parts of the roots fo EXT.
!
!    Output, real ( kind = rk ) QI(M), the imaginary parts of the roots
...
!
!    Output, integer ICHECK(M), root convergence flags.  Elements
!    1 to NODES give information on the convergence of the roots of the
!    polynomial EXT.
!    * 0, Convergence of I th root satisfactory;
!    * 1, Convergence of I th root unsatisfactory.
!
!    Worskpace, real ( kind = rk ) WORK(M+1,2).
!
!    Input, integer LDW, the leading dimension of WORK (which must
!    be at least M+1).
!
!    Output, integer IERR
!    * 0, No error detected
...
!
  ...
  real ( kind = rk ) sfa1
  logical symmet
  real ( kind = rk ), parameter :: vrt1 = 0.0000001D+00
  real ( kind = rk ) work(0:ldw-1,2)
  real ( kind = rk ) zi1
  real ( kind = rk ) zi2
  real ( kind = rk ) zr1
  real ( kind = rk ) zr2
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) T(0:N), the coefficients TI of the
...
!
!    Input, integer M, the lower limit to the expansion of T.
!
!    Input, integer N, the upper limit to the expansion of T.
!
!    Input, external RECUR ( ), the function which defines the orthogonal
!    polynomials.  See EXTEND for a full description.
!
!    Input, integer IFLAG, the operation to be carried out.
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(0:N), the coefficients TI of the polynomial
...
!    input, integer N, the upper limit to the expansion of T.
!
!    Input, real ( kind = rk ) XNODE, the node whose weight is required
!
!    Input, external RECUR ( ), the function which defines the orthogonal
!    polynomials.  See EXTEND for a full description.
!
!    Input, real ( kind = rk ) H0, th eintegral of the orthogonality weight
...
!    when the Christoffel-Darboux identity is used (N = M).
!
!    Output, real ( kind = rk ) WT, the weight associated with XNODE.
!
  implicit none
