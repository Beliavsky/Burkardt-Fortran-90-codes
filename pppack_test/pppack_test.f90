...
!
    do i = 1, n
      tau(i) =  ( real ( n - i,     kind = rk ) * ( -1.0D+00 )   &
...
!
      do j = 1, istep
        ...
        x = ( real ( istep - j, kind = rk ) * tau(i-1)   &
            + real (         j, kind = rk ) * tau(i)   ) &
            / real ( istep,     kind = rk )
    ...
    if ( 2 < n ) then
      decay = ( aloger - algerp ) &
            / log ( real ( n, kind = rk ) /real ( n - 2, kind = rk ) )
    end if
...
!
  ...
  real ( kind = rk ) bcoef(23)
  real ( kind = rk ), dimension ( 4 ) :: d0 = (/ &
...
!
     x = ( real ( npoint - i,     kind = rk ) * t(k) &
         + real (          i - 1, kind = rk ) * t(n+1) ) &
         / real ( npoint     - 1, kind = rk )
!
...
!
  ...
  real ( kind = rk ) biatx(4)
  integer i
  integer left
  real ( kind = rk ), dimension ( 11 ) :: t = (/ &
    0.0D+00, 0.0D+00, 0.0D+00, 0.0D+00, 1.0D+00, &
    3.0D+00, 4.0D+00, 6.0D+00, 6.0D+00, 6.0D+00, &
    6.0D+00 /)
  real ( kind = rk ) values(4)
  real ( kind = rk ) x
  ...
  do i = 1, 40
    ...
    x = ( real ( 40 - i, kind = rk ) * ( -1.0D+00 )   &
        + real (      i, kind = rk ) * (  7.0D+00 ) ) &
        / real ( 40,     kind = rk )
...
!
  ...
  real ( kind = rk ), dimension ( n) :: bcoef = (/ &
    0.0D+00, 0.0D+00, 0.0D+00, 1.0D+00, 0.0D+00, &
    0.0D+00, 0.0D+00 /)
  real ( kind = rk ) break(5)
  real ( kind = rk ) coef(4,4)
  integer i
  integer l
  real ( kind = rk ) ppvalu
  real ( kind = rk ) scrtch(4,4)
  real ( kind = rk ), dimension(n+k) :: t = (/ &
    0.0D+00, 0.0D+00, 0.0D+00, 0.0D+00, 1.0D+00, &
    3.0D+00, 4.0D+00, 6.0D+00, 6.0D+00, 6.0D+00, &
    6.0D+00 /)
...
!
  ...
  real ( kind = rk ) bcoef(1)
  real ( kind = rk ) bvalue
  integer i
  real ( kind = rk ), dimension ( 5 ) :: t = (/ &
...
!
  ...
  do run = 1, 3
    ...
    do n = nlow, nhigh, 2
      ...
      do i = 1, n
        tau(i) = &
          ( real ( ( n - 1 )**r - ( i - 1 )**r, kind = rk ) * ( -1.0D+00 )   &
          + real (                ( i - 1 )**r, kind = rk ) * ( +1.0D+00 ) ) &
          / real ( ( n - 1 )**r,                kind = rk )
      end do
...
!
  ...
  real ( kind = rk ) pnatx
  integer run
  real ( kind = rk ) scrtch(2,n_max)
  real ( kind = rk ) step
  real ( kind = rk ) tau(n_max)
  real ( kind = rk ) taunew(n_max)
  real ( kind = rk ) x
...
!
  ...
  real ( kind = rk ) algerp
  real ( kind = rk ) aloger
  real ( kind = rk ) bcoef(22)
  real ( kind = rk ) break(n_max)
  real ( kind = rk ) c(4,n_max)
  real ( kind = rk ) decay
  real ( kind = rk ) dg
  real ( kind = rk ) ddg
...
!
      ...
      do i = 5, m
        t(i) = &
          ( real ( ( m - 3 )**r - ( i - 4 )**r, kind = rk ) * ( -1.0D+00 )   &
          + real (                ( i - 4 )**r, kind = rk ) * ( +1.0D+00 ) ) &
          / real ( ( m - 3 )**r,                kind = rk )
      end do
...
!
  ...
  real ( kind = rk ) amax
  real ( kind = rk ) bcoef(n_max)
  real ( kind = rk ) break(n_max)
  real ( kind = rk ) coef(n_max*4)
  real ( kind = rk ) dx
  real ( kind = rk ) fx
  real ( kind = rk ) g
  real ( kind = rk ) gtau(n_max)
  real ( kind = rk ) h
  integer i
  integer iflag
  integer istep
  ...
  real ( kind = rk ) round
  integer run
  real ( kind = rk ) scrtch(n_max*7)
  real ( kind = rk ) size
  real ( kind = rk ) step
  real ( kind = rk ) t(n_max+4)
  real ( kind = rk ) tau(n_max)
  real ( kind = rk ) x
!
...
!
      ...
      do i = 4, n
        ...
        do j = 2, istep
          ...
          x = break(i-2) - dx * real ( j - 1, kind = rk )
...
!
  ...
  real ( kind = rk ) scrtch(n_max*7)
  real ( kind = rk ) step
  real ( kind = rk ) t(n_max+6)
  real ( kind = rk ) tau(n_max)
  real ( kind = rk ) temp
  real ( kind = rk ) tnew(n_max)
  real ( kind = rk ) x
...
!
    do n = nlow, nhigh, 2
      ...
      h = 2.0D+00 / real ( n - 3, kind = rk )

      t(1:4) = -1.0D+00

      do i = 5, n
        t(i) = real ( i - 4, kind = rk ) * h - 1.0D+00
      end do
...
!
      ...
      if ( nlow < n ) then
        temp = real ( n, kind = rk ) / real ( n - 2, kind = rk )
...
!
  ...
  real ( kind = rk ) a(n)
  real ( kind = rk ) bvalue
  real ( kind = rk ) gtitan(ntitan)
  real ( kind = rk ) gtau(ntitan)
  integer i
  integer iflag
  integer, dimension ( n ) :: ipick = (/ &
    1, 5, 11, 21, 27, 29, 31, 33, 35, 40, 45, 49 /)
  integer lx
  real ( kind = rk ) scrtch(lenscr)
  real ( kind = rk ) t(n+k)
  real ( kind = rk ) tau(n)
  real ( kind = rk ) x(ntitan)
...
!
  do i = 1, npoint
    y(i) = real ( int ( y(i) * tenton + 0.5D+00 ), kind = rk ) / tenton
  end do
...
!
  ...
  real ( kind = rk ) break(122)
  real ( kind = rk ) coef(4,22)
  real ( kind = rk ) :: gamma = 2.5D+00
  real ( kind = rk ) gtau(49)
  real ( kind = rk ) gtitan(49)
  integer i
  integer iflag
  integer, dimension ( n ) :: ipick = (/ &
    1, 5, 11, 21, 27, 29, 31, 33, 35, 40, 45, 49 /)
  ...
  integer l
  integer lx
  real ( kind = rk ) plotf(npoint)
  real ( kind = rk ) plotts(npoint)
  real ( kind = rk ) plott(npoint)
  real ( kind = rk ) ppvalu
  real ( kind = rk ) scrtch(119)
  real ( kind = rk ) step
  real ( kind = rk ) tau(n)
  real ( kind = rk ) x(49)
  ...
  step = ( tau(n) - tau(1) ) / real ( npoint - 1, kind = rk )

  do i = 1, npoint
    plott(i) = tau(1) + step * real ( i - 1, kind = rk )
...
!
  ...
  real ( kind = rk ) break(npiece)
  real ( kind = rk ) ds
  integer i
  integer iflag
  integer l
  real ( kind = rk ) ppvalu
  real ( kind = rk ) q(n,kpkm1)
  integer run
  real ( kind = rk ) s(n)
  ...
  real ( kind = rk ) xcoef(k,npiece)
  real ( kind = rk ) xx(npoint)
  real ( kind = rk ) y(n)
  real ( kind = rk ) ycoef(k,npiece)
  real ( kind = rk ) yy(npoint)
...
!
    ss = s(3)
    ds = ( s(6) - s(3) ) / real ( npoint - 1, kind = rk )
...
!
  ...
  real ( kind = rk ) bcoef(nx,ny)
  real ( kind = rk ) bvalue
  real ( kind = rk ), external :: f03
  real ( kind = rk ) fdata(nx,ny)
  integer i
  integer iflag
  integer j
  integer jj
  ...
  integer mflag
  real ( kind = rk ) q(nq)
  real ( kind = rk ) taux(nx)
  real ( kind = rk ) tauy(ny)
  real ( kind = rk ) tx(nx+kx)
  real ( kind = rk ) ty(ny+ky)
  real ( kind = rk ) work1(nx,ny)
  real ( kind = rk ) work2(nx)
  real ( kind = rk ) work3(nx,ny)
...
!
  ...
  real ( kind = rk ) bcoef(nx,ny)
  real ( kind = rk ) breakx(6)
  real ( kind = rk ) breaky(4)
  real ( kind = rk ) coef(kx,5,ky,3)
  real ( kind = rk ), external :: f03
  real ( kind = rk ) fdata(nx,ny)
  integer i
  integer iflag
  ...
  real ( kind = rk ) work2(nx)
  real ( kind = rk ) work3(nx,ny)
  real ( kind = rk ) work4(kx,kx,ny)
  real ( kind = rk ) work5(ny,kx,5)
  real ( kind = rk ) work6(ky,ky,21)
!
...
!
  ...
  integer prfun
  integer prpco
  real ( kind = rk ) q(ltkmax)
  integer run
  real ( kind = rk ) scrtch(ntmax)
  real ( kind = rk ) t(ntmax)
  real ( kind = rk ) tau
  real ( kind = rk ) totalw
  real ( kind = rk ) weight
...
!
  ...
  l = 6
  step = 1.0D+00 / real ( l, kind = rk )
  k = 2

  do i = 1, l + 1
    break(i) = real ( i - 1, kind = rk ) * step
  end do
...
!
  ...
  roun ( x ) = real ( int ( x * 100.0D+00 ), kind = rk ) / 100.0D+00

  icount = icount + 1
  ntau = 65

  do i = 1, ntau
    tau(i) = ( real ( ntau - i,     kind = rk ) * 0.0D+00   &
             + real (        i - 1, kind = rk ) * 3.0D+00 ) &
             / real ( ntau     - 1, kind = rk )
  end do
...
!
  ...
  real ( kind = rk ) break
  real ( kind = rk ), dimension ( n ) :: brkpic = (/ &
...
!
        do j = 1, istep
          ...
          x = ( real ( istep - j,     kind = rk ) * tau(i-1) &
              + real (         j - 1, kind = rk ) * tau(i) ) &
              / real ( istep     - 1, kind = rk )
...
!
!    Output, real ( kind = rk ) RUNGE, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) RUNGEP, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) RUNGEPP, the value of the function.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) rungepp
  real ( kind = rk ) u
  real ( kind = rk ) up
  real ( kind = rk ) v
  real ( kind = rk ) vp
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) F02, the value of the function.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, Y, the arguments of the function.
!
!    Output, real ( kind = rk ) F03, the value of the function.
!
  implicit none
