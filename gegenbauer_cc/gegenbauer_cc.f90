...
!  Parameters:
!
!    Input, real ( kind = rk ) ORDER, the order.
!    0.0 <= ORDER.
!
!    Input, real ( kind = rk ), X, the evaluation point. 
!
!    Output, real ( kind = rk ) BESSELJ, the value.
!
  implicit none
...
!    1 <= N.
!
!    Input, real ( kind = rk ), external F(x), the function to be 
!    integrated with the Gegenbauer weight.
!
!    Output, real ( kind = rk ) A2(0:N/2), the even Chebyshev coefficients of F.
!
  implicit none
...
!    1 <= N.
!
!    Input, real ( kind = rk ) F(x), thefunction to be 
!    integrated with the Gegenbauer weight.
!
!    Output, real ( kind = rk ) B2(0:N/2), the even Chebyshev coefficients of F.
!
  implicit none
  ...
  do r = 0, 2 * s, 2
    total = 0.0D+00
    do j = 0, n
      x1 = real ( 2 * j + 1, kind = rk ) * r8_pi &
        / 2.0D+00 / real ( n + 1, kind = rk )
      x2 = real ( r * ( 2 * j + 1 ), kind = rk ) * r8_pi &
        / 2.0D+00 / real ( n + 1, kind = rk )
      total = total + f ( cos ( x1 ) ) * cos ( x2 )
    end do
    rh = r / 2
    b2(rh) = ( 2.0D+00 / real ( n + 1, kind = rk ) ) * total
  end do
...
!    1 <= N.
!
!    Input, real ( kind = rk ) LAMBDA, used in the exponent of (1-x^2).
!    -0.5 < LAMBDA.
!
!    Input, real ( kind = rk ), external, F(x), the function to be integrated 
!    with the Gegenbauer weight.
!
!    Output, real ( kind = rk ) WEIGHT, the estimate for the Gegenbauer 
!    integral of F.
!
  implicit none
  ...
  rh = s
  u = 0.5D+00 * real ( sigma + 1, kind = rk ) * a2(rh)
  do rh = s - 1, 1, -1
    u = ( real ( rh, kind = rk ) - lambda ) &
      / ( real ( rh, kind = rk ) + lambda + 1.0D+00 ) * u + a2(rh)
  end do
...
!    -0.5 < LAMBDA.
!
!    Input, real ( kind = rk ), external F(x), the function to be integrated with
...
!
  ...
  rh = s
  u = real ( sigma + 1, kind = rk ) * b2(rh)
  do rh = s - 1, 1, -1
    u = ( real ( rh, kind = rk ) - lambda ) &
      / ( real ( rh, kind = rk ) + lambda + 1.0D+00 ) * u + b2(rh)
  end do
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, non-negative real argument for which
!    J's are to be calculated.
!
!    Input, real ( kind = rk ) ALPHA, fractional part of order for which
!    J's or exponentially scaled J'r (J*exp(X)) are
!    to be calculated.  0 <= ALPHA < 1.0.
!
!    Input, integer NB, number of functions to be calculated, 
!    NB > 0.  The first function calculated is of order ALPHA, and the
!    last is of order (NB - 1 + ALPHA).
!
!    Output, real ( kind = rk ) B(NB).  If RJBESL
!    terminates normally (NCALC=NB), the vector B contains the
!    functions J/ALPHA/(X) through J/NB-1+ALPHA/(X), or the
!    corresponding exponentially scaled functions.
!
!    Output, integer NCALC, indicates possible errors.
