...
!  Parameters:
!
!    Input, real ( kind = rk ) FATOL, an absolute error tolerance for
...
!    to be sought.
!
!    Input, real ( kind = rk ) XATOL, an absolute error tolerance for the root.
!
!    Input/output, real ( kind = rk ) XA, XB, two points at which the
!    function differs in sign.  On output, these values have been adjusted
!    to a smaller interval.
!
!    Input/output, real ( kind = rk ) FXA, FXB, the value of the function
!    at XA and XB.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) FATOL, an absolute error tolerance for the
...
!    to be sought.
!
!    Input, real ( kind = rk ) XATOL, XRTOL, absolute and relative error
...
!    tolerances  for the root.
!
!    Input/output, real ( kind = rk ) XA, XB, XC, three points.
!
!    Input/output, real ( kind = rk ) FXA, FXB, FXC, the value of the
...
!    Input, real ( kind = rk ) XATOL, an absolute error tolerance for the root.
!
!    Input, real ( kind = rk ) XMIN, XMAX, the interval in which the root should
!    be sought.
!
!    Input/output, real ( kind = rk ) XA.  On input, the starting point for
!    the iteration.  On output, the current approximation to the root.
!
!    Input/output, real ( kind = rk ) FXA, the function value at XA.
!
  implicit none
...
!
!    Output, real ( kind = rk ) FX, the value of the function at X.
!
  implicit none
...
!    Input, integer PROB, the number of the problem.
!
!    Input, real ( kind = rk ) X, the abscissa.
!
!    Output, real ( kind = rk ) FX1, the first derivative of the function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the abscissa.
!
!    Output, real ( kind = rk ) FX2, the second derivative of the function at X.
!
  implicit none
...
!    Input, integer PROB, the number of the problem.
!
!    Output, real ( kind = rk ) RANGE(2), the minimum and maximum values of
...
!
!    Output, real ( kind = rk ) X, the I-th root.
!
  implicit none
...
!
!    Output, real ( kind = rk ) X, the starting point.
!
  implicit none
...
!
!    Output, real ( kind = rk ) X, the value of the root.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) X, the value of the root.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the abscissa.
!
!    Output, real ( kind = rk ) FX2, the second derivative at X.
!
  implicit none
...
!    A must not be zero.
!
!    Output, real ( kind = rk ) R1, R2, the real parts of the two roots
!    of the polynomial.
!
  implicit none
...
!    Input, real ( kind = rk ) XATOL, an absolute error tolerance for the root.
!
!    Input, real ( kind = rk ) XMAX, XMIN, the interval in which the root should
!    be sought.
!
!    Input/output, real ( kind = rk ) XA, XB, two points at which the
!    function differs in sign.  On output, these values have been adjusted
