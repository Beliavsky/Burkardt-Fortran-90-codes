...
!
!    Input/output, real ( kind = rk ) TABLE(M,N).  On input, the dataset to
...
!
  ...
  do i = 1, m
    ...
    do j = 1, n
      table(i,indx(j)) =  ( real ( n - j,     kind = rk ) * v_min   &
                          + real (     j - 1, kind = rk ) * v_max ) &
                          / real ( n     - 1, kind = rk )
    end do
...
!
!    Output, real ( kind = rk ) DVEC(N), the values read from the string.
!
!    Output, integer IERROR, error flag.
!    0, no errors occurred.
