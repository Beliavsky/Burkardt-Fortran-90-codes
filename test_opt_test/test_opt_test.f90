...
!*****************************************************************************80
!
!! MAIN is the main program for TEST_OPT_TEST.
!
!  Discussion:
!
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: g_dif
  integer problem_num
  integer n
  integer n_min
  integer problem
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: h_dif
  integer i
  integer problem_num
  integer n
  integer n_min
...
!
  ...
  real ( kind = rk ) f
  real ( kind = rk ) f2
  real ( kind = rk ) g(n_max)
  integer i
  integer, parameter :: max_step = 6
  integer problem_num
  integer n
  integer n_min
  integer problem
  ...
  logical quit
  integer reduce
  integer, parameter :: reduce_max = 10
  real ( kind = rk ) s
  integer seed
  character ( len = 50 ) title
  real ( kind = rk ) x(n_max)
  real ( kind = rk ) x2(n_max)
!
!  Initialize the P25 parameter.
!
  p = 0.0D+00
