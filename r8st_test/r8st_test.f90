...
!
  ...
  integer, parameter :: nz_max = 30

  real ( kind = rk ) a(nz_max)
  real ( kind = rk ), allocatable :: b(:)
  integer col(nz_max)
  integer i
  integer it_max
...
!  Set the matrix.
!
  call r8st_random ( m, n, nz_num, row, col, a )
!
!  Make an R8GE copy.
!
...
!
  ...
  real ( kind = rk ), allocatable :: a_r8ge(:,:)
  real ( kind = rk ), allocatable :: a_r8st(:)
  integer, allocatable :: col(:)
  integer m
  integer n
  integer nz_num
