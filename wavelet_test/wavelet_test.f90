...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'wavelet_test():'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Test wavelet().'
!
!  Test transforms.
!
  call test01 ( )
...
!
  ...
  real ( kind = rk ) daub2_scale
  real ( kind = rk ) daub4_scale
  real ( kind = rk ) daub6_scale
  real ( kind = rk ) daub8_scale
  real ( kind = rk ) daub10_scale
  integer n
  integer seed
  integer test
  real ( kind = rk ) x
  ...
  do test = 1, 5
    write ( *, '(a)' ) ' '
    call random_number ( harvest = x_save )
    do n = 1, 6
      x = x_save
      y1 = daub2_scale ( n, x )
      x = x_save
