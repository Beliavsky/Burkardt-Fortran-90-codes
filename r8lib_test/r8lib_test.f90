...
!
  ...
  call r8vec_shift_circular_test ( )
...
!
  ...
  real ( kind = rk ) :: rhi = 200.0D+00
  real ( kind = rk ) rhi2
  real ( kind = rk ) :: rlo = 100.0D+00
  real ( kind = rk ) rlo2
  ...
  write ( *, '(a)' ) '  i4int_to_r8int() converts a value in an interval'
  write ) *, '(a)' ) '  from an integer to a real;'
  write ( *, '(a)' ) ''
...
!
  ...
  integer ierror
  integer, dimension ( n ) :: p1 = (/ &
    5, 2, 3, 4, 1 /)
  integer, dimension ( n ) :: p2 = (/ &
    4, 1, 3, 0, 2 /)
  integer, dimension ( n ) :: p3 = (/ &
    0, 2, 1, 3, 2 /)
  integer perm0_check
...
!
  ...
  real ( kind = rk ) r8
  real ( kind = rk ) r8_abs
  real ( kind = rk ) r8_absolute
  real ( kind = rk ) r8_uniform_ab
  real ( kind = rk ) :: r8_hi = +5.0D+00
  real ( kind = rk ) :: r8_lo = -5.0D+00
...
!
  ...
  do test = 0, 8
    ...
    x = 1.0D+00 + real ( test, kind = rk ) / 2.0D+00
    a = r8_acosh ( x )
    x2 = cosh ( a )
...
!
  ...
  write ( *, '(a)' ) ''
  do test = 0, test_num
    x = r8_pi ( ) * real ( test, kind = rk ) / real ( test_num, kind = rk )
...
!
  ...
  do i = 0, 24
    angle = r8_pi * real ( i, kind = rk ) / 12.0D+00
    if ( mod ( i, 12 ) == 0 ) then
...
!
  ...
  do i = 0, 24
    angle = real ( i, kind = rk ) * r8_pi / 12.0D+00
    if ( mod ( i, 12 ) == 0 ) then
      write ( *, '(2x,f8.2,2x,a)' ) angle, '  Undefined'
    else
...
!
  ...
  do i = 0, 24
    radians = real ( i, kind = rk ) * r8_pi / 12.0D+00
...
!
  ...
  do i = 1, 10
    ...
    i1 = i4_uniform_ab ( -100, +100 )
    i2 = i4_uniform_ab ( -100, +100 )
    if ( i2 == 0 ) then
      i2 = 1
    end if
...
!
  ...
  n = 1
  do i = 0, 20
    value2 = ( real ( n + 1, kind = rk ) / real ( n, kind = rk ) ) ** n
...
!
  ...
  real ( kind = rk ) value
  real ( kind = rk ) x
  real ( kind = rk ), dimension ( 11 ) :: x_test = (/ &
...
!
  ...
  real ( kind = rk ) fractional
  real ( kind = rk ) r8
  real ( kind = rk ) r8_fractional
  real ( kind = rk ) r8_uniform_ab
  real ( kind = rk ) :: r8_hi = 5.0D+00
  real ( kind = rk ) :: r8_lo = -3.0D+00
...
!
  ...
  do i = 1, 10
    a = r8_uniform_ab ( r8_lo, r8_hi )
    b = r8_uniform_ab ( r8_lo, r8_hi )
    c = r8_max ( a, b )
...
!
  ...
  real ( kind = rk ) x
  real ( kind = rk ) :: x_hi =  10.0D+00
  real ( kind = rk ) :: x_lo = -10.0D+00
  real ( kind = rk ) y
  real ( kind = rk ) z1
  real ( kind = rk ) z2
  ...
  do test = 1, test_num
    ...
    x = r8_uniform_ab ( x_lo, x_hi )
    y = r8_uniform_ab ( x_lo, x_hi )
...
!
  ...
  do test = 1, 10
    i4 = i4_uniform_ab ( i4_min, i4_max )
    r8 = r8_mop ( i4 )
...
!
  ...
  do i = 0, 24
    degrees = real ( i, kind = rk ) * 360.0D+00 / 24.0D+00
...
!
  ...
  real ( kind = rk ) value
  real ( kind = rk ) x
  real ( kind = rk ), dimension ( 25 ) :: x_test = (/ &
...
!
  ...
  do i = 1, 10
    r1 = r8_uniform_ab ( -5.0D+00, +5.0D+00 )
    r2 = r8_uniform_ab ( -5.0D+00, +5.0D+00 )
...
!
  ...
  a = r8_uniform_ab ( -5.0D+00, +5.0D+00 )
  b = r8_uniform_ab ( -5.0D+00, +5.0D+00 )
  c = r8_uniform_ab ( -5.0D+00, +5.0D+00 )
  ...
  do i = 0, 10
    x = real ( i, kind = rk ) * r8_pi / 10.0D+00
...
!
  ...
  real ( kind = rk ) r
  real ( kind = rk ) rd
  real ( kind = rk ) :: rhi = 10.0D+00
  real ( kind = rk ) rhi2
  real ( kind = rk ) :: rlo = 1.0D+00
  real ( kind = rk ) rlo2
...
!
  ...
  real ( kind = rk ), parameter :: b = 10.0D+00
  real ( kind = rk ), parameter :: c = 20.0D+00
  real ( kind = rk ) r8_uniform_ab
  integer i
  real ( kind = rk ) r
...
!
  ...
  real ( kind = rk ), parameter :: a = - 2.0D+00
  real ( kind = rk ), parameter :: b = 12.0D+00
  real ( kind = rk ) r
  real ( kind = rk ) r2
  real ( kind = rk ) r8_uniform_ab
  real ( kind = rk ) r8_wrap
  real ( kind = rk ), parameter :: rhi = 6.5D+00
  real ( kind = rk ), parameter :: rlo = 3.0D+00
...
!
  real ( kind = rk ), dimension ( n, 2 ) :: v = reshape ( (/ &
...
!
  ...
  do test = 1, test_num
    ...
    call r8mat_uniform_01 ( 2, n, x )

    x(1:2,1:n) = real ( nint ( 3.0D+00 * x(1:2,1:n) ), kind = rk )
...
!
  ...
  real ( kind = rk ) a(2,n)
  real ( kind = rk ) :: b = 0.0D+00
  real ( kind = rk ) :: c = 10.0D+00
  integer l
  integer r
  ...
  call r8mat_uniform_ab ( 2, n, b, c, a )
...
!  FORTRAN 2D arrays are listed in row order.
!
  real ( kind = rk ), dimension ( n, 3 ) :: v = reshape ( (/ &
    11.0,  21.0, 31.0, 41.0, 51.0, 61.0, 71.0, 81.0, 91.0, 101.0, &
...
!  FORTRAN arrays are listed in row order.
!
  real ( kind = rk ), dimension ( 3, n ) :: v = reshape ( (/ &
     11.0,  21.0, 31.0, &
     12.0,  22.0, 32.0, &
     13.0,  23.0, 33.0, &
     14.0,  24.0, 34.0, &
...
!
  ...
  integer, parameter :: lfat = 1
  integer, parameter :: mfat = 2
  integer, parameter :: nfat = 1
  integer, parameter :: l2 = ( l - 1 ) * ( lfat + 1 ) + 1
  integer, parameter :: m2 = ( m - 1 ) * ( mfat + 1 ) + 1
  integer, parameter :: n2 = ( n - 1 ) * ( nfat + 1 ) + 1
  real ( kind = rk ), dimension(l,m,n) :: x = reshape ( (/ &
...
!
  ...
  real ( kind = rk ) :: x_max = 4.0D+00
  real ( kind = rk ) :: x_min = 1.0D+00
  real ( kind = rk ) xval
  real ( kind = rk ) y(n_max)
  real ( kind = rk ) :: y_max = 3.0D+00
  real ( kind = rk ) :: y_min = 1.0D+00
  real ( kind = rk ) yval
  ...
  do i = 1, n_max
    ...
    xval = r8_uniform_ab ( x_min, x_max )
    xval = real ( nint ( xval ), kind = rk )
    yval = r8_uniform_ab ( y_min, y_max )
    yval = real ( nint ( yval ), kind = rk )
...
!
  ...
  do i = 1, n_max
    ...
    xval = r8_uniform_ab ( 1.0D+00, 4.0D+00 )
    xval = real ( nint ( xval ), kind = rk )
    yval = r8_uniform_ab ( 1.0D+00, 3.0D+00 )
    yval = real ( nint ( yval ), kind = rk )
    zval = r8_uniform_ab ( 1.0D+00, 4.0D+00 )
    zval = real ( nint ( zval ), kind = rk )
...
!
  ...
  call r8mat_uniform_01 ( n, n, a )
  a = ( a + transpose ( a ) ) / 2.0D+00
  do i = 1, n
    a(i,i) = a(i,i) + real ( n, kind = rk )
  end do
...
!
  ...
  do i = 1, m
    do j = 1, n
      a(i,j) = 10.0D+00 * real ( i, kind = rk ) + real ( j, kind = rk )
    end do
  end do
...
!
  x(1:n,1) = 1.0D+00
  do i = 1, n
    x(i,2) = real ( i, kind = rk )
  end do
  do i = 1, n
    x(i,3) = real ( 1 + mod ( i - 1, 3 ), kind = rk )
  end do
...
!
!    Input, real ( kind = rk ) X(N), the parameter values.
!
...
!    Input, real ( kind = rk ) X(N), the parameter values.
!
!    Output, real ( kind = rk ) H(N,N), the Hessian values.
!
  implicit none
...
!
  ...
  call r8mat_uniform_ab ( n, n, r8_lo, r8_hi, a )
...
!
  ...
  real ( kind = rk ) a(n,n)
  real ( kind = rk ) ah(n,n)
  real ( kind = rk ), parameter :: b = 0.0D+00
  real ( kind = rk ), parameter :: c = 5.0D+00
  real ( kind = rk ) h(n,n)
  integer row
  integer col
  ...
  call r8mat_uniform_ab ( n, n, b, c, a )
...
!    Input, real ( kind = rk ) X(N), the parameter values.
!
!    Output, real ( kind = rk ) F(M), the function values.
!
  implicit none
...
!    Input, real ( kind = rk ) X(N), the parameter values.
!
!    Output, real ( kind = rk ) FPRIME(M,N), the jacobian values.
!
  implicit none
...
!
  ...
  integer, parameter :: m = m1 * m2
  integer, parameter :: n1 = 3
  integer, parameter :: n2 = 2
  integer, parameter :: n = n1 * n2

  real ( kind = rk ), dimension (m1,n1) :: a = reshape ( (/ &
    1.0D+00, 4.0D+00, &
    2.0D+00, 5.0D+00, &
    3.0D+00, 6.0D+00 /), (/ 2, 3 /) )
  real ( kind = rk ), dimension (m2,n2) :: b = reshape ( (/ &
                        ...
                        33.0D+00, 43.0D+00, 53.0D+00, 63.0D+00, 73.0D+00, &
                                  44.0D+00, 54.0D+00, 64.0D+00, 74.0D+00, &
                                            55.0D+00, 65.0D+00, 75.0D+00, &
                                                      66.0D+00, 76.0D+00, &
                                                                77.0D+00 /)
  real ( kind = rk ), dimension(18) :: a2 = (/ &
    11.0D+00, 21.0D+00, 31.0D+00, 41.0D+00, 51.0D+00, 61.0D+00, 71.0D+00, &
              22.0D+00, 32.0D+00, 42.0D+00, 52.0D+00, 62.0D+00, 72.0D+00, &
                        33.0D+00, 43.0D+00, 53.0D+00, 63.0D+00, 73.0D+00 /)
  real ( kind = rk ), dimension(10) :: a3 = (/ &
    11.0D+00, 21.0D+00, 31.0D+00, 41.0D+00, &
              22.0D+00, 32.0D+00, 42.0D+00, &
                        33.0D+00, 43.0D+00, &
                                  44.0D+00 /)
  integer, parameter :: m1 = 7
  integer, parameter :: m2 = 7
  integer, parameter :: m3 = 4
...
!
  ...
  real ( kind = rk ) a(m,n)
  real ( kind = rk ), parameter :: b = 0.0D+00
  real ( kind = rk ), parameter :: c = 10.0D+00
  ...
  call r8mat_uniform_ab ( m, n, b, c, a )
...
!
  ...
  real ( kind = rk ), dimension ( m, n ) :: a = reshape ( (/ &
    ...
    1.0, 1.0, 1.0, 1.0 /), (/ m, n /) )
  real ( kind = rk ) b(m)
  real ( kind = rk ), dimension ( n ) :: x = (/ 1.0, 2.0 /)
...
!
  ...
  call r8mat_uniform_ab ( m, n, x1, x2, a )
...
!
  ...
  call r8mat_uniform_01 ( m, n, a )
  call r8mat_uniform_01 ( m, n, b )
...
!
  ...
  call r8mat_uniform_ab ( m, n, a, b, x )
...
!
  ...
  real ( kind = rk ), dimension ( 2, 2 ) :: a1 = reshape ( (/ &
    1.0D+00, 3.0D+00, &
    2.0D+00, 4.0D+00 /), (/ 2, 2 /) )
  real ( kind = rk ), dimension ( 3, 3 ) :: a2 = reshape ( (/ &
    2.0D+00, 1.0D+00, 1.0D+00, &
    1.0D+00, 1.0D+00, 0.0D+00, &
    1.0D+00, 0.0D+00, 1.0D+00 /), (/ 3, 3 /) )
  real ( kind = rk ), dimension ( 4, 4 ) :: a3 = reshape ( (/ &
...
!
  ...
  real ( kind = rk ), dimension (m,n) :: a
  real ( kind = rk ), parameter :: b = 2.0D+00
  real ( kind = rk ), parameter :: c = 10.0D+00
...
!
  ...
  real ( kind = rk ), dimension ( m * n ) :: r8rows = (/ &
              ...
              32.0D+00, 42.0D+00, 52.0D+00, 62.0D+00, 72.0D+00, &
                        43.0D+00, 53.0D+00, 63.0D+00, 73.0D+00, &
                                  54.0D+00, 64.0D+00, 74.0D+00, &
                                            65.0D+00, 75.0D+00, &
                                                      76.0D+00 /)
  real ( kind = rk ), dimension ( 15 ) :: a2 = (/ &
    21.0D+00, 31.0D+00, 41.0D+00, 51.0D+00, 61.0D+00, 71.0D+00, &
              32.0D+00, 42.0D+00, 52.0D+00, 62.0D+00, 72.0D+00, &
                        43.0D+00, 53.0D+00, 63.0D+00, 73.0D+00 /)
  real ( kind = rk ), dimension ( 6 ) :: a3 = (/ &
    21.0D+00, 31.0D+00, 41.0D+00, &
              32.0D+00, 42.0D+00, &
                        43.0D+00 /)
  integer m
  integer, dimension ( test_num ) :: m_test = (/ 7, 7, 4 /)
  integer n
  integer, dimension ( test_num ) :: n_test = (/ 7, 3, 7 /)
  integer size
  integer, dimension ( test_num ) :: size_test = (/ 21, 15, 6 /)
  integer test
...
!
  ...
  call r8vec_uniform_ab ( n, r8_lo, r8_hi, a )
...
!
  ...
  integer, parameter :: n2 = 3
  integer, parameter :: n3 = n1 + n2

  real ( kind = rk ), dimension ( n1 ) :: a1 = (/ &
    91.1, 31.2, 71.3, 51.4, 31.5 /)
  real ( kind = rk ), dimension ( n2 ) :: a2 = (/ &
    42.6, 22.7, 12.8 /)
  real ( kind = rk ) a3(n3)
...
!
  ...
  real ( kind = rk ) z(n)
  real ( kind = rk ), dimension ( n ) :: z_correct = (/ &
...
!
  ...
  real ( kind = rk ) r
  real ( kind = rk ), dimension(n) :: v1 = (/ &
    43.0D+00, 21.0D+00, 25.0D+00, 42.0D+00, 57.0D+00, 59.0D+00 /)
  real ( kind = rk ), dimension(n) :: v2 = (/ &
...
!
  ...
  real ( kind = rk ) fdif
  real ( kind = rk ) :: h = 0.01D+00
  integer i
  real ( kind = rk ) r8vec_dif_f
  real ( kind = rk ) :: x = 1.0D+00
  real ( kind = rk ) xi
...
!
  ...
  call r8vec_uniform_ab ( n, r8_lo, r8_hi, v1 )
  call r8vec_uniform_ab ( n, r8_lo, r8_hi, v2 )
...
!
  ...
  integer, parameter :: factor_num = 3
  integer, parameter :: point_num = 24

  integer factor_index
  integer factor_order
  real ( kind = rk ), allocatable, dimension ( : ) :: factor_value
  integer j
...
!
  ...
  call r8vec_uniform_01 ( n, v1 )
  call r8vec_uniform_01 ( n, v2 )
...
!
  ...
  integer istar
  integer jstar
  integer, dimension(nold-1) :: nfill = (/ 4, 3, 5, 0 /)
  integer nval
  real ( kind = rk ), dimension(nold) :: xold = (/ &
...
!
  ...
  integer, parameter :: n = 6
  integer, parameter :: fat = 3
  integer, parameter :: nfat = ( n - 1 ) * ( fat + 1 ) + 1

  real ( kind = rk ), dimension(n) :: x =  (/ &
    16.0D+00, 4.0D+00, 0.0D+00, 4.0D+00, 16.0D+00, 36.0D+00 /)
  real ( kind = rk ) xfat(nfat)
...
!
  ...
  integer, parameter :: histo_num = 20
  ...
  real ( kind = rk ) bin_hi
  real ( kind = rk ) bin_lo
  integer, dimension (0:histo_num+1) :: histo_gram
  integer i
  ...
  do test = 1, test_num
    ...
    do i = 0, histo_num+1
      ...
      if ( i == 0 ) then
        ...
        bin_lo = ( real ( histo_num - i + 1, kind = rk ) * a_lo   &
                 + real (             i - 1, kind = rk ) * a_hi ) &
                 / real ( histo_num,         kind = rk )

        bin_hi = ( real ( histo_num - i,     kind = rk ) * a_lo   &
                 + real (             i,     kind = rk ) * a_hi ) &
                 / real ( histo_num,         kind = rk )
...
!
  ...
  do i = 1, 20
    xval = r8_uniform_ab ( 0.0D+00, 20.0D+00 )
    xval = real ( nint ( xval ), kind = rk )
    write ( *, '(4x,f6.2)' ) xval
    call r8vec_index_insert ( n, x, indx, xval )
  end do
...
!
  ...
  b = 0.0D+00
  c = real ( n_max, kind = rk )


  do i = 1, n_max
    xval = r8_uniform_ab ( b, c )
    xval = real ( nint ( xval ), kind = rk )
    write ( *, '(4x,f6.2)' ) xval
...
!
  ...
  do test = 1, 5
    ...
    call random_number ( harvest = r_lo )
    call random_number ( harvest = r_hi )
...
!
  ...
  call r8vec_uniform_ab ( n, x1, x2, a )
...
!
  ...
  call r8vec_uniform_ab ( n, r8_lo, r8_hi, x )
...
!
  ...
  call r8vec_uniform_ab ( n, r8_lo, r8_hi, x )
  call r8vec_uniform_ab ( n, r8_lo, r8_hi, y )
  z(1:n) = x(1:n) - y(1:n)
...
!
  ...
  call r8vec_uniform_ab ( n, a_lo, a_hi, a )
...
!
  ...
  integer test
  real ( kind = rk ) x(n)
  real ( kind = rk ), dimension ( n, test_num ) :: x_test = reshape ( (/ &
...
!
  ...
  integer, dimension ( n ) :: p = (/ 2, 4, 5, 1, 3 /)
  real ( kind = rk ), dimension (n) :: x = (/ &
...
!
  ...
  call r8vec_permute_uniform ( n, a )
...
!
  ...
  real ( kind = rk ) rsquared_adjusted
  real ( kind = rk ), dimension ( n ) :: y_model = (/ &
     0.00D+00,  9.00D+00, 16.00D+00, 21.00D+00, 24.00D+00, &
    25.00D+00, 24.00D+00, 21.00D+00, 16.00D+00,  9.00D+00,  &
     0.00D+00 /)
...
subroutine r8vec_shift_circular_test ( )
...
!
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'r8vec_shift_circular_test():'
  write ( *, '(a)' ) '  r8vec_shift_circular() performs a circular shift'
  write ( *, '(a)' ) '  of K positions on an R8VEC.'

  a(1:n) = (/ 1.0D+00, 2.0D+00, 3.0D+00, 4.0D+00, 5.0D+00 /)
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  Original vector A:'
  write ( *, '(a)' ) ''
  write ( *, '(2x,2x,2x,5f4.1)' ) a(1:n)
  ...
  do k = -2, 3
    ...
    b(1:n) = a(1:n)
    call r8vec_shift_circular ( k, n, b )
    write ( *, '(2x,i2,2x,5f4.1)' ) k, b(1:n)
...
!
  ...
  integer, dimension ( mask_num ) :: mask = (/ &
...
!
  ...
  call r8vec_uniform_01 ( na, a )
  call r8vec_uniform_01 ( nb, b )
...
!
  ...
  a(1:n) = real ( nint ( a(1:n) ), kind = rk ) / 2.0D+00
...
!
  ...
  real ( kind = rk ), dimension ( x_num ) :: x_val = (/ &
    11.0, 11.0, 11.0, 22.0, 22.0, 33.0, 33.0, 55.0, 55.0 /)
  integer xdnu(x_num)
  real ( kind = rk ), allocatable, dimension ( : ) :: xu_val
...
!
  ...
  real ( kind = rk ), parameter :: tol = 0.25D+00
  integer unique_num
...
!
  ...
  integer, parameter :: unique_max = 30
  integer, parameter :: n = 30

  real ( kind = rk ) a(n)
  integer acount(unique_max)
  real ( kind = rk ) auniq(unique_max)
  real ( kind = rk ) b
  real ( kind = rk ) c
  integer i
  integer unique_num
  ...
  a(1:n) = real ( int ( a(1:n) ), kind = rk ) + 0.5D+00
...
!
  ...
  real ( kind = rk ), parameter :: a = 10.0D+00
  real ( kind = rk ), parameter :: b = 20.0D+00
  real ( kind = rk ) r(n)
...
!
  ...
  call r8vec_uniform_ab ( n, b,  c, a1 )

  b = 5.0D+00
  c = 10.0D+00

  call r8vec_uniform_ab ( n, b, c, a2 )
...
!
  ...
  do i = 1, n
    ...
    x(i) = real ( i4_uniform_ab ( 0, n ), kind = rk ) / real ( n, kind = rk )
    y(i) = real ( i4_uniform_ab ( 0, n ), kind = rk ) / real ( n, kind = rk )
