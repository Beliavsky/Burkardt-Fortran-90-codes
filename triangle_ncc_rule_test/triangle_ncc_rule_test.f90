...
!
  ...
  write ( *, '(a)' ) '  Test TRIANGLE_NCC_RULE().'
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: xytab
...
!
  ...
  real ( kind = rk ) area2
  integer node
  real ( kind = rk ), dimension ( dim_num, node_num ) :: node_xy = &
    reshape ( (/ &
    0.0D+00, 0.0D+00, &
    1.0D+00, 0.0D+00, &
    0.0D+00, 1.0D+00 /), (/ dim_num, node_num /) )
  real ( kind = rk ), dimension ( dim_num, node_num ) :: node_xy2 = &
    reshape ( (/ &
    1.0D+00, 2.0D+00, &
    1.0D+00, 1.0D+00, &
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: xy2
