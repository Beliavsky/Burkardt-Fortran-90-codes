...
!  Parameters:
!
!    Input, real ( kind = rk ) TET(3,4), the coordinates of the vertices.
...
!
  ...
  suborder_xyz(1:4,1:suborder_num) = &
      real ( 1 + suborder_xyz_n(1:4,1:suborder_num), kind = rk ) &
    / real ( 4 + suborder_xyz_d,                     kind = rk )
