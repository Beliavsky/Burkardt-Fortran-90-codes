...
!
!    Output, real ( kind = rk ) XYZ(3,4), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(4) = (/ &
    ...
    0.25000000000000000000D+00 /)
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,4) = reshape ( (/ &
...
!
!    Output, real ( kind = rk ) XYZ(3,ORDER), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(14) = (/ &
    ...
    0.042546020777081466438D+00 /)
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,14) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(14), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,14), the abscissas.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(15), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,15), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,15) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(24), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,24), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(24) = (/ &
    ...
    0.048214285714285714286D+00 /)
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,24) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) TETRAHEDRON_UNIT_VOLUME, the volume.
!
  implicit none
