...
!
  ...
  do problem = 1, problem_num
    ...
    do i = 1, 12
      ...
      quad = volume * sum ( fx(1:n) ) / real ( n, kind = rk )
  ...
  do problem = 1, problem_num
    ...
    do i = 1, 12
      ...
      do ix = 1, nx
        ...
        xval = ( real ( 2 * nx - 2 * ix + 1, kind = rk ) * a(1)   &
               + real (          2 * ix - 1, kind = rk ) * b(1) ) &
               / real ( 2 * nx,              kind = rk )

        do iy = 1, ny

          yval = ( real ( 2 * ny - 2 * iy + 1, kind = rk ) * a(2)   &
                 + real (          2 * iy - 1, kind = rk ) * b(2) ) &
                 / real ( 2 * ny,              kind = rk )
