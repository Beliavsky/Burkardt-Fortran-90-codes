...
!*****************************************************************************80
!
!! heat_mpi() solves the heat equation, using MPI for parallel processing.
!
!  Licensing:
!
...
!
  ...
  real ( kind = rk ) boundary_condition
  real ( kind = rk ) cfl
  real ( kind = rk ) h(0:n+1)
  integer h_file
  real ( kind = rk ) h_new(0:n+1)
  integer i
  integer id
  integer ierr
  real ( kind = rk ) initial_condition
  integer j
  integer j_max
  integer j_min
  real ( kind = rk ) k
  ...
  integer tag
  real ( kind = rk ) time
  real ( kind = rk ) time_delta
  real ( kind = rk ) time_max
  real ( kind = rk ) time_min
  real ( kind = rk ) time_new
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, TIME, the position and time.
!
!    Output, real ( kind = rk ) BOUNDARY_CONDITION, the boundary condition.
!
  implicit none
...
!    Input, real ( kind = rk ) X, TIME, the position and time.
!
!    Output, real ( kind = rk ) INITIAL_CONDITION, the initial condition.
!
  implicit none
...
!    Input, real ( kind = rk ) X, TIME, the position and time.
!
!    Output, real ( kind = rk ) RHS, the right hand side.
!
  implicit none
