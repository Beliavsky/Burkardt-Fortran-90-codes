...
!  Parameters:
!
!    Input, integer I, J, K, the indices of a voxel.
!
!    Input, integer NX, NY, NZ, the X, Y and Z dimensions
!    of the voxel data.
!
...
!
!    Input, integer IUNIT, the output unit number.
!
  implicit none
...
!    of the voxel data.
!
!    Input, integer I4VOXEL(NX,NY,NZ), an array of voxel data.
!
!    Input, integer IUNIT, the output unit number.
!
!    Output, integer NUM_FACE, the number of faces printed.
!
  implicit none
...
!    Input, integer I4VOXEL(NX,NY,NZ), an array of voxel data.
!
!    Output, integer NUM_POS, the number of strictly positive
...
!    of the voxel data.
!
!    Output, integer I4VOXEL(NX,NY,NZ), an array of voxel data.
!
...
!
  ...
  if ( ios /= 0 ) then
    ...
    write ( *, '(a)' ) '  Could not open the file "' // trim ( filename ) // '"'
    stop 1
  end if
!
!  Read the data.
...
!    Input, integer I4VOXEL(NX,NY,NZ), an array of voxel data.
!
!    Output, integer VSUM, the sum of the entries in the
!    voxel array.
!
  implicit none
...
!    of the voxel data.
!
!    Input/output, integer I4VOXEL(NX,NY,NZ), an array of
...
!    of the voxel data.
!
!    Input/output, integer I4VOXEL(NX,NY,NZ), an array
!    of voxel data.  On output, values below the threshhold value have
!    been reset to 0.
!
!    Input, integer THRESH, the threshhold value.
!
  implicit none
...
!  Parameters:
!
!    Input, integer NX, NY, NZ, the X, Y and Z dimensions of
!    the voxel data.
!
!    Input, integer I4VOXEL(NX,NY,NZ), an array of voxel data.
!
...
!  Parameters:
!
!    Input, integer NX, NY, NZ, the number of voxels in the
!    X, Y and Z directions.
!
!    Input/output, integer I4VOXEL(NX,NY,NZ).
...
!
!    Input, integer MAXLIST, the maximum length of the array used
!    to list the elements of the regions.
!
!    Output, integer NLIST, the number of entries of LIST that
...
!
  ...
  integer, parameter :: maxstack = 5000
  integer maxlist
  integer nx
  integer ny
  integer nz
...
!  Parameters:
!
!    Input, integer NX, NY, NZ, the X, Y and Z dimensions of the
!    voxel data.
!
!    Input, integer I4VOXEL(NX,NY,NZ), an array of voxel data.
!
...
!
  ...
  xmax = real ( nx, kind = rk )
  ymax = real ( ny, kind = rk )
  zmax = real ( nz, kind = rk )

  do k = 0, nz
    z = zmax * real ( k  ) / real ( nz, kind = rk )
    do j = 0, ny
      y = ymax * real ( j ) / real ( ny, kind = rk )
      do i = 0, nx
        x = xmax * real ( i ) / real ( nx, kind = rk )
...
!    of the voxel data.
!
!    Input, real ( kind = rk ) R8VOXEL(NX,NY,NZ), an array of real voxel data.
!
!    Output, integer I4VOXEL(NX,NY,NZ), a copy of the real data,
...
!    the voxel data.
!
!    Input, integer MAX_REGION, the maximum number of regions
...
!    for which storage has been allocated.
!
!    Input, integer NREGION, the number of regions.
!
!    Output, real ( kind = rk ) CENTER(4,MAX_REGION), contains the coordinates
...
!
  do iregion = 1, nregion
    if ( center(4,iregion) /= 0 ) then
      do i = 1, 3
        a = real ( center(i,iregion), kind = rk )
        b = real ( center(4,iregion), kind = rk )
...
!    the voxel data.
!
!    Output, real ( kind = rk ) RVOXEL(NX,NY,NZ), ...
!
!    Input, integer C(3), ...
!
!    Input, integer CENTER(3), ...
!
!    Input, integer I4VOXEL(NX,NY,NZ), an array of voxel data.
!
!    Input, integer MAX_REGION, the maximum number of regions
    ...
    do j = 1, ny
      do k = 1, nz
        ...
        if ( i4voxel(i,j,k) /= 0 ) then
          ...
          percent = 100.0D+00 / real ( center(4,iregion), kind = rk )
...
!    Input, integer A(3), ...
!
!    Input/output, real ( kind = rk ) RVOXEL(NX,NY,NZ), ...
!
!    Input, integer B(3), ...
!
!    Input, integer C(3), ...
!
!    Input, integer IREGION, ...
!
!    Input, real ( kind = rk ) PERCENT, ...
!
  implicit none
...
!
  if ( rvoxel(i,j,k) == 0.0D+00 ) then
    rvoxel(i,j,k) = 100.0D+00 * real ( iregion, kind = rk )
  end if
...
!
  ...
  do
    ...
    if ( rvoxel(i2,j2,k2) == 0.0D+00 ) then
      rvoxel(i2,j2,k2) = 100.0D+00 * real ( iregion, kind = rk )
    end if
...
!  Parameters:
!
!    Input, integer I1, J1, K1, the coordinates of the base
...
!
  ...
  real ( kind = rk ) alphaj
  real ( kind = rk ) alphak
  real ( kind = rk ), parameter :: big = 100000.0D+00
  integer i1
  integer i2
  integer inc
  integer j1
...
!
  if ( 0 < inc ) then
    alphai = real ( i2 - i1 + 0.5D+00, kind = rk ) / real ( inc, kind = rk )
  else if ( inc < 0 ) then
    alphai = real ( i2 - i1 - 0.5D+00, kind = rk ) / real ( inc, kind = rk )
  else
    alphai = big
  ...
  if ( 0 < jnc ) then
    alphaj = real ( j2 - j1 + 0.5D+00, kind = rk ) / real ( jnc, kind = rk )
  else if ( jnc < 0 ) then
    alphaj = real ( j2 - j1 - 0.5D+00, kind = rk ) / real ( jnc, kind = rk )
  else
    alphaj = big
  ...
  if ( 0 < knc ) then
    alphak = real ( k2 - k1 + 0.5D+00, kind = rk ) / real ( knc, kind = rk )
  else if ( knc < 0 ) then
    alphak = real ( k2 - k1 - 0.5D+00, kind = rk ) / real ( knc, kind = rk )
  else
    alphaj = big
...
!    of the voxel data.
!
!    Output, integer N000, N001, N010, N011, N100, N101,
