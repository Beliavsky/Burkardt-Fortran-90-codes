...
!*****************************************************************************80
!
!! sandia_cvt_test() tests sandia_cvt().
!
!  Discussion:
!
...
!    of type "real ( kind = rk )".  This is meant to be equivalent to
...
!
!    real ( kind = rk ) BOX_MIN(NDIM), BOX_MAX(NDIM), the minimum and maximum
!    coordinates of the bounding box.
!
!    real ( kind = rk ) DR, a tolerance used by DIATOM when testing whether
...
!
!    real ( kind = rk ) CELL_GENERATOR(NDIM,N), the Voronoi cell generators
...
!
!    real ( kind = rk ) CELL_VOLUME(N), the volume of the Voronoi cells.
!
!    real ( kind = rk ) CELL_CENTROID(NDIM,N), the centroids of the
!    Voronoi cells.
!
!    real ( kind = rk ) CELL_MOMENT(NDIM,NDIM,N), the second moments of the
!    Voronoi cells.
!
!
...
!
  ...
  real ( kind = rk ), parameter, dimension ( ndim ) :: box_min = &
    (/ 0.0D+00, 0.0D+00, 0.0D+00 /)
  real ( kind = rk ), parameter, dimension ( ndim ) :: box_max = &
  ...
  integer clock_max
  integer clock_rate
  real ( kind = rk ),parameter :: dr = 0.00001D+00
  integer i
  integer ios
  integer it
  ...
  real ( kind = rk ), parameter :: region_volume = 20.0D+00 * 1700.0D+00
