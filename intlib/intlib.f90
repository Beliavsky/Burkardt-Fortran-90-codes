...
!
!    Input, real ( kind = rk8 ) XTAB(NTAB), the abscissas at which the
!    function values are given.  The XTAB's must be distinct
!    and in ascending order.
!
!    Input, real ( kind = rk8 ) YTAB(NTAB), the function values,
!    YTAB(I) = F(XTAB(I)).
!
!    Input, real ( kind = rk8 ) A, the lower limit of integration.  A should
!    be, but need not be, near one endpoint of the interval
!    (X(1), X(NTAB)).
!
!    Input, real ( kind = rk8 ) B, the upper limit of integration.  B should
!    be, but need not be, near one endpoint of the interval
!    (X(1), X(NTAB)).
!
!    Output, real ( kind = rk8 ) RESULT, the approximate value of the integral.
!
  implicit none
  ...
  real ( kind = rk8 ) x23
  real ( kind = rk8 ) x3
  real ( kind = rk8 ) xtab(ntab)
  real ( kind = rk8 ) ytab(ntab)
...
!  Parameters:
!
!    Input, real ( kind = rk8 ), external FUNC, the name of the function to be
...
!    in FUNC.
!
!    Input, real ( kind = rk8 ) A, the lower limit of integration.
!
!    Input, real ( kind = rk8 ) B, the upper limit of integration.
!
!    Input, real ( kind = rk8 ) ABSERR, the absolute error tolerance.
!
!    Input, real ( kind = rk8 ) RELERR, the relative error tolerance.
!
!    Output, real ( kind = rk8 ) ERROR, an estimate of the absolute error.
!
!    Output, real ( kind = rk8 ) RESULT, the approximate value of the integral.
!
...
!
  ...
  real ( kind = rk8 ) ait(maxtbl)
  logical aitken
  real ( kind = rk8 ), parameter :: aitlow = 1.1D+00
  real ( kind = rk8 ), parameter :: aittol = 0.1D+00
  real ( kind = rk8 ) astep
  real ( kind = rk8 ) b
  real ( kind = rk8 ) beg
  real ( kind = rk8 ) begin(mxstge)
  real ( kind = rk8 ) bma
  real ( kind = rk8 ) curest
  real ( kind = rk8 ) dif(maxtbl)
  real ( kind = rk8 ) diff
  real ( kind = rk8 ) end
  real ( kind = rk8 ) ergoal
  real ( kind = rk8 ) erra
  real ( kind = rk8 ) errer
  real ( kind = rk8 ) error
  real ( kind = rk8 ) errr
  real ( kind = rk8 ) est(mxstge)
  real ( kind = rk8 ) fbeg
  real ( kind = rk8 ) fbeg2
  real ( kind = rk8 ) fend
  real ( kind = rk8 ) fextm1
  real ( kind = rk8 ) fextrp
  real ( kind = rk8 ) finis(mxstge)
  real ( kind = rk8 ) fn
  real ( kind = rk8 ) fnsize
  real ( kind = rk8 ), external :: func
  logical h2conv
  real ( kind = rk8 ) h2next
  real ( kind = rk8 ) h2tfex
  real ( kind = rk8 ), parameter :: h2tol = 0.15D+00
  real ( kind = rk8 ) hovn
  integer i
  integer ibeg
  ...
  integer n2
  integer nnleft
  real ( kind = rk8 ) prever
  real ( kind = rk8 ) r(maxtbl)
  logical reglar
  ...
  logical right
  real ( kind = rk8 ) rn(4)
  real ( kind = rk8 ) rnderr
  real ( kind = rk8 ) sing
  real ( kind = rk8 ) singnx
  real ( kind = rk8 ) slope
  real ( kind = rk8 ) stage
  real ( kind = rk8 ) step
  real ( kind = rk8 ) stepmn
  real ( kind = rk8 ) sum1
  real ( kind = rk8 ) sumabs
  real ( kind = rk8 ) t(maxtbl,maxtbl)
  real ( kind = rk8 ) tabs
  real ( kind = rk8 ) tabtlm
  real ( kind = rk8 ), parameter :: tljump = 0.01D+00
  real ( kind = rk8 ) ts(2049)
  real ( kind = rk8 ) vint
...
!    Input, real ( kind = rk8 ) B, the upper limit of integration.
!
!    Input, real ( kind = rk8 ) EPSIN, the relative error tolerance.
!
!    Output, real ( kind = rk8 ) EPSOUT, estimated integration error.
!
!    Output, real ( kind = rk8 ) RESULT, the approximate value of the integral.
!
  implicit none
  ...
  real ( kind = rk8 ) a1
  real ( kind = rk8 ) a2
  real ( kind = rk8 ) acof(257)
  real ( kind = rk8 ) alf
  real ( kind = rk8 ) alfnj
  real ( kind = rk8 ) alfno
  real ( kind = rk8 ) b
  real ( kind = rk8 ) bcof(257)
  real ( kind = rk8 ) bet
  real ( kind = rk8 ) betnj
  real ( kind = rk8 ) betno
  real ( kind = rk8 ) bounds
  real ( kind = rk8 ) ccof(513)
  real ( kind = rk8 ) cof
  real ( kind = rk8 ) cofmax
  real ( kind = rk8 ) const1
  real ( kind = rk8 ) const2
  real ( kind = rk8 ) deln
  real ( kind = rk8 ) deltan
  real ( kind = rk8 ) epsin
  real ( kind = rk8 ) epsout
  real ( kind = rk8 ) error
  real ( kind = rk8 ) etank
  real ( kind = rk8 ), external :: func
  real ( kind = rk8 ) gamman
  real ( kind = rk8 ) hnstep
  integer i
  integer index
  integer j
  integer k
  ...
  real ( kind = rk8 ) rn
  real ( kind = rk8 ) rnderr
  real ( kind = rk8 ) rounde
  real ( kind = rk8 ) tend
  real ( kind = rk8 ) tnew
  real ( kind = rk8 ) triarg
  real ( kind = rk8 ) umid
  real ( kind = rk8 ) wmean
  real ( kind = rk8 ) xmin
  real ( kind = rk8 ) xplus
...
subroutine class ( kinda, n, alpha, beta, b, a, muzero )
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) ALPHA, BETA, parameters needed for Laguerre 
!    and Jacobi polynomials.
!
!    Input, integer KINDA, specifies which polynomial is to be 
!    handled:
...
!    calculate.
!
!    Input, real ( kind = rk8 ) ALPHA, the value of the ALPHA parameter,
!    required only for Jacobi or Laguerre polynomials.
!
!    Input, real ( kind = rk8 ) BETA, the value of the BETA parameter,
!    required only for Jacobi polynomials.
!
!    Output, real ( kind = rk8 ) B(N-1), the offdiagonal coefficients.
!
!    Output, real ( kind = rk8 ) A(N), the diagonal coefficients.
!
!    Output, real ( kind = rk8 ) MUZERO, the zero-th moment, Integral W(X) DX,
...
!
  if ( kinda == 1 ) then
    ...
    do i = 1, n-1
      b(i) = real ( i, kind = rk8 ) &
        / sqrt ( 4.0D+00 * real ( i * i, kind = rk8 ) - 1.0D+00 )
    end do
!
!  KINDA = 2:
!
...
!
    ...
    a(1:n) = 0.0D+00
    do i = 1, n-1
      b(i) = sqrt ( real ( i, kind = rk8 ) / 2.0D+00 )
    end do
!
!  KINDA = 5:
!
...
!
    ...
    do i = 1, n
      a(i) = ( beta**2 - alpha**2 ) / &
        ( ( 2.0D+00 * real ( i - 1, kind = rk8 ) + alpha + beta ) &
        * ( 2.0D+00 * real ( i, kind = rk8 ) + alpha + beta ) )
    end do
    ...
    do i = 2, n-1
      ...
      b(i) = sqrt ( 4.0D+00 * real ( i, kind = rk8 ) &
        * ( real ( i, kind = rk8 ) + alpha ) &
        * ( real ( i, kind = rk8 ) + beta ) &
        * ( real ( i, kind = rk8 ) + alpha + beta ) / &
...
!
    ...
    do i = 1, n
      a(i) = 2.0D+00 * real ( i, kind = rk8 ) - 1.0D+00 + alpha
    end do
 
    do i = 1, n-1
      b(i) = sqrt ( real ( i, kind = rk8 ) * ( real ( i, kind = rk8 ) + alpha ) )
    end do
...
!
!    Input, real ( kind = rk8 ) XTAB(NTAB), contains the points at which the
!    function was evaluated.  The XTAB's must be distinct and
!    in ascending order.
!
!    Input, real ( kind = rk8 ) FTAB(NTAB), contains the tabulated values of
!    the function, FTAB(I) = F(XTAB(I)).
!
!    Input, real ( kind = rk8 ) A, lower limit of integration.
!
!    Input, real ( kind = rk8 ) B, upper limit of integration.
!
!    Output, real ( kind = rk8 ) Y(3,NTAB), will contain the coefficients
...
!                   + Y(3,I)*(X-XTAB(I))**3
!
!    Output, real ( kind = rk8 ) E(NTAB), E(I) = the definite integral from
!    XTAB(1) to XTAB(I) of S(X).
!
!    Workspace, real ( kind = rk8 ) WORK(NTAB).
!
!    Output, real ( kind = rk8 ) RESULT, the estimated value of the integral.
!
  implicit none
  ...
  real ( kind = rk8 ) a
  real ( kind = rk8 ) b
  real ( kind = rk8 ) e(ntab)
  real ( kind = rk8 ) ftab(ntab)
  integer i
  integer j
  real ( kind = rk8 ) r
  real ( kind = rk8 ) result
  ...
  real ( kind = rk8 ) term
  real ( kind = rk8 ) u
  real ( kind = rk8 ) work(ntab)
  real ( kind = rk8 ) xtab(ntab)
  real ( kind = rk8 ) y(3,ntab)
...
!
!    Input, real ( kind = rk8 ) FTAB(NTAB), contains the tabulated function
...
!    NTAB.
!
!    Output, real ( kind = rk8 ) RESULT, the approximate value of the
!    integral from XTAB(IA) to XTAB(IB) of the function.
!
!    Output, real ( kind = rk8 ) ERROR, an estimate of the error in
!    integration.
!
  implicit none
...
!
!    Input, real ( kind = rk8 ) FTAB(NTAB), contains the value of the function
!    at A, A+H, A+2*H, ... , B-H, B, where H = (B-A)/(NTAB-1).
!
!    Input, real ( kind = rk8 ) A, B, the limits of integration.
!
!    Input, real ( kind = rk8 ) T, the multiplier of the X argument of the cosine.
!
!    Output, real ( kind = rk8 ) RESULT, the approximate value of the integral.
!
  implicit none
...
!    Input, real ( kind = rk8 ) A, B, the limits of integration.
!
!    Input, real ( kind = rk8 ) T, multiplier of the X argument of the sine.
!
!    Output, real ( kind = rk8 ) RESULT, the approximate value of the integral.
!
  implicit none
...
!    algorithm.
!
!    The function is intended primarily for high accuracy integration or
...
!  Parameters:
!
!    Input, real ( kind = rk8 ), external FUNC, name of external function to
...
!    Input, real ( kind = rk8 ) B, the upper limit of integration.
!
!    Input/output, real ( kind = rk8 ) ERR.
...
!
!    Output, real ( kind = rk8 ) RESULT, the computed value of the integral.
!
...
!
  ...
  real ( kind = rk8 ) vl(30)
  real ( kind = rk8 ) vr
  real ( kind = rk8 ), save :: w1 = 3.62683783378361983D-01
  real ( kind = rk8 ), save :: w2 = 3.13706645877887287D-01
  real ( kind = rk8 ), save :: w3 = 2.22381034453374471D-01
  real ( kind = rk8 ), save :: w4 = 1.01228536290376259D-01
  real ( kind = rk8 ) x
  real ( kind = rk8 ), save :: x1 = 1.83434642495649805D-01
  real ( kind = rk8 ), save :: x2 = 5.25532409916328986D-01
  real ( kind = rk8 ), save :: x3 = 7.96666477413626740D-01
  real ( kind = rk8 ), save :: x4 = 9.60289856497536232D-01
!
...
!
  ...
  anib = log10 ( 2.0D+00 ) * real ( k, kind = rk8 ) / 0.30102000D+00
  nbits = int ( anib )
...
!    Dubrulle.
!
!    The code is a modified version of the EISPACK routine IMTQL2.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
!    Input/output, real ( kind = rk8 ) D(N).
...
!
!    Input/output, real ( kind = rk8 ) E(N).
...
!
!    Input/output, real ( kind = rk8 ) Z(N).
...
subroutine gaussq ( kinda, norder, alpha, beta, kpts, endpts, b, xtab, &
  weight )
...
!
!    The code allows the user to modify the rule to require that
...
!  Parameters:
!
!    Input, integer KINDA, chooses the rule to be calculated.
...
!         -1 < ALPHA
!
!    Input, integer NORDER, the number of points used for 
!    the quadrature rule.
!
!    Input, real ( kind = rk8 ) ALPHA, is only required for Gauss-Jacobi and
!    Gauss-Laguerre quadrature.  Its value is ignored in other cases.
!
!    Input, real ( kind = rk8 ) BETA, is only required for Gauss-Jacobi
...
!
!    Input, real ( kind = rk8 ) ENDPTS(2).
!    If KPTS is 1 or 2, ENDPTS contains the locations of the
!    endpoints to be fixed.
!
!    Workspace, real ( kind = rk8 ) B(NORDER).
!
!    Output, real ( kind = rk8 ) XTAB(NORDER), the nodes for the quadrature rule.
!
!    Output, real ( kind = rk8 ) WEIGHT(NORDER), the weights for the 
!    quadrature rule.
!
  implicit none
  ...
  real ( kind = rk8 ) alpha
  real ( kind = rk8 ) b(norder)
  real ( kind = rk8 ) beta
  real ( kind = rk8 ) endpts(2)
  real ( kind = rk8 ) gam
  integer i
  integer ierr
  integer kinda
  ...
  real ( kind = rk8 ) solve
  real ( kind = rk8 ) t1
  real ( kind = rk8 ) weight(norder)
  real ( kind = rk8 ) xtab(norder)
!
!  Get the diagonal coefficients XTAB(1:NORDER) and off-diagonal
!  coefficients B(1:NORDER-1) and MUZERO.
!
  call class ( kinda, norder, alpha, beta, b, xtab, muzero )
!
...
!
!    Input, real ( kind = rk8 ) DELT, the spacing between the X values of the
!    data.  The actual X values are not needed!
!
!    Input, real ( kind = rk8 ) Y(NTAB), the Y values of the data.
!
!    Work array, real ( kind = rk8 ) WORK(2*(NTAB-1)).  The actual minimum amount
!    of workspace required is two times the number of integer
!    divisors of NTAB-1.
!
!    Output, real ( kind = rk8 ) RESULT, the approximation to the integral.
!
  implicit none
  ...
  real ( kind = rk8 ) sum2
  real ( kind = rk8 ) sum1
  real ( kind = rk8 ) work(2*(ntab-1))
  real ( kind = rk8 ) y(ntab)
!
...
!
      ...
      j = j + 2
      work(j) = delt * sum2 * real ( ( ntab - 1 ) / k, kind = rk8 )
      work(j+1) = real ( ( ( ntab - 1 ) / k )**2, kind = rk8 )
!
...
!  Parameters:
!
!    Input, real ( kind = rk8 ), external FUNC.
...
!    Input, real ( kind = rk8 ) B, the upper limit of integration.
!
!    Input, real ( kind = rk8 ) EPSIN, the requested relative error tolerance.
!
!    Output, real ( kind = rk8 ) EPSOUT, an estimate of the error in the
!    integration.
!
!    Output, real ( kind = rk8 ) RESULT, the approximate value of the integral.
!
...
!  Parameters:
!
!    Input, real ( kind = rk8 ), external FUNC, name of external function to be
...
!    and in ascending order.
!
!    Input, real ( kind = rk8 ) FTAB(NTAB), the function values, 
!    FTAB(I) = F(XTAB(I)).
!
!    Input, real ( kind = rk8 ) A, the lower limit of integration.  A should
!    be, but need not be, near one endpoint of the interval
...
!    which evaluates the function at the point X.
!
!    Input, real ( kind = rk8 ) A, lower limit of integral.
!
!    Input, real ( kind = rk8 ) B, upper limit of integral.
!
!    Input, real ( kind = rk8 ) ERR, is a requested error tolerance.
!    Normally pick a value, 0 < ERR < 1.0D-03.
!
!    Output, real ( kind = rk8 ) RESULT, computed value of the integral.
...
!
  nbits = int ( log10 ( 2.0D+00 ) &
    * real ( digits ( result ), kind = rk8 ) / 0.30102000 )
...
3 continue
  ...
  do i = 1, 11, 2
    f(i) = func ( a + real ( i - 1, kind = rk8 ) * hh(1) )
  end do
...
5 continue
  ...
  do i = 2, 12, 2
    f(i) = func ( aa(l) + real ( i - 1, kind = rk8 ) * hh(l) )
  end do
...
!  Parameters:
!
!    Input, real ( kind = rk8 ), external FUNC, the name of the function to 
...
!    Input, real ( kind = rk8 ) RELERR, the relative error tolerance.
!
!    Input, integer NLEAST, the least number of times the 
...
!
!    Workspace, real ( kind = rk8 ) WORK(NMOST+1).
!
!    Output, real ( kind = rk8 ) RESULT, the approximate value of the integral.
!
  implicit none
  ...
  do i = 1, nmost
    ...
    sumabs = 0.0D+00
    do j = 1, nx
      fcnxi = func ( a + h * real ( 2 * j - 1, kind = rk8 ) )
...
!
!    Input, real ( kind = rk8 ) ALO, AHI, the low and high values.
!
!    Output, real ( kind = rk8 ) A(N), N evenly spaced values.
...
!    Input, real ( kind = rk8 ) B, upper limit of integration.
!
!    Input, real ( kind = rk8 ) EPSIN, requested relative error tolerance.
!
!    Output, real ( kind = rk8 ) EPSOUT, estimated achieved relative error.
!
...
!
  ...
  real ( kind = rk8 ) a
  real ( kind = rk8 ) acof(11)
  real ( kind = rk8 ) alf
  real ( kind = rk8 ) alfnj
  real ( kind = rk8 ) alfno
  real ( kind = rk8 ) ar
  real ( kind = rk8 ) b
  real ( kind = rk8 ) bcof(nupper+1)
  real ( kind = rk8 ) bet
  real ( kind = rk8 ) betnj
  real ( kind = rk8 ) betno
  real ( kind = rk8 ) const1
  real ( kind = rk8 ) const2
  real ( kind = rk8 ) deltan
  real ( kind = rk8 ) endpts
  real ( kind = rk8 ) epsin
  real ( kind = rk8 ) epsout
  real ( kind = rk8 ) error
  real ( kind = rk8 ), parameter :: fac1 = 0.411233516712057D+00
  real ( kind = rk8 ), parameter :: fac2 = 0.822467033441132D+00
...
!    Input, real ( kind = rk8 ) B, the upper limit of integration.
!
!    Input, real ( kind = rk8 ) EPS, the requested error tolerance.
!
!    Output, real ( kind = rk8 ) RESULT, the approximation to the integral.
!
  implicit none
  ...
  real ( kind = rk8 ) b
  real ( kind = rk8 ) da
  real ( kind = rk8 ) dx(maxlev)
  real ( kind = rk8 ) ep
  real ( kind = rk8 ) eps
  real ( kind = rk8 ) epsp(maxlev)
  real ( kind = rk8 ) est
  real ( kind = rk8 ) est1
  real ( kind = rk8 ) est2(maxlev)
  real ( kind = rk8 ) est3(maxlev)
  real ( kind = rk8 ) f1
  real ( kind = rk8 ) f2(maxlev)
  real ( kind = rk8 ) f3(maxlev)
  real ( kind = rk8 ) f4(maxlev)
  real ( kind = rk8 ) fa
  real ( kind = rk8 ) fb
  real ( kind = rk8 ) fbp(maxlev)
  real ( kind = rk8 ) fm
  real ( kind = rk8 ) fmp(maxlev)
  real ( kind = rk8 ), external :: func
  integer l
  integer lvl
  integer nrtr(maxlev)
  real ( kind = rk8 ) pval(maxlev,3)
  real ( kind = rk8 ) result
  real ( kind = rk8 ) sum1
  real ( kind = rk8 ) sx
  real ( kind = rk8 ) x2(maxlev)
  real ( kind = rk8 ) x3(maxlev)
...
!
!    Input, real ( kind = rk8 ) X(NTAB), contains the X values of the data,
!    in order.
!
!    Input, real ( kind = rk8 ) Y(NTAB), contains the Y values of the data.
!
!    Output, real ( kind = rk8 ) RESULT.
...
!    NTAB must be at least 3.
!
!    Input, real ( kind = rk8 ) H, specifies the increment between the
!    X values.  Note that the actual X values are not needed,
!    just the constant spacing!
!
!    Input, real ( kind = rk8 ) Y(NTAB), the data.
!
!    Output, real ( kind = rk8 ) RESULT, the value of the integral
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) SHIFT, the value of the factor that multiplies
...
!  Parameters:
!
!    Input, integer NTAB, is the number of data points.  
!    (NTAB-1) must be divisible by 6.
!
!    Input, real ( kind = rk8 ) H, is the spacing between the points at which
!    the data was evaluated.
!
!    Input, real ( kind = rk8 ) FTAB(NTAB), contains the tabulated data values.
!
!    Output, real ( kind = rk8 ) RESULT, is the approximation to the integral.
!
  implicit none
