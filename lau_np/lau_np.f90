...
!
!    Input, integer INODE(NEDGE), JNODE(NEDGE), the two
!    end nodes of each edge.
!
!    Output, integer LOOP(NEDGE), the Euler circuit, as a
!    series of nodes.
!
  implicit none
...
!    Input, integer NEDGE, the number of edges in the graph.
!
!    Input, integer INODE(NEDGE), JNODE(NEDGE), the edges
!    of the graph, describe by pairs of nodes.
!
!    Input, real ( kind = rk ) COST(NEDGE), the distance or cost of each edge.
!
!    Input, integer START, LAST, are the two nodes between which a
...
!
!    Output, real ( kind = rk ) TREE_COST, the total cost or length of the
!    spanning tree.
!
  implicit none
...
!
!    Output, integer INODE(NNODE), JNODE(NNODE); entries 1
...
!
!      and positivity and integer conditions:
!
!        0 <= X(J),    for 1 <= J <= N,
!        X(J) integer, for 1 <= J <= N.
!
...
!  Parameters:
!
!    Input, integer M, the number of constraints.
!
!    Input, integer N, the number of variables.
!
!    Input, real ( kind = rk ) A(M+1,N+M+1), contains the coefficients of the
...
!    working storage.
!
!    Input, real ( kind = rk ) B(M), the right hand side of the constraints.
!
!    Input, real ( kind = rk ) C(N), the coefficients of the objective function.
!
!    Output, real ( kind = rk ) X(max ( M+1, N ) ), contains the solution.
!
!    Output, integer INFS, error indicator.
...
!
  ...
  integer moveon
  integer n1
  real ( kind = rk ) row_norm
  real ( kind = rk ) temp
  real ( kind = rk ) wk1(n,n)
  real ( kind = rk ) wk2(m+1,m+1)
  real ( kind = rk ) wk3(n)
  real ( kind = rk ) wk4(n)
  real ( kind = rk ) wk5(n)
  real ( kind = rk ) wk6(n)
  ...
  real ( kind = rk ) wk11(m)
  real ( kind = rk ) wk12(m)
  real ( kind = rk ) wk13(m+1)
  real ( kind = rk ) wk14(m+1)
  real ( kind = rk ) wk15(m+1)
  real ( kind = rk ) wk16(n)
  logical wk17(n)
  real ( kind = rk ) x(n)
...
!
!    Given an integer K between 1 and N, the K-center location
...
!    Input, integer NNODE, the number of nodes.
!
!    Input, real ( kind = rk ) COST(NNODE,NNODE), the cost matrix, which contains
!    the "cost" for each pair of nodes, or the distance between them.
!
!    Input, integer KMAX, the maximum size of the subset of nodes
!    to be found.
!
!    Output, integer KNUM, the size of the subset of nodes found.
!    KNUM will be no greater than KMAX.
!
!    Output, integer KSET(KMAX), contains in entries 1 through KNUM
...
!
  ...
  real ( kind = rk ) big
  real ( kind = rk ) cost(nnode,nnode)
  real ( kind = rk ) great
  integer high
  integer i
  integer ifirst
  integer inode
  integer iwk1(nnode,nnode)
  integer iwk2(nnode)
  integer iwk3(nnode)
  integer iwk4((nnode*(nnode-1))/2)
  integer iwk5((nnode*(nnode-1))/2)
  integer j
  integer k
  integer kmax
  integer knum
  ...
  integer numk
  real ( kind = rk ) small
  real ( kind = rk ) work6((nnode*(nnode-1))/2)
...
!    of the cost matrix.
!
!    Input, real ( kind = rk ) C(M,N), the M by N cost matrix.
!
!    Input, integer K, the number of rows to be used
!    in the maximizing set.
!
!    Output, integer ISOL(M), contains in entries 1 through
...
!
  ...
  real ( kind = rk ) work3(m)
  real ( kind = rk ) work4(m)
  real ( kind = rk ) z
...
!    Input, integer N, the number of variables.
!
!    Input, real ( kind = rk ) A(N), the coefficients of the constraints.
!
!    Input, real ( kind = rk ) B, the right hand side of the constraints.
!
!    Input, real ( kind = rk ) C(N), the coefficients of the objective function.
!
!    Input, real ( kind = rk ) EPS, a positive real number prescribing the
!    required degree of accuracy in the solution.
!
!    Input, integer M, the smallest integer greater than ( 3 / EPS )^2.
!
!    Output, real ( kind = rk ) OBJVAL, the value of the objective function for
!    the suggested solution.
!
!    Output, integer NUMSOL, the number of nonzero variables in the
!    suggested solution.
!
!    Output, integer ISOL(N), contains the indices of the nonzero
...
!
  ...
  logical switch
  real ( kind = rk ) value
  real ( kind = rk ) work1(2,m)
  real ( kind = rk ) work2(2,m)
  real ( kind = rk ) work3(n)
  real ( kind = rk ) work4(n)
  real ( kind = rk ) work5(n)
  real ( kind = rk ) xtemp
  real ( kind = rk ) ytemp
!
...
!    Input, integer N, the number of variables.
!
!    Input, real ( kind = rk ) A(M,N), the matrix of coefficients of the
!    constraints.
!
!    Input, real ( kind = rk ) B(M), the right hand sides of the constraints.
!
!    Input, real ( kind = rk ) C(N), the coefficients of the objective function.
!
!    Output, real ( kind = rk ) OBJVAL, the value of the objective function
!    for the suggested solution.
!
!    Output, integer NUMSOL, the number of nonzero variables in the
!    suggested solution.
!
!    Output, integer ISOL(N), contains the indices of the
...
!  Parameters:
!
!    Input, integer N, the number of nodes in each partition.
!
!    Input, real ( kind = rk ) COST(2*N,2*N), the edge cost matrix.
!
!    Input, integer INIT:
...
!
!    Output, integer KP(N), KQ(N), the final partition of
!    the nodes.
!
!    Output, real ( kind = rk ) TCOST, the total cost of the edges in the
!    cut set.
!
  implicit none
...
!  Parameters:
!
!    Input, integer N, the number of nodes in the complete graph.
!    N is assumed to be even.
!
!    Input, real ( kind = rk ) COST(N*(N-1)/2), the strict upper triangle of the
!    cost matrix, stored by rows.  In other words, the first elements of
!    COST are the costs C(1,2), C(1,3), ..., C(1,N), C(2,3), C(2,4),
!    ..., C(2,N).
!
!    Output, integer PAIR(N), contains the minimum weight perfect
...
!    1, the linear program is infeasible.  No solution was found.
!
!    Workspace, real ( kind = rk ) WK2((M+1)*(M+1)).
!
!    Workspace, real ( kind = rk ) WK13(M+1).
!
!    Workspace, real ( kind = rk ) WK14(M+1).
!
!    Workspace, real ( kind = rk ) WK16(max(M+1,N)).
!
!    Workspace, integer IWK19(M+1).
!
!    Output, integer IWK20(N+M).
!
  implicit none
...
!  Parameters:
!
!    Input, integer N, the number of nodes in the graph.
!
!    Input, integer M, the number of edges in the graph.
!
!    Input, integer INODE(M), JNODE(M), the edges of the graph,
!    described by pairs of nodes.
!
!    Input, real ( kind = rk ) COST(M), the length of each edge in the graph.
!
!    Input, integer NS, the number of nodes which are
!    Steiner points.
!
!    Input, logical SPOINT(N), is TRUE for each node that is a Steiner point.
!
!    Output, integer NSP, the number of edges in the Steiner tree.
!
!    Output, integer ISTREE(N), JSTREE(N), the edges of the
!    Steiner tree, stored as pairs of nodes, in entries 1 through NSP.
!
!    Output, real ( kind = rk ) XLEN, the total length of the edges in the
!    Steiner tree.
!
  implicit none
...
!    Input, integer NNODE, the number of nodes.
!
!    Input, real ( kind = rk ) DIST(NNODE,NNODE), the distance between
!    each pair of nodes.
!
!    Output, integer ISOL(NNODE), contains the nodes
