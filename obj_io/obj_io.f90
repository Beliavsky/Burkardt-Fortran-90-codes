...
!  Input:
!
!    character C, the character to be tested.
!
!  Output:
!
!    logical CH_IS_CONTROL, TRUE if C is a control character, and
!    FALSE otherwise.
!
  implicit none
...
!
!    integer ORDER_MAX, the maximum number of vertices per face.
!
!    integer FACE_ORDER(FACE_NUM), the number of vertices per face.
!
!    integer FACE_NODE(ORDER_MAX,FACE_NUM), the nodes that
!    make up each face.
!
  implicit none
  ...
  integer face
  integer face_node(order_max,face_num)
...
!  Input:
!
!    integer NORMAL_NUM, the number of normal vectors.
!
!    real ( kind = rk ) NORMAL_VECTOR(3,NORMAL_NUM), the normal vectors.
!
  implicit none
  ...
  integer normal
  real ( kind = rk ) normal_vector(3,normal_num)
...
!
!    real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the coordinates
!    of the nodes.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the coordinates of points.
!
!    integer FACE_ORDER(FACE_NUM), the number of vertices per face.
!
!    integer FACE_NODE(ORDER_MAX,FACE_NUM), the nodes
!    making faces.
!
!    real ( kind = rk ) NORMAL_VECTOR(3,NORMAL_NUM), normal vectors.
!
!    integer VERTEX_NORMAL(ORDER_MAX,FACE_NUM), the indices
...
!
  ...
  real ( kind = rk ) temp
  integer vertex
  integer vertex_normal(order_max,face_num)
...
!  Input:
!
!    character ( len = * ) INPUT_FILENAME, the input file name.
!
!  Output:
!
!    integer NODE_NUM, the number of points.
...
!    integer NORMAL_NUM, the number of normal vectors.
!
!    integer ORDER_MAX, the maximum face order.
!
  implicit none
...
!
!    integer NODE_NUM, the number of vertices defined.
!
!    integer FACE_NUM, the number of faces defined.
!
!    integer NORMAL_NUM, the number of normal vectors defined.
!
!    integer ORDER_MAX, the maximum number of vertices per face.
!
  implicit none
...
!    integer FACE_ORDER(FACE_NUM), the number of vertices per face.
!
!    integer VERTEX_NORMAL(ORDER_MAX,FACE_NUM), the
...
!  Input:
!
!    character ( len = * ) OUTPUT_FILENAME, the name of the output file.
!
!    integer NODE_NUM, the number of points.
!
!    integer FACE_NUM, the number of faces.
...
!    integer FACE_ORDER(FACE_NUM), the number of vertices per face.
!
!    integer FACE_NODE(ORDER_MAX,FACE_NUM), the nodes making faces.
!
!    real ( kind = rk ) NORMAL_VECTOR(3,NORMAL_NUM), normal vectors.
!
!    integer VERTEX_NORMAL(ORDER_MAX,FACE_NUM), the
...
!    1, an error occurred.
!
!    integer LENGTH, the number of characters of S
!    used to make IVAL.
!
  implicit none
