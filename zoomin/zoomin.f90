...
!  Parameters:
!
!    Input/output, real ( kind = rk ) X, X1, two points defining the interval
...
!    between X and X1.
!
!    Input, real ( kind = rk ) ABSERR, an error tolerance.
!
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) X, X1.  On input, two points defining
...
!      complex x
!
!    Input, real ( kind = rk ) FATOL, the absolute error tolerance for F(X).
!
!    Input, integer ITMAX, the maximum number of
!    steps allowed.
!
!    Input, complex ( kind = ck ) X1, X2, X3, three distinct points to start the
!    iteration.
!
!    Input, real ( kind = rk ) XATOL, XRTOL, absolute and relative
!    error tolerances for the root.
!
!    Output, complex ( kind = ck ) XNEW, the estimated root.
!
!    Output, complex ( kind = ck ) FXNEW, the value of the function at XNEW.
!
  implicit none
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) X, X1, X2.
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) X, X1.
...
!    abs ( F(X) ) <= ABSERR.
!
!    Input, real ( kind = rk ) BETA, a parameter in the algorithm.
!
!    Input, real ( kind = rk ) ABSERR, an error tolerance.
!
!    Input, integer KMAX, the maximum number of iterations allowed.
...
!    abs ( F(X) ) <= ABSERR.
!
!    Input, integer IPOLY, the polynomial degree of the function.
...
!
  ...
  beta = 1.0D+00 / real ( ipoly - 1, kind = rk )
...
!    abs ( F(X) ) <= ABSERR.
!
!    Input, integer NSUB, the number of steps in the sub-iteration.
...
!    abs ( F(X) ) <= ABSERR.
!
!    Input, integer M, the multiplicity of the root.
!
!    Input, real ( kind = rk ) ABSERR, an error tolerance.
!
!    Input, integer KMAX, the maximum number of iterations allowed.
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) X, X1, X3.
!    On input, three distinct points that start the method.
!    On output, X is an approximation to a root of the equation
...
!    abs ( F(X) ) <= ABSERR.
!
!    Output, real ( kind = rk ) EM, an estimate of the multiplicity of the root.
!
!    Input, real ( kind = rk ) ABSERR, an error tolerance.
!
!    Input, integer KMAX, the maximum number of iterations allowed.
...
!    abs ( F(X) ) <= ABSERR.
!
!    Input, real ( kind = rk ) A, ???
!
!    Input, real ( kind = rk ) RHO, ???
!
!    Input, real ( kind = rk ) ABSERR, an error tolerance.
!
!    Input, integer KMAX, the maximum number of iterations allowed.
...
!  Parameters:
!
!    Input, real ( kind = rk ) A, B, C, three estimates for a root of the
...
!      function f ( x, ider )
!
!    Input, integer IPOLY, is 0 if the function is not known to be
!    a polynomial, or is equal to the polynomial degree otherwise.
!
!    Input, integer KMAX, the maximum number of iterations allowed.
!
!    Input, integer MULT, the multiplicity of the root being 
!    sought.  If the multiplicity of the root is not known, set MULT to 1.
!
!    Input, integer NDER, the highest order derivative that can be
...
!
!    Input, integer NSUB, the number of substeps to take.  A few
