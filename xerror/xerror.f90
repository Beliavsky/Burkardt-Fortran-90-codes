...
!
!    Output, integer I1MACH, the value of the constant.
!
  implicit none
...
!
!    Input, integer VALUE, the value to be set for the WHICH-th 
!    parameter, if SET is TRUE.
!
!    Input, logical SET.
!    TRUE: the WHICH-th parameter will be given the value, VALUE.
!
!    Output, integer J4SAVE, the old value of the WHICH-th
!    parameter.
!
  implicit none
...
!
!    Input, integer NMESSG, the actual number of characters in
...
!
!    Input/output, integer KONTRL.  This routine receives the
...
!  Parameters:
!
!    Input, integer MAXMES, the maximum number of times any one
...
!
!    Input, integer NERR, the error number, chosen by the library
...
!
!    Input, integer LEVEL, a value in the range 0 to 2 that 
...
!
!    Input, integer NPREF, the number of characters to use from 
...
!
!    Input, integer NWRAP, the maximum size piece into which to 
...
!
!    Input, integer NR, the number of real values to be printed. 
!    (0 to 2)
!
...
!
  ...
  call xgetua(lun,nunit)
  isizei = 1 + int ( log10 ( real ( i1mach(9), kind = rk ) ) )
  isizef = 1 + int ( log10 ( real ( i1mach(10), kind = rk )**i1mach(14) ) )
...
!  Parameters:
!
!    Output, integer IUNIT, the logical unit number of the first
...
!  Parameters:
!
!    Input, integer IUNIT(NUNIT), unit numbers to which the error
!    messages should be printed.  Normally these numbers should all be 
!    different but duplicates are not prohibited.
!
!    Input, integer NUNIT, the number of unit numbers provided
!    in IUNIT.  1 <= N <= 5.
!
  implicit none
