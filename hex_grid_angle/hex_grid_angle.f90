...
!  Parameters:
!
!    Input, real ( kind = rk8 ) BOX(2,2), the lower left and upper right
!    corners of the box.
!
!    Input, real ( kind = rk8 ) P(2), the point to be checked.
!
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) ANGLE, the angle, in degrees.
!
!    Output, real ( kind = rk8 ) COS_DEG, the cosine of the angle.
!
  implicit none
  ...
  real ( kind = rk8 ) angle
  real ( kind = rk8 ) cos_deg
  real ( kind = rk8 ), parameter :: degrees_to_radians &
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) DEGREES, the angle measure in degrees.
!
!    Output, real ( kind = rk8 ) DEGREES_TO_RADIANS, the angle measure in radians.
!
  implicit none
  ...
  real ( kind = rk8 ) degrees_to_radians
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) CENTER(2), the center of the grid.
!    This point must be inside the unit square.
!
!    Input, real ( kind = rk8 ) ANGLE, the angle, in degrees, of the grid.
!    Normally, 0 <= ANGLE <= 180, but any value is allowed.
!
!    Input, real ( kind = rk8 ) H, the spacing between neighboring
...
!
!    Output, real ( kind = rk8 ) R(2,N), the grid points.
!
  implicit none
  ...
  real ( kind = rk8 ) angle
  real ( kind = rk8 ) angle2
  logical box_01_contains_point_2d
  real ( kind = rk8 ) center(dim_num)
  real ( kind = rk8 ) cos_deg
  real ( kind = rk8 ) h
  integer i
  integer j
  ...
  integer layer
  integer layer_size
  real ( kind = rk8 ) point(dim_num)
  real ( kind = rk8 ) r(dim_num,n)
  real ( kind = rk8 ) r8_modp
  real ( kind = rk8 ) sin_deg
!
!  Ninny checks.
!
...
!    Input, real ( kind = rk8 ) CENTER(2), the "center" of the grid.
!
!    Input, real ( kind = rk8 ) ANGLE, the angle of the grid.
!
!    Input, real ( kind = rk8 ) H, the spacing between points on a grid line.
!
!    Input, integer N, the number of elements in the subsequence.
!
!    Input, real ( kind = rk8 ) R(2,N), the points.
!
...
!
!    Output, real ( kind = rk8 ) R8_UNIFORM_01, a new pseudorandom variate,
...
!
  r8_uniform_01 = real ( seed, kind = rk8 ) * 4.656612875D-10
...
!  Discussion:
!
!    An R8VEC is a vector of real ( kind = rk8 ) values.
!
!    For now, the input quantity SEED is an integer variable.
!
!  Licensing:
...
!    Input, real ( kind = rk8 ) ANGLE, the angle, in degrees.
!
!    Output, real ( kind = rk8 ) SIN_DEG, the sine of the angle.
!
  implicit none
