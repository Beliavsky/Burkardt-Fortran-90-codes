...
!*****************************************************************************80
!
!! lattice_rule_test() tests lattice_rule().
!
!  Licensing:
!
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'lattice_rule_test():'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Test LATTICE_RULE().'
...
!
  ...
  real ( kind = rk ), external :: e_01_2d
  real ( kind = rk ) error
  real ( kind = rk ) exact
  real ( kind = rk ), external :: f_01_2d
  integer fibonacci
  integer k
  integer m
  real ( kind = rk ) quad
