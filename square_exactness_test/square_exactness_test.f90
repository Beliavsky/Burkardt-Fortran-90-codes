...
!  Parameters:
!
!    Input, real ( kind = rk ) A(2), B(2), the lower and upper integration
!    limits.
!
!    Input, integer NX, NY, the orders in the X and Y directions.
!    NX and NY must be between 1 and 10.
!
!    Output, real ( kind = rk ) X(N), Y(N), the abscissas.
!
...
!
  ...
  integer j
  integer k
  real ( kind = rk ) w(nx*ny)
  real ( kind = rk ) wx(nx)
  real ( kind = rk ) wy(ny)
  real ( kind = rk ) x(nx*ny)
  real ( kind = rk ) xx(nx)
  real ( kind = rk ) y(nx*ny)
  real ( kind = rk ) yy(ny)
!
...
!    0 <= L <= 10.
!
!    Output, real ( kind = rk ) X(N), Y(N), the Padua points.
!
  implicit none
  ...
  real ( kind = rk ) x(((l+1)*(l+2))/2)
  real ( kind = rk ) y(((l+1)*(l+2))/2)
