...
!
!    Output, integer NNZ, the number of nonzero entries required
!    to store the matrix, if REP = 'coordinate'.
!
!    Input, integer NNZMAX, the maximum dimension of the 
!    appropriate data array.
!
!    Output, integer INDX(NNZ), the row indices for coordinate 
!    format.  Not used if REP is 'array'.
!
!    Output, integer JNDX(NNZ), the column indices for coordinate
!    format.  Not used if REP is 'array'.
!
!    Output, integer IVAL(NNZ), matrix values, if FIELD 
!    is 'integer'.
!
!    Output, real RVAL(NNZ), matrix values, if FIELD is 'real'.
!
!    Output, real ( kind = rk ) DVAL(NNZ), matrix values, if FIELD is 'double'.
!
!    Output, complex CVAL(NNZ), matrix values, if FIELD 
!    is 'complex'.
!
  implicit none
...
!
!    Input, integer NNZ, the number of nonzero entries required
!    to store the matrix, if REP = 'coordinate'.
!
!    Input, integer INDX(NNZ), the row indices for coordinate
!    format.  Not used if REP is 'array'.
!
!    Input, integer JNDX(NNZ), the column indices for coordinate
!    format.  Not used if REP is 'array'.
!
!    Input, integer IVAL(NNZ), matrix values, if FIELD is
!    'integer'.
!
!    Input, real RVAL(NNZ), matrix values, if FIELD is 'real'.
!
!    Input, real ( kind = rk ) DVAL(NNZ), matrix values, if FIELD is 'double'.
!
!    Input, complex CVAL(NNZ), matrix values, if FIELD
!    is 'complex'.
!
  implicit none
...
!  Parameters:
!
!    Input, character ( len = * ) REP, the Matrix Market 'representation'
!    indicator.  Possible values include:
!    'coordinate'   (for sparse data)
!    'array'        (for dense data)
!    'elemental'    (to be added)
!
!    Input, character ( len = * ) SYMM, the Matrix Market symmetry.
!    Possible values include:
!    'symmetric'
!    'hermitian'
!    'skew-symmetric'
...
!    'pattern'
!
!    Input, integer NNZ, the number of nonzero entries required to
...
!    format.  Not used if REP is 'array'.
!
!    Input, integer IVAL(NNZ), matrix values, if FIELD 
!    is 'integer'.
!
!    Input, real RVAL(NNZ), matrix values, if FIELD is 'real'.
!
