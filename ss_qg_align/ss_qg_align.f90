...
!
!    Output, integer A_TO_I4, the index of the character,
...
!    values.
!
!    Input/output, integer ISEED, a seed for the random number
!    generator.
!
...
!*****************************************************************************80
!
!! I4_TO_AMINO_CODE converts an I4 to an amino code.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!  Parameters:
!
!    Input, integer I, the index of an amino acid, 
!    between 1 and 23.
!
...
!
!    Input/output, integer ISEED, a seed for the random 
!    number generator.
!
  implicit none
...
!
  ...
  integer, parameter, dimension ( acid_num, acid_num ) :: &
    weight2 = &
    reshape ( (/&
...
!
!    Output, real PAM120_SCORE, the score for matching the two
!    characters.
!
  implicit none
...
!    C1 is from sequence A, C2 from sequence B.
!
!    Output, real PAM200_SCORE, the score for matching the
!    two characters.
!
  implicit none
...
!
!    Output, integer IVAL, the integer read from the string.
...
!    C1 is from sequence A, C2 from sequence B.
!
!    Output, real SIMPLE_SCORE, the score for matching 
!    the two characters.
!
  implicit none
...
!
!    Input, integer M1, M2, the minimum and maximum rows of the 
!    score matrix to compute.  0 <= M1 <= M2 <= M.
!
!    Input, integer N, the number of entries in sequence B.
!
!    Input, integer N1, N2, the minimum and maximum columns of the 
!    score matrix to compute.  0 <= N1 <= N2 <= N.
!
...
!
!    Input, real GAP_OPEN, GAP_EXTEND, the penalties for opening 
!    and extending a gap.  A gap of length 7, for example, will result in a
...
!
!    Input, integer M1, M2, the minimum and maximum rows of the 
!    computed score table.
!
!    Input, integer N, the number of entries in sequence B.
!
!    Input, integer N1, N2, the minimum and maximum columns of 
!    the computed score table.
!
!    Input, integer LDS, the declared upper first dimension of S.
!    LDS must be at least M.
!
...
!    Output, integer T(0:N), the backward pointer vector.
!
!    Output, integer I1, J1, the location of an optimal quasiglobal
!    score.  I1 will equal M1, or J1 will equal N1.
!
...
!
!    Input, integer M1, M2, the lowest and highest table rows 
!    to compute.
!
!    Input, integer N, the number of entries in the sequence B.
!
!    Input, integer N1, N2, the lowest and highest table columns
!    to compute.
!
!    Input, external SS_SCORE, the name of a function of the form
!      function ss_score ( c1, c2 )
...
!
!    Input, integer N1, N2, the minimum and maximum columns of 
!    the score matrix to compute.  0 <= N1 <= N2 <= N.
!
!    Input, external SS_SCORE, the name of a function of the form
!      function ss_score ( c1, c2 )
!    which returns a real value SS_SCORE for the matching of the character
...
!    certain penalties.
!
!    Input, integer LDS, the declared upper first dimension of 
!    S, E, and F.  LDS must be at least M.
!
!    Output, real S(0:LDS,0:N), the forward optimal score table.
!    Entries in the M1:M2 by N1:N2 block have been computed.
!
...
!    Input, integer M, the number of entries in sequence A.
!
!    Input, integer M1, M2, the minimum and maximum entries of A 
!    to align.  For a full alignment, use M1 = 0, M2 = M.
!
!    Input, integer N, the number of entries in sequence B.
!
...
!  Parameters:
!
!    Output, integer I1SUB, J1SUB, J1TYPE, 
!    real T1, the row and column of the first cell, the type 
!    of the match there, and the appropriate base.
!
...
!    the row and column of the first cell, the type of the match there, and
!    the appropriate base.
!
...
!  Parameters:
!
!    Input/output, integer ISEED, the "seed" used to generate
!    the output random number, and updated in preparation for the
!    next one.  ISEED should not be zero.
!
!    Output, real UNIFORM_01_SAMPLE, a random value 
!    between 0 and 1.
!
!  Local Parameters:
!
