...
!
  ...
  external qdrtgh
  integer uip(1)
  real ( kind = rk ) urp(4,3)
  real ( kind = rk ) v(lv)
  real ( kind = rk ) x(4)

  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TOMS611_TEST():'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Test TOMS611().'
!
...
!  arbitrarily pass qdrtf for ufparm to satisfy the calling sequence.
!
  iv(1) = 0
...
!
  ...
  integer nf, uip(*)
  real ( kind = rk ) x(n), f, urp(n,3)
  external ufp
  integer i
  real ( kind = rk ) dn, f2, t, t1
...
!
  ...
  integer nf, uip(*)
  real ( kind = rk ) x(n), g(n)
  real ( kind = rk ) h(n*(n+1)/2)
  real ( kind = rk ) urp(n,3)
  external qdrtf
  integer i, j, k
  real ( kind = rk ) dn, f, t1, t2
