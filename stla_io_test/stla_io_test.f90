...
!
  ...
  call test01 ( 'cube.stl' )
  call test02 ( 'cube.stl' )
  call test03 ( 'cube.stl' )
  call test04 ( 'cube_new.stl' )
  call test05 ( )
!
!  Terminate.
!
...
!
  ...
  real ( kind = rk ), allocatable, dimension(:,:) :: face_normal
  integer face_num
  integer ierror
  character ( len = * ) input_file_name
  integer node_num
...
!
  ...
  integer, dimension ( 3, face_num ) :: face_node = reshape ( (/ &
    1, 3, 2, &
    2, 3, 4, &
    1, 6, 5, &
    1, 2, 6, &
    ...
    2, 4, 6, &
    6, 4, 8 /), (/ 3, face_num /) )
  real ( kind = rk ), dimension ( 3, face_num ) :: face_normal = reshape ( (/ &
...
!
  ...
  real ( kind = rk ), dimension ( 3, face_num ) :: face_normal = reshape ( (/ &
   ...
   +1.0D+00,  0.0D+00,  0.0D+00, &
   +1.0D+00,  0.0D+00,  0.0D+00 /), (/ 3, face_num /) )
  real ( kind = rk ), dimension ( 3, face_num ) :: face_normal2
  real ( kind = rk ), dimension ( 3, node_num ) :: node_xyz = reshape ( (/ &
    0.0D+00, 0.0D+00, 0.0D+00, &
    1.0D+00, 0.0D+00, 0.0D+00, &
    0.0D+00, 1.0D+00, 0.0D+00, &
