...
!
!    Output, real ( kind = rk ) A(N,NDIAG), the R8GD matrix.
!
  implicit none
...
!    storage.
!
!    Input, real ( kind = rk ) A(N,NDIAG), the R8GD matrix.
!
...
!    NDIAG must be at least 1, and no more than 2 * N - 1.
!
!    Input, integer OFFSET(NDIAG), the offsets for the 
!    diagonal storage.
!
!    Input, real ( kind = rk ) A(N,NDIAG), the R8GD matrix.
!
