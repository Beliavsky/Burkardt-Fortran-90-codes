...
!
  ...
  integer seed
  real ( kind = rk ), allocatable, dimension ( : ) :: x0
...
!
  ...
  real      ( kind = rk ) dx
  real      ( kind = rk ), allocatable, dimension ( : ) :: fx0
  real      ( kind = rk ), allocatable, dimension ( : ) :: fx1
  real      ( kind = rk ), allocatable, dimension ( : ) :: fx2
  integer i
  integer j
  integer nvar
  integer option
...
!
!    Input, real ( kind = rk ) X(NVAR), the starting point.
!
!    Input, integer PAR_INDEX, the continuation parameter.
!    If the program is free to choose this value, set it to 0.
!
!    Input, real ( kind = rk ) H, HMIN, HMAX, the suggested step, and
!    the minimum and maximum stepsizes.  H may be negative.
!
!    Output, real ( kind = rk ) XT(NVAR), the computed point.
!
!    Output, real ( kind = rk ) HT, the  actual stepsize that was used.
!
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: jac
  real ( kind = rk ), allocatable, dimension ( : ) :: jt
  real ( kind = rk ) jtd
  real ( kind = rk ) jtn
  integer nvar
  integer option
  ...
  real ( kind = rk ) r8vec_norm_l2
  integer seed
  real ( kind = rk ), allocatable, dimension ( : ) :: tan
  real ( kind = rk ), allocatable, dimension ( : ) :: x0
...
!
  ...
  integer step_max
  real ( kind = rk ), allocatable, dimension ( : ) :: tan
  real ( kind = rk ), allocatable, dimension ( : ) :: tan1
  real ( kind = rk ), allocatable, dimension ( : ) :: tan2
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: x1
  real ( kind = rk ), allocatable, dimension ( : ) :: x2
  real ( kind = rk ), allocatable, dimension ( : ) :: xt
