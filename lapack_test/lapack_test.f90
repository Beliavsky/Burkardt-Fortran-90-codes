...
!
  ...
  integer, parameter :: lwork = 3*min(m,n) + max ( max(m,n), 2*min(m,n) )
  ...
  character jobu
  character jobvt
  real ( kind = rk8 ) s(min(m,n))
  integer seed
  real ( kind = rk8 ) sigma(m,n)
  real ( kind = rk8 ) u(m,m)
  real ( kind = rk8 ) vt(n,n)
  real ( kind = rk8 ) work(lwork)
...
!
  ...
  real ( kind = rk8 ) a(nband+1,n)
  integer i
  integer info
  integer j
  real ( kind = rk8 ) l(nband+1,n)
  real ( kind = rk8 ) l_row(n)
...
!  Parameters:
!
!    real ( kind = rk8 ) AB(LDAB,N), contains, on input, the upper or lower
...
!
!    real ( kind = rk8 ) ABSTOL, the absolute error tolerance for the eigenvalues.
!    If the input value of ABSTOL is not positive, then an appropriate
!    value will be determined internally and used instead.
!
!    real ( kind = rk8 ) BB(LDBB,N), contains, on input, the upper or lower
...
!
!    integer IFAIL(N), if JOBZ = 'V', then if INFO is 0, the first
...
!
!    integer KB, the number of superdiagonals (if UPLO = 'U') or
!    subdiagonals (if UPLO = 'L') of B that are nonzero.
!
!    integer LDAB, the leading dimension of the array AB, which
!    must be at least KA+1.
!
!    integer LDBB, the leading dimension of the array BB, which
!    must be at least KB+1.
!
!    integer LDQ, the leading dimension of the array Q.
!    If JOBZ = 'N', then Q is not used, and LDQ should be any
!    positive value such as 1.  If JOBZ = 'V', then LDQ must be
!    at least N.
!
!    integer LDZ, the leading dimension of the array Z.
!    If JOBZ = 'N', then Z is not used, and LDZ should be any
!    positive value such as 1.  If JOBZ = 'V', then LDZ must be
!    at least N.
!
!    integer M, the number of eigenvalues found by DSBGVX.
!
!    integer N, the order of the matrices A and B.
!
!    real ( kind = rk8 ) Q(LDQ,N), if JOBZ = 'V', the N by N matrix used to
...
!
!    real ( kind = rk8 ) VL, VU, the lower and upper bounds of an interval to be
!    searched for eigenvalues.  In this case, VL must be less than VU.
!    These values are used only if RANGE = 'V'.
!
!    real ( kind = rk8 ) W(N), the requested eigenvalues, in ascending order.
!
!    real ( kind = rk8 ) WORK(7*N), workspace.
!
!    real ( kind = rk8 ) Z(LDZ,N), if JOBZ = 'V', the I-th column of Z contains
...
!
  ...
  integer, parameter :: ldab = ka+1
  integer, parameter :: ldbb = kb+1
  integer, parameter :: ldq = 1
  integer, parameter :: ldz = 1

  real ( kind = rk8 ) ab(ldab,n)
  real ( kind = rk8 ), parameter :: abstol = 0.0D+00
  real ( kind = rk8 ) bb(ldbb,n)
  integer i
  integer ifail(n)
  integer il
  integer ilo
  ...
  character :: jobz = 'N'
  integer m
  real ( kind = rk8 ) q(ldq,n)
  character :: range = 'I'
  integer test
  character :: uplo = 'U'
  real ( kind = rk8 ) value
  real ( kind = rk8 ), parameter :: vl = 0.0D+00
  real ( kind = rk8 ), parameter :: vu = 1.0D+00
  real ( kind = rk8 ) w(n)
  real ( kind = rk8 ) work(7*n)
  real ( kind = rk8 ) z(ldz,n)
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'dgeev_test():'
  write ( *, '(a)' ) '  (D): for a double precision real matrix,'
  write ( *, '(a)' ) '  (GE): in general storage mode,'
  write ( *, '(a)' ) '  (EV): compute eigenvalues and eigenvectors.'
  write ( *, '(a)' ) ' '
!
!  Set A.
!
...
!
  ...
  if ( info /= 0 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a,i8)' ) '  dgeev() returned nonzero INFO = ', info

  else

    call r8vec2_print ( n, wr, wi, '  The eigenvalues:' )
...
!
!    Input, real ( kind = rk8 ) A(M,N), the matrix whose L-oo
!    norm is desired.
!
!    Output, real ( kind = rk8 ) R8MAT_NORM_LI, the L-oo norm of A.
!
  implicit none
