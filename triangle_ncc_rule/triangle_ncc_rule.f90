...
subroutine triangle_ncc_degree ( rule, degree )
...
!
  ...
  if ( 1 <= rule .and. rule <= 9 ) then
    ...
    degree = -1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGLE_NCC_DEGREE - Fatal error!'
...
subroutine triangle_ncc_order_num ( rule, order_num )
...
!
  ...
  call triangle_ncc_suborder ( rule, suborder_num, suborder )
...
subroutine triangle_ncc_rule ( rule, order_num, xy, w )
...
!
  ...
  do s = 1, suborder_num
    ...
    if ( suborder(s) == 1 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'TRIANGLE_NCC_RULE - Fatal error!'
...
subroutine triangle_ncc_rule_num ( rule_num )
...
subroutine triangle_ncc_suborder ( rule, suborder_num, suborder )
...
!
  ...
  if ( rule == 1 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGLE_NCC_SUBORDER - Fatal error!'
    write ( *, '(a,i8)' ) '  Illegal RULE = ', rule
    stop 1
...
subroutine triangle_ncc_suborder_num ( rule, suborder_num )
...
!
  ...
  if ( 1 <= rule .and. rule <= 9 ) then
    ...
    suborder_num = -1
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGLE_NCC_SUBORDER_NUM - Fatal error!'
    write ( *, '(a,i8)' ) '  Illegal RULE = ', rule
    stop 1
...
subroutine triangle_ncc_subrule ( rule, suborder_num, suborder_xyz, suborder_w )
...
!
  ...
  if ( rule == 1 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGLE_NCC_SUBRULE - Fatal error!'
    write ( *, '(a,i8)' ) '  Illegal RULE = ', rule
    stop 1
  ...
  suborder_xyz(1:3,1:suborder_num) = &
      real ( suborder_xyz_n(1:3,1:suborder_num), kind = rk ) &
