...
!  Parameters:
!
!    Input, real ( kind = rk ) A(2), B(2), the lower and upper limits.
!
...
!
  ...
  do i = 1, 2
    ...
    if ( mod ( expon(i), 2 ) == 1 ) then
      ...
      value = value * 2.0D+00 / real ( expon(i) + 1, kind = rk )
    end if
...
!
!    Output, real ( kind = rk ) W(ORDER_1D(1)*ORDER_1D(2)), the weights.
!
!    Output, real ( kind = rk ) XY(2,ORDER_1D(1)*ORDER_1D(2)), the abscissas.
!
  implicit none
  ...
  integer order
  integer order_1d(2)
  real ( kind = rk ) w(order_1D(1)*order_1D(2))
  real ( kind = rk ), allocatable :: w_1d(:)
  real ( kind = rk ), allocatable :: x_1d(:)
  real ( kind = rk ) xy(2,order_1D(1)*order_1D(2))
...
!    Input, real ( kind = rk ) A(2), B(2), the lower and upper limits.
!
!    Output, real ( kind = rk ) SQUARE_VOLUME, the volume.
!
  implicit none
