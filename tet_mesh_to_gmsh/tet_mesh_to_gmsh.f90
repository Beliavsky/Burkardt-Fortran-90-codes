...
!*****************************************************************************80
!
!! TET_MESH_TO_GMSH converts a tet mesh to a Gmsh file.
!
!  Usage:
!
...
!
  ...
  write ( *, '(a)' ) '  Convert a linear or quadratic tet mesh to Gmsh format.'
...
!
  ...
  if ( element_order == 4 ) then
    ...
    write ( *, * ) ' '
    write ( *, '(a)' ) 'TET_MESH_TO_GMSH - Fatal error!'
    write ( *, '(a)' ) '  Data must be for 4, 10 or 20-node elements.'
    stop
  end if

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read the header of "' // trim ( element_filename ) //'".'
  write ( *, '(a)' ) ' '
...
!
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  Created gmsh file "' // trim ( gmsh_filename ) // '".'
!
!  Free memory.
!
...
subroutine gmsh_write ( gmsh_filename, dim_num, node_num, node_xyz, &
  element_order, element_num, element_node )

!*****************************************************************************80
!
!! GMSH_WRITE writes the tet mesh data as a Gmsh mesh file.
!
!  Discussion:
!
...
!
!    Input, real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the node coordinates.
!
