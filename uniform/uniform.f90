...
!
!    Output, integer BEC(N), a pseudorandom binary vector.
!
  implicit none
...
!
!    Output, complex ( kind = ck ) C8_UNIFORM_01, a pseudorandom complex value.
!
  implicit none
...
!
  ...
  i = ilo + int ( r8_uniform_01 ( seed ) * real ( ihi + 1 - ilo ) )
...
!  Parameters:
!
!    Input, integer A, B, C, the coefficients of the 
...
!
!    Output, integer I4_SEED_ADVANCE, the "next" seed.
!
  implicit none
...
!
!    Output, integer I4_UNIFORM_0I, a uniform random value 
...
!    LCRG is to be applied.  It is required that 0 <= N.
!
!    Input, integer SEED, the starting value of SEED.  It is 
!    customary that 0 < SEED.
!
!    Output, integer SEED_LCRG, the value of SEED that would 
...
!  Set the new SEED.
!
  value2 = int ( an, kind = ik ) * int ( seed, kind = ik ) &
    + int ( bn, kind = ik )

  value2 = mod ( value2, int ( c, kind = ik ) )
!
!  Guarantee that the value is positive.
!
  if ( value2 < 0 ) then
    value2 = value2 + int ( c, kind = ik )
  end if

  seed_lcrg = int ( value2 )
...
!
  ...
  do j = 1, n
    ...
    do i = 1, m
      ...
      r(i,j) = a &
        + ( b - a ) * real ( seed, kind = rk ) * 4.656612875D-10
...
!
  ...
  do j = 1, n
    ...
    do i = 1, m
      ...
      r(i,j) = a(i) &
        + ( b(i) - a(i) ) * real ( seed, kind = rk ) * 4.656612875D-10
...
!
  ...
  do i = 1, m
    do j = 1, n
      ...
      r(i,j) = a(j) &
        + ( b(j) - a(j) ) * real ( seed, kind = rk ) * 4.656612875D-10
