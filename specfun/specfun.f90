...
!
!    Output, real ( kind = rk ) BESEI0, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESEI1, the value of the function.
!
  implicit none
...
!    0 < X.
!
!    Output, real ( kind = rk ) BESK0, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESEK1, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESI0, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESI1, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESJ0, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESJ1, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESK1, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESY0, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) BESY1, the value of the function.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) ARG, the argument.  The argument must not
...
!  Parameters:
!
!    Input, real ( kind = rk ) ARG, the argument.  If JINT = 1, then
...
!  Parameters:
!
!    Input, real ( kind = rk ) ARG, the argument.  0 < ARG is
...
!  Parameters:
!
!    Input, real ( kind = rk ) ARG, the argument.  XLEAST < ARG is
!    always required.  If JINT = 1, then the argument must also be
!    less than XMAX.
!
!    Output, real ( kind = rk ) RESULT, the value of the function,
...
!  Parameters:
!
!    Input, real ( kind = rk ) ARG, the argument.  If JINT = 0, ARG
!    must satisfy
!     -XMAX < ARG < XMAX;
...
!
!    Output, real ( kind = rk ) DAW, the value of the function.
!
  implicit none
...
!
!    Input, real ( kind = rk ) XMAX, the largest finite floating-point number,
!    as computed, for instance, by MACHAR.
!
!    Output, real ( kind = rk ) D(0:NMAX), the value and first NMAX derivatives
!    of Ei(X).
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) EI, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) EONE, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) EXPEI, the value of the function.
!
  implicit none
...
!    by  -(IT+3).
!
!    Output, integer IEXP, the number of bits (decimal places if
!    IBETA = 10) reserved for the representation of the exponent, including the
!    bias or sign, of a floating-point number.
!
!    Output, integer MINEXP, largest magnitude negative integer
!    such that FLOAT(IBETA)^MINEXP is positive and normalized.
!
!    Output, integer MAXEXP, the smallest positive power of BETA
!    that overflows.
!
!    Output, real ( kind = rk ) EPS, the value FLOAT(IBETA)^MACHEP.
!
!    Output, real ( kind = rk ) EPSNEG, the value FLOAT(IBETA)^NEGEPS.
!
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) R8_ERF, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) R8_ERFC, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the function.
!
!    Output, real ( kind = rk ) R8_ERFCX, the value of the function.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) REN, a pseudorandom value.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ), parameter :: c1 = 2796203.0D+00
  real ( kind = rk ), parameter :: c2 = 1.0D-06
  real ( kind = rk ), parameter :: c3 = 1.0D-12
  integer iy
  real ( kind = rk ) ren
  ...
  ren = real ( iy, kind = rk ) / c1 * ( 1.0D+00 + c2 + c3 )
...
!
!    Output, integer NCALC, error indicator.  If NCALC = NB, 
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument for which the
!    J's are to be calculated.
!
!    Input, real ( kind = rk ) ALPHA, the fractional part of order for which
...
!
!    Output, real ( kind = rk ) B(NB).  If RJBESL terminates normally, with
...
!
      if ( 3 <= nbmx ) then
        ...
        nstart = magx + 2
        nend = nb - 1
        en = real ( nstart + nstart, kind = rk ) - 2.0D+00 + ( alpha + alpha )
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the non-negative argument for which
...
!
!    Output, real ( kind = rk ) BK(NB), the results.  If the routine
...
!    K(ALPHA+I-1,X)/K(ALPHA+I-2,X) for the rest of the array.
!
!    Output, integer NCALC, error indicator.  If NCALC = NB, then 
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument.  0 <= X.
!
!    Input, real ( kind = rk ) ALPHA, the fractional part of the order
...
!    last is of order (NB - 1 + ALPHA).
!
!    Output, real ( kind = rk ) BY(NB).  If the routine terminates normally
...
!
!    Output, integer NCALC, error flag.  Before using the vector 
