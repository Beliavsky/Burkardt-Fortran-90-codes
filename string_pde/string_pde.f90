...
!*****************************************************************************80
!
!! MAIN is the main program for string_pde.
!
!  Discussion:
!
...
!  Local Parameters:
!
!    Local, real ( kind = rk ) ALPHA, the CFL stability parameter.
!
!    Local, real ( kind = rk ) C, the wave speed.
!
!    Local, real ( kind = rk ) DT, the time step.
!
!    Local, real ( kind = rk ) DX, the spatial step.
!
!    Local, integer M, the number of time steps.
!
!    Local, integer N, the number of spatial intervals.
!
!    Local, real ( kind = rk ) T1, T2, the initial and final times.
!
!    Local, real ( kind = rk ) U(M+1,N+1), the computed solution.
!
!    Local, real ( kind = rk ) X1, X2, the left and right spatial endpoints.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the location.
!
!    Output, real ( kind = rk ) G, the value of the time derivative of the 
