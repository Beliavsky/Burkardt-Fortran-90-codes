...
!
  ...
  logical converged
  real ( kind = rk ) damp
  real ( kind = rk ) damptol
  real ( kind = rk ), allocatable, dimension ( : ) :: f
  real ( kind = rk ) fnrm
  real ( kind = rk ) fnrm_old
  real ( kind = rk ) fnrm_zero
  real ( kind = rk ), allocatable, dimension ( :, : ) :: fprime
  integer ijac
  integer iknow
  integer info
  integer it
  ...
  real ( kind = rk ) snrm
  real ( kind = rk ), allocatable, dimension ( : ) :: step
  character ( len = 80 ) title
  real ( kind = rk ), allocatable, dimension ( : ) ::  x
  real ( kind = rk ), allocatable, dimension ( : ) ::  xnew
  real ( kind = rk ) xnrm
  real ( kind = rk ) xold
...
!
        if ( ijac == 0 ) then
          call p00_jac ( problem, n, x, fprime )
        else
          call p00_dif ( problem, n, x, fprime )
        end if
!
...
!
      call p00_sol ( problem, n, iknow, x )
