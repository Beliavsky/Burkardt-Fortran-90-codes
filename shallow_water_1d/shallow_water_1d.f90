...
!*****************************************************************************80
!
!! SHALLOW_WATER_1D solves the 1D shallow water equations.
!
!  Discussion:
!
...
!
!    Output, real UH_ARRAY(NX,NT+1), the mass velocity for all space and 
!    time points.
!
...
!
  ...
  real ( kind = rk ) dx
  real ( kind = rk ) dt
  character ( len = 255 ) filename_h
  character ( len = 255 ) filename_t
  character ( len = 255 ) filename_uh
  character ( len = 255 ) filename_x
  real ( kind = rk ) g
  real ( kind = rk ), allocatable :: h(:)
  real ( kind = rk ), allocatable :: h_array(:,:)
  real ( kind = rk ), allocatable :: hm(:)
  integer iarg
  integer ierror
  integer it
  integer last
  ...
  real ( kind = rk ), allocatable :: uh(:)
  real ( kind = rk ), allocatable :: uh_array(:,:)
  real ( kind = rk ), allocatable :: uhm(:)
  real ( kind = rk ), allocatable :: x(:)
  real ( kind = rk ) x_length
...
!
  ...
  dx = x_length / real ( nx - 1, kind = rk )
  dt = t_length / real ( nt, kind = rk )
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) &
    '  X  values saved in file "' // trim ( filename_x ) // '".'
  write ( *, '(a)' ) &
    '  T  values saved in file "' // trim ( filename_t ) // '".'
  write ( *, '(a)' ) &
    '  H  values saved in file "' // trim ( filename_h ) // '".'
  write ( *, '(a)' ) &
    '  UH values saved in file "' // trim ( filename_uh ) // '".'
!
!  Free memory.
!
  deallocate ( h )
...
!*****************************************************************************80
!
!! boundary_conditions() sets the boundary conditions.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
!    Input/output, real ( kind = rk ) H(NX), the height, with H(1) and H(NX) 
!    adjusted for boundary conditions.
!
!    Input/output, real ( kind = rk ) UH(NX), the mass velocity, with UH(1) 
...
!    Input, real ( kind = rk ) T, the current time.
!
!    Output, real ( kind = rk ) H(NX), the initial height for all space.
!
!    Output, real ( kind = rk ) UH(NX), the initial mass velocity for all space.
!
  implicit none
