...
!
!    Input, real ( kind = rk ) F(N), the data values at the nodes.
!
...
!
!    Output, real ( kind = rk ) xyzmin(3), xyzdel(3), containing 
!    minimum nodal coordinates and cell dim-
!    ensions, respectively.  Refer to store3.
!
!    Output, real ( kind = rk ) RMAX, square root of the largest element in 
!    RSQ, maximum radius R(K).
!
!    Output, real ( kind = rk ) RSQ(N) = the squares of the radii r(k)
!    which enter into the weights W(K).
!
!    Output, real ( kind = rk ) A(9,N), the coefficients for
...
!
  ...
  real ( kind = rk ) av
  real ( kind = rk ) avsq
  real ( kind = rk ) b(10,10)
  real ( kind = rk ) c
  real ( kind = rk ) dmin
  real ( kind = rk ), parameter :: dtol = 0.01D+00
  real ( kind = rk ) f(n)
  real ( kind = rk ) fk
  integer i
  integer ib
  ...
  real ( kind = rk ) rsold
  real ( kind = rk ) rsq(n)
  real ( kind = rk ), parameter :: rtol = 1.0D-05
  real ( kind = rk ) rws
  real ( kind = rk ) s
  real ( kind = rk ), parameter :: sf = 1.0D+00
  real ( kind = rk ) sum2
  real ( kind = rk ) t
  real ( kind = rk ) x(n)
  real ( kind = rk ) xk
  real ( kind = rk ) xyzdel(3)
  real ( kind = rk ) xyzdl(3)
  real ( kind = rk ) xyzmin(3)
  real ( kind = rk ) xyzmn(3)
  real ( kind = rk ) y(n)
  real ( kind = rk ) yk
  real ( kind = rk ) z(n)
  real ( kind = rk ) zk
...
!  Parameters:
!
!    Input, real ( kind = rk ) PX, PY, PZ, the point P at which Q is 
!    to be evaluated.
!
!    Input, integer N, the number of nodes and data values 
!    defining Q.  10 <= N.
!
!    Input, real ( kind = rk ) X(N), Y(N), Z(N), F(N), the node coordinates
...
!    to STORE3.
!
!    Input, real ( kind = rk ) XYZMIN(3), XYZDEL(3), the minimum nodal
!    coordinates and cell dimensions, respectively.  XYZDEL elements 
!    must be positive.  Refer to STORE3.
!
!    Input, real ( kind = rk ) RMAX, the square root of the largest 
!    element in RSQ, the maximum radius.
!
!    Input, real ( kind = rk ) RSQ(N), the squared radii which enter 
!    into the weights defining Q.
!
!    Input, real ( kind = rk ) A(9,N), the coefficients for the nodal 
!    functions defining Q.
!
!    Output, real ( kind = rk ) QS3VAL, the function value Q(PX,PY,PZ)
...
!  Parameters:
!
!    Input, real ( kind = rk ) PX, PY, PZ, the point P at which Q and
!    its partials are to be evaluated.
!
!    Input, integer N, the number of nodes and data values 
!    defining Q.  10 <= N.
!
!    Input, real ( kind = rk ) X(N), Y(N), Z(N), F(N), the node coordinates and
...
!
!    Input, real ( kind = rk ) RMAX, the square root of the largest element
!    in RSQ, the maximum radius.
!
!    Input, real ( kind = rk ) RSQ(N), the squared radii which enter into
!    the weights defining Q.
!
!    Input, real ( kind = rk ) A(9,N), the coefficients for the nodal 
!    functions defining Q.
!
!    Output, real ( kind = rk ) Q, the value of Q at (PX,PY,PZ) unless
!    IER == 1, in which case no values are returned.
!
!    Output, real ( kind = rk ) QX, QY, QZ, the first partial derivatives of Q at
...
!  Parameters:
!
!    Input, real ( kind = rk ) PX, PY, PZ, the coordinates of the point P
...
!    negatives).
!
!    Input, real ( kind = rk ) XYZMIN(3), XYZDEL(3), minimum nodal 
...
!
!    Output, real ( kind = rk ) DSQ, squared euclidean distance between 
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) A.  On input, the first component of
!    the vector.  On output, overwritten by r = +/-sqrt(a*a + b*b)
!
!    Input/output, real ( kind = rk ) B.  On input, the second component of 
...
!
!    Output, real ( kind = rk ) C, c = +/-(a/r)
!
!    Output, real ( kind = rk ) S, s = +/-(b/r)
!
!  Local parameters:
!
...
!
!    Input, real ( kind = rk ) C, S, the elements of the Givens rotation.
!    These may be determined by subroutine GIVENS.
!
!    Input/output, real ( kind = rk ) X(N), Y(N), the vectors to be rotated.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) XK, YK, ZK, FK = coordinates and data value
!    at node K (interpolated by q).
!
!    Input, real ( kind = rk ) XI, YI, ZI, FI = coordinates and data value
!    at node I.
!
!    Input, real ( kind = rk ) S1, S2 = reciprocals of the scale factors.
!
!    Input, real ( kind = rk ) R = radius of influence about node K defining the
!    weight.
!
!    Output, real ( kind = rk ) ROW(10), a row of the augmented
!    regression matrix.
!
!  Local parameters:
!
...
!    if ier /= 0.
!
!    Output, real ( kind = rk ) XYZMIN(3), the minimum
...
!
!    Output, real ( kind = rk ) XYZDEL(3), the dimensions
...
!
  delx = ( xmx - xmn ) / real ( nnr, kind = rk )
  dely = ( ymx - ymn ) / real ( nnr, kind = rk )
  delz = ( zmx - zmn ) / real ( nnr, kind = rk )
