...
!    0 and 2^(LEVEL_MAX+1).
!
!    Output, integer TEST_LEVEL(TEST_NUM), the value of LEVEL at
...
!  Parameters:
!
!    Input, integer ORDER, the order of the Fejer type 1 rule.
!    1 <= ORDER.
!
!    Input, integer I, the index of the desired abscissa.
!    1 <= I <= ORDER.
!
!    Output, real ( kind = rk ) F1_ABSCISSA, the value of the I-th
...
!
  ...
  if ( order < 1 ) then
    ...
    value = cos ( real ( 2 * order + 1 - 2 * i, kind = rk ) * pi &
...
!
  ...
  do i = 1, order
    theta(i) = real ( 2 * ( order + 1 - i ) - 1, kind = rk ) * pi &
             / real ( 2 * order,     kind = rk )
  end do
  ...
  w(1:order) = 2.0D+00 * w(1:order) / real ( order, kind = rk )
...
!  Parameters:
!
!    Input, integer ORDER, the order of the Fejer type 2 rule.
!    1 <= ORDER.
!
!    Input, integer I, the index of the desired abscissa.
!    1 <= I <= ORDER.
!
!    Output, real ( kind = rk ) F2_ABSCISSA, the value of the I-th
...
!
  ...
  if ( order < 1 ) then
    ...
    value = cos ( real ( order + 1 - i, kind = rk ) * pi &
                / real ( order + 1,     kind = rk ) )
  end if
...
!
  ...
  do i = 1, order
    theta(i) = real ( order + 1 - i, kind = rk ) * pi &
             / real ( order + 1, kind = rk )
  end do
...
!
!    Input, integer GRID_BASE(DIM_NUM), the "base" of the
!    rule being used in each dimension.
!
!    Output, real ( kind = rk ) GRID_POINT(DIM_NUM), the grid points of
!    abscissas.
!
  implicit none
  ...
  integer point
  integer pointer
  integer, dimension ( 0:7 ) :: skip = (/ &
    0, 1, 4, 11, 26, 57, 120, 247 /)
  real ( kind = rk ), dimension ( 247 ) :: x = (/ &
    0.0D+00, &
...
!
!    Output, real ( kind = rk ) GP_ABSCISSA, the value of the I-th
...
!
!    Input, integer GRID_INDEX(DIM_NUM,POINT_NUM), the indices of
!    the points to be tested.
!
!    Input, integer GRID_BASE(DIM_NUM), the "base", which is
...
!    index applies.
!
!    Output, integer INDEX_TO_LEVEL_OPEN, the first level on which
...
!
  ...
  integer grid_base(dim_num,point_num)
  integer grid_index(dim_num,point_num)
  integer level_max
  integer rule
...
!
  ...
  integer grid_base(dim_num,point_num)
  integer, dimension ( dim_num ) :: grid_base2
  integer grid_index(dim_num,point_num)
  integer, allocatable, dimension ( :, : ) :: grid_index2
  integer h
  integer level
...
!
  ...
  if ( any ( mod ( expon(1:dim_num), 2 ) == 1 ) ) then
    ...
    value = 2.0D+00**dim_num &
      / real ( product ( expon(1:dim_num) + 1 ), kind = rk )
  end if
...
!    rule in each dimension.
!
!    Input, integer ORDER_ND, the product of the entries of
!    ORDER_1D.
!
!    Output, integer INDX(DIM_NUM,ORDER_ND), the indices of the
!    points in the grid.  The second dimension of this array is equal to the
...
!    of ORDER_1D.
!
!    Output, integer INDX(DIM_NUM,ORDER_ND), the indices of
...
!
  ...
  real ( kind = rk ) cc_abscissa
  integer dim
  integer, allocatable, dimension ( :, : ) :: grid_base
...
!
  ...
  integer, allocatable, dimension ( : ) :: grid_level
  real ( kind = rk ) grid_point(dim_num,point_num)
  real ( kind = rk ) grid_point_temp(dim_num)
  real ( kind = rk ) grid_weight(point_num)
  real ( kind = rk ), allocatable, dimension ( : ) :: grid_weight2
  integer h
  integer, parameter :: i4_1 = 1
