...
!
  ...
  call i4vec_shift_circular_test ( )
...
!
  ...
  do test = 1, test_num
    i = i4_uniform_ab ( i4_lo, i4_hi )
    j = i4_uniform_ab ( i4_lo, i4_hi )
    k = i4_and ( i, j )
    l = iand ( i, j )
...
!
  ...
  integer k
  integer, save :: k_test(4) = (/ &
    3, 999, 3, 10 /)
  integer n
  integer, save :: n_test(4) = (/ &
    10, 1000, 100, 100 /)
...
!
  ...
  integer a
  integer, parameter :: a_hi =  100
  integer, parameter :: a_lo = -100
  integer b
  integer, parameter :: b_hi =  10
  integer, parameter :: b_lo = -10
  real ( kind = rk8 ) c0
  integer c1
  integer c2
  integer c3
  ...
  do test = 1, test_num
    a = i4_uniform_ab ( a_lo, a_hi )
    b = i4_uniform_ab ( b_lo, b_hi )
    if ( b == 0 ) then
      b = 7
    end if
    c0 = real ( a, kind = rk8 ) / real ( b, kind = rk8 )
    c1 = i4_div_rounded ( a, b )
    c2 = nint ( real ( a, kind = rk8 ) / real ( b, kind = rk8 ) )
    c3 = a / b
    c4 = int ( real ( a, kind = rk8 ) / real ( b, kind = rk8 ) )
...
!
  ...
  integer i
  integer i4_gcd
  integer, dimension(test_num) :: i_test = (/ &
    36, 49, 0, 12, 36, 1, 91 /)
  integer j
  integer, dimension(test_num) :: j_test = (/ &
    30, -7, 71, 12, 49, 42, 28 /)
  integer test
...
!
  ...
  real ( kind = rk8 ) b
  real ( kind = rk8 ), dimension(test_num) :: b_test = (/ &
...
!
  ...
  integer i
  integer, dimension ( 7 ) :: i_test = (/ &
    0, 1, 2, 3, 10, -1, -2 /)
  integer i4_power
  integer j
  integer, dimension ( 7 ) :: j_test = (/ &
    1, 2, 3, 3, 3, 4, 5 /)
  integer test
  integer test_num
...
!
  ...
  integer i4
  integer i4_sign
  integer, dimension(test_num) :: i4_vec = (/ -10, -7, 0, 5, 9 /)
  integer s
  integer test
...
!
  ...
  integer, dimension ( dim_num ) :: base = (/ 2, 3, 5 /)
  integer, dimension ( dim_num ) :: leap = (/ 1, 1, 1 /)
  integer n
  real ( kind = rk8 ) r(dim_num)
  integer, dimension ( dim_num ) :: seed = (/ 0, 0, 0 /)
  integer step
...
!
  ...
  integer, dimension(l,m,n) :: x = reshape ( (/ &
        1,  2,  3,   4,  1, &
        4,  9, 16,   1,  8, &
       27, 64,  2,   4,  6, &
        8,  2,  8,  18, 32, &
...
!
  ...
  call i4mat_uniform_ab ( m, n, b, c, a )
...
!
  ...
  integer, dimension ( 3, 3 ) :: a1 = reshape ( (/ &
    1, 4, 7, &
    2, 5, 8, &
    3, 6, 9 /), (/ 3, 3 /) )
  integer, dimension ( 3, 3 ) :: a2 = reshape ( (/ &
    1, 4, 7, &
    2, 5, 8, &
    3, 6, 0 /), (/ 3, 3 /) )
  integer, dimension ( 4, 3 ) :: a3 = reshape ( (/ &
    1,  4,  7, 10, &
    2,  5,  8, 11, &
    3,  6,  0, 12 /), (/ 4, 3 /) )
  integer, dimension ( 3, 4 ) :: a4 = reshape ( (/ &
    1, 4, 7, &
    2, 5, 8, &
    3, 6, 9, &
    7, 8, 3 /), (/ 3, 4 /) )
  integer, dimension ( 5, 3 ) :: a5 = reshape ( (/ &
    1, 4, 7, 10, 3, &
    2, 5, 8, 11, 3, &
    3, 6, 9, 12, 3 /), (/ 5, 3 /) )
  integer, dimension ( 3, 2 ) :: a6 = reshape ( (/ &
    0, 0, 0, &
    0, 0, 0 /), (/ 3, 2 /) )
  integer rank_a
...
!
  ...
  integer, parameter :: m1 = 3
  integer, parameter :: m2 = 3
  integer, parameter :: n1 = 3
  integer, parameter :: n2 = 3

  integer :: a1(m1,n1) = reshape ( (/ &
    11, 211, 3111, &
    12, 222, 3222, &
    13, 233, 3333 /), (/ 3, 3 /) )
  integer :: a2(m2,n2) = reshape ( (/ &
    10,    23, 45, &
    42, -1000, 63, &
    77,    63, 90 /), (/ 3, 3 /) )
  integer i4mat_width
...
!
  ...
  call i4mat_perm2_uniform ( m, n, a )
...
!
  ...
  integer a(n_max)
  integer, dimension (test_num) :: atest = (/ &
    -10, 2, 9, 10, 20, 24 /)
  integer aval
  integer i
  integer left
...
!
  ...
  call i4vec_uniform_ab ( m, 0, 6, j )
  call i4vec_uniform_ab ( m, 0, 6, k )
  n(1:m) = j(1:m) + k(1:m)
...
!
  ...
  v_lo = -5
  v_hi = 10
  call i4vec_uniform_ab ( n, v_lo, v_hi, v )
...
!
  ...
  integer, allocatable, dimension ( : ) :: factor_value
  integer j
...
!
  ...
  integer :: locate(5) = (/ 0, 3, 10, 20, 100 /)
...
!
  ...
  integer a(n)
  integer, allocatable, dimension ( : ) :: histo_gram
  integer histo_num
  integer i
  ...
  call i4vec_uniform_ab ( n, 0, 25, a )
...
!  Each ROW of this definition is a COLUMN of the matrix.
!
  integer, dimension(n,test_num) :: x_test = reshape ( (/ &
    1, 3, 2, 4, &
    2, 2, 2, 2, &
    1, 2, 2, 4, &
    1, 2, 3, 4, &
...
!
  ...
  call i4vec_uniform_ab ( n1, b, c, a1 )
  ...
  call i4vec_uniform_ab ( n2, b, c, a2 )
...
!
  ...
  do test = 1, test_num
    ...
    call i4vec_uniform_ab ( n, ilo, ihi, a )
    a_save(1:n) = a(1:n)
...
!
  ...
  call i4vec_permute_uniform ( n, a )
...
!
  ...
  write ( *, '(a)' ) '  assuming ascending order.'
...
subroutine i4vec_shift_circular_test ( )
...
!
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'i4vec_shift_circular_test():'
  write ( *, '(a)' ) '  i4vec_shift_circular() performs a circular shift'
  write ( *, '(a)' ) '  of K positions on an I4VEC.'
  ...
  do k = -2, 3
    ...
    b(1:n) = a(1:n)
    call i4vec_shift_circular ( k, n, b )
    write ( *, '(2x,i2,2x,5i4)' ) k, b(1:n)
...
!
  ...
  integer, dimension ( x_num ) :: x_val = (/ &
    11, 11, 11, 22, 22, 33, 33, 55, 55 /)
  integer, allocatable, dimension ( : ) :: xu_val
  integer xdnu(x_num)
...
!
  ...
  integer, parameter :: unique_max = 20
  integer, parameter :: n = 20
...
!
  ...
  call i4vec_uniform_ab ( n, 0, 10, a )
  call i4vec_transpose_print ( n, a, '  A:' )

  call i4vec_uniform_ab ( n, 0, 10, b )
...
!
  ...
  call i4vec_uniform_ab ( n, b, c, ivec )

  call i4vec_uniform_ab ( n, b, c, jvec )
