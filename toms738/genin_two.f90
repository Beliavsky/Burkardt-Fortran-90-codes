...
!
    ...
    do i = 1, seqlen
      ...
      if ( mod ( i, step ) == 0 ) then
        estimate = real ( total, kind = rk ) / real ( i, kind = rk )
...
!
!    Output, real ( kind = rk ) EXACTF, the exact value of the integral.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) QUASI(DIMEN), the next quasirandom vector.
!
!  Local Parameters:
!
!    Local, integer MAXDIM, the maximum dimension that will be used.
...
!    fixed-point integer.
!
!    Local, real ( kind = rk ) RECIP is the multiplier which changes the
...
!
    ...
    testf = 1.0D+00
    do i = 1, dimen
      testf = testf * real ( i, kind = rk ) * cos ( real ( i, kind = rk ) * quasi(i) )
    end do
!
!  Integral over [0,+1]**DIMEN Product ( 1 <= i <= DIMEN )
!  T(N(i))( 2 * x(i) - 1 ) dx
