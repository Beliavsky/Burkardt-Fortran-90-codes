...
!  Local:
!
!    integer MAXBAS, the maximum asymptotically-optimal base 
!    used up to MAXDIM.
!
!    integer MAXDIM, the maximum spatial dimension that will be used.
!
!    integer OPTBAS(2:MAXDIM), gives the asymptotically-optimal
!    base for the respective dimension.
!
!    integer OUNIT, the unit to save the output.
!
!    integer POWER(2:MAXBAS), values used in a possible
!    warm-up calculation.
!
  implicit none
...
!
    ...
    do i = 1, seqlen
      ...
      if ( mod ( i, step ) == 0 ) then
        estimate = integral_est / real ( i, kind = rk )
...
!
!    Global, real ( kind = rk ) RECIP = 1.0 / Q**NFIGS.
!
  implicit none
...
!
    ...
    exactf = 1.0D+00
    do i = 1, dimen
      exactf = exactf * sin ( real ( i, kind = rk ) )
    end do
!
!  Integral over [0,+1]**DIMEN Product ( 1 <= i <= DIMEN )
!  T(N(i))( 2 * x(i) - 1 ) dx
...
!
    ...
    x = 1.0D+00 / real ( 2**dimen, kind = rk )
...
!  Parameters:
!
!    Output, real ( kind = rk ) QUASI(*), the next vector in the sequence.
!
!  Local Parameters:
!
...
!  quasi-random vector.
!
  quasi(1:dimen) = real ( nextq(1:dimen), kind = rk ) * recip
!
...
!
  i_max = int ( sqrt ( real ( q, kind = rk ) ) ) + 1
  q_copy = q
...
!
  temp = log ( 2.0D+00**nbits - 1.0D+00 ) / log ( real ( q, kind = rk ) )
...
!  Set RECIP.
!
  recip = 1.0D+00 / real ( q**nfigs, kind = rk )
!
...
!    Input, integer DIMEN, the spatial dimension.
!
!    Input, real ( kind = rk ) QUASI(DIMEN), the argument.
!
!    Output, real ( kind = rk ) TESTF, the integrand at QUASI(*).
!
  implicit none
...
!
    ...
    testf = 1.0D+00
    do i = 1, dimen
      testf = testf * real ( i, kind = rk ) &
        * cos ( real ( i, kind = rk ) * quasi(i) )
    end do
!
!  Integral over [0,+1]**DIMEN Product ( 1 <= i <= DIMEN )
!  T(N(i))( 2 * x(i) - 1 ) dx
