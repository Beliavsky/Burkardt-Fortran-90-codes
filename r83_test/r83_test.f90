...
!
  ...
  do j = 1, n
    do i = max ( 1, j - 1 ), min ( n, j + 1 )
      if ( j == i - 1 ) then
        value = real ( j, kind = rk )
      else if ( j == i ) then
        value = 4.0D+00 * real ( j, kind = rk )
      else if ( j == i + 1 ) then
        value = real ( j, kind = rk )
      end if
      a(i-j+2,j) = value
...
!
  ...
  real ( kind = rk ), allocatable :: a_83(:,:)
  real ( kind = rk ), allocatable :: a_ge(:,:)
  real ( kind = rk ), allocatable :: ax_83(:)
