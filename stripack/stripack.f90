...
!
!    Input, real ( kind = rk ) X(K), Y(K), Z(K), the coordinates of the nodes.
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) V1(3), V2(3), V3(3), the Cartesian coordinates
!    of unit vectors (the three triangle vertices in any order).  These 
!    vectors, if nonzero, are implicitly scaled to have length 1.
!
!    Output, real ( kind = rk ) AREAS, the area of the spherical triangle 
...
!  Parameters:
!
!    Input, real ( kind = rk ) V1(3), V2(3), V3(3), the coordinates of the 
!    three triangle vertices (unit vectors) in counter clockwise order.
!
!    Output, real ( kind = rk ) C(3), the coordinates of the circumcenter unless
...
!    IER = 1.
!
!    Output, real ( kind = rk ) XC(2*N-4), YC(2*N-4), ZC(2*N-4), the coordinates
...
!    if 0 < NB.
!
!    Output, real ( kind = rk ) RC(2*N-4), the circumradii (the arc lengths or
...
!
  ...
  real ( kind = rk ) v3(3)
  real ( kind = rk ) x(n)
  real ( kind = rk ) xc(2*n-4)
  real ( kind = rk ) y(n)
  real ( kind = rk ) yc(2*n-4)
  real ( kind = rk ) z(n)
  real ( kind = rk ) zc(2*n-4)
...
!    4 <= N.
!
!    Input, integer IO1, IO2, indexes (in the range 1 to N) of
...
!  Parameters:
!
!    Input, integer N0, NB, indexes, in the range 1 to N, of a 
...
!
!    Input, integer LPH, list pointer to the hole (NB as a 
...
!    deleted.  1 <= K <= N.
!
!    Input/output, integer N, the number of nodes in the 
!    triangulation.  4 <= N.  Note that N will be decremented following the 
!    deletion.
!
!    Input/output, real ( kind = rk ) X(N), Y(N), Z(N), the coordinates of 
...
!
!    Input/output, integer LWK, the number of columns reserved for 
...
!
!    Output, integer IWK(2,LWK), indexes of the endpoints of the 
...
!  Parameters:
!
!    Input, integer IN1, IN2, indexes (of X, Y, and Z) in the 
!    range 1 to N defining a pair of nodes to be connected by an arc.
!
!    Input, real ( kind = rk ) X(N), Y(N), Z(N), the coordinates of the nodes.
!
!    Input/output, integer LWK.  On input, the number of columns 
...
!
!    Output, integer IWK(2*LWK), the indexes of the endpoints of 
...
!
!    Input, integer L, the number of nodes in the sequence on 
!    output.  2 <= L <= N.
!
!    Input/output, integer NPTS(L).  On input, the indexes of 
...
!
!    Output, real ( kind = rk ) DF, value of an increasing function (negative
...
!  Parameters:
!
!    Input, real ( kind = rk ) P(3), the coordinates of the point (unit vector)
!    to be located.
!
!    Input, integer LV, the length of arrays XV, YV, and ZV.
!
!    Input, real ( kind = rk ) XV(LV), YV(LV), ZV(LV), the coordinates of unit
...
!  Parameters:
!
!    Input, real ( kind = rk ) P1(3), P2(3), the coordinates of unit vectors.
!
!    Input, real ( kind = rk ) CN(3), the coordinates of a nonzero vector 
...
!
!    Output, real ( kind = rk ) P(3), point of intersection defined above 
...
!
  ...
  x = ( real ( ix, kind = rk ) / 30269.0D+00 ) &
    + ( real ( iy, kind = rk ) / 30307.0D+00 ) &
    + ( real ( iz, kind = rk ) / 30323.0D+00 )

  u = x - int ( x )
  jrand = int ( real ( n, kind = rk ) * u + 1.0D+00 )
...
!  Parameters:
!
!    Input, real ( kind = rk ) X1, Y1, Z1 = Coordinates of N1.
!
!    Input, real ( kind = rk ) X2, Y2, Z2 = Coordinates of N2.
!
!    Input, real ( kind = rk ) X0, Y0, Z0 = Coordinates of N0.
!
