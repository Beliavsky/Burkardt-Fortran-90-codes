...
!    Input, integer N, the spatial dimension.
!
!    Output, integer CN_LEG_03_1_SIZE, the order.
!
  implicit none
...
!    Input, integer N, the spatial dimension.
!
!    Input, integer EXPON(N), the polynomial exponents.
!    0 <= ALPHA(*).
!
!    Output, real ( kind = rk ) VALUE, the value of the integral.
!
  ...
  if ( any ( expon(1:n) < 0 ) ) then
    ...
    value = 1.0D+00
    do i = 1, n
      arg = ( real ( expon(i) + 1, kind = rk ) ) / 2.0D+00
