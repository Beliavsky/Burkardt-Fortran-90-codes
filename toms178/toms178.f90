...
!  Parameters:
!
!    Input, real ( kind = rk ) DELTA(NVARS), the size of a step in
!    each direction.
!
!    Input/output, real ( kind = rk ) POINT(NVARS); on input, the current
!    candidate.  On output, the value of POINT may have been updated.
!
!    Input, real ( kind = rk ) PREVBEST, the minimum value of the function seen
!    so far.
!
...
!      real ( kind = rk ) x(n)
!
!    Input/output, integer FUNEVALS, the number of function
!    evaluations.
!
!    Output, real ( kind = rk ) BEST_NEARBY, the minimum value of the function
...
!
  ...
  real ( kind = rk ) best_nearby
  real ( kind = rk ) delta(nvars)
  real ( kind = rk ), external :: f
  real ( kind = rk ) ftmp
  integer funevals
  integer i
  real ( kind = rk ) minf
  real ( kind = rk ) point(nvars)
  real ( kind = rk ) prevbest
  real ( kind = rk ) z(nvars)
...
!
!    Input, real ( kind = rk ) STARTPT(NVARS), the user-supplied
!    initial estimate for the minimizer.
!
!    Output, real ( kind = rk ) ENDPT(NVARS), the estimate for the
!    minimizer, as calculated by the program.
!
!    Input, real ( kind = rk ) RHO, a user-supplied convergence parameter
...
!
!    Input, real ( kind = rk ) EPS, the criterion for halting
...
!      real ( kind = rk ) x(n)
!
!    Output, integer HOOKE, the number of iterations taken.
!
  implicit none
  ...
  integer keep
  real ( kind = rk ) newf
  real ( kind = rk ) newx(nvars)
  real ( kind = rk ) rho
  real ( kind = rk ) startpt(nvars)
  real ( kind = rk ) steplength
  real ( kind = rk ) tmp
  logical, parameter :: verbose = .false.
  real ( kind = rk ) xbefore(nvars)
