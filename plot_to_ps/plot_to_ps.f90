...
!   end if
  ...
  write ( *, '(a)' ) 'plot_to_ps() converts ' // trim ( filein_name) &
    // ' to ' // trim ( fileout_name )
!
!  Open the input file.
!
...
!    of the ellipse.
!
!    Input, real ( kind = rk ) R1, R2, the "radius" of the ellipse in the major
!    and minor axis directions.  A circle has these values equal.
!
!    Input, real ( kind = rk ) THETA, the angle that the major axis of the
...
!    N must be at least 1.
!
!    Output, real ( kind = rk ) X(N), Y(N), the coordinates of points on
!    the ellipse.
!
  implicit none
...
!  Parameters
!
!    Input, integer FILEIN_UNIT, the unit number associated with 
...
!
  ...
  real ( kind = rk ) g
  real ( kind = rk ), save :: hist_fat = 1.0D+00
  integer i
  integer ierror
  integer ios
  integer lchar
  ...
  real ( kind = rk ) :: xmax = 1.0D+00
  real ( kind = rk ) :: xmin = 0.0D+00
  real ( kind = rk ) xvec(maxn)
  real ( kind = rk ) x1
  real ( kind = rk ) x2
  real ( kind = rk ) x3
  real ( kind = rk ), allocatable :: y(:)
  real ( kind = rk ) :: ymax = 1.0D+00
  real ( kind = rk ) :: ymin = 0.0D+00
  real ( kind = rk ) yvec(maxn)
  real ( kind = rk ) y1
  real ( kind = rk ) y2
  real ( kind = rk ) y3
...
!
    if ( s_eqi ( word, 'ARC' ) ) then
    ...
    else if ( s_eqi ( word, 'STAR_POINTS' ) ) then
      ...
      allocate ( x(10) )
      allocate ( y(10) )
      call ps_star_points ( x1, y1, r1, x, y )
      do i = 1, 10
        write ( *, '(g14.6,g14.6)' ) x(i), y(i)
      end do
      deallocate ( x )
      deallocate ( y )
...
!  Parameters
!
!    Input/output, integer FILEIN_LINECOUNT, the number of lines
...
!    when input comes from the user, and not a file.
!
!    Input, real ( kind = rk ) HIST_FAT, the width of each histogram bar.
!
!    Output, integer IERROR, error flag.
!    0, no error detected,
!    nonzero, an error occurred.
!
  ...
  logical s_eqi
  character ( len = 80 ) word
  real ( kind = rk ) xvec(maxpoly)
  real ( kind = rk ) x1
  real ( kind = rk ) yvec(maxpoly)
  real ( kind = rk ) y1
  real ( kind = rk ) y1_old
...
!  Parameters:
!
!    Input, real ( kind = rk ) X1, Y1, the starting point of the arrow.
!
!    Input, real ( kind = rk ) X2, Y2, the ending point of the arrow.
!
  implicit none
...
!    Input, real ( kind = rk ) R, the radius of the circle.
!
!    Input, real ( kind = rk ) THETA1, THETA2, the angular coordinates
...
!  Parameters:
!
!    Input, real ( kind = rk ) FILL_GRAY, the gray fill used to fill polygons.
...
!  Parameters:
!
!    Input, real ( kind = rk ) XMIN, XMAX, the minimum and maximum values
...
!
!    Input, real ( kind = rk ) YMIN, YMAX, the minimum and maximum values
...
!
  do i = 1, nx
    ...
    if ( 1 < nx ) then
      ...
      x = ( real ( nx - i,     kind = rk ) * xmin   &
          + real (      i - 1, kind = rk ) * xmax ) &
          / real ( nx     - 1, kind = rk )
...
!
  do i = 1, ny
    ...
    if ( 1 < ny ) then
      ...
      y = ( real ( ny - i,     kind = rk ) * ymin   &
          + real (      i - 1, kind = rk ) * ymax ) &
          / real ( ny     - 1, kind = rk )
...
!    Input, character ( len = * ) STRING, the string to be printed.
!
!    Input, real ( kind = rk ) ANGLE, the angle of rotation, in degrees.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, Y, the X and Y components of the new point.
!
  implicit none
...
!    Input, real ( kind = rk ) R, the radius of the circle.
!
!    Input, real ( kind = rk ) ANGLE_MIN, ANGLE_MAX, the minimum and
...
!  Parameters:
!
!    Input, real ( kind = rk ) X0, Y0, the coordinates of the center of the box.
!
!    Input, real ( kind = rk ) R, the radius of the box.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X0, Y0, the coordinates of the center of the star.
!
!    Input, real ( kind = rk ) R, the radius of the star.
!
  implicit none
  ...
  do i = 1, n
    angle = pi * real ( mod ( ( - 7 + i * 8 ), 20 ), kind = rk ) / 10.0D+00
...
!    Input, real ( kind = rk ) R, the radius of the star.
!
!    Output, real ( kind = rk ) X(10), Y(10), the coordinates of points
!    on the star.
!
  implicit none
  ...
  do i = 1, 5
    ...
    y(j) = y0 + r * sin ( angle )
    j = j + 1
    angle = pi * real ( mod ( ( - 5 + i * 8 ), 20 ), kind = rk ) / 10.0D+00
...
!
  ...
  do i = 2, n
    angle = ( real ( n - i,     kind = rk ) * angle_min   &
            + real (     i - 2, kind = rk ) * angle_max ) &
            / real ( n     - 2, kind = rk  )
