...
!
!    Output, real ( kind = rk ) XYZ(3,1), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(1) = (/ &
    1.0D+00 /)
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,1) = reshape ( (/ &
...
!
!    Output, real ( kind = rk ) XYZ(3,5), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,5) = reshape ( (/ &
...
!
!    Output, real ( kind = rk ) XYZ(3,6), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,6) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(8), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,8), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(8) = (/ &
   ...
   0.17441058844013092706D+00 /)
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,8) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(9), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,9), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,9) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(13), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,13), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,13) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(18), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,18), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(18) = (/ &
   ...
   0.05383042853090460712D+00 /)
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,18) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(27), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,27), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(27) = (/ &
   ...
   0.006933033103838124540D+00 /)
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,27) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(48), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,48), the abscissas.
!
  implicit none
  ...
  1.84346316995826843D-003 /)
  real ( kind = rk ) xyz(3,order)
  real ( kind = rk ) :: xyz_save(3,48) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) PYRAMID_UNIT_VOLUME, the volume.
!
  implicit none
