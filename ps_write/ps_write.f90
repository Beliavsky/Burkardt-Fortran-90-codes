...
!
!    real ( kind = 8 ) X(N), Y(N), the coordinates of the points to mark.
!
  implicit none
...
!
  ...
  do i = 1, n
    ...
    if ( point_inside_box_2d ( xmin, ymin, xmax, ymax, x(i), y(i) ) ) then
      pxcen = plotxmin2 + nint ( alpha * ( x(i) - xmin ) )
      pycen = plotymin2 + nint ( alpha * ( y(i) - ymin ) )
      write ( unit, '(3i6,a)' ) pxcen, pycen, marker_size, &
        ' 0 360 arc closepath stroke'
      call ps_setting_real ( 'SET', 'XCUR', x(i) )
      call ps_setting_real ( 'SET', 'YCUR', y(i) )
    end if
...
!    integer N, the number of points.
!
!    real ( kind = 8 ) X(N), Y(N), the coordinates of the point to mark.
!
  implicit none
