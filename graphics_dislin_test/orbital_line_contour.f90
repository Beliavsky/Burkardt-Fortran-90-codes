...
!*****************************************************************************80
!
!! orbital_line_contour() uses DISLIN to display a contour plot of Z(X,Y) data.
!
!  Licensing:
!
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'orbital_line_contour:'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Use DISLIN to make a contour plot of Z(X,Y) data.'
!
!  Read the data.
!
  call r8mat_header_read ( 'orbital_data.txt', m, nn )

  allocate ( xyz(1:m,1:nn) )

  call r8mat_data_read ( 'orbital_data.txt', m, nn, xyz )
!
!  Split the data.
...
!
  call setfil ( 'orbital_line_contour.png' )
!
...
!
  value1 = 1.0D+00
  value2 = 1.0D+00
  call graf ( xmin, xmax, xmin, value1, ymin, ymax, ymin, value2 )
!
...
!  Define the title.
!
  call titlin ( 'Orbital line contour', 1 )
  call title ( )
!
...
!
  ...
  do level = 1, level_num
    ...
    level_value = real ( ( real ( level_num + 1 - level, kind = rk ) * zmin   &
                  + real (                 level, kind = rk ) * zmax ) &
                  / real ( level_num + 1,         kind = rk ), kind = rk )
