...
!    Input, real ( kind = rk ) A(M,N), the array to be examined.
!
!    Output, real ( kind = rk ) MEAN(M), the means, or averages, of the rows.
!
  implicit none
...
!
!    Input, real ( kind = rk ) V(M,N), the data.
!
!    Output, real ( kind = rk ) A(M,N+1), the running average.  A(I,J) is the 
...
!
  do j = 2, n + 1
    a(1:m,j) = a(1:m,j) /  real ( j - 1, kind = rk )
  end do
...
!    Input, real ( kind = rk ) V(M,N), the data.
!
!    Output, real ( kind = rk ) S(M,N+1), the running sums.  S(I,J) is the sum
!    of V(i,1:J-1).
!
  implicit none
...
!    Input, real ( kind = rk ) A(M,N), the M by N array.
!
!    Output, real ( kind = rk ) ROWSUM(M), the sum of the entries of
!    each row.
!
  implicit none
...
!    Input, real ( kind = rk ) A(M,N), the M by N array.
!
!    Output, real ( kind = rk ) X(M*N), a vector containing the M rows of A.
!
  implicit none
...
subroutine r8row_uniform_ab ( m, n, a, b, r )
...
!
!    Output, real ( kind = rk ) VARIANCE(M), the variances of the rows.
!
  implicit none
  ...
  do i = 1, m
    ...
    mean = sum ( a(i,1:n) ) / real ( n, kind = rk )
