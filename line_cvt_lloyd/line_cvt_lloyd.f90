...
!
!    Input, real ( kind = rk ) E_PLOT(0:IT_NUM), the energy per iteration.
!
...
!    Input, integer ( kind = 4 ) IT_NUM, the number of iterations to take.
!
!    Input, real ( kind = rk ) X_PLOT(N,IT_NUM), the point locations over time.
!
!    Input, character ( len = * ) HEADER, an identifying string.
!
  implicit none
  ...
  integer ( kind = 4 ) it
  character ( len = 255 ) plot_filename
  real ( kind = rk ) x_plot(n,0:it_num)
!
!  Write data file.
!
  call get_unit ( data_unit )
...
!    Input, character ( len = * ) HEADER, an identifying string.
!
!    Input/output, real ( kind = rk ) X(N), the point locations.
!
  implicit none
  ...
  do it = 1, it_num
    ...
    xm = sum ( ( x_old(1:n) - x(1:n) ) ** 2 ) / real ( n, kind = rk )
    xm_plot(it) = xm
...
!    Input, real ( kind = rk ) A, B, the left and right endpoints.
!
!    Input, real ( kind = rk ) X(N), the generator locations.
!
!    Output, real ( kind = rk ) E, the energy of the generator distribution.
!
  implicit none
...
!    Input, real ( kind = rk ) A, B, the left and right endpoints.
!
!    Input, real ( kind = rk ) X(N), the point locations.
!
  implicit none
...
!    Input, integer ( kind = 4 ) IT_NUM, the number of iterations to take.
!
!    Input, real ( kind = rk ) XM_PLOT(IT_NUM), the average motion per iteration.
!
!    Input, character ( len = * ) HEADER, an identifying string.
!
  implicit none
