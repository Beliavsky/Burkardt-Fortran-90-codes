...
!
  ...
  do i = 1, n
    theta(i) = real ( 2 * n - 2 * i + 1, kind = rk ) * pi &
             / real ( 2 * n,             kind = rk )
  end do
...
!
  ...
  real ( kind = rk ) l_interp(data_num,interp_num)
  real ( kind = rk ) p_data(m,data_num)
  real ( kind = rk ) p_interp(m,interp_num)
  real ( kind = rk ) t_data(data_num)
  real ( kind = rk ) t_interp(interp_num)
!
...
!
!    Input, real ( kind = rk ) T_DATA(DATA_NUM), the data points.
!
!    Input, integer INTERP_NUM, the number of
!    interpolation points.
!
!    Input, real ( kind = rk ) T_INTERP(INTERP_NUM), the
!    interpolation points.
!
!    Output, real ( kind = rk ) L_INTERP(DATA_NUM,INTERP_NUM), the values
...
!
  ...
  do i = 1, n
    x(i) = ( real ( n - i + 1, kind = rk ) * ( -1.0D+00 )   &
           + real (     i,     kind = rk ) * ( +1.0D+00 ) ) &
           / real ( n     + 1, kind = rk )
  end do
...
!    Input, integer DATA_NUM, the number of data points.
!
!    Input, real ( kind = rk ) P_DATA(M,DATA_NUM), the data values.
!
!    Output, real ( kind = rk ) T_DATA(DATA_NUM), parameter values
...
!
!    Input, real ( kind = rk ) A(M,N), a "small" M by N array.
!
!    Input, integer M2, N2, the number of rows and columns in A2.
!
!    Output, real ( kind = rk ) A2(M2,N2), the expanded array, which
...
!
!    Output, real ( kind = rk ) XFAT(BEFORE+(N-1)*(FAT+1)+1+AFTER), the
!    "fattened" data.
!
  implicit none
...
!
  do j = 1, after
    k = k + 1
    xfat(k) = &
      ( real ( fat - j + 1, kind = rk ) * x(n)     &
      + real (       j,     kind = rk ) * ( x(n) + ( x(n) - x(n-1) ) ) ) &
      / real ( fat     + 1, kind = rk )
  end do
...
!
!    Input, real ( kind = rk ) A(N), a sorted vector.
!
!    Input, real ( kind = rk ) VALUE, the value whose nearest vector
!    entry is sought.
!
!    Output, integer R8VEC_SORTED_NEAREST, the index of the nearest
!    entry in the vector.
!
  implicit none
