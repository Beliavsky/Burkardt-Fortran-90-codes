...
!
!    Input, real ( kind = rk ) X(N), the points.
!
  implicit none
  ...
  do i = 1, n
    angle(i) = r8_pi * real ( 2 * ( i - 1 ) + 1, kind = rk ) &
                     / real ( 2 * n, kind = rk )
  end do
...
!
  ...
  if ( n == 1 ) then
    ...
    do i = 1, n
      angle(i) = r8_pi * real ( n - i, kind = rk ) / real ( n - 1, kind = rk )
    end do
    x(1:n) = cos ( angle(1:n) )
  end if
...
!
  ...
  do i = 1, n
    angle(i) = r8_pi * real ( 2 * n - 2 * i + 1, kind = rk ) &
                     / real ( 2 * n         + 1, kind = rk )
  end do
...
!
  ...
  do i = 1, n
    angle(i) = r8_pi * real ( 2 * ( n - i + 1 ), kind = rk ) &
                     / real ( 2 * n + 1, kind = rk )
  end do
...
!
  ...
  do i = 1, n
    x(i) = real ( - n  - 1 + 2 * i, kind = rk ) / real ( n + 1, kind = rk )
  end do
...
!
  ...
  do i = 1, n
    x(i) = real ( - n  - 1 + 2 * i, kind = rk ) / real ( n, kind = rk )
  end do
...
!
  ...
  do i = 1, n
    theta(i) = r8_pi * real ( 2 * n + 1 - 2 * i, kind = rk ) &
                     / real ( 2 * n, kind = rk )
  end do
...
!
  ...
  do i = 1, n
    theta(i) = r8_pi * real ( n + 1 - i, kind = rk ) &
                     / real ( n + 1, kind = rk )
  end do
...
!
!    Input, real ( kind = rk ) X(N), the interpolation points.
!
!    Input, integer NFUN, the number of evaluation points.
!
!    Input, real ( kind = rk ) XFUN(*), the evaluation points.
!
!    Output, real ( kind = rk ) LMAX, an estimate of the Lebesgue constant 
!    for the points.
!
  implicit none
  ...
  real ( kind = rk ) lfun(nfun)
  real ( kind = rk ) lmax
  real ( kind = rk ) x(n)
  real ( kind = rk ) xfun(nfun)
...
!    Input, integer NFUN, the number of evaluation points.
!
!    Input, real ( kind = rk ) XFUN(NFUN), the evaluation points.
!
!    Output, real ( kind = rk ) LFUN(NFUN), the Lebesgue function values.
!
  implicit none
