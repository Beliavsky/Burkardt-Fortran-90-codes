...
!
!    Input, integer ORDER_1D, the order of the 1D rule to use.
!
...
!
  ...
  real ( kind = rk ) volume
  real ( kind = rk ) w
  real ( kind = rk ) weight(order_1d)
  real ( kind = rk ) x(dim_num,point_num)
  real ( kind = rk ) xtab(order_1d)
!
...
!
  ...
  result = result * volume / real ( 2.0D+00**dim_num, kind = rk )
...
!
  ...
  real ( kind = rk ) a(dim_num)
  real ( kind = rk ) a_sub(dim_num)
  real ( kind = rk ) b(dim_num)
  real ( kind = rk ) b_sub(dim_num)
  integer dim
  integer indx(dim_num)
  integer problem
  integer j
  integer k
  real ( kind = rk ) result
  integer sub_indx(dim_num)
  integer sub_num
  real ( kind = rk ) value(point_num)
  real ( kind = rk ) volume
  real ( kind = rk ) w
  real ( kind = rk ), save, dimension ( norder ) :: weight = (/ &
    ...
    0.236926885056189087514264040720D+00 /)
  real ( kind = rk ) x(dim_num,point_num)
  real ( kind = rk ), save, dimension ( norder ) :: xtab = (/ &
...
!
  ...
  do
    ...
    do dim = 1, dim_num
      ...
      a_sub(dim) = ( real ( sub_num + 1 - sub_indx(dim), kind = rk ) * a(dim)   &
                   + real (         - 1 + sub_indx(dim), kind = rk ) * b(dim) ) &
                   / real ( sub_num,                     kind = rk )

      b_sub(dim) = ( real ( sub_num     - sub_indx(dim), kind = rk ) * a(dim)   &
                   + real (               sub_indx(dim), kind = rk ) * b(dim) ) &
                   / real ( sub_num,                     kind = rk )
...
!
  ...
  result = result * volume / real ( ( 2 * sub_num )**dim_num, kind = rk )
...
!
  ...
  result = sum ( value(1:point_num) ) * volume / real ( point_num, kind = rk )
...
!  Parameters:
!
!    Input, integer PROBLEM, the index of the desired problem.
!
...
!
!    Input, real ( kind = rk ) X(DIM_NUM,POINT_NUM), the evaluation points.
!
!    Output, real ( kind = rk ) VALUE(POINT_NUM), the function values.
!
  implicit none
...
!  Parameters:
!
!    Input, integer PROBLEM, the index of the test problem.
!
!    Input, integer DIM_NUM, the dimension of the argument.
!
!    Output, real ( kind = rk ) A(DIM_NUM), B(DIM_NUM), the lower and upper 
...
!  Parameters:
!
!    Input, integer PROBLEM, the number of the test problem.
!
...
!
!    Input/output, real ( kind = rk ) VALUE(DIM_NUM), the value of the variable.
!
  implicit none
...
!    Input, integer POINT_NUM, the number of points.
!
!    Input, real ( kind = rk ) X01(DIM_NUM,POINT_NUM), the points, in [0,1], 
!    to be transformed.
!
!    Output, real ( kind = rk ) XAB(DIM_NUM,POINT_NUM), the transformed points.
!
  implicit none
  ...
  integer dim
  integer problem
  real ( kind = rk ) x01(dim_num,point_num)
  real ( kind = rk ) xab(dim_num,point_num)
...
!    Input, integer DIM_NUM, the dimension of the problem.
!
!    Output, real ( kind = rk ) VOLUME, the volume of the integration region.
!
  implicit none
...
!
  ...
  exact = real ( ( dim_num ) * ( 3 * dim_num + 1 ), kind = rk ) / 12.0D+00
...
!
  ...
  exact = real ( dim_num * ( 5 * dim_num - 2 ), kind = rk ) / 15.0D+00
...
!    Input, integer DIM_NUM, the dimension of the argument.
!
!    Output, real ( kind = rk ) A(DIM_NUM), B(DIM_NUM), the limits
!    of integration.
!    Note that if A = -HUGE(A), the lower limit is
!    actually negative infinity, and if B = HUGE(B), the upper limit
!    is actually infinity.
...
!  Parameters:
!
!    Input, integer N, the dimension of the argument.
!
!    Output, real ( kind = rk ) A(DIM_NUM),B(DIM_NUM), the lower and upper
!    limits of integration.
!    Note that if A = -HUGE(A), the lower limit is
...
!
  ...
  exact = 0.5D+00 - sqrt ( 2.0D+00**( 3 * dim_num ) ) &
    * cos ( 0.25D+00 * pi * real ( dim_num, kind = rk ) ) &
    / ( 2.0D+00 * pi**dim_num )
...
!
!    Input/output, real ( kind = rk ) VALUE(DIM_NUM), the value of the object.
!
  implicit none
  ...
  character ( len = * ) action
  real ( kind = rk ), allocatable, save, dimension ( : ) :: c
  character ( len = * ) name
  ...
  if ( action(1:1) == 'D' .or. action(1:1) == 'd' ) then
    ...
    if ( name == 'C' .or. name == 'c' .or. name == '*' ) then
      c(1:dim_num) = 1.0D+00 / real ( dim_num, kind = rk )
    end if
    ...
    if ( name == 'C' .or. name == 'c' ) then
      ...
      c(1:dim_num) = c(1:dim_num) * 60.0D+00 / real ( dim_num, kind = rk )
      value(1:dim_num) = c(1:dim_num)
    else
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'P09_R8VEC - Fatal error!'
...
!
  ...
  exact = real ( dim_num, kind = rk ) / 4.0D+00
...
!
  ...
  exact = 1.0D+00
  do dim = 1, dim_num
    exact = exact * sin ( real ( dim, kind = rk ) )
  end do
...
!
  ...
  do point = 1, point_num
    do dim = 1, dim_num
      value(point) = value(point) * real ( dim, kind = rk ) &
        * cos ( real ( dim, kind = rk ) * x(dim,point) )
    end do
  end do
...
!
  ...
  do dim = 1, dim_num
    if ( dim <= remainder ) then
      exact = exact / real ( fraction + 2, kind = rk )
    else
      exact = exact / real ( fraction + 1, kind = rk )
    end if
  end do
...
!
  ...
  integer, save :: dim_num_save = 0
  real ( kind = rk ) value(dim_num)
  real ( kind = rk ), allocatable, save, dimension ( : ) :: z
...
!
  ...
  exact = 0.0D+00
  exponent = real ( dim_num + p, kind = rk )

  minus_one = -1.0D+00
  do dim = 0, dim_num
    minus_one = - minus_one
    exact = exact + minus_one * r8_choose ( dim_num, dim ) &
      * ( real ( dim_num - dim, kind = rk ) * bval + real ( dim, kind = rk ) &
      * aval )**exponent
  end do

  do dim = 1, dim_num
    exact = exact / ( p + real ( dim, kind = rk ) )
  end do
...
!
  ...
  real ( kind = rk ), save :: a = 0.0D+00
  character ( len = * ) action
  real ( kind = rk ), save :: b = 1.0D+00
  character ( len = * ) name
  real ( kind = rk ) value
  real ( kind = rk ), save :: p = 2.0D+00
...
!
  ...
  exact = 2.0D+00 * c
  do dim = 1, dim_num
    arg = real ( e(dim) + 1, kind = rk ) / 2.0D+00
    exact = exact * gamma ( arg )
  end do

  arg = real ( sum ( e(1:dim_num) ) + dim_num, kind = rk ) / 2.0D+00
...
!
  ...
  character ( len = * ) action
  integer, allocatable, save, dimension ( : ) :: e
  character ( len = * ) name
  integer, save :: dim_num_save = 0
  integer value(dim_num)
...
!
  ...
  arg = real ( e_sum, kind = rk ) / 2.0D+00

  exact = exact / gamma ( arg )

  exact = exact * r**( e_sum ) / real ( e_sum, kind = rk )
...
!
  ...
  exact = c
  do dim = 1, dim_num
    exact = exact * gamma ( real ( e(dim) + 1, kind = rk ) )
  end do

  exact = exact / gamma ( real ( sum ( e(1:dim_num) ) + 1, kind = rk ) )
...
!
  ...
  do 
    ...
    i = i + 1
    term = term * c / real ( i, kind = rk )

    term2 = term / real ( ( i + 1 )**dim_num, kind = rk )
...
!
  ...
  if ( action(1:1) == 'D' .or. action(1:1) == 'd' ) then
    ...
    if ( name == 'C' .or. name == 'c' ) then
      call random_number ( harvest = c(1:dim_num) )
      c(1:dim_num) = c(1:dim_num) * 25.0D+00 / real ( dim_num, kind = rk )
      value(1:dim_num) = c(1:dim_num)
    else
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'P27_R8VEC - Fatal error!'
...
! 
  ...
  do
    ...
    total = total + (-1.0D+00)**sum(ivec(1:dim_num)) &
      / ( 1.0D+00 &
      + dot_product ( real ( ivec(1:dim_num), kind = rk ), c(1:dim_num) ) )**r

  end do

  a = 1.0D+00
  do dim = 0, dim_num-1
    a = a * ( r + real ( dim, kind = rk ) )
  end do
...
!
  ...
  if ( action(1:1) == 'D' .or. action(1:1) == 'd' ) then
    ...
    if ( name == 'C' .or. name == 'c' ) then
      call random_number ( harvest = c(1:dim_num) )
      c(1:dim_num) = c(1:dim_num) * 80.0D+00 / real ( dim_num**2, kind = rk )
      value(1:dim_num) = c(1:dim_num)
    else
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'P29_R8VEC - Fatal error!'
...
!
  ...
  if ( action(1:1) == 'D' .or. action(1:1) == 'd' ) then
    ...
    if ( name == 'C' .or. name == 'c' .or. name == '*' ) then
      c(1:dim_num) = 0.5D+00**( 1.0D+00 / real ( dim_num, kind = rk ) )
    end if

    if ( name == 'Z' .or. name == 'z' .or. name == '*' ) then
      z(1:dim_num) = 0.5D+00**( 1.0D+00 / real ( dim_num, kind = rk ) )
    end if
...
!
  ...
  exact = real ( dim_num, kind = rk ) / 2.0D+00 * sqrt ( pi**dim_num ) &
    / gamma ( real ( dim_num + 4 ) / 2.0D+00 )
...
!
!    Output, real ( kind = rk ) R8_ERROR, the value of the error function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument of the error function.
!
!    Output, real ( kind = rk ) R8_ERRORC, the value of the complementary 
!    error function at X.
!
  implicit none
...
!
  ...
  volume = 1.0D+00
  do dim = 1, dim_num
    volume = volume / real ( dim, kind = rk )
  end do
...
!
  ...
  if ( mod ( dim_num, 2 ) == 0 ) then
    m = dim_num / 2
    do i = 1, m
      volume = volume * pi / real ( i, kind = rk )
    end do
  else
    m = ( dim_num - 1 ) / 2
    do i = 1, m
