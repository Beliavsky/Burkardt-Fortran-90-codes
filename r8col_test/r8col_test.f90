...
!
  ...
  real ( kind = rk ) dtab(m,n)
  integer i
  integer icol
  integer j
  integer k
  real ( kind = rk ) r8vec(m)

  k = 1
  do i = 1, m
    do j = 1, n

      dtab(i,j) = real ( k, kind = rk )

      if ( j == 3 ) then
        r8vec(i) = real ( k, kind = rk )
      end if
...
!
  ...
  real ( kind = rk ), dimension (m,n_max) :: a = reshape ( (/ &
  ...
  integer col
  real ( kind = rk ), dimension(m) :: r8vec1 = (/ 3.0D+00, 7.0D+00, 11.0D+00 /)
  real ( kind = rk ), dimension(m) :: r8vec2 = (/ 3.0D+00, 4.0D+00, 18.0D+00 /)
  integer n
...
!
  ...
  integer, dimension ( n ) :: perm = (/ 2, 4, 5, 1, 3 /)
...
!
  ...
  call r8col_uniform_ab ( m, n, b, c, a )
...
!
  ...
  real ( kind = rk ), dimension (m,n) :: a = reshape ( (/ &
    ...
    1.0D+00,  5.0D+00,  9.1D+00, &
    1.0D+00,  5.1D+00,  9.0D+00 /), (/ m, n /) )
  real ( kind = rk ), allocatable, dimension ( :, : ) :: au
  integer i
  integer j
  integer n_unique
  real ( kind = rk ) tol
...
!
  ...
  real ( kind = rk ) tol
  integer, allocatable, dimension ( : ) :: undx
  integer, allocatable, dimension ( : ) :: xdnu
