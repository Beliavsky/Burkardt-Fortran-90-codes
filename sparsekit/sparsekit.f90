...
!
!    Input, integer JMASK(*), IMASK((NROW+1), defining mask 
...
!    Input, integer NCOL, the column dimension of the matrix.
!
!    Input, integer JOB, job indicator.  When JOB = 0, only the
...
!
!    Output, integer NDEGR(NROW), contains the degrees (the number
!    of nonzeros in each row of the matrix A * B.
!
!    Output, integer NNZ, the number of nonzero elements 
!    found in A * B.
!
...
!    Input, integer NROW, the row dimension of the matrix.
!
!    Input, integer JOB, job indicator. Job=0 means get array b
!    only job = 1 means get b, and the integer arrays ib, jb.
!
!    Input, real A(*), integer JA(*), IA(NROW+1), the matrix in CSR
!    Compressed Sparse Row format.
!
...
!
!    Input, integer NDIAG, the first dimension of array adiag as 
!    declared in the calling program.
!
!    Input, integer IDIAG, the number of diagonals in the matrix.
!
!    Input, integer IOFF(IDIAG), the offsets of the diagonals of 
...
!
!    a, ja = the real and integer arrays of the Ellpack/Itpack
!    format.
...
!    the A matrix.
!
!    Input, integer JDIAG, the number of jagged-diagonals in the
!    data structure.
!
...
!
! ia     = integer array containing the lengths of the  
!          jagged diagonals
!
!    Output, real Y(N), the product A*X.
!
...
!
!    Output, real A(*), integer JA(*), IA(NODE_NUM+1), the 
...
!
!    Input, integer ELEMENT_NODE(NPE,ELEMENT_NUM), the 
!    connectivity matrix.  ELEMENT_NODE(I,J) is the global index of the I-th
!    local node in element J.
!
!    Input, integer NODE_CODE(NODE_NUM), boundary information for
...
!      Input, integer NEL, the element index.
...
!      Input, integer IJK(NODE,NELX), the nodes making up 
!      each element.
...
!    each element.
!
!    Output, real ( kind = 8 ) A(*), integer JA(*), IA(NX+1), 
...
!
!    Input, integer NODCODE(NX), the boundary information list 
...
!
!    Workspace, integer IWK(NX).
!
!    Workspace, integer JWK(NX+1).
!
!    Output, integer IERR, error message.
!    * 0, normal return
!    * 1, negative area encountered, due to bad numbering of nodes
!    of an element.
!
!    Input, integer IOUT, output unit.
!
...
!
! lowd  = integer. this should be set to the row number in abd 
...
!
! ml      = integer. equal to the bandwidth of the strict lower
!         part of A.
!
! mu      = integer. equal to the bandwidth of the strict upper
...
!
! len   = integer. length of arrays a and ja. bndcsr will stop if
...
!    Input, integer NELX, the number of elements.
!
!    Input/output, integer IJK(NODE,NELX), lists the nodes that 
...
!    in CSR format.
!
! ja      = integer array of length nnz containing the column 
!    positions of the corresponding elements in a.
!
! ia      = integer array of length nnz containing the row 
!    positions of the corresponding elements in a.
!
...
! on return:
!
!    Output, real A(*), integer JA(*), IA(NROW+1), the matrix in
...
!
!    Input, integer NNZ, the number of nonzero elements in the
!    sparse matrix.
!
!    Input, integer NCA, the first dimension of output arrays
!    CA and JAC.
!
!  A(NNZ)  - Real array.
...
!            entries in A.
!
!  NER     - integer. Must be set greater than or equal to the
...
!    Compressed Sparse Row format.
!
!    Input, integer IPOS, indicates the position in the array AO,
...
!        we have B = A * Diag
!
!    Output, real B(*), integer JB(*), IB(NROW+1), the scaled
...
!
! iao   = integer array of length n/nblk+1. ia(i) points to the
!         beginning of block row number i in the arrays a and ja.
!
  implicit none
...
!         of A on return
!
! ioff  = integer array of length idiag, containing the offsets 
...
! work arrays:
!
! ind   = integer array of length 2*n-1 used as work space.
...
!
!    Output, real B(*), integer JB(*), 
!    integer IB(NROW+1), the resulting 
...
!    be at least NE.
!
!    Output, integer NE, the number of nonzero elements in the 
...
!
!    Output, integer IA(NN), pointers to specify rows for the
!    stored nonzero entries in AR.
!
!    Output, integer JA(NN), pointers to specify columns for the 
!    stored nonzero entries in AR.
!
!    Output, integer IERR, an error parameter, returned as zero on
...
!
  ...
  integer m1
  complex ( kind = ck8 ) rd(ndmx+1)
  real ( kind = 8 ) rm
  real ( kind = 8 ) t
  real ( kind = 8 ) u(n,*)
  real ( kind = 8 ) w(*)
  complex ( kind = ck8 ) wkc(ih)
  real ( kind = 8 ) x(*)
  real ( kind = 8 ) y(*)
  real ( kind = 8 ) z(m+1)
...
!    Input, integer N, the order of the matrix.
!
!    Input, integer M, the dimension of the Krylov subspace. 
...
!
! ioff           = integer array containing the offsets  of the
! ndiag diagonals
!
...
!    Compressed Sparse Row format.
!
!    Input, integer NBLK, the dimension of each diagonal block.
...
!
!    Output, integer IERR, error flag, which is nonzero if an
!    error occurred.
!
  implicit none
...
!
  ...
  complex ( kind = ck8 ) coef(*)
  real ( kind = 8 ) coef0
  real ( kind = 8 ) dt
  real ( kind = 8 ) hh(ih,*)
  complex ( kind = ck8 ) hloc(mmax+1,mmax)
  integer i
  integer ii
  integer j
  integer m
  complex ( kind = ck8 ) root(ndg)
  complex ( kind = ck8 ) t
  complex ( kind = ck8 ) w2(*)
  real ( kind = 8 ) y(*)
  real ( kind = 8 ) yloc(mmax)
  complex ( kind = ck8 ) zpiv
!
...
!
     do j = 1, m
        do i = 1, j+1
           hloc(i,j) = cmplx ( dt * hh(i,j), kind = ck8 )
        end do
        hloc(j,j) = hloc(j,j) - root(ii)
        w2(j) = cmplx ( yloc(j), kind = ck8 )
     end do
!
!  Forward solve.
!
...
!    Input, integer N, the order of the matrix.
!
!    Input, real ( kind = 8 ) A(*), integer JA(*), IA(N+1), 
!    the matrix in Compressed Sparse Row (CSR) format.
!
!    Input, integer LFIL, the fill-in parameter.  Each row of L 
...
!
!    Output, real ( kind = 8 ) ALU(*), integer JUL(*),
...
!
!    Output, integer JU(N), pointers to the beginning of each 
!    row of U in the matrix ALU, JLU.
!
!    Input, integer IWK, the minimum length of arrays ALU and 
!    JLU.
!
!    Workspace, real ( kind = 8 ) WU(N+1), WL(N).
!
!    Workspace, integer JR(N), JWL(N), JWU(N).
!
...
!    Input, integer N, the order of the matrix.
!
!    Input, integer IM, the size of the Krylov subspace.  IM 
...
!    Compressed Sparse Row format.
!
! perm       = integer array of length nrow containing the 
!    permutation arrays
...
!                      used).
!
!    Output, real AO(*), integer JAO(*), IAO(NROW+1), the permuted
!    matrix in CSR Compressed Sparse Row format.
!
! note :
...
!
!    Input, real A(*), integer JA(*), IA(NCOL+1), the transposed
!    matrix in CSR Compressed Sparse Row format.
!
