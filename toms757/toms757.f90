...
!  Parameters:
!
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) ABRAM0, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ), dimension(0:8) :: ab0f = (/ &
    ...
    -0.1076888d-13, &
     0.599d-17 /)
  real ( kind = rk8 ), dimension(0:8) :: ab0g = (/ &
     ...
     -0.112887d-14, &
      0.55d-18 /)
  real ( kind = rk8 ), dimension(0:8) :: ab0h = (/ &
     ...
     -0.30123d-15, &
      0.14d-18 /)
  real ( kind = rk8 ), dimension(0:27) :: ab0as = (/ &
    ...
    -0.4d-19, &
     0.2d-19, &
    -0.1d-19 /)
  real ( kind = rk8 ) abram0
  real ( kind = rk8 ) asln
  real ( kind = rk8 ) asval
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ) fval
  real ( kind = rk8 ) gval
  real ( kind = rk8 ), parameter :: gval0 = 0.13417650264770070909D+00
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  real ( kind = rk8 ) hval
  real ( kind = rk8 ), parameter :: lnxmin = -708.3964D+00
  integer, parameter :: nterma = 22
  integer, parameter :: ntermf = 8
  integer, parameter :: ntermg = 8
  integer, parameter :: ntermh = 8
  real ( kind = rk8 ), parameter :: onerpi = 0.56418958354775628695D+00
  real ( kind = rk8 ), parameter :: rt3bpi = 0.97720502380583984317D+00
  real ( kind = rk8 ), parameter :: rtpib2 = 0.88622692545275801365D+00
  real ( kind = rk8 ), parameter :: six = 6.0D+00
  real ( kind = rk8 ) t
  real ( kind = rk8 ), parameter :: three = 3.0D+00
  real ( kind = rk8 ), parameter :: two = 2.0D+00
  real ( kind = rk8 ) v
  real ( kind = rk8 ) x
  real ( kind = rk8 ), parameter :: xlow1 = 1.490116D-08
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00
...
!
!    Output, real ( kind = rk8 ) X, the argument of the function.
!
!    Output, real ( kind = rk8 ) FX, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) ABRAM1, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) ab1as(0:27)
  real ( kind = rk8 ) ab1f(0:9)
  real ( kind = rk8 ) ab1g(0:8)
  real ( kind = rk8 ) ab1h(0:8)
  real ( kind = rk8 ) abram1
  real ( kind = rk8 ) asln
  real ( kind = rk8 ) asval
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ) fval
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) ABRAM2, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) abram2
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  integer, parameter :: nterma = 23
  integer, parameter :: ntermf = 9
  ...
  real ( kind = rk8 ) ab2f(0:9),ab2g(0:8),ab2h(0:7),ab2as(0:26), &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) AIRY_AI_INT, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) aaint1(0:25)
  real ( kind = rk8 ) aaint2(0:21)
  real ( kind = rk8 ) aaint3(0:40)
  real ( kind = rk8 ) aaint4(0:17)
  real ( kind = rk8 ) aaint5(0:17)
  real ( kind = rk8 ) airy_ai_int
  real ( kind = rk8 ) airzer
  real ( kind = rk8 ) arg
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: eight = 8.0D+00
  real ( kind = rk8 ) forty1
  real ( kind = rk8 ), parameter :: four = 4.0D+00
  real ( kind = rk8 ) fr996
  real ( kind = rk8 ) gval
  real ( kind = rk8 ) hval
  real ( kind = rk8 ) nine
  real ( kind = rk8 ) ninhun
  ...
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ) piby4
  real ( kind = rk8 ) pitim6
  real ( kind = rk8 ) rt2b3p
  real ( kind = rk8 ) t
  real ( kind = rk8 ) temp
  real ( kind = rk8 ), parameter :: three = 3.0D+00
  real ( kind = rk8 ), parameter :: two = 2.0D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xhigh1
  real ( kind = rk8 ) xlow1
  real ( kind = rk8 ) xneg1
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) AIRY_BI_INT, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) abint1(0:36)
  real ( kind = rk8 ) abint2(0:37)
  real ( kind = rk8 ) abint3(0:37)
  real ( kind = rk8 ) abint4(0:20)
  real ( kind = rk8 ) abint5(0:20)
  real ( kind = rk8 ) airy_bi_int
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: eight = 8.0D+00
  real ( kind = rk8 ), parameter :: four = 4.0D+00
  ...
  real ( kind = rk8 ) arg,birzer,f1,f2,nine,ninhun, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) AIRY_GI, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) airy_gi
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: four = 4.0D+00
  ...
  real ( kind = rk8 ) argip1(0:30),argip2(0:29),argin1(0:42), &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) AIRY_HI, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) airy_hi
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: four = 4.0D+00
  ...
  real ( kind = rk8 ) arhip(0:31),arbip(0:23),argip1(0:29), &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) ARCTAN_INT, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ), dimension ( 0:22 ) :: atnina = (/ &
  ...
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ) t
  real ( kind = rk8 ), parameter :: twobpi = 0.63661977236758134308D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ), parameter :: xlow = 7.4505806D-09
  real ( kind = rk8 ), parameter :: xupper = 4.5036D+15
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) BESSEL_I0_INT, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) ari01(0:28)
  real ( kind = rk8 ) ari0a(0:33)
  real ( kind = rk8 ), parameter :: ateen = 18.0D+00
  real ( kind = rk8 ) bessel_i0_int
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  integer ind
  real ( kind = rk8 ), parameter :: lnr2pi = 0.91893853320467274178D+00
  integer, parameter :: nterm1 = 25
  integer, parameter :: nterm2 = 27
  real ( kind = rk8 ) t
  real ( kind = rk8 ) temp
  real ( kind = rk8 ), parameter :: thirt6 = 36.0D+00
  real ( kind = rk8 ), parameter :: three = 3.0D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ), parameter :: xhigh = 713.758339D+00
  real ( kind = rk8 ), parameter :: xlow = 0.5161914D-07
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) BESSEL_J0_INT, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) bessel_j0_int
  real ( kind = rk8 ) cheval
  integer ind
  integer, parameter :: nterm1 = 22
  ...
  real ( kind = rk8 ) arj01(0:23),arj0a1(0:21),arj0a2(0:18), &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) BESSEL_K0_INT, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) bessel_k0_int
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  ...
  real ( kind = rk8 ) ak0in1(0:15),ak0in2(0:15),ak0ina(0:27), &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) BESSEL_Y0_INT, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) bessel_y0_int
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: nine = 9.0D+00
  integer, parameter :: nterm1 = 22
  ...
  real ( kind = rk8 ) arj01(0:23),ary01(0:24),ary0a1(0:21), &
...
!
!    Input, real ( kind = rk8 ) A(0:N), the coefficients of the Chebyshev series.
!
!    Input, real ( kind = rk8 ) T, the value at which the series is
!    to be evaluated.
!
!    Output, real ( kind = rk8 ) CHEVAL, the value of the Chebyshev series at T.
!
  implicit none
  ...
  integer i
  real ( kind = rk8 ) :: t
  real ( kind = rk8 ), parameter :: test = 0.6D+00
  real ( kind = rk8 ) :: tt
  real ( kind = rk8 ), parameter :: two = 2.0D+00
  real ( kind = rk8 ) :: u0
  real ( kind = rk8 ) :: u1
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) CLAUSEN, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) aclaus(0:15)
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ) clausen
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  integer indx
  integer, parameter :: nterms = 13
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ), parameter :: pi = 3.1415926535897932385D+00
  real ( kind = rk8 ), parameter :: pisq = 9.8696044010893586188D+00
  real ( kind = rk8 ) t
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) twopi,twopia,twopib,xhigh,xsmall
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) DEBYE1, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) adeb1(0:18)
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ) debye1
  real ( kind = rk8 ), parameter :: eight = 8.0D+00
  real ( kind = rk8 ), parameter :: four = 4.0D+00
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  integer i
  ...
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ), parameter :: quart = 0.25D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) debinf,expmx, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) DEBYE2, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ) adeb2(0:18),debinf,expmx, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) DEBYE3, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ) adeb3(0:18),debinf,expmx, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) DEBYE4, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ) adeb4(0:18),debinf,eightn,expmx, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) EXP3_INT, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ) aexp3(0:24),aexp3a(0:24), &
         funinf,sixten,t, &
         xlow,xupper
  data sixten /16.0d0 /
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) GOODWIN, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ) agost(0:28),agosta(0:23), &
       fval,gamby2,rtpib2, &
       t,xhigh,xlow
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) I0ML0, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) ai0l0(0:23)
  real ( kind = rk8 ) ai0l0a(0:23)
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ) i0ml0
...
!
!    Output, real ( kind = rk8 ) I1ML1, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ) ai1l1(0:23),ai1l1a(0:25),atehun, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) LOBACHEVSKY, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ) arlob1(0:15),arlob2(0:10), &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) STROMGEN, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) astrom(0:26)
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ) epngln
  real ( kind = rk8 ) epsln
  real ( kind = rk8 ) f15bp4
  real ( kind = rk8 ), parameter :: four = 4.0D+00
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  integer k1
  integer k2
  ...
  integer numexp
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ) one5ln
  real ( kind = rk8 ) pi4b3
  real ( kind = rk8 ) rk
  real ( kind = rk8 ) stromgen
  real ( kind = rk8 ), parameter :: two = 2.0D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) seven,sumexp,sum2,t,valinf,xhigh, &
       xk,xk1,xlow0,xlow1
  data seven/ 7.0d0 /
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) STRUVE_H0, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) arrh0(0:19)
  real ( kind = rk8 ) arrh0a(0:20)
  real ( kind = rk8 ) ay0asp(0:12)
  real ( kind = rk8 ) ay0asq(0:13)
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: eight = 8.0D+00
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  integer indsgn
  ...
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ) struve_h0
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) eleven,h0as, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) STRUVE_H1, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) arrh1(0:17)
  real ( kind = rk8 ) arrh1a(0:21)
  real ( kind = rk8 ) ay1asp(0:14)
  real ( kind = rk8 ) ay1asq(0:15)
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: eight = 8.0D+00
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  ...
  real ( kind = rk8 ) struve_h1
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) fortp5, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) STRUVE_L0, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ) struve_l0
  real ( kind = rk8 ), parameter :: two = 2.0D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) arl0(0:27),arl0as(0:15),ai0ml0(0:23), &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) STRUVE_L1, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ), parameter :: sixten = 16.0D+00
  real ( kind = rk8 ) struve_l1
  real ( kind = rk8 ), parameter :: two = 2.0D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) arl1(0:26),arl1as(0:16),ai1ml1(0:25), &
       atehun,ch1,ch2,lnr2pi, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) SYNCH1, the value of the function.
!
  implicit none
  ...
  integer, parameter :: nterm3 = 21
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ) synch1
  real ( kind = rk8 ), parameter :: three = 3.0D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) async1(0:13),async2(0:11),asynca(0:24), &
       cheb1,cheb2,conlow, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) SYNCH2, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ) asyn21(0:14),asyn22(0:13),asyn2a(0:18), &
       cheb1,cheb2,conlow, &
       lnrtp2,t,ten,xhigh1, &
       xhigh2,xlow,xpowth
  data ten/ 10.0d0 /
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) TRAN02, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ) tran02
  real ( kind = rk8 ), parameter :: valinf = 0.32898681336964528729D+01
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xvalue
  real ( kind = rk8 ), parameter :: zero = 0.0D+00

  real ( kind = rk8 ) atran(0:19),rk, &
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) TRAN03, the value of the function.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) atran(0:19)
  real ( kind = rk8 ) cheval
  real ( kind = rk8 ), parameter :: eight = 8.0D+00
  real ( kind = rk8 ), parameter :: four = 4.0D+00
  real ( kind = rk8 ), parameter :: half = 0.5D+00
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) TRAN04, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) TRAN05, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) TRAN06, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) TRAN07, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) TRAN08, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk8 ) XVALUE, the argument of the function.
!
!    Output, real ( kind = rk8 ) TRAN09, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ) rk
  real ( kind = rk8 ), parameter :: rnumjn = 9.0D+00
  real ( kind = rk8 ) sumexp
  real ( kind = rk8 ) sum2
  real ( kind = rk8 ) t
  real ( kind = rk8 ) tran09
  real ( kind = rk8 ), parameter :: valinf = 0.36360880558872871397d6
  real ( kind = rk8 ) x
  real ( kind = rk8 ) xhigh1
  real ( kind = rk8 ) xhigh2
  real ( kind = rk8 ) xhigh3
  real ( kind = rk8 ) xk
  real ( kind = rk8 ) xk1
  real ( kind = rk8 ) xlow1
  real ( kind = rk8 ) xlow2
