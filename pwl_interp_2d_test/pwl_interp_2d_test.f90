...
!
  ...
  integer, parameter :: n_test_num = 5

  integer i
  integer n
  integer, dimension ( n_test_num ) :: n_test = (/ &
    2, 3, 4, 5, 9 /)
  integer prob
  integer prob_num
...
!
!    Input, integer N, the grid size in each dimension.
!
  implicit none
  ...
  real ( kind = rk ), allocatable :: xd_1d(:)
  real ( kind = rk ), allocatable :: xi(:)
  real ( kind = rk ), allocatable :: xi_1d(:)
  real ( kind = rk ), allocatable :: yd(:)
  real ( kind = rk ), allocatable :: yd_1d(:)
  real ( kind = rk ), allocatable :: yi(:)
  real ( kind = rk ), allocatable :: yi_1d(:)
  real ( kind = rk ), allocatable :: zd(:)
  real ( kind = rk ), allocatable :: zdm(:)
...
!
  if ( 1 < nd ) then
    ...
    app_error = r8vec_norm_affine ( ni, zi, zdm ) / real ( ni, kind = rk )
