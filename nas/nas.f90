...
!*****************************************************************************80
!
!! nas() is the NAS kernel benchmark program.
!
!  Discussion:
!
...
!
  ...
  real ( kind = rk ) a(5,5,md,md)
  real ( kind = rk ) ans
  real ( kind = rk ) b(5,5,md,md)
  real ( kind = rk ) bx(5,5,md,md)
  real ( kind = rk ) c(5,5,md,md)
  real ( kind = rk ) er
  real ( kind = rk ) f7
  real ( kind = rk ) fp
  integer i
  ...
  integer nb
  integer ns
  real ( kind = rk ) s(jd,kd,ld,5)
  real ( kind = rk ) sx(jd,kd,ld,5)
  real ( kind = rk ) t
  real ( kind = rk ) t30
  real ( kind = rk ) time1
  real ( kind = rk ) time2
...
!*****************************************************************************80
!
!! COPY copies a real ( kind = rk ) vector.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
  ...
  real ( kind = rk ) time2
  real ( kind = rk ) tm
  complex ( kind = ck ) w1(m)
  complex ( kind = ck ) w2(n)
  complex ( kind = ck ) x(m1,n)
...
!
  ...
  real ( kind = rk ) a(0:ida,-m:0,0:n)
  real ( kind = rk ) ans
  real ( kind = rk ) ax(0:ida,-m:0,0:n)
  real ( kind = rk ) b(0:nrhs,0:nmat,0:n)
  real ( kind = rk ) bx(0:nrhs,0:nmat,0:n)
  real ( kind = rk ) er
  real ( kind = rk ) f7
  real ( kind = rk ) fp
  integer i
...
!
  ...
  real ( kind = rk ) a(0:ida,-m:0, 0:n)
  real ( kind = rk ) b(0:nrhs,0:idb,0:n)
  real ( kind = rk ) eps
  real ( kind = rk ) epss(0:256)
  integer i
  integer i0
  integer j
  integer jj
...
!
  ...
  integer lw
  integer nwall(nb)
  complex ( kind = ck ) proj(nw,nb)
  real ( kind = rk ) rmatrx(nw*nb,nw*nb)
  real ( kind = rk ) t1
  real ( kind = rk ) t2
  real ( kind = rk ) t30
  real ( kind = rk ) time1
  real ( kind = rk ) time2
  real ( kind = rk ) tm
  complex ( kind = ck ) wall(nw,nb)
  real ( kind = rk ) xmax(nb)
  complex ( kind = ck ) zcr(nw,nb)
...
!
  ...
  real ( kind = rk ) ans
  real ( kind = rk ) cp(nw,nb)
  real ( kind = rk ) dpds(nw,nb)
  real ( kind = rk ) er
  complex ( kind = ck ) expmz(nvm)
  complex ( kind = ck ) expz(nvm)
  real ( kind = rk ) f7
  complex ( kind = ck ) force(nb)
  real ( kind = rk ) fp
  real ( kind = rk ) gamma(nvm)
  integer i
  integer it
  integer j
  integer nwall(nb)
  real ( kind = rk ) ps(nvm)
  real ( kind = rk ) psi(nw)
  complex ( kind = ck ) refpt(nb)
  real ( kind = rk ) rhs(nw*nb)
  real ( kind = rk ) rmatrx(nw*nb,nw*nb)
  real ( kind = rk ) rmom(nb)
  real ( kind = rk ) t1
  real ( kind = rk ) t2
  real ( kind = rk ) t30
  real ( kind = rk ) time1
...
!
  ...
  real ( kind = rk ) delt
  real ( kind = rk ) dpds(nw,nb)
  complex ( kind = ck ) dum3
  complex ( kind = ck ) expmwk
  complex ( kind = ck ) expmz(nvm)
  complex ( kind = ck ) expwkl
  complex ( kind = ck ) expz(nvm)
  complex ( kind = ck ) force(nb)
  real ( kind = rk ) gamma(nvm)
  integer i
...
!
  ...
  real ( kind = rk ) a(nja,njb)
  real ( kind = rk ) ans
  real ( kind = rk ) b(nja,njb)
  real ( kind = rk ) c(nja,njb)
  real ( kind = rk ) d(nja,njb)
  real ( kind = rk ) e(nja,njb)
  real ( kind = rk ) er
  real ( kind = rk ) f(nja,njb,3)
  real ( kind = rk ) f7
  real ( kind = rk ) fp
  real ( kind = rk ) fx(nja,njb,3)
  integer i
  integer it
  integer j
  integer k
  ...
  real ( kind = rk ) time2
  real ( kind = rk ) tm
  real ( kind = rk ) x(nja,njb)
  real ( kind = rk ) y(nja,njb)
