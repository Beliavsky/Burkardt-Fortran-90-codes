...
!    the triangulation.
!
!    Output, real ( kind = rk ) ALPHA_MEASURE, the ALPHA quality measure.
!
  implicit none
...
!    the triangulation.
!
!    Output, real ( kind = rk ) AREA_MEASURE, the AREA quality measure.
!
  implicit none
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) Z(DIM_NUM,N), the points.
!
!    Output, real ( kind = rk ) BETA_MEASURE, the BETA quality measure.
!
  implicit none
  ...
  real ( kind = rk ) beta_measure
  real ( kind = rk ) gamma(n)
  real ( kind = rk ) gamma_ave
  real ( kind = rk ) gamma_std
  real ( kind = rk ) z(dim_num,n)

  call pointset_spacing ( dim_num, n, z, gamma )

  gamma_ave = sum ( gamma(1:n) ) / real ( n, kind = rk )
...
!*****************************************************************************80
!
!! chi_measure() determines the CHI pointset quality measure.
!
!  Discussion:
!
...
!
!    Input, integer SEED_INIT, the initial value of the random
!    number seed.
!
!    Output, real ( kind = rk ) CHI_MEASURE, the CHI quality measure.
!
  implicit none
...
!    number seed.
!
!    Output, real ( kind = rk ) D_MEASURE, the D quality measure.
!
  implicit none
  ...
  real ( kind = rk ) centroid(dim_num,n)
  integer closest(1)
  real ( kind = rk ) d_measure
  real ( kind = rk ) dge_det
  real ( kind = rk ) di
  integer hit(n)
  integer i
  integer i1
  ...
  integer j
  integer k
  real ( kind = rk ) moment(dim_num,dim_num,n)
  integer ns
  external sample_routine
  integer seed
  integer seed_init
  ...
  do j = 1, n
    ...
    if ( 0 < hit(j) ) then
      ...
      centroid(1:dim_num,j) = centroid(1:dim_num,j) / real ( hit(j), kind = rk )
  ...
  do j = 1, n
    do i = 1, dim_num
      moment(i,i,j) = moment(i,i,j) - tri(j) / real ( dim_num, kind = rk )
    end do
  end do
...
!    N must be positive.
!
!    Input/output, real ( kind = rk ) A(N,N), the matrix to be analyzed.
!    On output, the matrix has been overwritten by factorization information.
!
!    Output, real ( kind = rk ) DGE_DET, the determinant of the matrix.
!
  implicit none
...
!    number seed.
!
!    Output, real ( kind = rk ) E_MEASURE, the E quality measure.
!
  implicit none
  ...
  integer closest(1)
  real ( kind = rk ) dist
  real ( kind = rk ) e_measure
  real ( kind = rk ) e_vec(n)
  integer k
  integer ns
  external sample_routine
  integer seed
  ...
  e_measure = sum ( e_vec(1:n) ) / real ( ns, kind = rk )
...
!    Input, real ( kind = rk ) Z(DIM_NUM,N), the points.
!
!    Output, real ( kind = rk ) GAMMA_MEASURE, the GAMMA quality measure.
!
!  Local parameters:
!
!    Local, real ( kind = rk ) GAMMA_SQ_MAX, the maximum, over all points,
!    of the minimum squared distance to a distinct point.
!
!    Local, real ( kind = rk ) GAMMA_SQ_MIN, the minimum, over all points,
!    of the minimum squared distance to a distinct point.
!
  implicit none
  ...
  real ( kind = rk ) dist_sq
  real ( kind = rk ) dist_sq_min
  real ( kind = rk ) gamma_measure
  real ( kind = rk ) gamma_sq_max
  real ( kind = rk ) gamma_sq_min
  real ( kind = rk ) z(1:dim_num,1:n)
!
...
!    number seed.
!
!    Output, real ( kind = rk ) H_MEASURE, the H quality measure.
!
  implicit none
...
!  Parameters:
!
!    Input, integer IVAL, the value to be wrapped.
!
...
!    Input, real ( kind = rk ) Z(DIM_NUM,N), the points.
!
!    Output, real ( kind = rk ) LAMBDA_MEASURE, the LAMBDA quality measure.
!
!  Local parameters:
!
!    Local, real ( kind = rk ) GAMMA_MAX, the maximum, over all points,
!    of the minimum distance to a distinct point.
!
!    Local, real ( kind = rk ) GAMMA_MIN, the minimum, over all points,
!    of the minimum distance to a distinct point.
!
  implicit none
...
!
  if ( gamma_ave <= 0.0D+00 ) then
    ...
    lambda_measure = &
      sqrt ( &
             sum ( ( gamma(1:n) - gamma_ave )**2 ) / real ( n, kind = rk ) &
           ) / gamma_ave
  end if
...
!    number seed.
!
!    Output, real ( kind = rk ) MU_MEASURE, the MU quality measure.
!
  implicit none
...
!    number seed.
!
!    Output, real ( kind = rk ) NU_MEASURE, the NU quality measure.
!
  implicit none
  ...
  integer k
  integer ns
  real ( kind = rk ) nu_measure
  external             sample_routine
  integer seed
  integer seed_init
  real ( kind = rk ) volume(n)
  real ( kind = rk ) x(dim_num)
  real ( kind = rk ) z(dim_num,n)
  ...
  volume(1:n) = real ( hit(1:n), kind = rk ) &
              / real ( ns, kind = rk )
!
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) Z(DIM_NUM,N), the point distribution.
!
!    Output, real ( kind = rk ) GAMMA(N), the minimum distance between each
...
!    the triangulation.
!
!    Output, real ( kind = rk ) Q_MEASURE, the Q quality measure.
!
  implicit none
...
!    Input, real ( kind = rk ) Z(DIM_NUM,N), the points.
!
!    Output, real ( kind = rk ) R0_MEASURE, the R0 quality measure.
!
  implicit none
...
!
  ...
  if ( 1 < n ) then
    r0_measure = r0_measure / real ( n * ( n - 1 ), kind = rk )
  else
    r0_measure = 0.0D+00
  end if
...
!
!    Local, integer SAVED, is 0 or 1 depending on whether there is
!    a single saved value left over from the previous call.
!
!    Local, integer X_LO_INDEX, X_HI_INDEX, records the range of 
!    entries of X that we need to compute.  This starts off as 1:N, but is 
!    adjusted if we have a saved value that can be immediately stored in X(1),
!    and so on.
!
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) Z(DIM_NUM,N), the point coordinates.
...
!
!    Output, real ( kind = rk ) RADIUS(N), the radius of the
...
!    number generator.
!
!    Output, real ( kind = rk ) X(DIM_NUM,N), the sample points.
!
  implicit none
...
!    Input, real ( kind = rk ) Z(DIM_NUM,N), the points.
!
!    Output, real ( kind = rk ) SPHERE_MEASURE, the amount of volume taken up
...
!
  ...
  integer j
  logical r8mat_in_01
  real ( kind = rk ), dimension ( n ) :: radius
  real ( kind = rk ) radius_ave
  real ( kind = rk ) radius_max
  real ( kind = rk ) radius_min
  real ( kind = rk ) sphere_measure
  logical, parameter :: verbose = .false.
  real ( kind = rk ) volume
  logical, parameter :: walls = .true.
  real ( kind = rk ), dimension ( dim_num, n ) :: z
...
!   write ( *, '(a)' ) '  Some data is not in the unit hypercube.'
  ...
  if ( verbose ) then
    ...
    radius_ave = sum ( radius(1:n) ) / real ( n, kind = rk )
...
!    number seed.
!
!    Output, real ( kind = rk ) TAU_MEASURE, a quality measure.
!
  implicit none
  ...
  t_bar = sum ( t(1:n) ) / real ( n, kind = rk )
