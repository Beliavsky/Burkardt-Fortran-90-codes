...
!
  ...
  integer, parameter :: max_tab = 12

  real ( kind = rk ) diftab(max_tab)
  integer i
  integer j
  integer jhi
  character mark
  ...
  character ( len = 80 ) title
  real ( kind = rk ) x
  real ( kind = rk ) xdata(max_tab)
  real ( kind = rk ) yapprox
  real ( kind = rk ) ydata(max_tab)
...
!
      do i = 1, ntab
        ...
        do j = 1, jhi
          ...
          if ( i < ntab ) then
            x = ( real ( jhi - j + 1, kind = rk ) * xdata(i)     &
                + real (       j - 1, kind = rk ) * xdata(i+1) ) &
                / real ( jhi,         kind = rk )
          else
            x = xdata(ntab)
          end if
...
!
  ...
  integer prob_num
  character ( len = 80 ) title
  real ( kind = rk ), allocatable, dimension ( : ) :: xdata
  real ( kind = rk ) xval
  real ( kind = rk ), allocatable, dimension ( : ) :: ydata
  real ( kind = rk ) ypval
  real ( kind = rk ) yval
...
!
    do i = 1, data_num - 1
      ...
      do j = 1, jhi
        ...
        xval = ( real ( jmax - j,     kind = rk ) * xdata(i)     &
               + real (        j - 1, kind = rk ) * xdata(i+1) ) &
               / real ( jmax     - 1, kind = rk )
...
!
  ...
  real ( kind = rk ) ybcend
  real ( kind = rk ), allocatable, dimension ( : ) :: ydata
  real ( kind = rk ), allocatable, dimension ( : ) :: ypp
  real ( kind = rk ) yppval
  real ( kind = rk ) ypval
  real ( kind = rk ) yval
...
!
  ...
  integer prob
  real ( kind = rk ), allocatable, dimension ( : ) :: xdata
  real ( kind = rk ), allocatable, dimension ( : ) :: xplot
  real ( kind = rk ) xval
  real ( kind = rk ), allocatable, dimension ( : ) :: ydata
  real ( kind = rk ), allocatable, dimension ( : ) :: yplot
  real ( kind = rk ) yval
...
!
  ...
  do plot = 1, nplot
    ...
    xval = ( real ( nplot - plot,     kind = rk ) * a     &
           + real (         plot - 1, kind = rk ) * b ) &
           / real ( nplot        - 1, kind = rk )
...
!
  ...
  integer prob
  real ( kind = rk ), allocatable, dimension ( : ) :: xdata
  real ( kind = rk ) xplot(nplot)
  real ( kind = rk ) xval
  real ( kind = rk ) ybcbeg
  real ( kind = rk ) ybcend
  real ( kind = rk ), allocatable, dimension ( : ) :: ydata
  real ( kind = rk ) yplot(nplot)
  real ( kind = rk ), allocatable, dimension ( : ) :: ypp
  real ( kind = rk ) yppval
  real ( kind = rk ) ypval
  real ( kind = rk ) yval
...
!
  do j = 1, nplot
    ...
    xval = ( real ( nplot - j,     kind = rk ) * xdata(1)    &
           + real (         j - 1, kind = rk ) * xdata(data_num) ) &
           / real ( nplot     - 1, kind = rk )
