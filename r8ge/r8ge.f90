...
!
!    Output, real ( kind = rk ) RCOND, an estimate of the reciprocal
...
!
!    Output, real ( kind = rk ) D(M), the DILU factor.
!
  implicit none
...
!
  ...
  do j = 1, n
    do i = 1, m
      a(i,j) = 1.0D+00 / real ( i + j - 1, kind = rk )
    end do
  end do
...
!
!    Output, real ( kind = rk ) A(N,N), the inverse Hilbert matrix.
!
  implicit none
...
!
  ...
  do j = 2, n
    a(i,j) = - a(i,j-1) &
      * real ( ( n + j - 1 ) * ( i + j - 2 ) * ( n + 1 - j ), kind = rk ) &
      / real ( ( i + j - 1 ) * ( j - 1 ) * ( j - 1 ), kind = rk )
  end do
!
...
!
  do i = 2, n
    do j = 1, n
      ...
      a(i,j) = - a(i-1,j) &
        * real ( (n+i-1) * (i+j-2) * (n+1-i), kind = rk ) &
        / real ( (i+j-1) * ( i - 1 ) * ( i - 1 ), kind = rk )
...
!
!    Output, real ( kind = rk ) A(M,N), the identity matrix.
!
  implicit none
...
!    Input, real ( kind = rk ) A(M,N), the R8GE matrix.
!
!    Output, real ( kind = rk ) L(M,M), the M by M unit lower triangular factor.
!
!    Output, real ( kind = rk ) U(M,N), the M by N upper triangular factor.
!
  implicit none
...
!
!    Output, real ( kind = rk ) B(N), the result of the multiplication.
!
...
!    N must be positive.
!
!    Input, real ( kind = rk ) A(N1,N2), B(N2,N3), the factors.
!
!    Output, real ( kind = rk ) C(N1,N3), the product.
!
  implicit none
...
!    N must be positive.
!
!    Input, real ( kind = rk ) A(N2,N1), B(N2,N3), the factors.
!
!    Output, real ( kind = rk ) C(N1,N3), the product.
!
  implicit none
...
!
!    Input, real ( kind = rk ) A_LU(M,N), the LU factors from R8GE_TRF.
!
...
!
!    Input, integer PIVOT(*), the pivot vector computed 
!    by R8GE_TRF.
!
!    Input, real ( kind = rk ) X(*), the vector to be multiplied.
!    For the untransposed case, X should have N entries.
!    For the transposed case, X should have M entries.
!
!    Output, real ( kind = rk ) B(*), the result of the multiplication.
...
!    Input, real ( kind = rk ) A(M,N), the R8GE matrix.
!
!    Output, real ( kind = rk ) P(M,M), the M by M permutation factor.
!
!    Output, real ( kind = rk ) L(M,M), the M by M unit lower triangular factor.
!
!    Output, real ( kind = rk ) U(M,N), the M by N upper triangular factor.
...
!    N must be positive.
!
!    Input, real ( kind = rk ) A(N,N), the R8GE matrix.
!
!    Output, real ( kind = rk ) P(0:N), the coefficients of the characteristic
...
!
!    Input, real ( kind = rk ) A(M,N), the original, UNFACTORED R8GE matrix.
!
!    Input, real ( kind = rk ) X(N), the estimated solution.
!
!    Input, real ( kind = rk ) B(M), the right hand side vector.
!
!    Output, real ( kind = rk ) R(M), the residual vector, b - A * x.
!
  implicit none
...
!    N must be positive.
!
!    Input, real ( kind = rk ) A(N,N), the original, UNFACTORED R8GE matrix.
!
!    Input, real ( kind = rk ) A_LU(N,N), the LU factors from R8GE_FA.
!
!    Input, integer PIVOT(N), the pivot vector from R8GE_FA.
...
!    nonzero, solve A'*X=B.
!
!    Input/output, real ( kind = rk ) X(N), an estimate of the solution 
!    of A * x = b.  On output, the solution has been improved by one 
!    step of iterative refinement.
!
!    Output, real ( kind = rk ) R(N), contains the correction terms added to X.
!
  implicit none
...
!    the array.
!
!    Input, real ( kind = rk ) A(M,N), the array to be copied.
!
!    Output, real ( kind = rk ) X(M*N), the vector.
!
  implicit none
...
!    'C':  A'* X = B  (Conjugate transpose = Transpose)
!
!    Input, real ( kind = rk ) A(N,N), the factors L and U from the factorization
!    A = P*L*U as computed by R8GE_TRF.
!
!    Input, integer PIVOT(N), the pivot indices from R8GE_TRF.
!
!    Input/output, real ( kind = rk ) B(N,NRHS).
...
!    in the array.
!
!    Input, real ( kind = rk ) X(M*N), the vector to be copied into the array.
!
!    Output, real ( kind = rk ) A(M,N), the array.
!
  implicit none
