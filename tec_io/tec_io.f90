...
!
!    Input, real ( kind = rk8 ) TABLE(M,N), the table data.
!
  implicit none
...
! character ( len = 40 ) string
  real ( kind = rk8 ), parameter :: x = 1.0D+00
...
subroutine tec_data_read ( tec_file_unit, dim_num, &
...
!    per node.
!
!    Output, real ( kind = rk8 ) NODE_COORD(DIM_NUM,NODE_NUM), the coordinates 
!    of nodes.
!
!    Output, integer ELEMENT_NODE(ELEMENT_ORDER,ELEMENT_NUM); 
!    the global index of local node I in element J.
!
!    Output, real ( kind = rk8 ) NODE_DATA(NODE_DATA_NUM,NODE_NUM), the 
...
!
  ...
  integer node
  real ( kind = rk8 ) node_coord(dim_num,node_num)
  real ( kind = rk8 ) node_data(node_data_num,node_num)
...
subroutine tec_data_write ( tec_file_unit, dim_num, &
  node_num, element_num, element_order, node_data_num, node_coord, &
  element_node, node_data )
...
!    per node.
!
!    Input, real ( kind = rk8 ) NODE_COORD(DIM_NUM,NODE_NUM), the coordinates 
!    of nodes.
!
!    Input, integer ELEMENT_NODE(ELEMENT_ORDER,ELEMENT_NUM); 
!    the global index of local node I in element J.
!
!    Input, real ( kind = rk8 ) NODE_DATA(NODE_DATA_NUM,NODE_NUM), the 
!    data values associated with each node.
!
  implicit none
...
subroutine tec_header_read ( tec_file_unit, dim_num, node_num, &
...
!  Parameters:
!
!    Input, integer TEC_FILE_UNIT, the unit number associated 
!    with the TEC file.
!
!    Output, integer DIM_NUM, the spatial dimension, inferred 
!    from the names of the variables.
!
!    Output, integer NODE_NUM, the number of nodes, determined by 
!    the "N=" argument.
!
...
!  Write the node coordinates and node data.
!
  call tec_data_write ( tec_file_unit, dim_num, &
    node_num, element_num, element_order, node_data_num, node_coord, &
    element_node, node_data )
!
!  Close the file.
