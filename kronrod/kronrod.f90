...
!
!    Input, real ( kind = rk ) EPS, the requested absolute accuracy of the
!    abscissas.
!
!    Output, real ( kind = rk ) X(N+1), the abscissas.
!
!    Output, real ( kind = rk ) W1(N+1), the weights for 
!    the Gauss-Kronrod rule.
!
!    Output, real ( kind = rk ) W2(N+1), the weights for 
!    the Gauss rule.
!
  implicit none
...
!
  coef2 = 2.0D+00 / real ( 2 * n + 1, kind = rk )
  do i = 1, n
    coef2 = coef2 * 4.0D+00 * real ( i, kind = rk ) / real ( n + i, kind = rk )
  end do
!
...
!
!    Input/output, real ( kind = rk ) X(N+1), W1(N+1), W2(N+1), the abscissas
!    and weights.
!
  implicit none
...
!    abscissas.
!
!    Input, real ( kind = rk ) COEF2, a value needed to compute weights.
!
!    Input, logical EVEN, is TRUE if N is even.
!
!    Input, real ( kind = rk ) B(M+1), the Chebyshev coefficients.
!
!    Input/output, real ( kind = rk ) X; on input, an estimate for
!    the abscissa, and on output, the computed abscissa.
!
!    Output, real ( kind = rk ) W, the weight.
!
  implicit none
...
!    the abscissa, and on output, the computed abscissa.
!
!    Output, real ( kind = rk ) W1, the Gauss-Kronrod weight.
!
!    Output, real ( kind = rk ) W2, the Gauss weight.
!
  implicit none
