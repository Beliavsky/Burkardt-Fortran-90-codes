...
!*****************************************************************************80
!
!! football_rank() ranks college football teams.
!
!  Discussion:
!
...
!    the teams for each game.
!
!    Input, integer ( kind = 4 ) IUNIT, the FORTRAN unit number of the file 
!    being read.
!
!    Input, character ( len = 5 ) TEAM_ABBREV(TEAM_NUM), the abbreviations for
!    each team.
...
!    whose beginning a value is to be extracted.  On output,
!    the value, if found, has been removed.
!
!    Output, integer ( kind = 4 ) IVAL.  If IERROR is 0, then IVAL contains the
!    "next" value read from LINE; otherwise IVAL is 0.
!
!    Output, integer ( kind = 4 ) IERROR.
!    0, no error.
!    nonzero, a value could not be extracted from the beginning of the
