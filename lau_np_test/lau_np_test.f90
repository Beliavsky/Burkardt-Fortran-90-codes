...
!
  ...
  real ( kind = rk ), dimension ( nnode,nnode) :: dist = reshape ( (/ &
...
!
  ...
  real ( kind = rk ), dimension ( m ) :: b = (/ 1.0D+00, -1.0D+00, 4.0D+00 /)
  real ( kind = rk ), dimension ( n ) :: c = (/ &
  ...
  write ( *, '(a)' ) '  integer linear programming problem.'
...
!
  ...
  integer, parameter :: m = ( nnode * ( nnode - 1 ) ) / 2
...
subroutine knapsack_test ( )

!*****************************************************************************80
!
!! knapsack_test tests knapsack().
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
  ...
  real ( kind = rk ) :: b = 170.0D+00
  real ( kind = rk ), dimension ( n ) :: c = &
    (/ 442.0D+00, 525.0D+00, 511.0D+00, 593.0D+00, 546.0D+00, &
    564.0D+00, 617.0D+00 /)
  real ( kind = rk ) :: eps = 0.8D+00
  integer isol(n)
  integer numsol
  real ( kind = rk ) objval

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'knapsack_test():'
...
!
  ...
  real ( kind = rk ) a(m,n)
  real ( kind = rk ), dimension ( m ) :: b = (/ 19.0D+00, 14.0D+00, 17.0D+00 /)
  real ( kind = rk ), dimension ( n ) :: c = (/ &
...
!
  ...
  real ( kind = rk ), dimension ((n*(n-1))/2) :: cost = (/ &
