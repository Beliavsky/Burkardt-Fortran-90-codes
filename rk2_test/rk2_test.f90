...
!
  ...
  write ( *, '(a)' ) '  Test rk2() on several ODE''s.'
  ...
  y0(2) = 100.0D+00
  n = 200
  call predator_prey_rk2_test ( tspan, y0, n )

  tspan(1) = 0.0D+00
  tspan(2) = 1.0D+00
  y0(1) = 0.0D+00
  n = 50
  call stiff_rk2_test ( tspan, y0(1), n )
!
!  Terminate.
!
  write ( *, '(a)' ) ''
...
subroutine predator_prey_rk2_test ( tspan, p0, n )

!*****************************************************************************80
!
!! predator_prey_rk2_test: predator-prey ODE using rk2().
!
!  Discussion:
!
...
!
  ...
  integer data_unit
  character ( len = * ), parameter :: header = 'predator_prey_rk2'
  integer i
  real ( kind = rk ) p0(m)
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  predator_prey_rk2_test: data stored in "' &
    // trim ( data_filename ) // '".'
!
...
!
  ...
  write ( command_unit, '(a)' ) 'set title "rk2(): predator prey ODE"'
  ...
  write ( *, '(a)' ) '  predator_prey_rk2_test: plot commands stored in "' &
...
subroutine stiff_rk2_test ( tspan, y0, n )

!*****************************************************************************80
!
!! stiff_rk2_test tests stiff_rk2().
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'stiff_rk2_test'
  write ( *, '(a)' ) '  Solve stiff ODE using rk2().'

  call rk2 ( stiff_deriv, tspan, y0, n, m, t, y )

  call r8vec_linspace ( n2, tspan(1), tspan(2), t2 )

  call stiff_exact ( n2, t2, y2 )

  call plot2 ( n, t, y, n2, t2, y2, 'stiff_rk2', &
    'Stiff ODE: rk2() method' )
