...
!
!    Output, integer I4VEC(N), the values read from the string.
!
...
!    Input, integer N, the number of values expected.
!
!    Output, real ( kind = rk8 ) R8VEC(N), the values read from the string.
!
!    Output, integer IERROR, error flag.
!    0, no errors occurred.
!    -K, could not read data for entries -K through N.
...
!
!    Input, real ( kind = rk8 ) XY(2,POINT_NUM), the point coordinates.
!
  implicit none
...
!
!    Input, integer POINT_NUM, the number of points.  The program
!    will stop reading data once POINT_NUM values have been read.
!
!    Output, real ( kind = rk8 ) XY(2,POINT_NUM), the point coordinates.
!
  implicit none
...
!
  ...
  real ( kind = rk8 ) theta
  integer, parameter :: turns = 5
  real ( kind = rk8 ) xy(2,point_num)

  do j = 1, point_num
    r = real ( j - 1, kind = rk8 ) / real ( point_num - 1, kind = rk8 )
    theta = real ( turns, kind = rk8 ) * r * ( 2.0D+00 * r8_pi )
    xy(1,j) = r * cos ( theta )
    xy(2,j) = r * sin ( theta )
  end do
...
!
  ...
  character ( len = * ) input_filename
  integer point_num
  real ( kind = rk8 ), allocatable, dimension ( : ) :: xy(:,:)
!
!  Read the header.
!
...
subroutine xyf_data_print ( face_num, face_data_num, &
...
!
!    Input, integer FACE_POINTER(LINE_NUM+1), pointers to the
!    first line item for each face.
!
!    Input, integer FACE_DATA(LINE_DATA_NUM), indices
...
!
  ...
  integer i
  integer face
  integer face_data(face_data_num)
  integer face_pointer(face_num+1)
...
subroutine xyf_data_write ( output_unit, face_num, face_data_num, &
  face_pointer, face_data )
...
!    Input, integer POINT_NUM, the number of points.
!
!    Input, integer FACE_NUM, the number of lines.
!
!    Input, integer FACE_DATA_NUM, the number of line items.
!
!    Output, real ( kind = rk8 ) XY(2,POINT_NUM), the point coordinates.
!
!    Output, integer FACE_POINTER(FACE_NUM+1), pointers to the
...
!
  ...
  integer k
  real ( kind = rk8 ) r
  real ( kind = rk8 ), parameter :: r_min = 1.0D+00
  real ( kind = rk8 ), parameter :: r_max = 3.0D+00
  real ( kind = rk8 ) t
  real ( kind = rk8 ), parameter :: t_min = 3.141592653589793D+00
  real ( kind = rk8 ), parameter :: t_max = 0.0D+00
  real ( kind = rk8 ) xy(2,point_num)

  k = 0
  do j = 1, n_r

    r = ( real ( n_r - j ,    kind = rk8 ) * r_min &
        + real (       j - 1, kind = rk8 ) * r_max ) &
        / real ( n_r     - 1, kind = rk8 )

    do i = 1, n_t

      t = ( real ( n_t - i,     kind = rk8 ) * t_min &
          + real (       i - 1, kind = rk8 ) * t_max ) &
          / real ( n_t     - 1, kind = rk8 )
...
!
!    Output, integer FACE_NUM, the number of face.
!
...
!    Input, integer POINT_NUM, the number of points.
!
!    Input, integer FACE_NUM, the number of face.
!
!    Input, integer FACE_DATA_NUM, the number of face items.
!
  implicit none
...
!  Write the data.
!
  call xyf_data_write ( output_unit, face_num, face_data_num, &
    face_pointer, face_data )
!
!  Close the file.
!
...
subroutine xyl_data_print ( line_num, line_data_num, &
...
!
  ...
  integer i
  integer line
  integer line_data(line_data_num)
  integer line_pointer(line_num+1)
...
subroutine xyl_data_write ( output_unit, line_num, line_data_num, &
  line_pointer, line_data )
...
!  Write the data.
!
  call xyl_data_write ( output_unit, line_num, line_data_num, &
    line_pointer, line_data )
!
!  Close the file.
!
