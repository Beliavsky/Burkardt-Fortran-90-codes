...
!  Parameters:
!
!    Input, integer M, the number of objects to cluster.
!
!    Input, real ( kind = rk8 ) T(M,M), a distance matrix which records the
!    distances, or the squares of distances, between all pairs of
!    objects.  T(M,M) should be symmetric, and the diagonal entries
!    should be 0.
!
!    Input/output, integer P(M), the cluster assignments.
!
!    Input, integer NC, the number of clusters.
!
!    Output, real ( kind = rk8 ) E(NC), the sum, for each cluster, of the pairwise
!    distances between points in the cluster.
!
!    Output, real ( kind = rk8 ) D, the sum of the entries in E.
!
  implicit none
...
!  Parameters:
!
!    Input, integer M, the number of rows of X.
!
!    Input, integer N, the number of columns of data.
!
!    Input, real ( kind = rk8 ) X(M,N), the data to be clustered.
!
!    Input, real ( kind = rk8 ) W(M), the weights associated with each data point.
!
!    Input/output, integer P(M), the cluster assignments.
!
!    Input, integer NC, the number of clusters.
!
!    Output, real ( kind = rk8 ) S(NC,N), the location of the cluster centers.
!    These points are NOT required to be data points.
!
!    Output, real ( kind = rk8 ) E(NC), the per-cluster objective functions.
!
!    Output, real ( kind = rk8 ) D, the total objective function, which is just
...
!
  ...
  real ( kind = rk8 ) d
  real ( kind = rk8 ) e(nc)
  real ( kind = rk8 ), parameter :: eps = 0.001D+00
  real ( kind = rk8 ) f
  real ( kind = rk8 ) g(m)
  integer h
  integer i
  ...
  real ( kind = rk8 ) w_sum
  real ( kind = rk8 ) x(m,n)
  real ( kind = rk8 ), allocatable, dimension ( :, : ) :: y
...
!    Input, real ( kind = rk8 ) X(M,N), the data to be clustered.
!
!    Input, integer NC, the number of clusters created.
!
!    Input, integer P(M), the cluster assignments.
!
!    Output, real ( kind = rk8 ) S(NC,N), the cluster centroids.
!
  implicit none
...
!
  ...
  do j = 1, nc
    s(j,1:n) = s(j,1:n) / real ( max ( q(j), 1 ), kind = rk8 )
  end do
...
!    Input, integer P(M), the cluster assignments.
!
!    Output, real ( kind = rk8 ) MEDIAN(NC,N), the cluster medians.
!
  implicit none
  ...
  integer j
  integer k
  real ( kind = rk8 ) median(nc,n)
  integer p(m)
  integer r
  integer r_pop
  real ( kind = rk8 ) temp(m)
  real ( kind = rk8 ) x(m,n)
...
!    Input, integer P(M), the cluster assignments.
!
!    Output, real ( kind = rk8 ) D, the cluster median distance.
!
  implicit none
...
!    Input, integer NC, the number of clusters created.
!
!    Output, integer Q(NC), the cluster populations.
!
  implicit none
...
!    Input, integer P(M), the cluster assignments.
!
!    Output, real ( kind = rk8 ) E(NC), the cluster variances.
!
  implicit none
...
!  Parameters:
!
!    Input, integer A(M,N), the data matrix.
!
!    Input, integer M, the number of rows in the matrix.
!
!    Input, integer N, the number of columns in the matrix.
!
!    Output, integer P(N), a permutation of the column indices
!    which maximizes the objective function.
!
!    Input, real ( kind = rk8 ) POWER, the power to which the entries of A should
...
!
  ...
  b(1:m,1:n) = ( real ( a(1:m,1:n), kind = rk8 ) )**power
...
!
!    Input, integer M, the number of data items.
!
!    Input, integer N, the dimension of each data item.
!
!    Output, real ( kind = rk8 ) X(M,N), the data values.
!
  implicit none
...
!    Input, integer N, the dimension of each data item.
!
!    Input, real ( kind = rk8 ) X(M,N), the data values.
!
...
!    Input, integer M, the number of data items.
!
!    Input, integer N, the dimension of the data items.
!
!    Input, real ( kind = rk8 ) X(M,N), the data items.
!
!    Input, integer J1, J2, the columns of data corresponding to
!    X and Y.
!
!    Input, integer ROWS, COLUMNS, the number of rows and columns
!    of "pixels" to use.
!
  implicit none
  ...
  real ( kind = rk8 ) x(m,n)
  real ( kind = rk8 ) x1_max
  real ( kind = rk8 ) x1_min
  real ( kind = rk8 ) x2_max
  real ( kind = rk8 ) x2_min
  ...
  do i = 1, m
    ...
    i2 = 1 + nint ( &
      real ( columns, kind = rk8 ) * ( x(i,j1) - x1_min ) / ( x1_max - x1_min ) )
    i2 = max ( i2, 1 )
    i2 = min ( i2, columns )

    i1 = 1 + nint ( &
      real ( rows, kind = rk8    ) * ( x(i,j2) - x2_min ) / ( x2_max - x2_min ) )
    i1 = max ( i1, 1 )
    i1 = min ( i1, rows )
...
!    Input, character ( len = * ) FILE_NAME, the name of the file to read.
!
!    Output, integer M, the number of nonblank, noncomment lines.
!
!    Output, integer N, the number of values per line.
!
  implicit none
...
!
!    Output, real ( kind = rk8 ) A(N,N), the matrix.
!
  implicit none
  ...
  do i = 1, n
    do j = 1, n
      if ( i <= j ) then
        a(i,j) = real ( i * ( n - j + 1 ), kind = rk8 ) / real ( n + 1, kind = rk8 )
      else
        a(i,j) = real ( j * ( n - i + 1 ), kind = rk8 ) / real ( n + 1, kind = rk8 )
      end if
    end do
  end do
...
!    Input, real ( kind = rk8 ) X(M,N), the data to be clustered.
!
!    Input/output, integer P(M), contains the cluster assignments.
!
!    Input, integer NC_OLD, NC_NEW, the initial and final number of
...
!
!    Input/output, real ( kind = rk8 ) E(NC_NEW), the cluster variances.
!
!    Input/output, real ( kind = rk8 ) D, the total variance.
!
  implicit none
...
!
  ...
  integer r
  real ( kind = rk8 ) x(m,n)
  real ( kind = rk8 ) x1(m,n)
  real ( kind = rk8 ) x2(m,n)
!
!  1 -> 2
!
  nd = 1
...
!
  ...
  real ( kind = rk8 ) ss
  integer t
  real ( kind = rk8 ) u(nc,n)
  real ( kind = rk8 ) ua(n)
  real ( kind = rk8 ) ub(n)
  real ( kind = rk8 ) uf(n)
  real ( kind = rk8 ) uj
  real ( kind = rk8 ) ur
  real ( kind = rk8 ) v(nc,n)
  real ( kind = rk8 ) va(n)
  real ( kind = rk8 ) vb(n)
  real ( kind = rk8 ) vf(n)
  real ( kind = rk8 ) vj
...
!  Parameters:
!
!    Input, integer M, the number of objects to be clustered.
!
!    Input/output, real ( kind = rk8 ) D(M,M), the distance matrix.
!    D(I,J) is the distance from I to J.  D should be symmetric, and
!    have a zero diagonal.  On output, D has been destroyed.
!
!    Input, integer METHOD, specifies the method to be used.  
!    METHOD should be between 1 and 7.
!
!    Output, integer A(M-1), B(M-1), contains the two indices of
!    the two clusters being merged at each step.
!
!    Output, real ( kind = rk8 ) H(M-1), the distance between the two clusters
      ...
      if ( i /= ic .and. p(i) == 0 ) then
        ...
        if ( method == 1 ) then
          ...
          d(j,l) = ( q(ic) * d(j,l) + q(jc) * d(k1,k2) ) &
            / real ( q(ic) + q(jc), kind = rk8 )

        else if ( method == 6 ) then

          d(j,l) = ( q(ic) * d(j,l) + q(jc) * d(k1,k2) &
            - ( q(ic) * q(jc) * dmin / real ( q(ic) + q(jc), kind = rk8 ) ) ) &
            / real ( q(ic) + q(jc), kind = rk8 )

        else if ( method == 7 ) then

          d(j,l) = ( ( q(ic) + q(i) ) * d(j,l) + ( q(jc) + q(i) ) * d(k1,k2) &
            - q(i) * dmin ) / real ( q(i) + q(ic) + q(jc), kind = rk8 )
...
!  Parameters:
!
!    Input, integer N, the size of the vector being permuted.
!
!    Input/output, integer X(N), the vector to be permuted.
!
!    Input/output, integer Q(N), information about the permutation.
!
...
!
  ...
  integer i4_factorial
  integer, save :: l = 0
  integer, save, allocatable, dimension ( : ) :: q
  integer x(n)
...
!    Input, integer M, the number of rows of X.
!
!    Input, integer N, the number of columns of X.
!
!    Input, real ( kind = rk8 ) X(M,N), the data to be clustered.
!
!    Input, real ( kind = rk8 ) RHO, a clustering tolerance.  To join a cluster,
!    a new point has to be within RHO of the representative.
!    RHO should not be negative.
!
!    Output, integer NC, the number of clusters created.
!
!    Output, integer P(M), the cluster assignments.
!
  implicit none
...
!
      ...
      s(nc,1:n) = ( real ( q(nc) - 1, kind = rk8 ) * s(nc,1:n) + x(r,1:n) ) &
        / real ( q(nc), kind = rk8 )
...
!    Output, real ( kind = rk8 ) E(NC), the cluster variances.
!
!    Output, real ( kind = rk8 ) D, the total variance.
!
  implicit none
...
!
    ...
    a = real ( q(r), kind = rk8 ) &
      * sum ( ( s(r,1:n) - x(i,1:n) )**2 ) / real ( q(r) - 1, kind = rk8 )
    b = huge ( b )
    ...
    do j = 1, nc
      ...
      if ( j /= r ) then
        ...
        f = real ( q(j), kind = rk8 ) * sum ( ( s(j,1:n) - x(i,1:n) )**2 ) &
          / real ( q(j) + 1, kind = rk8 )
    ...
    if ( a <= b ) then
      ...
      s(r,1:n) = ( real ( q(r), kind = rk8 ) * s(r,1:n) - x(i,1:n) ) &
        / real ( q(r) - 1, kind = rk8 )
      s(v,1:n) = ( real ( q(v), kind = rk8 ) * s(v,1:n) + x(i,1:n) ) &
        / real ( q(v) + 1, kind = rk8 )
...
!
!    Input, real ( kind = rk8 ) D(M,M), the distance matrix.  D(I,J) is the
!    distance from I to J.  D should be symmetric, and have a zero diagonal.
!
!    Output, integer Q(M-1), P(M-1), T(M-1), the index of a point,
...
!    Input, integer M, the number of rows of X.
!
!    Input, real ( kind = rk8 ) X(M), the data to be clustered.  The data must be
!    sorted in ascending order.
!
!    Input, integer NC, the number of clusters to create.
!
!    Output, integer Q(NC,NC), describes the clusters of data.
...
!
!    Output, real ( kind = rk8 ) S(M,NC), contains pointwise variances for a
...
!  Parameters:
!
!    Input, integer M, the dimension of the objects.
!
!    Input, integer N, the number of objects.
!
!    Input, real ( kind = rk8 ) A(M,N), the objects as columns of a matrix.
!
!    Output, integer B(N,N), a distance matrix for the objects.
!
...
!
!    Input, real ( kind = rk8 ) Z, is used if TRANS is TRUE on input, and
!    represents the upper bound for entries of A.
!
!    Output, integer IND(N), the optimal ordering of the variables.
!
  implicit none
...
!  Parameters:
!
!    Input/output, real ( kind = rk8 ) X, Y.  On output, the values of X and
...
!
!    Input, real ( kind = rk8 ) A(N,N), the matrix whose determinant is desired.
!
!    Output, real ( kind = rk8 ) DET, the determinant of the matrix.
!
  implicit none
...
!
!    Input, real ( kind = rk8 ) A(M,N), the matrix to be printed.
!
...
!
!    Input, real ( kind = rk8 ) X(N), the array to be examined.
!
...
!  Parameters:
!
!    Input, integer M, the number of items to assign.
!
!    Input, integer M0, the minimum number of items in 
!    each cluster.
!
!    Input, integer NC, the number of clusters.
!
!    Output, integer P(M), the cluster to which each item 
!    is assigned.
!
!    Input/output, integer SEED, a seed used by the random
!    number generator.
!
  implicit none
...
!
  do i = k+1, m
    ...
    j = int ( real ( nc, kind = rk8 ) * urand ( seed ) ) + 1
    j = min ( j, nc )
    j = max ( j, 1 )
...
!
!    Output, real ( kind = rk8 ) R, the real value that was read from the string.
!
...
!    Input, real ( kind = rk8 ) W(M), the weights associated with each data point.
!
!    Input/output, real ( kind = rk8 ) S(N).  On input, if IS is nonzero, then
!    S is the initial estimate of the location of X*.  On output,
!    S contains the program's estimate of the location.
!
!    Input, real ( kind = rk8 ) EPS, a tolerance used in an accuracy test.
!
!    Input, integer ITMAX, the maximum number of 
!    iterations allowed.
!
!    Input, integer IS, is 0 if the initial guess for the location
!    of X* should be made by the program, or nonzero if the user
!    has supplied a guess for the location in S.
!
!    Output, real ( kind = rk8 ) F, the value of the objective function.
!
  implicit none
...
!    Input, integer M, the number of rows of X.
!
!    Input, integer L, the number of columns of X.
!
!    Input/output, real ( kind = rk8 ) X(M,L), the data to be transformed.
!
  implicit none
...
!
  do k = 1, l
    ...
    s = sqrt ( real ( m - 1, kind = rk8 ) / ( u - t * q ) )
...
!    number generator.
!
!    Output, real ( kind = rk8 ) URAND, the pseudo-random number.
!
  implicit none
...
!    Input/output, integer P(M), the cluster assignments.
!
!    Output, real ( kind = rk8 ) DET, the determinant.
!
  implicit none
  ...
  real ( kind = rk8 ) am(n,n)
  real ( kind = rk8 ) bm(n,n)
  real ( kind = rk8 ) det
  real ( kind = rk8 ) detn
  real ( kind = rk8 ) detv
  real ( kind = rk8 ) dm(n,n)
  real ( kind = rk8 ) dn(n,n)
  real ( kind = rk8 ) dv(n,n)
  integer i
  integer ii
  integer ir
  integer it
    ...
    do j = 1, nc
      ...
      if ( r /= j ) then
        ...
        dn(1:n,1:n) = dm(1:n,1:n) &
          - real ( q(r), kind = rk8 ) * am(1:n,1:n) &
          / real ( q(r) - 1, kind = rk8 ) &
          + real ( q(j), kind = rk8 ) * bm(1:n,1:n) &
          / real ( q(j) + 1, kind = rk8 )
...
!
    if ( d1 < d2 ) then
      p(i1) = 1
      s1(1:n) = ( real ( g1, kind = rk8 ) * s1(1:n) + x(i1,1:n) ) &
        / real ( g1 + 1, kind = rk8 )
      g1 = g1 + 1
!
...
!
      ...
      p(i2) = 2
      s2(1:n) = ( real ( g2, kind = rk8 ) * s2(1:n) + x(i2,1:n) ) &
        / real ( g2 + 1, kind = rk8 )
      g2 = g2 + 1
    end if
