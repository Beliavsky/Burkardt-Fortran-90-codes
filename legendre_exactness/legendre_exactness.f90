...
!*****************************************************************************80
!
!! legendre_exactness() tests a Legendre quadrature rule for exactness.
!
!  Discussion:
!
...
!
  ...
  if ( order == 1 ) then
    ...
    w2(1:order) = 0.0D+00
    w2(1:order-1) = w2(1:order-1) + 1.0D+00 / real ( order - 1, kind = rk )
    w2(2:order)   = w2(2:order)   + 1.0D+00 / real ( order - 1, kind = rk )

    do i = 1, order
      x2(i) = ( real ( order - i,     kind = rk ) * ( -1.0D+00 ) &
