...
!*****************************************************************************80
!
!! normal_dataset() generates a multivariate normal random dataset.
!
!  Discussion:
!
...
!
  ...
  real ( kind = rk8 ), allocatable, dimension ( :, : ) :: a
  integer arg_num
  integer i
  integer iarg
  integer iargc
  integer j
  integer m
  real ( kind = rk8 ), allocatable, dimension ( : ) :: mu
  integer n
...
!
!    real ( kind = rk8 ) A(M,M), the variance-covariance 
!    matrix.  A must be positive definite symmetric.
!
!    real ( kind = rk8 ) MU(M), the mean vector.
!
!    integer SEED, the random number seed.
!
!  Output:
!
!    integer SEED, the updated random number seed.
!
!    real ( kind = rk8 ) X(M,N), the points.
!
  implicit none
...
!
!    Ireal ( kind = rk8 ) A(M,N), an M by N matrix to be printed.
!
...
!
!    real ( kind = rk8 ) A(N,N): the matrix in R8PO storage.
!
!  Output:
!
!    real ( kind = rk8 ) A(N,N): the Cholesky factor R in R8GE storage.
!
...
!    and so on.
!
!    real ( kind = rk8 ) Y, the value from the previous call, if SAVED is 1.
!
  implicit none
