...
!  Parameters:
!
!    Input, integer CENTER(2), the center of the hexagon, in 
!    PostScript coordinates.
!
!    Input, integer RADIUS, the radius of the hexagon, in 
!    PostScript coordinates.
!
!    Iniput, integer ANGLE, the angle through which the hexagon
...
!
!    Input, integer IUNIT, the PostScript output unit.
!
...
!  Parameters:
!
!    Input, integer NGRID, the number of "grid lines" drawn in
...
!  Parameters:
!
!    Input, integer N, specifies the number of grid lines drawn.
!
!    Input, character ( len = * ) FILE_NAME, the name of the file to which the
!    output should be written.
!
...
!
  ...
  do i = 1, 2*n-1
    ...
    center(2) = 360 + int ( ( i - n ) * 1.5 * radius )
!
...
!  Parameters:
!
!    Input, integer ANGLE_NUM, the number of angular grid lines.
!
!    Input, integer CIRCLE_NUM, the number of circular grid lines.
!
!    Input, character ( len = * ) FILE_NAME, the name of the file to which the
!    output should be written.
!
...
!
  ...
  pxcen = int ( 0.5D+00 * ( pxmin + pxmax ) )
  pycen = int ( 0.5D+00 * ( pymin + pymax ) )
  ...
  do i = 0, angle_num
    ...
    angle = real ( ( i - 1 ) * 360, kind = rk ) / real ( angle_num, kind = rk )
    ...
    px = pxcen + int ( prmax * cos ( degrees_to_radians ( angle ) ) )

    py = pycen + int ( prmax * sin ( degrees_to_radians ( angle ) ) )
...
!
  ...
  do circle = 1, circle_num
    ...
    pr = int ( real ( circle * prmax, kind = rk ) &
      / real ( circle_num, kind = rk ) )
...
!
  ...
  integer grid_px_min
  integer grid_px_num
  integer grid_px_space
  integer grid_px_width
  integer grid_py_max
  integer grid_py_min
  integer grid_py_num
  integer grid_py_space
  integer grid_py_width
  integer grid_row
  integer ierror
  integer iunit
  integer pxmax
...
!
  ...
  integer pymax
  integer pymin
  integer, dimension ( 9, 9 ) :: value = reshape ( &
    (/ &
    0, 6, 0, 0, 7, 9, 4, 0, 0, &
    0, 0, 0, 0, 0, 4, 0, 3, 6, &
    3, 2, 4, 0, 0, 1, 0, 0, 0, &
...
!  Parameters:
!
!    Input, integer IUNIT, the FORTRAN output unit.
!
!    Input, integer GRID_PX_MIN, GRID_PY_MIN, GRID_PX_MAX, 
...
!    the sudoku grid to be drawn.
!
!    Input, integer VALUE(9,9), contains 81 values.  Values between
...
!  Input:
!
!    integer N, specifies the number of grid lines drawn.
!
!    character ( len = * ) FILE_NAME, the name of the file to which the
!    output should be written.
!
  implicit none
...
!  Parameters:
!
!    Input, integer N, the number of lines or boxes per inch.
!
!    Input, character ( len = * ) FILE_NAME, the name of the file to which the
!    output should be written.
!
...
!
  ...
  do i = 0, ( 15 * n ) / 2
    ...
    px = pxmin + nint ( real ( i * 72, kind = rk ) / real ( n, kind = rk ) )
  ...
  do j = 0, 10 * n
    ...
    py = pymin + nint ( real ( j * 72, kind = rk ) / real ( n, kind = rk ) )
