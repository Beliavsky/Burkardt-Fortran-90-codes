...
!
  ...
  real ( kind = rk ), dimension ( n ) :: x =  (/ 0.0D+00, 1.0D+00,  2.0D+00 /)
  real ( kind = rk ), dimension ( n ) :: y =  (/ 1.0D+00, 6.0D+00, 17.0D+00 /)
  real ( kind = rk ), dimension ( n ) :: yp = (/ 2.0D+00, 8.0D+00, 14.0D+00 /)
...
!
  ...
  call random_number ( harvest = c(0:2*n-1) )
...
!
  ...
  real ( kind = rk ) f12
  integer i
  real ( kind = rk ), dimension ( nd ) :: xd = (/ 0.0D+00, 10.0D+00 /)
  real ( kind = rk ) xv
  real ( kind = rk ) yd(nd)
  real ( kind = rk ) yh
  real ( kind = rk ) ypd(nd)
