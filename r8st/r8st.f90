...
!    matrix entry.
!
!    Output, integer K, the r8st index of the (I,J) entry.
!
  implicit none
...
!
!    Output, real ( kind = rk ) B(N), the product vector A'*X.
!
  implicit none
...
subroutine r8st_random ( m, n, nz_num, row, col, a )
...
!
  ...
  call i4_fake_use ( m )
  call i4_fake_use ( n )
  call i4_fake_use ( row(1) )
  call i4_fake_use ( col(1) )
  call random_number ( harvest = a(1:nz_num) )
...
!    of the matrix.
!
!    Output, integer NZ_NUM, the number of nonzero elements 
!    in the matrix.
!
  implicit none
...
!    Input, real ( kind = rk ) A(NZ_NUM), the nonzero elements of the matrix.
!
!    Output, integer ROWCOL(2,NZ_NUM), the R8NCF row and column
!    index vector.
!
  implicit none
