...
!  Parameters:
!
!    Input, real ( kind = rk8 ) ALPHA_MAX, the maximum legal value of 
!    LEVEL_WEIGHT, after all entries have been divided by the minimum 
!    nonzero entry.  1 <= ALPHA_MAX.
!
!    Input, integer DIM_NUM, the spatial dimension.
!
!    Input, real ( kind = rk8 ) LEVEL_WEIGHT(DIM_NUM), the anisotropic weights.  
!    The values must be positive.  
!
!    Output, real ( kind = rk8 ) LEVEL_WEIGHT2(DIM_NUM), the balanced 
...
!
  ...
  real ( kind = rk8 ) alpha_max
  integer dim
  real ( kind = rk8 ) level_weight(dim_num)
  real ( kind = rk8 ) level_weight2(dim_num)
  real ( kind = rk8 ) level_weight_min
  integer nonzero_num
...
!    Input, integer DIM_NUM, the spatial dimension.
!
!    Input/output, real ( kind = rk8 ) LEVEL_WEIGHT(DIM_NUM), the anisotropic
...
!
  ...
  real ( kind = rk8 ) level_weight(dim_num)
  real ( kind = rk8 ) level_weight_min
  real ( kind = rk8 ) level_weight_sum
  integer nonzero_num
  integer option
  real ( kind = rk8 ) r8_huge
!
...
!
    ...
    level_weight(1:dim_num) = ( real ( dim_num, kind = rk8 ) &
...
!    Input, integer DIM_NUM, the spatial dimension.
!
!    Input, real ( kind = rk8 ) IMPORTANCE(DIM_NUM), the importance vector.
!    All entries must be nonnegative, and at least one entry must be positive.
!
!    Output, real ( kind = rk8 ) LEVEL_WEIGHT(DIM_NUM), the anisotropic
!    weights.
!
  implicit none
  ...
  integer dim
  integer found
  real ( kind = rk8 ) importance(dim_num)
  real ( kind = rk8 ) level_weight(dim_num)
...
!
  ...
  integer level_1d_max(dim_num)
  integer level_max
  real ( kind = rk8 ) level_weight(dim_num)
  real ( kind = rk8 ) level_weight_min_pos
  logical more_grids
  logical more_points
...
!
  ...
  q_min = real ( level_max, kind = rk8 ) * level_weight_min_pos &
    - sum ( level_weight(1:dim_num) )
  q_max = real ( level_max, kind = rk8 ) * level_weight_min_pos
  do dim = 1, dim_num
    if ( 0.0D+00 < level_weight(dim) ) then
...
!    6, "FE", full exponential.
!
!    Input, integer NP(DIM_NUM), the number of parameters 
!    used by each rule.
!
!    Input, real ( kind = rk8 ) P(*), the parameters needed by each rule.
!
!    Input, integer POINT_NUM, the number of unique points 
!    in the grid. 
!
...
!    it.  The indices are 1-based.
!
!    Output, real ( kind = rk8 ) SPARSE_POINT(DIM_NUM,POINT_NUM), the points.
!
  implicit none
  ...
  integer sparse_index(dim_num,point_num)
  integer sparse_order(dim_num,point_num)
  real ( kind = rk8 ) sparse_point(dim_num,point_num)
!
...
!
  ...
  do dim = 1, dim_num
    ...
    do level = 0, level_1d_max(dim)
      ...
      call level_growth_to_order ( 1, level_vec, rule(dim:dim), growth(dim:dim), order_vec )
      ...
      if ( rule(dim) == 1 ) then
        call clenshaw_curtis_compute_points_np ( order_vec, np(dim), p(p_index), &
          points )
      else if ( rule(dim) == 2 ) then
        call fejer2_compute_points_np ( order_vec, np(dim), p(p_index), points )
      else if ( rule(dim) == 3 ) then
        call patterson_lookup_points_np ( order_vec, np(dim), p(p_index), points )
      else if ( rule(dim) == 4 ) then
        call legendre_compute_points_np ( order_vec, np(dim), p(p_index), points )
      else if ( rule(dim) == 5 ) then
        call hermite_compute_points_np ( order_vec, np(dim), p(p_index), points )
      else if ( rule(dim) == 6 ) then
        call gen_hermite_compute_points_np ( order_vec, np(dim), p(p_index), &
          points )
      else if ( rule(dim) == 7 ) then
        call laguerre_compute_points_np ( order_vec, np(dim), p(p_index), points )
      else if ( rule(dim) == 8 ) then
        call gen_laguerre_compute_points_np ( order_vec, np(dim), p(p_index), &
          points )
      else if ( rule(dim) == 9 ) then
        call jacobi_compute_points_np ( order_vec, np(dim), p(p_index), points )
      else if ( rule(dim) == 10 ) then
        call hermite_genz_keister_lookup_points_np ( order_vec, np(dim), &
          p(p_index), points )
...
!    Input, real ( kind = rk8 ) P(*), the parameters needed by each rule.
!
!    Output, real ( kind = rk8 ) WEIGHT_ND(ORDER_ND), the product rule weights.
!
  implicit none
  ...
  integer p_index
  integer rule(dim_num)
  real ( kind = rk8 ), allocatable, dimension ( : ) :: weight_1d
  real ( kind = rk8 ) weight_nd(order_nd)
...
!    Input, real ( kind = rk8 ) P(*), the parameters needed by each rule.
!
!    Input, real ( kind = rk8 ) TOL, the tolerance for point equality.
!
...
!
  ...
  real ( kind = rk8 ), allocatable, dimension ( :, : ) :: sparse_total_point
  real ( kind = rk8 ) tol
!
!  Special cases.
!
...
!
!    Input, real ( kind = rk8 ) W(N), the weights, which should be nonnegative.
...
!
!    Input, real ( kind = rk8 ) Q_MIN, Q_MAX, the lower and upper
!    limits on the sum.
!
...
!    Input, integer DIM_NUM, the spatial dimension.
!
!    Input, real ( kind = rk8 ) LEVEL_WEIGHT(DIM_NUM), the weights.
!
!    Input, integer X(DIM_NUM), satisfies the Q constraint.
!
!    Input, real ( kind = rk8 ) Q_MAX, the Q constraint maximum.
!
!    Output, real ( kind = rk8 ) COEF, the combinatorial coefficient.
!
  implicit none
...
!
!    Input, integer X(DIM_NUM), a point which satisifies 
!    the constraints.
!
!    Input, real ( kind = rk8 ) Q_MIN, Q_MAX, the lower and upper
!    limits on the sum.
...
!
!    Input/output, integer X(DIM_NUM).  On first call, with 
!    MORE = FALSE, the input value of X is not important.  On subsequent calls, 
!    the input value of X should be the output value from the previous call.
!    On output, (with MORE = TRUE), the value of X will be the "next"
!    vector in the reverse lexicographical list of vectors that satisfy
...
!  Parameters:
!
!    Input, integer DIM_NUM, the number of components in 
!    the vector.
!
!    Input, real ( kind = rk8 ) LEVEL_WEIGHT(DIM_NUM), the anisotropic weights.
!
!    Input, integer X_MAX(DIM_NUM), the maximum values allowed in 
!    each component of X.
!
!    Input/output, integer X(DIM_NUM).  On first call, with 
!    MORE = FALSE, the input value of X is not important.  On subsequent calls, 
...
!
  ...
  logical more
  real ( kind = rk8 ) q_max
  real ( kind = rk8 ), save :: q_max2
  real ( kind = rk8 ) q_min
  real ( kind = rk8 ), save :: q_min2
  real ( kind = rk8 ) level_weight(dim_num)
  integer x(dim_num)
  integer x_max(dim_num)
!
...
!    the unique listing.
!
!    Output, real ( kind = rk8 ) SPARSE_WEIGHT(POINT_NUM), the weights
...
!
  ...
  real ( kind = rk8 ) coef
  integer dim
  real ( kind = rk8 ), allocatable, dimension ( : ) :: grid_weight
  integer growth(dim_num)
  integer level_1d(dim_num)
  integer level_1d_max(dim_num)
  integer level_max
  ...
  integer rule(dim_num)
  integer sparse_unique_index(point_total_num)
  real ( kind = rk8 ) sparse_weight(point_num)
...
!    in the grid. 
!
!    Input, real ( kind = rk8 ) SPARSE_WEIGHT(POINT_NUM), the weights.
!
!    Input, real ( kind = rk8 ) SPARSE_POINT(DIM_NUM,POINT_NUM), the points.
!
...
!
  ...
  integer rule(dim_num)
  real ( kind = rk8 ) sparse_point(dim_num,point_num)
  real ( kind = rk8 ) sparse_region(dim_num,2)
  real ( kind = rk8 ) sparse_weight(point_num)
!
