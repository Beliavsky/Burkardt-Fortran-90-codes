...
!
  ...
  do i = 1, npol
    xpol(i) = ( real ( npol - ( i - 1 ), kind = rk ) * xlo &
...
!
  ...
  real ( kind = rk ) dpdx(npol)
  integer ipol
  integer ival
  integer nx
  real ( kind = rk ) pval(npol)
  real ( kind = rk ) xhi
  real ( kind = rk ) xlo
  real ( kind = rk ) xpol(npol)
  real ( kind = rk ) xval
...
!
  ...
  real ( kind = rk ) a(0:maxn)
  real ( kind = rk ) b(0:maxn)
  real ( kind = rk ) c(0:maxn)
  integer na
  integer nb
  integer test
