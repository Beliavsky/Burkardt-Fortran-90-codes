...
subroutine r8col_duplicates ( m, n, n_unique, a )
...
!
  ...
  call random_number ( harvest = a(1:m,1:n_unique) )
!
!  Randomly copy unique columns.
!
  do j1 = n_unique + 1, n
    j2 = i4_uniform_ab ( 1, n_unique )
    a(1:m,j1) = a(1:m,j2)
  end do
!
!  Permute the columns.
...
!    Input, integer M, N, the number of rows and columns.
!
!    Input/output, real ( kind = rk ) A(M,N), the array to be flipped.
!
  implicit none
...
!    N must be positive.
!
!    Output, real ( kind = rk ) TABLE(M,N), the table.
!
  implicit none
  ...
  do i = 1, m
    do j = 1, n
      table(i,j) = real ( fac * i + j, kind = rk )
    end do
  end do
...
!
!    Input/output, real ( kind = rk ) A(M,N_MAX), a table of numbers, regarded
!    as an array of columns.  The columns must have been sorted
!    lexicographically.
!
!    Input, real ( kind = rk ) X(M), a vector of data which will be inserted
...
!
!    Output, real ( kind = rk ) AMAX(N), the maximums of the columns.
!
  implicit none
...
!    Input, integer M, N, the number of rows and columns.
!
!    Input/output, real ( kind = rk ) A(M,N), the array to be rescaled.
!
  implicit none
...
!    Input, real ( kind = rk ) A(M,N), the array to be examined.
!
!    Output, real ( kind = rk ) AMIN(N), the minimums of the columns.
!
  implicit none
...
!
!    Input/output, real ( kind = rk ) A(M,N), the array to be permuted.
!
  implicit none
...
!    Input, integer N, the number of data values.
!
!    Input, real ( kind = rk ) AL(M,N), the data values.
!
!    Input, integer UNIQUE_NUM, the number of unique values
!    in A.  This value is only required for languages in which the size of
!    UNDX must be known in advance.
...
!    Input, real ( kind = rk ) A(M,N), the array to be examined.
!
!    Output, real ( kind = rk ) COLSUM(N), the sums of the columns.
!
  implicit none
...
!    Input, real ( kind = rk ) A(M,N), the array.
!
!    Output, real ( kind = rk ) X(M*N), a vector containing the N columns of A.
!
  implicit none
...
subroutine r8col_uniform_ab ( m, n, a, b, r )
...
subroutine r8col_uniform_abvec ( m, n, a, b, r )
...
!    the array.
!
!    Input, real ( kind = rk ) A(M), B(M), the lower and upper limits.
!
!    Output, real ( kind = rk ) R(M,N), the array of pseudorandom values.
!
  implicit none
