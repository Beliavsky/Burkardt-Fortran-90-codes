...
!*****************************************************************************80
!
!! frieze() uses blending to tile a region with a pattern.
!
!  Discussion:
!
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) R, S, the (R,S) coordinates of a point.
!
!    Output, real ( kind = rk8 ) X, Y, the (X,Y) coordinates of the point.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) RE, RW, SN, SS, the extreme (east and west)
...
!
  ...
  real ( kind = rk8 ) sn
  real ( kind = rk8 ) ss
  real ( kind = rk8 ) xpoint(npoint)
  real ( kind = rk8 ) ypoint(npoint)
!
!  East cell border.
!
  r = re
  do i = 1, npoint
    s = ( real ( npoint - i,     kind = rk8 ) * ss   &
        + real (          i - 1, kind = rk8 ) * sn ) &
        / real ( npoint     - 1, kind = rk8 )
...
!
  ...
  do i = 1, npoint
    ...
    r = ( real ( npoint - i,     kind = rk8 ) * re   &
        + real (          i - 1, kind = rk8 ) * rw ) &
        / real ( npoint     - 1, kind = rk8 )
...
!
  r = rw
  do i = 1, npoint
    s = ( real ( npoint - i,     kind = rk8 ) * sn   &
        + real (          i - 1, kind = rk8 ) * ss ) &
        / real ( npoint     - 1, kind = rk8 )
    call blend1d2d ( r, s, xpoint(i), ypoint(i) )
  end do
...
!
  s = ss
  do i = 1, npoint
    r = ( real ( npoint - i,     kind = rk8 ) * rw   &
        + real (          i - 1, kind = rk8 ) * re ) &
        / real ( npoint     - 1, kind = rk8 )
    call blend1d2d ( r, s, xpoint(i), ypoint(i) )
  end do
...
!
  ...
  real ( kind = rk8 ) sn
  real ( kind = rk8 ) ss
  real ( kind = rk8 ) x(nfine)
  real ( kind = rk8 ) xcval(4)
  real ( kind = rk8 ) xmax
  real ( kind = rk8 ) xmin
  real ( kind = rk8 ) xne
  real ( kind = rk8 ) xnw
  real ( kind = rk8 ) xse
  real ( kind = rk8 ) xsw
  real ( kind = rk8 ) y(nfine)
  real ( kind = rk8 ) ycval(n)
  real ( kind = rk8 ) ymax
  real ( kind = rk8 ) ymin
  real ( kind = rk8 ) yne
  real ( kind = rk8 ) ynw
...
!
  ...
  r = 1.0D+00
  do i = 1, nfine
    s = real ( i - 1, kind = rk8 ) / real ( nfine - 1, kind = rk8 )
  ...
  s = 1.0D+00
  do i = nfine, 1, -1
    r = real ( i - 1, kind = rk8 ) / real ( nfine - 1, kind = rk8 )
    call boundary_2d ( r, s, x(i), y(i) )
  end do
...
!
  ...
  do irow = 1, nrow
    ...
    sn = real ( irow, kind = rk8 ) / real ( nrow, kind = rk8 )
    ss = real ( irow - 1, kind = rk8 ) / real ( nrow, kind = rk8 )

    do icol = 1, ncol

      re = real ( icol, kind = rk8 ) / real ( ncol, kind = rk8 )
      rw = real ( icol - 1, kind = rk8 ) / real ( ncol, kind = rk8 )
...
!
  ...
  do i = 1, nfine
    r = ( real ( nfine - i,     kind = rk8 ) * rw   &
        + real (         i - 1, kind = rk8 ) * re ) &
        / real ( nfine     - 1, kind = rk8 )

    s = ( real ( nfine - i,     kind = rk8 ) * sn   &
        + real (         i - 1, kind = rk8 ) * ss ) &
        / real ( nfine     - 1, kind = rk8 )
...
!
  ...
  integer, parameter :: max_end = 100
  integer, parameter :: max_pat = 100
  integer, parameter :: n = 4
  integer, parameter :: nfine = 21
...
!
  ...
  real ( kind = rk8 ) xmin
  real ( kind = rk8 ) xpat(max_pat)
  real ( kind = rk8 ) xpoint(MAX_POINT)
  real ( kind = rk8 ) y(nfine)
  real ( kind = rk8 ) ymax
  real ( kind = rk8 ) ymin
  real ( kind = rk8 ) ypat(max_pat)
  real ( kind = rk8 ) ypoint(MAX_POINT)
...
!  Parameters:
!
!    Input, integer IEND(NUM_END), contains the indices of XPAT and YPAT
...
!
!    Input, integer NUM_END, the number of entries used in IEND, and one
!    more than the number of line segments used to draw the pattern.
!
!    Input, integer NUM_PAT, the number of entries used in XPAT and YPAT.
!
!    Input, real ( kind = rk8 ) XPAT(NUM_PAT), YPAT(MAX_PAT), the X and Y
...
!
  ...
  real ( kind = rk8 ) ss
  real ( kind = rk8 ) xpoint(MAX_POINT)
  real ( kind = rk8 ) xpat(num_pat)
  real ( kind = rk8 ) ypoint(MAX_POINT)
  real ( kind = rk8 ) ypat(num_pat)
...
!  Parameters:
!
!    Output, integer IEND(MAX_END), contains the indices of XPAT
...
!
!    Input, integer MAX_END, the maximum number of entries in IEND.
!
!    Output, integer NUM_END, the number of entries used in IEND, 
!    and one more than the number of line segments used to draw the pattern.
!
!    Input, integer MAX_PAT, the maximum number of entries in 
!    XPAT and YPAT.
!
!    Output, integer NUM_PAT, the number of entries used in XPAT 
!    and YPAT.
!
!    Output, real ( kind = rk8 ) XPAT(MAX_PAT), YPAT(MAX_PAT), the X and Y
!    coordinates of points that define the line segments used to draw 
!    the pattern.
!
  implicit none
...
!
!    Input, real ( kind = rk8 ) R, G, B, the RGB values for the new line color.
!
  implicit none
  ...
  real ( kind = rk8 ) b
  real ( kind = rk8 ) b_old
  real ( kind = rk8 ) b_stack(nstack)
  real ( kind = rk8 ) g
  real ( kind = rk8 ) g_old
  real ( kind = rk8 ) g_stack(nstack)
  integer, save :: istack = 0
  real ( kind = rk8 ) r
  real ( kind = rk8 ) r_old
  real ( kind = rk8 ) r_stack(nstack)
  logical s_eqi
...
!  Input:
!
!    real ( kind = rk8 ) R, G, B, the RGB values for the new line color.
!
  implicit none
...
!
  if ( r_old /= r .or. g_old /= g .or. b_old /= b ) then
    ...
    call ps_setting_int ( 'GET', 'UNIT', iunit )

    call ps_comment ( 'Set RGB line color.' )

    write ( iunit, '(3f7.4,a)' ) r, g, b, ' setrgbcolor'
...
!
  ...
  real ( kind = rk8 ) fill_blue
  real ( kind = rk8 ) fill_green
  real ( kind = rk8 ) fill_red
  real ( kind = rk8 ) font_size
  real ( kind = rk8 ) line_blue
  real ( kind = rk8 ) line_green
  real ( kind = rk8 ) line_red
  integer line_width
  integer marker_size
...
subroutine ps_file_close ( iunit )
...
!  Input:
!
!    integer IUNIT, the FORTRAN unit to which output was written.
!
  implicit none
...
!
!  For some unfathomable reason, I can't say "iunit = 0" here.
!
! iunit = 0
  ival = 0
  call ps_setting_int ( 'SET', 'UNIT', ival )
...
!
  ...
  write ( iunit, '(a)' )     '%%Creator: ps_write.f90'
  write ( iunit, '(a)' )     '%%Title: ' // trim ( file_name )
  write ( iunit, '(a)' )     '%%CreationDate: ' // trim ( date )
  write ( iunit, '(a)' )     '%%Pages: (atend)'
  write ( iunit, '(a,4i6)' ) '%%BoundingBox:', plotxmin, plotymin, plotxmax, &
    plotymax
  write ( iunit, '(a)' )     '%%Document-Fonts: Times-Roman'
  write ( iunit, '(a)' )     '%%LanguageLevel: 1'
...
!
  ...
  write ( iunit, '(a)' ) '1.00 inch scalefont'
...
subroutine ps_file_open ( file_name, iunit, ierror )
...
!  Parameters:
!
!    Input, integer IUNIT, the FORTRAN unit to which output should
!    be written.
!
...
!
  ...
  call ps_setting_int ( 'SET', 'UNIT', iunit )
...
!
  ...
  write ( iunit, '(a,i6)' ) '%%Pages: ', num_pages
! write ( iunit, '(a)' ) 'end'
  write ( iunit, '(a)' ) '%%EOF'
!
...
!
!    Input, real ( kind = rk8 ) X(NPOINT), Y(NPOINT), the X and Y components
!    of the points.
!
  implicit none
  ...
  integer state
  integer iunit
  real ( kind = rk8 ) x(npoint)
  real ( kind = rk8 ) xmin
  real ( kind = rk8 ) y(npoint)
  real ( kind = rk8 ) ymin
!
...
!    Input, integer NPOINT, the number of points in the line.
!
!    Input, real ( kind = rk8 ) X(NPOINT), Y(NPOINT), the X and Y 
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) XMIN, YMIN, XMAX, YMAX, the minimum and maximum X
...
!
  ...
  real ( kind = rk8 ) xmin
  real ( kind = rk8 ) xmin2
  real ( kind = rk8 ) xvec(4)
  real ( kind = rk8 ) ycur
  real ( kind = rk8 ) ymax
  real ( kind = rk8 ) ymax2
  real ( kind = rk8 ) ymin
  real ( kind = rk8 ) ymin2
  real ( kind = rk8 ) yvec(4)
!
!  Determine if the PostScript state is acceptable.
!
  call ps_setting_int ( 'GET', 'STATE', state )
...
!
  ...
  write ( iunit, '(a,i6,i6)' ) '%%Page: ', num_pages, num_pages
  write ( iunit, '(a)' ) 'save'
!
!  Reset the state.
!
...
!
  ...
  alpha = min ( real ( plotxmax - plotxmin, kind = rk8 ) / ( xmax2 - xmin2 ), &
                real ( plotymax - plotymin, kind = rk8 ) / ( ymax2 - ymin2 ) )
!
!  Adjust PLOTXMIN and PLOTYMIN to center the image.
!
  plotxmin2 = nint ( 0.5D+00 * &
    ( real ( plotxmin + plotxmax, kind = rk8 ) - alpha * ( xmax2 - xmin2 ) ) )

  plotymin2 = nint ( 0.5D+00 * &
    ( real ( plotymin + plotymax, kind = rk8 ) - alpha * ( ymax2 - ymin2 ) ) )
!
!  Store data.
!
...
!
  ...
  write ( iunit, '(a)' ) 'restore showpage'
...
subroutine ps_setting_int ( my_action, variable, ival )
...
!  Parameters:
!
!    Input, character ( len = * ) my_action, the desired action:
...
!    If my_action = 'GET', then IVAL is an output quantity, and is the
!    current internal value of the variable.
!
!    If my_action = 'SET', then IVAL is an input quantity, and the
!    current internal value of the variable is set to this value.
!
!    If my_action = 'PRINT', then IVAL is ignored.
!
  implicit none

  character ( len = * ) my_action
  ...
  if ( variable == 'LINE_WIDTH' ) then
    ...
    if ( my_action == 'GET' ) then
      ival = line_width
    else if ( my_action == 'PRINT' ) then
      write ( *, '(a,i9)' ) 'Line width, LINE_WIDTH = ', line_width
    else if ( my_action == 'SET' ) then
      line_width = ival
    else if ( my_action == 'POP' ) then
      call i4_swap ( line_width, ival )
    else if ( my_action == 'PUSH' ) then
      call i4_swap ( line_width, ival )
    end if
    ...
    if ( my_action == 'GET' ) then
      ...
      write ( *, '(a,i9)' ) 'Current FORTRAN unit, UNIT = ', iunit
    else if ( my_action == 'SET' ) then
      iunit = ival
    else if ( my_action == 'POP' ) then
      call i4_swap ( iunit, ival )
    else if ( my_action == 'PUSH' ) then
      call i4_swap ( iunit, ival )
    end if
...
!
!    Input/output, real ( kind = rk8 ) VALUE.
...
!
  ...
  character ( len = * ) action
  real ( kind = rk8 ), save :: alpha = 0.0D+00
  real ( kind = rk8 ), save :: fill_blue = 0.7D+00
  real ( kind = rk8 ), save :: fill_green = 0.7D+00
  real ( kind = rk8 ), save :: fill_red = 0.7D+00
  real ( kind = rk8 ), save :: font_size = 0.1D+00
  real ( kind = rk8 ), save :: line_blue = 0.0D+00
  real ( kind = rk8 ), save :: line_green = 0.0D+00
  real ( kind = rk8 ), save :: line_red = 0.0D+00
  real ( kind = rk8 ) value
  character ( len = * ) variable
  real ( kind = rk8 ), save :: xcur = 0.0D+00
  real ( kind = rk8 ), save :: xmax = 1.0D+00
  real ( kind = rk8 ), save :: xmin = 0.0D+00
  real ( kind = rk8 ), save :: ycur = 0.0D+00
  real ( kind = rk8 ), save :: ymax = 0.0D+00
  real ( kind = rk8 ), save :: ymin = 0.0D+00
