...
!
  ...
  integer, parameter :: n2 = 1 + 5 * ( n + 1 )

  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ), external :: cosine_sum
  real ( kind = rk ) f2(n2)
  real ( kind = rk ) fa
  real ( kind = rk ) fb
  integer i
...
!
!    Output, real ( kind = rk ) COSINE_SUM, the value.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) cosine_sum
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) POLY5, the value of the polynomial at X.
!
  implicit none
