...
!
  ...
  write ( *, '(a)' ) '  Test poisson_simulation().'
...
!
  ...
  integer, parameter :: bin_num = 30
  integer, parameter :: event_num = 1000
  ...
  call poisson_fixed_events ( lambda, event_num, t, w )

  write ( *, '(a)' ) ''
  write ( *, '(a,g14.6)' ) '  Minimum wait = ', minval ( w(1:event_num) )
  write ( *, '(a,g14.6)' ) '  Average wait = ', &
    sum ( w(1:event_num) ) / real ( event_num, kind = rk )
...
!
  ...
  f_bin(1:bin_num) = 0
  do i = 0, event_num
    j = 1 + int ( real ( bin_num, kind = rk ) * ( w(i) - w_min ) &
      / ( w_max - w_min ) )
    j = min ( j, bin_num )
    f_bin(j) = f_bin(j) + 1
  end do
...
!
  ...
  width = 0.85D+00 * ( w_max - w_min ) / real ( bin_num, kind = rk )
  write ( command_unit, '(a,f8.2,a)' ) &
...
!
  ...
  integer, parameter :: bin_num = 30
  integer, parameter :: test_num = 20000

  character ( len = 80 ) command_filename
  integer command_unit
  character ( len = 80 ) data_filename
  integer data_unit
  real ( kind = rk ) f_bin(bin_num)
  integer i
  real ( kind = rk ) lambda
  integer n(test_num)
  real ( kind = rk ) n_bin(bin_num)
  real ( kind = rk ) n_max
  real ( kind = rk ) n_mean
  real ( kind = rk ) n_min
  real ( kind = rk ) n_var
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'TEST02:'
  write ( *, '(a)' ) '  POISSON_FIXED_TIME simulates a Poisson process'
  ...
  do test = 1, test_num
    call poisson_fixed_time ( lambda, t, n(test) )
  end do
  ...
  n_min = real ( minval ( n ), kind = rk )
  n_max = real ( maxval ( n ), kind = rk )

  call r8vec_midspace ( bin_num, n_min, n_max, n_bin )

  f_bin(1:bin_num) = 0
  do test = 1, test_num
    i = 1 + int ( real ( bin_num * ( n(test) - n_min ), kind = rk ) &
      / real ( n_max - n_min, kind = rk ) )
    i = min ( i, bin_num )
    f_bin(i) = f_bin(i) + 1
  end do
!
...
!
  ...
  write ( command_unit, '(a)' ) 'set grid'
  write ( command_unit, '(a)' ) 'set style fill solid'
  w = 0.85D+00 * ( n_max - n_min ) / real ( bin_num, kind = rk )
  write ( command_unit, '(a,f8.2,a)' ) &
