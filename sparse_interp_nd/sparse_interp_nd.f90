...
!
!    Output, real ( kind = rk ) XD(M,ND), the points at which data was sampled.
!
  implicit none
...
!    Input, integer ND, the number of points in the product grid.
!
!    Input, real ( kind = rk ) ZD(ND), the function evaluated at the points XD.
!
!    Input, integer NI, the number of points at which the 
!    interpolant is to be evaluated.
!
!    Input, real ( kind = rk ) XI(M,NI), the points at which the interpolant 
!    is to be evaluated.
!
!    Output, real ( kind = rk ) ZI(NI), the interpolant evaluated at the 
!    points XI.
!
  implicit none
...
!    1 <= M.
!
!    Output, integer C(0:L_MAX), the coefficients for objects 
!    at sublevels 0 through L_MAX.
!
!    Output, integer W(0:L_MAX), the number of objects at 
