...
!    1 <= I <= 16.
!
!    Output, integer I4_MACH, the value of the chosen parameter.
!
  implicit none
...
!
!    Output, real ( kind = rk ) R8_ACOS, the arc-cosine of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ), parameter :: pi2 = 1.57079632679489661923132169163975D+00
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_ACOSH, the arc-hyperbolic cosine of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ), parameter :: dln2 = 0.69314718055994530941723212145818D+00
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) AMPL, PHI, the modulus and phase of the 
...
!
  ...
  real ( kind = rk ) ampl
  real ( kind = rk ) an20cs(57)
  real ( kind = rk ) an21cs(60)
  real ( kind = rk ) an22cs(74)
  real ( kind = rk ) aph0cs(53)
  real ( kind = rk ) aph1cs(58)
  real ( kind = rk ) aph2cs(72)
  real ( kind = rk ) eta
  integer nan20
  integer nan21
  integer nan22
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_AI, the Airy function Ai of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) aifcs(13)
  real ( kind = rk ) aigcs(13)
  integer naif
  integer naig
  real ( kind = rk ) r8_ai
  real ( kind = rk ) r8_aie
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_AID, the derivative of the Airy function 
!    Ai of X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_AIDE, the exponentially scaled derivative of 
...
!
  ...
  real ( kind = rk ) aifcs(13)
  real ( kind = rk ) aigcs(13)
  real ( kind = rk ) aip1cs(57)
  real ( kind = rk ) aip2cs(37)
  real ( kind = rk ) eta
  integer naif
  integer naig
  integer naip1
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_AIE, the exponentially scaled Airy 
!    function Ai of X.
!
  implicit none
...
!
  ...
  real ( kind = rk ) am20cs(57)
  real ( kind = rk ) am21cs(60)
  real ( kind = rk ) am22cs(74)
  real ( kind = rk ) ampl
  real ( kind = rk ) ath0cs(53)
  real ( kind = rk ) ath1cs(58)
  real ( kind = rk ) ath2cs(72)
  real ( kind = rk ) eta
  integer nam20
  integer nam21
  integer nam22
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_AINT, the truncated version of X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_ASIN, the arc-sine of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) asincs(39)
  integer nterms
  real ( kind = rk ) pi2
  real ( kind = rk ) r8_asin
  real ( kind = rk ) r8_csevl
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_ASINH, the arc-hyperbolic sine of X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_ATAN, the arc-tangent of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) atancs(16)
  real ( kind = rk ) conpi8(4)
  integer n
  integer nterms
  real ( kind = rk ) pi8(4)
...
!  Parameters:
!
!    Input, real ( kind = rk ) SN, CS, the Y and X coordinates of a 
!    point on the angle.
!
!    Output, real ( kind = rk ) R8_ATAN2, the arc-tangent of the angle.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_ATANH, the arc-hyperbolic tangent of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) atnhcs(27)
  real ( kind = rk ) dxrel
  integer nterms
...
!
  ...
  real ( kind = rk ) ampl
  real ( kind = rk ) bm12cs(40)
  real ( kind = rk ) bm1cs(37)
  real ( kind = rk ) bt12cs(39)
  real ( kind = rk ) bth1cs(44)
  real ( kind = rk ) eta
  integer nbm1
  integer nbm12
  integer nbt12
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESI0, the Bessel function I of order 0 of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) bi0cs(18)
  integer nti0
  real ( kind = rk ) r8_besi0
  real ( kind = rk ) r8_besi0e
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESI0E, the exponentially scaled Bessel 
!    function I0(X).
!
  implicit none
  ...
  real ( kind = rk ) ai02cs(69)
  real ( kind = rk ) ai0cs(46)
  real ( kind = rk ) bi0cs(18)
  real ( kind = rk ) eta
  integer ntai02
  integer ntai0
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESJ1, the Bessel function J of order 1 of X.
!
  implicit none
  ...
  real ( kind = rk ) ampl
  real ( kind = rk ) bj1cs(19)
  integer ntj1
  real ( kind = rk ) r8_besj1
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) theta
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESK0, the Bessel function K of order 0 of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) bk0cs(16)
  integer ntk0
  real ( kind = rk ) r8_besi0
  real ( kind = rk ) r8_besk0
  real ( kind = rk ) r8_besk0e
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESK0E, the exponentially scaled Bessel 
!    function K0(X).
!
  implicit none
  ...
  real ( kind = rk ) ak02cs(33)
  real ( kind = rk ) ak0cs(38)
  real ( kind = rk ) bk0cs(16)
  real ( kind = rk ) eta
  integer ntak0
  integer ntak02
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESY0, the Bessel function Y of order 0 of X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BESY1, the Bessel function Y of order 1 of X.
!
  implicit none
  ...
  real ( kind = rk ) ampl
  real ( kind = rk ) by1cs(20)
  integer nty1
  real ( kind = rk ) r8_besj1
  real ( kind = rk ) r8_besy1
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) theta
...
!  Parameters:
!
!    Input, real ( kind = rk ) A, B, the arguments.
!
!    Output, real ( kind = rk ) R8_BETA, the beta function of A and B.
!
  implicit none
...
!    0.0 <= X <= 1.0.
!
!    Input, real ( kind = rk ) PIN, the first distribution parameter.
!    0.0 < PIN.
!
!    Input, real ( kind = rk ) QIN, the second distribution parameter.
!    0.0 < QIN.
!
!    Output, real ( kind = rk ) R8_BETAI, the incomplete beta function ratio.
!
  implicit none
  ...
  if ( 1.0D+00 < q ) then
    ...
    term = exp ( xb - real ( ib, kind = rk ) * alnsml )
    c = 1.0D+00 / ( 1.0D+00 - y )
    p1 = q * c / ( p + q - 1.0D+00 )

    finsum = 0.0D+00
    n = int ( q )
    if ( q == real ( n, kind = rk ) ) then
      n = n - 1
    end if
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BI, the Airy function Bi of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) bifcs(13)
  real ( kind = rk ) bif2cs(15)
  real ( kind = rk ) bigcs(13)
  real ( kind = rk ) big2cs(15)
  real ( kind = rk ) eta
  integer nbif
  integer nbif2
  integer nbig
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BID, the derivative of the Airy function 
!    Bi of X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BIDE, the exponentially scaled derivative of 
...
!
  ...
  real ( kind = rk ) big2cs(16)
  real ( kind = rk ) bigcs(13)
  real ( kind = rk ) bip1cs(47)
  real ( kind = rk ) bip2cs(88)
  real ( kind = rk ) btr
  real ( kind = rk ) eta
  integer nbif
  integer nbif2
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_BIE, the exponentially scaled Airy 
!    function Bi of X.
!
  implicit none
...
!
!    Output, real ( kind = rk ) R8_BINOM, the binomial coefficient.
!
  implicit none
  ...
  integer m
  integer n
  real ( kind = rk ) r8_binom
  real ( kind = rk ) r8_lgmc
  real ( kind = rk ) r8_lnrel
  real ( kind = rk ) r8_mach
  real ( kind = rk ) sq2pil
  real ( kind = rk ) xk
  real ( kind = rk ) xn
  ...
  if ( k <= 20 .and. real ( k, kind = rk ) &
    * log ( real ( max ( n, 1 ), kind = rk ) ) <= bilnmx ) then

    r8_binom = 1.0D+00

    do i = 1, k
      r8_binom = r8_binom * real ( n - i + 1, kind = rk ) / real ( i, kind = rk )
    end do
...
!
!    Output, real ( kind = rk ) R8_CBRT, the cube root of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) cbrt2(5)
  integer irem
  integer iter
  integer ixpnt
  integer n
  integer niter
  real ( kind = rk ) r8_cbrt
  real ( kind = rk ) r8_mach
  real ( kind = rk ) r8_pak
  real ( kind = rk ) value
  real ( kind = rk ) vsq
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_CHI, the hyperbolic cosine integral
!    evaluated at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_CHU, the function value.
!
  implicit none
  ...
  real ( kind = rk ) pochai
  real ( kind = rk ) r8_chu
  real ( kind = rk ) r8_chu_scaled
  real ( kind = rk ) r8_exprel
  real ( kind = rk ) r8_gamma
  real ( kind = rk ) r8_gamr
  real ( kind = rk ) r8_mach
  real ( kind = rk ) r8_mop
  real ( kind = rk ) r8_poch
  real ( kind = rk ) r8_poch1
  real ( kind = rk ) sum
  real ( kind = rk ) t
  real ( kind = rk ) x
  real ( kind = rk ) xeps1
...
!
    ...
    do i = 1, 1000
      xi = real ( istrt + i, kind = rk )
      xi1 = real ( istrt + i - 1, kind = rk )
      b0 = ( a + xi1 - beps ) * b0 * x &
        / ( ( xn + xi1 ) * ( xi - beps ) )
...
!    Input, real ( kind = rk ) A, B, the parameters.
!
!    Input, real ( kind = rk ) Z, the argument.
!
!    Output, real ( kind = rk ) R8_CHU_SCALED, the function value.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_CI, the cosine integral Ci evaluated at X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) cics(19)
  real ( kind = rk ) f
  real ( kind = rk ) g
  integer nci
  real ( kind = rk ) r8_ci
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_CIN, the cosine integral Cin evaluated at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_CINH, the hyperbolic cosine integral Cinh
!    evaluated at X.
!
  implicit none
  ...
  real ( kind = rk ) absx
  real ( kind = rk ) cinhcs(16)
  real ( kind = rk ) eul
  integer ncinh
  real ( kind = rk ) r8_chi
  real ( kind = rk ) r8_cinh
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_COS, the cosine of X.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument in degrees.
!
!    Output, real ( kind = rk ) R8_COS_DEG, the cosine of X.
!
  implicit none
  ...
  integer n
  real ( kind = rk ) r8_cos_deg
  real ( kind = rk ), parameter :: raddeg &
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_COSH, the hyperbolic cosine of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) r8_cosh
  real ( kind = rk ) r8_mach
  real ( kind = rk ) value
  real ( kind = rk ) x
  real ( kind = rk ) y
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_COT, the cotangent of X.
!
  implicit none
...
!
!    Input, real ( kind = rk ) A(N), the Chebyshev coefficients.
!
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_DAWSON, the value of Dawson's integral at X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) daw2cs(45)
  real ( kind = rk ) dawacs(75)
  real ( kind = rk ) dawcs(21)
  real ( kind = rk ) eps
  integer ntdaw
  integer ntdaw2
  integer ntdawa
  real ( kind = rk ) r8_csevl
  real ( kind = rk ) r8_dawson
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) x
  real ( kind = rk ) xbig
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_E1, the exponential integral E1.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) ae10cs(50)
  real ( kind = rk ) ae11cs(60)
  real ( kind = rk ) ae12cs(41)
  real ( kind = rk ) ae13cs(50)
  real ( kind = rk ) ae14cs(64)
  real ( kind = rk ) e11cs(29)
  real ( kind = rk ) e12cs(25)
  real ( kind = rk ) eta
  integer ntae10
  integer ntae11
  integer ntae12
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_EI, the exponential integral Ei.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_ERF, the error function at X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ), parameter :: erfcs(21) = (/ &
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_ERFC, the co-error function of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) erc2cs(49)
  real ( kind = rk ) erfccs(59)
  real ( kind = rk ) erfcs(21)
  real ( kind = rk ) eta
  integer nterc2
  integer nterf
  integer nterfc
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_EXP, the exponential of X.
!
  implicit none
  ...
  if ( x < xmin ) then
    ...
    n = int ( y )
    f = y - real ( n, kind = rk )
    n = int ( 23.0D+00 * xint + real ( n, kind = rk ) )
    n16 = n / 16
    if ( n < 0 ) then
      n16 = n16 - 1
    end if
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_EXPREL, the exponential relative error term
!    at X.
!
  implicit none
  ...
  if ( absx < xbnd ) then
    ...
    r8_exprel = 0.0D+00
    do i = 1, nterms
      r8_exprel = 1.0D+00 + r8_exprel * x / real ( nterms + 2 - i, kind = rk )
    end do
  else
...
!
!    Output, real ( kind = rk ) R8_FAC, the factorial of N.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) facn(31)
  integer n
  integer nmax
  real ( kind = rk ) r8_fac
  real ( kind = rk ) r8_lgmc
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_GAMI, the value of the incomplete 
!    gamma function.
!
  implicit none
  ...
  real ( kind = rk ) a
  real ( kind = rk ) factor
  real ( kind = rk ) r8_gami
  real ( kind = rk ) r8_gamit
  real ( kind = rk ) r8_lngam
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) X, the evaluation point.
!
!    Output, real ( kind = rk ) R8_GAMIC, the value of the incomplete 
!    gamma function.
!
  implicit none
  ...
  real ( kind = rk ) h
  integer izero
  real ( kind = rk ) r8_gamic
  real ( kind = rk ) r8_gmic
  real ( kind = rk ) r8_gmit
  real ( kind = rk ) r8_lgic
  real ( kind = rk ) r8_lgit
  real ( kind = rk ) r8_lngam
  real ( kind = rk ) r8_mach
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_GAMIT, the function value.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_GAMMA, the gamma function of X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_GAMR, the value of the reciprocal gamma
!    function at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Input, real ( kind = rk ) ALX, the logarithm of X.
!
!    Output, real ( kind = rk ) R8_GMIC, the complementary incomplete 
!    gamma function.
!
  implicit none
  ...
  do k = 1, m
    r8_gmic = r8_gmic + 1.0D+00 / real ( k, kind = rk )
  end do
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Input, real ( kind = rk ) ALGAP1, the logarithm of Gamma ( A + 1 ).
!
!    Input, real ( kind = rk ) SGNGAM, the sign of Gamma ( A + 1 ).
!
!    Input, real ( kind = rk ) ALX, the logarithm of X.
!
!    Output, real ( kind = rk ) R8_GMIT, the Tricomi incomplete gamma function.
!
  implicit none
  ...
  r8_gmit = 0.0D+00
  algs = - real ( ma, kind = rk ) * log ( x ) + algs
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_INT, the integer part of X.
!
  implicit none
  ...
  integer ibase
  integer ipart
  integer, save :: npart = 0
  real ( kind = rk ) part
  real ( kind = rk ) r8_int
  real ( kind = rk ) r8_mach
  real ( kind = rk ), save :: scale = 0.0D+00
  real ( kind = rk ) x
  real ( kind = rk ), save :: xbig = 0.0D+00
  real ( kind = rk ), save :: xmax = 0.0D+00
  real ( kind = rk ) xscl
  ...
  if ( npart == 0 ) then
    ibase = i4_mach ( 10 )
    xmax = 1.0D+00 / r8_mach ( 4 )
    xbig = min ( real ( i4_mach ( 9 ), kind = rk ), 1.0D+00 / r8_mach ( 4 ) )
    scale = ibase ** int ( log ( xbig ) &
      / log ( real ( ibase, kind = rk ) ) - 0.5D+00 )
...
!    Input, real ( kind = rk ) A, B, the arguments.
!
!    Output, real ( kind = rk ) R8_LBETA, the logarithm of the beta function of A
!    and B.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) ALGAM, the logarithm of the absolute value of
!    gamma ( X ).
!
!    Output, real ( kind = rk ) SGNGAM, the sign (+1 or -1 ) of gamma ( X ).
!
  implicit none
...
!    Input, real ( kind = rk ) ALX, the logarithm of X.
!
!    Output, real ( kind = rk ) R8_LGIC, the log complementary incomplete 
!    gamma function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Input, real ( kind = rk ) ALGAP1, the logarithm of the gamma 
!    function of A+1.
!
!    Output, real ( kind = rk ) R8_LGIT, the log of Tricomi's incomplete
!    gamma function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_LI, the logarithmic integral evaluated at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_LNGAM, the logarithm of the absolute value of
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_LNREL, the value of LOG ( 1 + X ).
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) alnrcs(43)
  integer nlnrel
  real ( kind = rk ) r8_csevl
  integer r8_inits
  real ( kind = rk ) r8_lnrel
...
!    Input, real ( kind = rk ) X, the evaluation point.
!
!    Output, real ( kind = rk ) R8_LOG, the logarithm of X.
!
  implicit none
  ...
  real ( kind = rk ) aln2
  real ( kind = rk ) alncen(5)
  real ( kind = rk ) alncs(11)
  real ( kind = rk ) center(4)
  integer n
  integer nterms
  integer ntrval
  real ( kind = rk ) r8_csevl
...
!    Input, real ( kind = rk ) X, the evaluation point.
!
!    Output, real ( kind = rk ) R8_LOG10, the logarithm, base 10, of X.
!
  implicit none
...
!  Parameters:
!
!    Output, integer ( kind = rk ) IBETA, the radix for the floating-point
!    representation.
!
!    Output, integer ( kind = rk ) IT, the number of base IBETA digits in 
!    the floating-point significand.
!
!    Output, integer ( kind = rk ) IRND:
...
!      partial underflow.
!
!    Output, integer ( kind = rk ) NGRD, the number of guard digits for 
...
!
  ...
  real ( kind = rk ) epsneg
  integer ( kind = rk ) i
  integer ( kind = rk ) ibeta
  integer ( kind = rk ) iexp
  integer ( kind = rk ) irnd
  integer ( kind = rk ) it
  integer ( kind = rk ) itemp
  integer ( kind = rk ) iz
  integer ( kind = rk ) j
  integer ( kind = rk ) k
  integer ( kind = rk ) machep
  integer ( kind = rk ) maxexp
  integer ( kind = rk ) minexp
  integer ( kind = rk ) mx
  integer ( kind = rk ) negep
  integer ( kind = rk ) ngrd
  integer ( kind = rk ) nxres
  real ( kind = rk ) one
  real ( kind = rk ) t
  real ( kind = rk ) temp
  real ( kind = rk ) temp1
...
!  Discussion:
!
!    An R8 is a real ( kind = rk ) real value.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!  Parameters:
!
!    Input, real ( kind = rk ) Y, the mantissa.
!
!    Input, integer N, the exponent.
!
!    Output, real ( kind = rk ) R8_PAK, the packed value.
!
  implicit none
  ...
  if ( nmin == 0 ) then
    ...
    nmin = int ( aln2b * real ( i4_mach ( 15 ), kind = rk ) )
    nmax = int ( aln2b * real ( i4_mach ( 16 ), kind = rk ) )
  end if
...
!  Parameters:
!
!    Input, real ( kind = rk ) A, X, the arguments.
!
!    Output, real ( kind = rk ) R8_POCH, the Pochhammer function of A and X.
!
  implicit none
...
!
  if ( real ( n, kind = rk ) == x .and. n <= 20 ) then
    r8_poch = 1.0D+00
    do i = 1, n
      r8_poch = r8_poch * ( a + real ( i - 1, kind = rk ) )
    end do
    return
  end if
...
!    Input, real ( kind = rk ) X, the evaluation point.
!
!    Output, real ( kind = rk ) R8_POCH1, the value of the function.
!
  implicit none
  ...
  real ( kind = rk ) alnvar
  real ( kind = rk ) b
  real ( kind = rk ) bern(20)
  real ( kind = rk ) binv
  real ( kind = rk ) bp
  real ( kind = rk ) gbern(21)
  real ( kind = rk ) gbk
  integer i
  integer ii
  integer incr
  ...
  if ( var < sqtbig ) then
    ...
    do k = 2, nterms
      ...
      gbern(k+1) = - rho * gbk / real ( k, kind = rk )
      term = term * ( real ( 2 * k - 2, kind = rk ) - x ) &
        * ( real ( 2 * k - 1, kind = rk ) - x ) * var2
...
!
  do ii = 1, incr
    i = incr - ii
    binv = 1.0D+00 / ( bp + real ( i, kind = rk ) )
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the base.
!
!    Input, real ( kind = rk ) Y, the power.
!
!    Output, real ( kind = rk ) R8_POW, the value of X^Y.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_PSI, the psi function of X.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) apsics(16)
  real ( kind = rk ) aux
  real ( kind = rk ) dxrel
  integer i
  integer n
  ...
  if ( y < 10.0D+00 ) then
    ...
    if ( n == 0 ) then
      ...
      if ( x < 0.0D+00 .and. &
        x + real ( n - 2, kind = rk ) == 0.0D+00 ) then
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 'R8_PSI(): Fatal error!'
        write ( *, '(a)' ) '  X is a negative integer.'
        stop 1
      ...
      do i = 1, n
        r8_psi = r8_psi - 1.0D+00 / ( x + real ( i - 1, kind = rk ) )
      end do

    else if ( 0 < n ) then

      do i = 1, n
        r8_psi = r8_psi + 1.0D+00 / ( y + real ( i, kind = rk ) )
      end do
...
!    random number x are represented in real ( kind = rk ) as two 11-bit
...
!  Parameters:
!
!    Input, real ( kind = rk ) R, determines the action.
...
!
!    Output, real ( kind = rk ) R8_RAND, a pseudo-random number between 
!    0.0 and 1.0.
!
  implicit none
  ...
  integer, parameter :: ia0 = 1029
  integer, parameter :: ia1 = 1536
  integer, parameter :: ia1ma0 = 507
  ...
  r8_rand = real ( ix1 * 2048 + ix0, kind = rk )
...
!  Parameters:
!
!    Input, real ( kind = rk ) XMEAN, the mean of the Gaussian distribution.
!
!    Input, real ( kind = rk ) SD, the standard deviation of the 
!    Gaussian function.
!
!    Input/output, integer SEED, a seed for the random 
!    number generator.
!
!    Output, real ( kind = rk ) R8_RANDGS, a normally distributed random number.
!
  implicit none
...
!  Parameters:
!
!    Input, integer N, the number of random numbers in an 
!    auxiliary table.
!
!    Input/output, real ( kind = rk ) T(N+1), an array of random numbers,
!    initialized before first call by R8_RANDOM_INIT.
!
!    Input/output, integer IX0, IX1, two parameters used
!    as seeds for the random number generator.
!
!    Output, real ( kind = rk ) R8_RANDOM, a random number between 0.0 and 1.0.
!
  implicit none
...
!
  j = int ( t(n+1) * real ( abs ( n ), kind = rk ) + 1.0D+00 )
  t(n+1) = t(j)
  r8_random = t(j)
!
...
!    auxiliary table.
!
!    Output, real ( kind = rk ) T(N+1), an array of random numbers. 
!
!    Input/output, integer IX0, IX1, two parameters used
...
!  Parameters:
!
!    Input/output, integer SEED, a seed for the
!    random number generator.
!
!    Output, real ( kind = rk ) R8_REN, the random value.
!
  implicit none
  ...
  r8_ren = real ( seed, kind = rk ) / 2796203.0D+00
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_SHI, the hyperbolic sine integral 
!    Shi evaluated at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_SI, the sine integral Si evaluated at X.
!
  implicit none
...
!
  ...
  real ( kind = rk ) f
  real ( kind = rk ) f1cs(43)
  real ( kind = rk ) f2cs(99)
  real ( kind = rk ) g
  real ( kind = rk ) g1cs(44)
  real ( kind = rk ) g2cs(44)
  real ( kind = rk ) g3cs(56)
  integer nf1
  integer nf2
  integer ng1
  integer ng2
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_SIN, the sine of X.
!
  implicit none
  ...
  integer n2
  integer, save :: ntsn = 0
  real ( kind = rk ), parameter :: pi2rec = &
    0.63661977236758134307553505349006D+00
  real ( kind = rk ), parameter :: pihi = 3.140625D+00
  real ( kind = rk ), parameter :: pilo = 9.6765358979323846264338327950288D-04
  real ( kind = rk ), parameter :: pirec = 0.31830988618379067153776752674503D+00
  integer r8_inits
  real ( kind = rk ) r8_csevl
  real ( kind = rk ) r8_mach
  real ( kind = rk ) r8_sin
  real ( kind = rk ) sgn
  real ( kind = rk ), parameter :: sincs(15) = (/ &
  ...
  real ( kind = rk ), save :: xwarn = 0.0D+00
  real ( kind = rk ) y
!
!  Initialize.
!
...
!    Input, real ( kind = rk ) X, the argument in degrees.
!
!    Output, real ( kind = rk ) R8_SIN_DEG, the sine of X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_SINH, the hyperbolic sine of X.
!
  implicit none
  ...
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) r8_sinh
  real ( kind = rk ) sinhcs(13)
  real ( kind = rk ), save :: sqeps = 0.0D+00
  real ( kind = rk ) value
  real ( kind = rk ) x
  real ( kind = rk ) y
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_SPENCE, Spence's function evaluated at X.
!
  implicit none
  ...
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) r8_spence
  real ( kind = rk ) spencs(38)
  real ( kind = rk ) x
  real ( kind = rk ) xbig
...
!    Input, real ( kind = rk ) X, the number whose square root is desired.
!
!    Output, real ( kind = rk ) R8_SQRT, the square root of X.
!
  implicit none
  ...
  real ( kind = rk ) r8_mach
  real ( kind = rk ) r8_pak
  real ( kind = rk ) r8_sqrt
  real ( kind = rk ) sqrt2(3)
  real ( kind = rk ) value
  real ( kind = rk ) x
  real ( kind = rk ) y
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_TAN, the tangent of X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) R8_TANH, the hyperbolic tangent of X.
!
  implicit none
  ...
  integer r8_inits
  real ( kind = rk ) r8_mach
  real ( kind = rk ) r8_tanh
  real ( kind = rk ), save :: sqeps = 0.0D+00
  real ( kind = rk ) tanhcs(31)
  real ( kind = rk ) value
  real ( kind = rk ) x
  real ( kind = rk ) xmax
  real ( kind = rk ) y
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the number to be unpacked.
!
!    Output, real ( kind = rk ) Y, the mantissa.
!
