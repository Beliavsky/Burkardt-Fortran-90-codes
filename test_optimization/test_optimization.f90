...
!
!    Output, real ( kind = rk ) A(M), B(M), the lower and upper bounds.
!
  implicit none
...
!
!    Input, real ( kind = rk ) X(M,N), the argument of the objective function.
!
!    Output, real ( kind = rk ) F(N), the objective function evaluated at
!    each argument.
!
  implicit none
...
!
!    Output, real ( kind = rk ) X(M), the solution, if known.
!
  implicit none
...
!    Input, integer N, the number of arguments.
!
!    Input, real ( kind = rk ) X(M,N), the arguments.
!
!    Output, real ( kind = rk ) F(N), the function evaluated at the arguments.
!
  implicit none
...
!
  ...
  real ( kind = rk ), parameter :: a = 20.0D+00
  real ( kind = rk ), parameter :: b = 0.2D+00
  real ( kind = rk ), parameter :: c = 0.2D+00
  real ( kind = rk ) f(n)
  integer j
  real ( kind = rk ), parameter :: pi = 3.141592653589793D+00
  real ( kind = rk ) x(m,n)
  ...
  do j = 1, n
    f(j) = - a * exp ( - b * sqrt ( sum ( x(1:m,j)**2 ) &
      / real ( m, kind = rk ) ) ) &
      - exp ( sum ( cos ( c * pi * x(1:m,j) ) ) / real ( m, kind = rk ) ) &
      + a + exp ( 1.0D+00 )
  end do
...
!
  do i = 1, m
    alpha(i) = real ( i, kind = rk ) / real ( m + 1, kind = rk )
  end do
