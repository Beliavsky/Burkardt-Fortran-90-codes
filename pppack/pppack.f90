...
!  Parameters:
! 
!    Input/output, real ( kind = rk ) W(NROWW,NROW).
...
!
!    Input, integer NBANDL, the number of bands of A below 
!    the main diagonal.
! 
!    Input, integer NBANDU, the number of bands of A above 
!    the main diagonal.
! 
...
!  Parameters:
!
!    Input, real ( kind = rk ) W(NROWW,NROW).  W contains the banded matrix,
...
!    main diagonal.
! 
!    Input/output, real ( kind = rk ) B(NROW).
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) W(NBANDS,NROW).
...
! 
!    Work array, real ( kind = rk ) DIAG(NROW).
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) W(NBANDS,NROW), the Cholesky factorization for C, 
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(N+K), the knot sequence.
! 
!    Input, real ( kind = rk ) BCOEF(N), the B spline coefficient sequence.
! 
!    Input, integer N, the number of B spline coefficients.
! 
!    Input, integer K, the order of the spline.
! 
!    Work array, real ( kind = rk ) SCRTCH(K,K).
! 
!    Output, real ( kind = rk ) BREAK(L+1), the piecewise polynomial breakpoint 
!    sequence.  BREAK contains the distinct points in the sequence T(K:N+1)
! 
!    Output, real ( kind = rk ) COEF(K,N), with COEF(I,J) = (I-1)st derivative 
...
!
  ...
  real ( kind = rk ) bcoef(n)
  real ( kind = rk ) biatx(k)
  real ( kind = rk ) break(*)
  real ( kind = rk ) coef(k,n)
  real ( kind = rk ) diff
  integer i
  integer j
  integer jp1
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(LEFT+JOUT), the knot sequence.  T is assumed to 
...
! 
!    Input, real ( kind = rk ) X, the point at which the B-splines 
!    are to be evaluated.
! 
!    Input, integer LEFT, an integer chosen so that 
!    T(LEFT) <= X <= T(LEFT+1).
! 
!    Output, real ( kind = rk ) BIATX(JOUT), with BIATX(I) containing the
...
!
  ...
  real ( kind = rk ) biatx(jhigh)
  real ( kind = rk ), save, dimension ( jmax ) :: deltal
  real ( kind = rk ), save, dimension ( jmax ) :: deltar
  integer i
  integer index
  integer, save :: j = 1
  integer left
  real ( kind = rk ) saved
  real ( kind = rk ) t(left+jhigh)
  real ( kind = rk ) term
  real ( kind = rk ) x
...
!    Input, real ( kind = rk ) T(N+K), the knot sequence.
! 
!    Input, real ( kind = rk ) BCOEF(N,M).  For each J, B(*,J) is the 
...
! 
!    Work array, real ( kind = rk ) SCRTCH(K,K,M).
! 
!    Output, real ( kind = rk ) BREAK(L+1), the breakpoint sequence
!    containing the distinct points in the sequence T(K),...,T(N+1)
! 
!    Output, real ( kind = rk ) COEF(M,K,N), with COEF(MM,I,J) = the (I-1)st
...
!
  ...
  real ( kind = rk ) bcoef(n,m)
  real ( kind = rk ) biatx(k)
  real ( kind = rk ) break(*)
  real ( kind = rk ) coef(m,k,*)
  real ( kind = rk ) diff
  real ( kind = rk ) fkmj
  integer i
  integer j
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(N+K), the knot sequence.  T is assumed 
!    to be nondecreasing.
!
!    Input, real ( kind = rk ) BCOEF(N), B-spline coefficient sequence.
! 
!    Input, integer N, the length of BCOEF.
! 
!    Input, integer K, the order of the spline.
!
!    Input, real ( kind = rk ) X, the point at which to evaluate.
! 
!    Input, integer JDERIV, the order of the derivative to 
!    be evaluated.  JDERIV is assumed to be zero or positive.
! 
!    Output, real ( kind = rk ) BVALUE, the value of the (JDERIV)-th 
...
!  Parameters:
!
!    Input, real ( kind = rk ) P, the smoothing parameter that defines 
!    the linear system.
!
!    Input/output, real ( kind = rk ) V(NPOINT,7), contains data used
!    to define the linear system, some of which is determined by
!    routine SETUPQ.
!
!    Input, real ( kind = rk ) QTY(NPOINT), the value of Q' * Y.
!
!    Input, integer NPOINT, the number of equations.
!
!    Input, integer NCOL, an unused parameter, which may be 
!    set to 1.
!
!    Output, real ( kind = rk ) U(NPOINT), the solution.
!
!    Output, real ( kind = rk ) QU(NPOINT), the value of Q * U.
!
  implicit none
  ...
  integer ncol
  real ( kind = rk ) p
  real ( kind = rk ) qty(npoint)
  real ( kind = rk ) qu(npoint)
  real ( kind = rk ) u(npoint)
  real ( kind = rk ) v(npoint,7)
  real ( kind = rk ) prev
  real ( kind = rk ) ratio
  real ( kind = rk ) six1mp
  real ( kind = rk ) twop
...
!  Parameters: 
!
!    Input, real ( kind = rk ) ALEFT, ARIGHT, the endpoints of the interval.
!
!    Input, integer LBEGIN, the initial number of polynomial 
!    pieces in the approximation.  A uniform breakpoint sequence will be chosen.
!
!    Input, integer IORDER, the order of the polynomial pieces 
!    to be used in the approximation
!
!    Input, integer NTIMES, the number of passes to be made 
!    through NEWNOT.
!
!    Input, real ( kind = rk ) ADDBRK, the number, possibly fractional, of 
...
!    through NEWNOT.
!
!    Input, real ( kind = rk ) RELERR, a tolerance.  Newton iteration is 
...
!
  ...
  real ( kind = rk ) amax
  real ( kind = rk ) aright
  real ( kind = rk ) asave(ndim)
  real ( kind = rk ) b(ndim)
  real ( kind = rk ) bloks(lenblk)
  real ( kind = rk ) break
  real ( kind = rk ) coef
  real ( kind = rk ) dx
  real ( kind = rk ) err
  ...
  real ( kind = rk ) rho
  real ( kind = rk ) t(ndim)
  real ( kind = rk ) templ(lenblk)
  real ( kind = rk ) temps(ndim)
  real ( kind = rk ) xside
...
!
  dx = ( aright - aleft ) / real ( lbegin, kind = rk )
...
!
    lnew = lbegin + int ( real ( nt, kind = rk ) * addbrk )
...
!
!    Output, real ( kind = rk ) RHO(K), the collocation points.
!
  implicit none
  ...
  if ( k == 1 ) then
    ...
    do j = 1, k
      rho(j) = ( real ( k - j,     kind = rk ) * ( -1.0D+00 )   &
               + real (     j - 1, kind = rk ) * ( +1.0D+00 ) ) &
               / real ( k     - 1, kind = rk )
    end do
...
!  Parameters:
!
!    Input, real ( kind = rk ) TAU(N), the abscissas or X values of
...
!
!    Input/output, real ( kind = rk ) C(4,N).
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) W(NEQU,NCOLS), on input, contains
...
!
!    Input/output, real ( kind = rk ) B(NEQU); on input, the right hand
...
!
!    Output, real ( kind = rk ) X(NEQU), the computed solution, if 
!    IFLAG is nonzero.
!
!    Output, integer IFLAG, error flag.
...
!  Parameters:
!
!    Input, integer MODE, an integer indicating the task to 
!    be performed.
!    1, initialization
...
!
!    Input, real ( kind = rk ) XX, a point at which information is wanted
!
!    Output, real ( kind = rk ) V, depends on the MODE.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) BLOKS(*), the factorization of A computed
!    by FCBLOK.
!
!    Input, integer INTEGS(3,NBLOKS), describes the block 
!    structure of A.
!
!    Input, integer NBLOKS, the number of blocks in A.
!
!    Input, integer IPIVOT(*), pivoting information.
!    The dimension of IPIVOT is the sum ( INTEGS(1,1:NBLOKS) ).
!
!    Input, integer IFLAG, = (-1)^(number of interchanges during
!    factorization) if successful, otherwise IFLAG = 0.
!
!    Output, real ( kind = rk ) DETSGN, the sign of the determinant.
!
!    Output, real ( kind = rk ) DETLOG, the natural logarithm of the 
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(N+KPM), the knot sequence.
!
!    Input, integer N, the dimension of the approximating spline 
!    space, that is, the order of the linear system to be constructed.
!
!    Input, integer KPM, = K + M, the order of the approximating 
!    spline.
!
!    Input, integer LENBLK, the maximum length of the array BLOKS,
!    as allowed by the dimension statement in COLLOC.
!
!    Workspace, real ( kind = rk ) WORK1(KPM,KPM), used in PUTIT.
!
!    Workspace, real ( kind = rk ) WORK2(KPM,M+1), used in PUTIT.
!
!    Output, real ( kind = rk ) BLOKS(*), the coefficient matrix of the
!    linear system, stored in almost block diagonal form, of size
!    KPM * sum ( INTEGS(1,1:NBLOKS) ).
!
!    Output, integer INTEGS(3,NBLOKS), describing the block 
!    structure.
...
!
!    Output, integer NBLOKS, the number of blocks, equals number
!    of polynomial pieces.
!
!    Output, real ( kind = rk ) B(*), the right hand side of the linear 
...
!
  ...
  integer nrow
  real ( kind = rk ) rho
  real ( kind = rk ) t(n+kpm)
  real ( kind = rk ) work1(kpm,kpm)
  real ( kind = rk ) work2(kpm,*)
  real ( kind = rk ) xside
...
!  Parameters:
!
!    Input, real ( kind = rk ) BREAK(L+1), real ( kind = rk ) COEF(K,L), 
!    integer L, integer K, the piecewise polynomial representation 
!    of a certain function F of order K.  Specifically,
!      d^(K-1) F(X) = COEF(K,I) for BREAK(I) <= X < BREAK(I+1).
!
!    Input, integer LNEW, the number of subintervals into which 
!    the interval (A,B) is to be sectioned by the new breakpoint
!    sequence BRKNEW.
!
!    Output, real ( kind = rk ) BRKNEW(LNEW+1), the new breakpoints.
!
!    Output, real (kind = rk ) COEFG(2,L), the coefficient part of the 
...
!
  ...
  real ( kind = rk ) break(l+1)
  real ( kind = rk ) brknew(lnew+1)
  real ( kind = rk ) coef(k,l)
  real ( kind = rk ) coefg(2,l)
  integer i
  ...
  if ( lnew == 0 ) then
    ...
    do i = 1, lnew + 1
      brknew(i) = ( real ( lnew - i + 1, kind = rk ) * break(1) &
                  + real (        i - 1, kind = rk ) * break(l+1) ) &
                  / real ( lnew,         kind = rk )
    end do
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) W(NROW,NCOL); on input, contains the
!    matrix to be partially factored; on output, the partial factorization.
!
!    Output, integer IPIVOT(NROW), contains a record of the 
!    pivoting strategy used; row IPIVOT(I) is used during the I-th elimination
!    step, for I = 1, ..., LAST.
!
!    Workspace, real ( kind = rk ) D(NROW), used to store the maximum entry
!    in each row.
!
...
!
!    Input, integer LAST, the number of elimination steps to 
!    be carried out.
!
!    Input/output, integer IFLAG.  On output, equals the input 
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) BLOKS(*).  On input, the almost 
...
!    Input, integer NBLOKS, the number of blocks in A.
!
!    Output, integer IPIVOT(*), which will contain pivoting 
...
!  Parameters:
!
!    Input, real ( kind = rk ) XT(LXT), a nondecreasing sequence of values.
!
!    Input, integer LXT, the dimension of XT.
!
!    Input, real ( kind = rk ) X, the point whose location with 
...
!  Parameters:
!
!    Input, real ( kind = rk ) BREAK(L+1), the breakpoint sequence.
!
!    Input, integer L, the number of intervals or pieces.
!
!    Input, integer KPM, = K+M, the order of the piecewise
!    polynomial function or spline.
!
!    Input, integer M, the order of the differential equation.
!
!    Output, real ( kind = rk ) T(N+KPM), the knot sequence.
!
...
!    Input, real ( kind = rk ) T(N+K), the knot sequence.
!
!    Input, integer N, the dimension of the space of splines 
!    of order K with knots T.
!
!    Input, integer K, the order of the splines.
!
!    Workspace, real ( kind = rk ) Q(K,N), used to store the K lower 
!    diagonals of the Gramian matrix C.
!
!    Workspace, real ( kind = rk ) DIAG(N), used in BCHFAC.
!
!    Output, real ( kind = rk ) BCOEF(N), the B-spline coefficients of
...
!    every data point.
!
!    Output, real ( kind = rk ) FTAU(NTAU), contains the value of the computed
!    approximation at each value TAU(1:NTAU).
!
!    Output, real ( kind = rk ) ERROR(NTAU), with 
...
!
  ...
  real ( kind = rk ) errl2
  real ( kind = rk ) errmax
  real ( kind = rk ) error(ntau)
  real ( kind = rk ) ftau(ntau)
  real ( kind = rk ) gtau
  integer ie
  integer iprfun
  integer k
...
!    Input, real ( kind = rk ) BREAK(L+1), the breakpoint sequence.
!
!    Output, real ( kind = rk ) T(N+K), the knot sequence.
!
!    Output, integer N, the dimension of the corresponding spline 
!    space of order K.
!
  implicit none
...
!      d^(k-1) F(X) = COEF(K,I) for BREAK(I) <= X < BREAK(I+1).
!
!    Input, integer LNEW, the number of intervals into which the 
!    interval (A,B) is to be divided by the new breakpoint sequence BRKNEW. 
!
!    Output, real ( kind = rk ) BRKNEW(LNEW+1), the new breakpoint sequence.
!
!    Output, real ( kind = rk ) COEFG(2,L), the coefficient part of the piecewise
...
!  Parameters:
!
!    Input, real ( kind = rk ) BREAK(L+1), real COEF(*), integer L, the
!    piecewise polynomial representation of the function F to be evaluated.
!
!    Input, integer K, the order of the polynomial pieces that 
!    make up the function F.  The usual value for K is 4, signifying a 
!    piecewise cubic polynomial.
!
!    Input, real ( kind = rk ) X, the point at which to evaluate F or
!    of its derivatives.
!
...
!
!    Output, real ( kind = rk ) PPVALU, the value of the JDERIV-th
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(LEFT+KPM), the knot sequence.
!
!    Input, integer KPM, the order of the spline.
!
!    Input, integer LEFT, indicates the interval of interest, 
!    that is, the interval ( T(LEFT), T(LEFT+1) ).
!
!    Workspace, real ( kind = rk ) SCRTCH(KPM,KPM).
!
!    Workspace, real ( kind = rk ) DBIATX(KPM,M+1), derivatives of B-splines, 
!    with DBIATX(J,I+1) containing the I-th derivative of the J-th B-spline 
!    of interest.
!
!    Output, real ( kind = rk ) Q(NROW,KPM), the block.
!
!    Input, integer NROW, number of rows in block to be 
!!   put together.
!
!    Output, real ( kind = rk ) B(NROW), the corresponding piece of 
!    the right hand side.
!
  implicit none
  ...
  integer mode
  integer mp1
  real ( kind = rk ) q(nrow,kpm)
  real ( kind = rk ) rho
  real ( kind = rk ) scrtch(kpm,kpm)
  real ( kind = rk ) t(left+kpm)
  real ( kind = rk ) v(20)
  real ( kind = rk ) xm
  real ( kind = rk ) xside
  real ( kind = rk ) xx
...
!  Parameters:
!
!    Input. real ( kind = rk ) X, the value to be perturbed.
!
!    Input, real ( kind = rk ) SIZE, the size of the perturbation.
!
!    Output, real ( kind = rk ) ROUND, the perturbed value.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ), save :: flip = -1.0D+00
  real ( kind = rk ) round
  real ( kind = rk ) size
  real ( kind = rk ) x
...
!  Parameters:
!
!    Input, real ( kind = rk ) BLOKS(*), integer INTEGS(3,NBLOKS), integer
!    NBLOKS, integer IPIVOT(*), are as on return from FCBLOK.
!
!    Input, real ( kind = rk ) B(*), the right hand side, stored corresponding 
!    to the storage of the equations.  See comments in SLVBLK for details.
!
!    Output, real ( kind = rk ) X(*), the solution vector.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X(NPOINT), the abscissas, assumed to be 
!    strictly increasing.
!
!    Input, real ( kind = rk ) DX(NPOINT), the data uncertainty estimates,
!    which are assumed to be positive.
!
!    Input, real ( kind = rk ) Y(NPOINT), the corresponding ordinates.
!
!    Input, integer NPOINT, the number of data points.
!
!    Output, real ( kind = rk ) V(NPOINT,7), contains data needed for
!    the smoothing computation.
!
!    Output, real ( kind = rk ) QTY(NPOINT), the value of Q' * Y.
!
  implicit none
  ...
  real ( kind = rk ) diff
  real ( kind = rk ) dx(npoint)
  integer i
  real ( kind = rk ) prev
  real ( kind = rk ) qty(npoint)
  real ( kind = rk ) v(npoint,7)
  real ( kind = rk ) x(npoint)
  real ( kind = rk ) y(npoint)
...
!  Parameters:
!
!    Input, real ( kind = rk ) AI(NROWI,NCOLI), the current block.
!
!    Input, integer IPIVOT(NROWI), the pivot vector.
!
!    Input, integer NROWI, NCOLI, the number of rows and columns
!    in block AI.
!
!    Input, integer LAST, indicates the last row on which pivoting
!    has been carried out.
!
!    Input/output, real ( kind = rk ) AI1(NROWI1,NCOLI1), the next block.
!
!    Input, integer NROWI1, NCOLI1, the number of rows and columns
!    in block AI1.
!
  implicit none
  ...
  real ( kind = rk ) ai(nrowi,ncoli)
  real ( kind = rk ) ai1(nrowi1,ncoli1)
  integer ip
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) BLOKS(*), a one-dimenional array, 
...
!
!    Input, integer INTEGS(3,NBLOKS), description of the block 
!    structure of A.
...
!
!    Input, integer NBLOKS, the number of blocks.
!
!    Input, real ( kind = rk ) B(NBROWS), the right hand side.  Certain entries 
!    are arbitrary, corresponding to rows of the blocks which overlap.  See 
!    the block structure in the example.
!
!    Output, integer IPIVOT(NBROWS), the pivoting sequence used.
!
!    Output, real ( kind = rk ) X(N), the computed solution, if iflag /= 0.
!
!    Output, integer IFLAG.
...
!    Input, real ( kind = rk ) Y(NPOINT), the corresponding ordinates.
!
!    Input, real ( kind = rk ) DY(NPOINT), the data uncertainty estimates,
!    which are assumed to be positive.
!
!    Input, integer NPOINT, the number of data points.
!
!    Input, real ( kind = rk ) S, an upper bound on the discrete weighted mean
!    square distance of the approximation F from the data.
!
!    Workspace, real ( kind = rk ) V(NPOINT,7).
!
!    Workspace, real ( kind = rk ) A(NPOINT,4).
!
!    Output, real ( kind = rk ) A(NPOINT,4).
...
!
!    Output, real ( kind = rk ) SMOOTH, the value of the smoothing parameter.
!
  implicit none
  ...
  real ( kind = rk ) a(npoint,4)
  real ( kind = rk ) change
  real ( kind = rk ) dy(npoint)
  integer i
  real ( kind = rk ) oosf
  real ( kind = rk ) ooss
  real ( kind = rk ) p
...
!  Parameters:
!
!    Input, real ( kind = rk ) TAU(N), contains the data point abscissas.
!    TAU must be strictly increasing
! 
!    Input, real ( kind = rk ) GTAU(N,M), contains the data point ordinates.
! 
!    Input, real ( kind = rk ) T(N+K), the knot sequence.
! 
...
!    Input, integer M, the number of data sets.
!
!    Work space, real ( kind = rk ) WORK(N).
! 
!    Output, real ( kind = rk ) Q(2*K-1)*N, the triangular 
...
!
  ...
  real ( kind = rk ) bcoef(m,n)
  real ( kind = rk ) gtau(n,m)
  integer i
  integer iflag
  integer ilp1mx
  integer j
...
!  Parameters:
!
!    Input, real ( kind = rk ) TAU(N), the data point abscissas.  The entries in
!    TAU should be strictly increasing.
!
!    Input, real ( kind = rk ) GTAU(N), the data ordinates.
!
!    Input, real ( kind = rk ) T(N+K), the knot sequence.
!
...
!    Input, integer K, the order of the spline.
!
!    Output, real ( kind = rk ) Q((2*K-1)*N), the triangular factorization
...
!  Parameters:
!
!    Input, real ( kind = rk ) TAU(N), the interpolation points.
!    assumed to be nondecreasing, with TAU(I) < TAU(I+K), for all I.
!
!    Input, integer N, the number of data points.
!
!    Input, integer K, the order of the optimal recovery scheme 
!    to be used.
!
!    Workspace, real ( kind = rk ) SCRTCH((N-K)*(2*K+3)+5*K+3).  The various
!    contents are specified in the text below.
!
!    Output, real ( kind = rk ) T(N+K), the optimal knots ready for
...
!
  ...
  integer nmk
  integer nx
  real ( kind = rk ) scrtch((n-k)*(2*k+3)+5*k+3)
  real ( kind = rk ) t(n+k)
  real ( kind = rk ) tau(n)
  real ( kind = rk ) sign
  real ( kind = rk ) signst
  real ( kind = rk ) tol
  real ( kind = rk ), parameter :: tolrte = 0.000001D+00
  real ( kind = rk ) xij
...
!
  ...
  do j = 1, nmk 
    scrtch(j+nx) = sum ( tau(j+1:j+k-1) ) / real ( k - 1, kind = rk )
  end do
!
...
!
  newton = 1
  tol = tolrte * ( tau(n) - tau(1) ) / real ( nmk, kind = rk )
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) W(NROW,NCOL), integer IPIVOT(NROW), integer
!    NROW, integer NCOL, integer LAST, are as on return from FACTRB.
!
!    Input/output, real ( kind = rk ) X(NCOL).
...
!  Parameters:
!
!    Input, real ( kind = rk ) W(NROW,LAST), integer IPIVOT(NROW), 
!    integer NROW, integer LAST, are as on return from FACTRB.
!
!    Output, real ( kind = rk ) B(2*NROW-LAST).  On input, B(1:NROW)
...
!
  ...
  real ( kind = rk ) b(nrow+nrow-last)
  integer ip
  integer ipivot(nrow)
  integer jhi
  integer k
  real ( kind = rk ) w(nrow,last)
  real ( kind = rk ) x(nrow)
...
!  Parameters:
!
!    Input, real ( kind = rk ) TAU(NTAU), the sequence of data points.  
!    TAU must be strictly increasing.
!
!    Input, real ( kind = rk ) GTAU(NTAU), the corresponding sequence of
!    function values.
!
!    Input, integer NTAU, the number of data points.  
!    NTAU must be at least 4.
!
!    Input, real ( kind = rk ) GAMMA, indicates whether additional flexibility
!    is desired.
...
!
!    Output, real ( kind = rk ) BREAK(L), real ( kind = rk ) COEF(K,L), 
!    integer L, integer K, give the piecewise polynomial 
...
!
!    Output, real ( kind = rk ) S(NTAU,6).  The individual columns of this
...
!
  ...
  real ( kind = rk ) onemzt
  real ( kind = rk ) ratio
  real ( kind = rk ) s(ntau,6)
  real ( kind = rk ) sixth
  real ( kind = rk ) tau(ntau)
  real ( kind = rk ) temp
  real ( kind = rk ) x
  real ( kind = rk ) z
  real ( kind = rk ) zeta
...
!  Parameters:
!
!    Output, real ( kind = rk ) T(N), the location of the data points.
!
!    Output, real ( kind = rk ) G(N), the value associated with the data points.
!
