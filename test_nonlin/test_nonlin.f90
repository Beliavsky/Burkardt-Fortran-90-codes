subroutine p00_dif ( problem, n, x, fjac )
...
!
!    real ( kind = rk ) X(N), the evaluation point.
!
!  Output:
!
!    real ( kind = rk ) FJAC(N,N), the approximante jacobian matrix.
!
  implicit none
...
subroutine p00_fx ( problem, n, x, f )
...
!  Output:
!
!    real ( kind = rk ) F(N), the function value.
!
  implicit none
...
subroutine p00_jac ( problem, n, x, fjac )
...
!  Output:
!
!    real ( kind = rk ) FJAC(N,N), the jacobian matrix.
!
  implicit none
  ...
  if ( problem == 1 ) then
    call p01_jac ( n, x, fjac )
  else if ( problem == 2 ) then
    call p02_jac ( n, x, fjac )
  else if ( problem  ==  3 ) then
    call p03_jac ( n, x, fjac )
  else if ( problem == 4 ) then
    call p04_jac ( n, x, fjac )
  else if ( problem == 5 ) then
    call p05_jac ( n, x, fjac )
  else if ( problem == 6 ) then
    call p06_jac ( n, x, fjac )
  else if ( problem == 7 ) then
    call p07_jac ( n, x, fjac )
  else if ( problem == 8 ) then
    call p08_jac ( n, x, fjac )
  else if ( problem == 9 ) then
    call p09_jac ( n, x, fjac )
  else if ( problem == 10 ) then
    call p10_jac ( n, x, fjac )
  else if ( problem == 11 ) then
    call p11_jac ( n, x, fjac )
  else if ( problem == 12 ) then
    call p12_jac ( n, x, fjac )
  else if ( problem == 13 ) then
    call p13_jac ( n, x, fjac )
  else if ( problem == 14 ) then
    call p14_jac ( n, x, fjac )
  else if ( problem == 15 ) then
    call p15_jac ( n, x, fjac )
  else if ( problem == 16 ) then
    call p16_jac ( n, x, fjac )
  else if ( problem == 17 ) then
    call p17_jac ( n, x, fjac )
  else if ( problem == 18 ) then
    call p18_jac ( n, x, fjac )
  else if ( problem == 19 ) then
    call p19_jac ( n, x, fjac )
  else if ( problem == 20 ) then
    call p20_jac ( n, x, fjac )
  else if ( problem == 21 ) then
    call p21_jac ( n, x, fjac )
  else if ( problem == 22 ) then
    call p22_jac ( n, x, fjac )
  else if ( problem == 23 ) then
    call p23_jac ( n, x, fjac )
  else
    write ( *, '(a)' ) ' '
...
subroutine p00_sol ( problem, n, iknow, x )
...
!
!    real ( kind = rk ) X(N), a solution value, if IKNOW is positive.
!
  implicit none
...
!
  if ( problem == 1 ) then
    call p01_sol ( n, iknow, x )
  else if ( problem == 2 ) then
    call p02_sol ( n, iknow, x )
  else if ( problem == 3 ) then
    call p03_sol ( n, iknow, x )
  else if ( problem == 4 ) then
    call p04_sol ( n, iknow, x )
  else if ( problem == 5 ) then
    call p05_sol ( n, iknow, x )
  else if ( problem == 6 ) then
    call p06_sol ( n, iknow, x )
  else if ( problem == 7 ) then
    call p07_sol ( n, iknow, x )
  else if ( problem == 8 ) then
    call p08_sol ( n, iknow, x )
  else if ( problem == 9 ) then
    call p09_sol ( n, iknow, x )
  else if ( problem == 10 ) then
    call p10_sol ( n, iknow, x )
  else if ( problem == 11 ) then
    call p11_sol ( n, iknow, x )
  else if ( problem == 12 ) then
    call p12_sol ( n, iknow, x )
  else if ( problem == 13 ) then
    call p13_sol ( n, iknow, x )
  else if ( problem == 14 ) then
    call p14_sol ( n, iknow, x )
  else if ( problem == 15 ) then
    call p15_sol ( n, iknow, x )
  else if ( problem == 16 ) then
    call p16_sol ( n, iknow, x )
  else if ( problem == 17 ) then
    call p17_sol ( n, iknow, x )
  else if ( problem == 18 ) then
    call p18_sol ( n, iknow, x )
  else if ( problem == 19 ) then
    call p19_sol ( n, iknow, x )
  else if ( problem == 20 ) then
    call p20_sol ( n, iknow, x )
  else if ( problem == 21 ) then
    call p21_sol ( n, iknow, x )
  else if ( problem == 22 ) then
    call p22_sol ( n, iknow, x )
  else if ( problem == 23 ) then
    call p23_sol ( n, iknow, x )
  else
    write ( *, '(a)' ) ' '
...
!  Output:
!
!    real ( kind = rk ) X(N), the starting point.
!
  implicit none
...
!
  ...
  do i = 1, 29
    ...
    ti = real ( i, kind = rk ) / 29.0D+00

    sum1 = 0.0D+00
    temp = 1.0D+00
    do j = 2, n
      sum1 = sum1 + real ( j - 1, kind = rk ) * temp * x(j)
      temp = ti * temp
    end do
    ...
    do k = 1, n
      f(k) = f(k) + temp * ( sum1 - sum2 * sum2 - 1.0D+00 ) &
        * ( real ( k - 1, kind = rk ) - 2.0D+00 * ti * sum2 )
      temp = ti * temp
    end do
...
!
  ...
  do i = 1, 29
    ...
    do k = 1, n
      ...
      do j = k, n
        fjac(k,j) = fjac(k,j) &
          + tj * ( ( real ( k - 1, kind = rk ) / ti - 2.0D+00 * sum2 ) &
          * ( real ( j - 1, kind = rk ) / ti - 2.0D+00 * sum2 ) - temp1 )
        tj = ti * tj
      end do
      tk = ti**2 * tk
    end do
...
!
  ...
  f(1:n) = f(1:n) / real ( n, kind = rk )

  do i = 1, n

    if ( mod ( i, 2 ) == 0 ) then
      f(i) = f(i) + 1.0D+00 / real ( i * i - 1, kind = rk )
    end if
...
!
  ...
  fjac(1:n,1:n) = fjac(1:n,1:n) / real ( n, kind = rk )
...
!
  ...
  do i = 1, n
    x(i) = real ( 2 * i - 1 - n, kind = rk ) / real ( n + 1, kind = rk )
  end do
...
!
  ...
  f(1:n-1) = x(1:n-1) + sum ( x(1:n) ) - real ( n + 1, kind = rk )

  f(n) = product ( x(1:n) ) - 1.0D+00

  return
end
subroutine p08_jac ( n, x, fjac )
...
!
  ...
  do k = 1, n
    ...
    f(k) = 2.0D+00 * x(k) + 0.5D+00 * h * h &
      * ( x(k) + real ( k, kind = rk ) * h + 1.0D+00 )**3
...
!
  ...
  do i = 1, n
    ...
    fjac(i,i) = 2.0D+00 + 1.5D+00 * ( x(i) + 1.0D+00 + real ( i, kind = rk ) &
      / real ( n + 1, kind = rk )  )**2 / real ( n + 1, kind = rk )**2
...
!
  ...
  do i = 1, n
    x(i) = real ( i * ( i - n - 1 ), kind = rk ) / real ( n + 1, kind = rk )**2
  end do
...
!
  ...
  do k = 1, n
    ...
    tk = real ( k, kind = rk ) / real ( n + 1, kind = rk )
...
!
  ...
  do i = 1, n
    ...
    ti = real ( i, kind = rk ) / real ( n + 1, kind = rk )

    do j = 1, n
      tj = real ( j, kind = rk ) / real ( n + 1, kind = rk )
      temp1 = ( x(j) + tj + 1.0D+00 )**2
      temp2 = min ( ti, tj ) - ti * tj
      fjac(i,j) = 1.5D+00 * temp2 * temp1 / real ( n + 1, kind = rk )
    end do
...
!
  ...
  do k = 1, n
    f(k) =  real ( n, kind = rk ) &
      - c_sum + real ( k, kind = rk ) * ( 1.0D+00 - cos ( x(k) ) ) &
      - sin ( x(k) )
  end do

  return
end
subroutine p11_jac ( n, x, fjac )
  ...
  do i = 1, n
    ...
    do j = 1, n
      ...
      if ( i /= j ) then
        ...
        fjac(i,j) = real ( j + 1, kind = rk ) * sin ( x(j) ) - cos ( x(j) )
      end if
...
!
  ...
  sum1 = 0.0D+00
  do j = 1, n
    sum1 = sum1 + real ( j, kind = rk ) * ( x(j) - 1.0D+00 )
  end do

  do k = 1, n
    f(k) = x(k) - 1.0D+00 + real ( k, kind = rk ) * sum1 &
      * ( 1.0D+00 + 2.0D+00 * sum1 * sum1 )
  end do

  return
end
subroutine p12_jac ( n, x, fjac )
...
!
  ...
  sum1 = 0.0D+00
  do i = 1, n
    sum1 = sum1 + real ( i, kind = rk ) * ( x(i) - 1.0D+00 )
  end do

  do i = 1, n
    do j = 1, n

      fjac(i,j) = real ( i * j, kind = rk ) * ( 1.0D+00 + 6.0D+00 * sum1**2 )
...
!
  ...
  do i = 1, n
    x(i) = 1.0D+00 - real ( i, kind = rk ) / real ( n, kind = rk )
  end do
...
!  Local:
!
!    real ( kind = rk ) C, the constant, which must be between 0 and 1.
!
  implicit none
  ...
  real ( kind = rk ), parameter :: c = 0.9D+00
  real ( kind = rk ) f(n)
  integer i
  real ( kind = rk ) mu(n)
  real ( kind = rk ) term
  ...
  do i = 1, n
    mu(i) = real ( 2 * i - 1, kind = rk ) / real ( 2 * n, kind = rk )
  end do
...
!  Discussion:
!
!    Errors in this calculation were corrected on 17 October 2020.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
  ...
  do i = 1, n
    ...
    do j = 1, n
      ...
      termp = c * mu(i) / ( mu(i) + mu(j) ) / real ( 2 * n, kind = rk )
...
!
!    real ( kind = rk ) A(N), the vector whose 2-norm is desired.
!
!  Output:
!
!    real ( kind = rk ) R8VEC_NORM2, the 2-norm of A.
!
  implicit none
...
!    N must be positive.
!
!    real ( kind = rk ) A_LU(N,N), the LU factors from R8GE_FA.
!
