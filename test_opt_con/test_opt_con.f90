...
!
!    Output, real ( kind = rk ) A(M), B(M), lower and upper bounds.
!
  implicit none
...
!  Parameters:
!
!    Output, integer PROBLEM_NUM, the number of defined problems.
!
  implicit none
...
!
!    Output, real ( kind = rk ) X(M), the solution.
!
  implicit none
...
!
  ...
  real ( kind = rk ), dimension ( 2, 5 ) :: a = reshape ( (/ &
    3.0D+00, 5.0D+00, &
    5.0D+00, 2.0D+00, &
    2.0D+00, 1.0D+00, &
    1.0D+00, 4.0D+00, &
...
!
  ...
  real ( kind = rk ), dimension ( 10, 30 ) :: a = reshape ( (/ &
