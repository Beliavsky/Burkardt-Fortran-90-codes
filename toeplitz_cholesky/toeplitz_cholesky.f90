...
subroutine t_cholesky_upper ( n, t, r )
...
!    Input, real ( kind = rk ) T(N), the first row.
!
!    Output, real ( kind = rk ) R(N,N), the upper Cholesky factor.
!
  implicit none
...
!    Input, integer N, the order of the matrix.
!
!    Input, real ( kind = rk ) G(2,N), the compressed Toeplitz matrix.
...
!    Input, integer N, the order of the matrix.
!
!    Input, real ( kind = rk ) A(N,N), the Toeplitz matrix.
!
!    Output, real ( kind = rk ) L(N,N), the lower Cholesky factor.
!
  implicit none
