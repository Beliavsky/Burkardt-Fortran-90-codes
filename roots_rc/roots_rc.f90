...
!
!    Input, real ( kind = rk ) X(N).  Before the first call, the user should
...
!
!    Input, real ( kind = rk ) FX(N), the value of the function at XNEW.
!
!    Output, real ( kind = rk ) FERR, the function error, that is, the sum of
!    the absolute values of the most recently computed function vector.
!
!    Output, real ( kind = rk ) XNEW(N), a new point at which a function 
!    value is requested.
!
!    Workspace, real ( kind = rk ) Q(2*N+2,N+2).  Before the first call 
...
!
  if ( q(2*n+1,1) == 0.0D+00 ) then
    ...
    jsus = 1
    do i = 2, n + 1
      if ( real ( 2 * n, kind = rk ) <= q(2*n+2,i) ) then
        q(2*n+1,i) = r8_huge ( )
      end if
