...
!
!    real ( kind = rk ) x, the argument of the objective function.
!
!  Output:
!
!    real ( kind = rk ) f, the value of the objective function.
!
  implicit none
...
!    integer problem, the problem number.
!
!    real ( kind = rk ) x, the value of the variable.
!
!  Output:
!
!    real ( kind = rk ) f1, the first derivative of the 
!    objective function.
!
  implicit none
...
!    integer problem, the problem number.
!
!    real ( kind = rk ) x, the point where the gradient is to 
!    be approximated.
!
!  Output:
!
!    real ( kind = rk ) F1_DIF, the approximated gradient vector.
!
  implicit none
...
!  Input:
!
!    real ( kind = rk ) x, the values of the variables.
!
!  Output:
!
!    real ( kind = rk ) f2, the second derivative.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) f2_dif, the approximate second derivative.
!
  implicit none
...
!
!    Output, real ( kind = rk ) p00_FMIN, the abcissa approximating the 
!    minimizer of f.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) a, b, two points, between which a local
...
!  Output:
!
!    real ( kind = rk ) p00_sol, the solution.
!
  implicit none
  ...
  real ( kind = rk ) p01_sol
  real ( kind = rk ) p02_sol
  real ( kind = rk ) p03_sol
  real ( kind = rk ) p04_sol
  real ( kind = rk ) p05_sol
  real ( kind = rk ) p06_sol
  real ( kind = rk ) p07_sol
  real ( kind = rk ) p08_sol
  real ( kind = rk ) p09_sol
  real ( kind = rk ) p10_sol
  real ( kind = rk ) p11_sol
  real ( kind = rk ) p12_sol
  real ( kind = rk ) p13_sol
  real ( kind = rk ) p14_sol
  real ( kind = rk ) x
...
!  Output:
!
!    real ( kind = rk ) x, a starting point for the optimization.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) x, the argument of the objective function.
!
...
!
!    Output, real ( kind = rk ) F1, the first derivative of the
!    objective function.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) x, the values of the variables.
!
!    Output, real ( kind = rk ) F2, the second derivative.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) A, B, two points defining an interval in which
...
!  Output:
!
!    real ( kind = rk ) p01_sol, the solution.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) x, a starting point for the optimization.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p02_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p03_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p04_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p05_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p06_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p07_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p08_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p09_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p10_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p11_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) a, b, two points defining an interval in which
!    the local minimizer should be sought.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p12_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p13_sol, the solution.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) p14_sol, the solution.
!
  implicit none
