...
!
!    Input, real ( kind = rk ) X(N), the argument of the objective function.
!
!    Output, real ( kind = rk ) F, the value of the objective function.
!
  implicit none
...
!    Input, integer N, the number of variables.
!
!    Input, real ( kind = rk ) X(N), the values of the variables.
!
!    Output, real ( kind = rk ) G(N), the gradient of the objective function.
!
  implicit none
...
!    Input, integer N, the number of variables.
!
!    Input, real ( kind = rk ) X(N), the point where the gradient 
!    is to be approximated.
!
!    Output, real ( kind = rk ) GDIF(N), the approximated gradient vector.
!
  implicit none
...
!    Input, real ( kind = rk ) X(N), the values of the variables.
!
!    Output, real ( kind = rk ) H(N,N), the Hessian matrix.
!
  implicit none
...
!    Input, real ( kind = rk ) X(N), the values of the variables.
!
!    Output, real ( kind = rk ) HDIF(N,N), the approximated N by N 
!    Hessian matrix.
!
  implicit none
...
!
!    Output, real ( kind = rk ) X(N), the solution, if known.
!
  implicit none
...
!
!    Output, real ( kind = rk ) X(N), a starting point for the optimization.
!
  implicit none
...
!    Input, real ( kind = rk ) X(N), the values of the variables.
!
!    Output, real ( kind = rk ) H(N,N), the N by N Hessian matrix.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X(2), the values of the variables.
!
!    Output, real ( kind = rk ) TH, the value of the term.
!
  implicit none
...
!
  ...
  real ( kind = rk ) dfdx5
  real ( kind = rk ) dfdx6
  real ( kind = rk ) d2fdx11
  real ( kind = rk ) d2fdx13
  real ( kind = rk ) d2fdx22
  real ( kind = rk ) d2fdx24
  real ( kind = rk ) d2fdx31
  real ( kind = rk ) d2fdx42
  real ( kind = rk ) d2fdx55
  real ( kind = rk ) d2fdx56
  real ( kind = rk ) d2fdx65
  real ( kind = rk ) fi
  real ( kind = rk ) h(n,n)
  integer i
  real ( kind = rk ) x(n)
...
!
  ...
  do i = 1, 15
    ...
    t = x(1) * exp ( - 0.5D+00 * x(2) * &
      ( 3.5D+00 - 0.5D+00 * real ( i - 1, kind = rk ) - x(3) )**2 ) - y(i)
...
!  Parameters:
!
!    Output, real ( kind = rk ) Y(15), data values needed for the 
!    objective function.
!
  implicit none
...
!
  ...
  real ( kind = rk ) d2f1dx12
  real ( kind = rk ) d2f1dx21
  real ( kind = rk ) d2f2dx11
  real ( kind = rk ) d2f2dx22
  real ( kind = rk ) df1dx1
  real ( kind = rk ) df1dx2
  real ( kind = rk ) df2dx1
  real ( kind = rk ) df2dx2
...
!
  ...
  real ( kind = rk ) c
  real ( kind = rk ) d2fidx11
  real ( kind = rk ) d2fidx22
  real ( kind = rk ) dfidx1
  real ( kind = rk ) dfidx2
  real ( kind = rk ) dfidx3
  real ( kind = rk ) fi
...
!
  ...
  f1 = 0.0D+00
  do i = 1, n
    f1 = f1 + real ( i, kind = rk ) * ( x(i) - 1.0D+00 )
  end do
...
!
  ...
  do i = 1, n
    x(i) = real ( n - i, kind = rk ) / real ( n, kind = rk )
  end do
...
!
  ...
  do i = 1, 29
    ...
    d1 = real ( i, kind = rk ) / 29.0D+00
    s1 = 0.0D+00
    d2 = 1.0D+00
    do j = 2, n
      s1 = s1 + real ( j - 1, kind = rk ) * d2 * x(j)
      d2 = d2 * real ( i, kind = rk ) / 29.0D+00
    end do
    ...
    t = s1 - s2 * s2 - 1.0D+00
    s3 = 2.0D+00 * s2 * real ( i, kind = rk ) / 29.0D+00
    d2 = 2.0D+00 / d1

    do j = 1, n
      g(j) = g(j) + d2 * ( real ( j - 1, kind = rk ) - s3 ) * t
      d2 = d2 * real ( i, kind = rk ) / 29.0D+00
    end do
...
!
  ...
  do i = 1, 29
    ...
    d2 = 1.0D+00 / d1
    do j = 1, n
      t1 = real ( j - 1, kind = rk ) - s3
      h(j,j) = h(j,j) + 2.0D+00 * ( t1 * t1 - t ) * d2 * d2
      d3 = 1.0D+00 / d1
      do k = 1, j-1
        h(j,k) = h(j,k) &
          + 2.0D+00 * ( t1 * ( real ( k - 1, kind = rk ) - s3 ) - t ) * d2 * d3
        d3 = d1 * d3
      end do
      d2 = d1 * d2
    end do
...
!
  ...
  real ( kind = rk ), parameter :: ap = 0.00001D+00
  real ( kind = rk ) f
  real ( kind = rk ) t1
  real ( kind = rk ) t2
  real ( kind = rk ) x(n)
...
!
  ...
  s2 = 0.0D+00
  do j = 1, n
    t1 = t1 + real ( n - j + 1, kind = rk ) * x(j)**2
    s1 = exp ( x(j) / 10.0D+00 )
    if ( 1 < j ) then
...
!
  ...
  t1 = - 1.0D+00
  do j = 1, n
    t1 = t1 + real ( n - j + 1, kind = rk ) * x(j) * x(j)
  end do
  ...
  do j = 1, n
    ...
    h(j,j) = 8.0D+00 * ( real ( n - j + 1, kind = rk ) * x(j) )**2 &
      + real ( n - j + 1, kind = rk ) * th
    ...
    if ( 1 < j ) then
      ...
      do k = 1, j-1
        t1 = exp ( real ( k, kind = rk ) / 10.0D+00 )
        h(j,k) = 8.0D+00 * real ( n - j + 1, kind = rk ) &
          * real ( n - k + 1, kind = rk ) * x(j) * x(k)
      end do
...
!
  ...
  f = 0.0D+00
  do j = 1, n
    t = real ( n + j, kind = rk ) - sin ( x(j) ) &
      - s1 - real ( j, kind = rk ) * cos ( x(j) )
    f = f + t * t
  end do
...
!
  ...
  do j = 1, n
    t = real ( n + j, kind = rk ) - sin ( x(j) ) &
      - s1 - real ( j, kind = rk ) * cos ( x(j) )
    s2 = s2 + t
    g(j) = ( real ( j, kind = rk ) * sin ( x(j) ) - cos ( x(j) ) ) * t
  end do
...
!
  ...
  do j = 1, n
    ...
    th = cos ( x(j) )
    t = real ( n + j, kind = rk ) - h(j,j) - s1 - real ( j, kind = rk ) * th
    s2 = s2 + t
    do k = 1, j-1
      h(j,k) = 2.0D+00 * ( sin ( x(k) ) &
        * ( real ( n + j + k, kind = rk ) * h(j,j) - th ) - &
        h(j,j) * cos ( x(k) ) )
    end do

    h(j,j) = real ( j * ( j + 2 ) + n, kind = rk ) * h(j,j) * h(j,j) + th * &
      ( th - real ( 2 * j + 2, kind = rk ) * h(j,j) ) &
      + t * ( real ( j, kind = rk ) * th + h(j,j) )
...
!
  ...
  real ( kind = rk ) df1dxj
  real ( kind = rk ) df1dxjp1
  real ( kind = rk ) df2dxjp2
  real ( kind = rk ) df2dxjp3
  real ( kind = rk ) df3dxjp1
  real ( kind = rk ) df3dxjp2
  real ( kind = rk ) df4dxj
  real ( kind = rk ) df4dxjp3
  real ( kind = rk ) f1
  real ( kind = rk ) f2
  real ( kind = rk ) f3
  real ( kind = rk ) f4
...
!
  ...
  real ( kind = rk ) d2f1dx12
  real ( kind = rk ) d2f1dx21
  real ( kind = rk ) d2f2dx12
  real ( kind = rk ) d2f2dx21
  real ( kind = rk ) d2f2dx22
  real ( kind = rk ) d2f3dx12
  real ( kind = rk ) d2f3dx21
  real ( kind = rk ) d2f3dx22
  real ( kind = rk ) df1dx1
  real ( kind = rk ) df1dx2
  real ( kind = rk ) df2dx1
  real ( kind = rk ) df2dx2
...
!
  ...
  real ( kind = rk ) d2f1dx11
  real ( kind = rk ) d2f3dx33
  real ( kind = rk ) df1dx1
  real ( kind = rk ) df1dx2
  real ( kind = rk ) df2dx1
  real ( kind = rk ) df3dx3
...
!    Input, real ( kind = rk ) X(N), the argument of the objective function.
!
!    Output, real ( kind = rk ) FVEC(N), an auxilliary vector.
!
  implicit none
  ...
  do i = 1, n
    ...
    if ( mod ( i, 2 ) == 0 ) then
      fvec(i) = fvec(i) + 1.0D+00 / ( real ( i, kind = rk )**2 - 1.0D+00 )
    end if
  end do
...
!
  ...
  g(1:n) = 2.0D+00 * g(1:n) / real ( n, kind = rk )
...
!
  ...
  do i = 1, n
    do j = 1, n
      f = f + x(i) * x(j) / real ( i + j - 1, kind = rk )
    end do
  end do
...
!
  ...
  do i = 1, n
    ...
    g(i) = 0.0D+00
    do j = 1, n
      g(i) = g(i) + 2.0D+00 * x(j) / real ( i + j - 1, kind = rk )
    end do
...
!
  ...
  do i = 1, n
    do j = 1, n
      h(i,j) = 2.0D+00 / real ( i + j - 1, kind = rk )
    end do
  end do
...
!
  ...
  real ( kind = rk ) x(n)
  real ( kind = rk ), parameter :: xhi = 5.12D+00
  real ( kind = rk ), parameter :: xlo = -5.12D+00
...
!
  ...
  real ( kind = rk ) x(n)
  real ( kind = rk ), parameter :: xhi =  2.048D+00
  real ( kind = rk ), parameter :: xlo = -2.048D+00
...
!
  ...
  do i = 1, n
    g(i) = real ( i, kind = rk ) * 4.0D+00 * x(i)**3
  end do
...
!
  ...
  do i = 1, n
    h(i,i) = real ( i, kind = rk ) * 12.0D+00 * x(i)**2
  end do
...
!  Parameters:
!
!    Output, real ( kind = rk ) P, the value of the parameter that multiplies the
!    Gaussian noise.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) P, the value of the parameter that multiplies the 
!    Gaussian noise.
!
  implicit none
...
!
!    Input/output, real ( kind = rk ) P, the value of the parameter that
...
!
  ...
  real ( kind = rk ) p
  real ( kind = rk ), save :: p_save = 1.0D+00
...
!
  ...
  real ( kind = rk ) x(n)
  real ( kind = rk ), parameter :: xhi = 1.28D+00
  real ( kind = rk ), parameter :: xlo = -1.28D+00
...
!
  ...
  integer j1
  integer j2
  integer, parameter :: jroot = 5
  integer, parameter :: k = 500
  real ( kind = rk ) x(n)
  ...
  do j = 1, jroot * jroot
    ...
    fj = real ( j, kind = rk ) + ( x(1) - real ( a1, kind = rk ) )**6 &
      + ( x(2) - real ( a2, kind = rk ) )**6
...
!
  ...
  do j = 1, jroot * jroot
    ...
    dfidx1 = dfidx1 - ( 1.0D+00 / fj**2 ) * 6.0D+00 &
      * ( x(1) - real ( a1, kind = rk ) )**5
    dfidx2 = dfidx2 - ( 1.0D+00 / fj**2 ) * 6.0D+00 &
      * ( x(2) - real ( a2, kind = rk ) )**5
...
!
  ...
  real ( kind = rk ) b
  real ( kind = rk ) c
  real ( kind = rk ), parameter :: d = 6.0D+00
  real ( kind = rk ), parameter :: e = 10.0D+00
  real ( kind = rk ) f
  real ( kind = rk ) ff
  real ( kind = rk ), parameter :: pi = 3.141592653589793D+00
  real ( kind = rk ) x(n)
...
!
  ...
  real ( kind = rk ), parameter, dimension ( 4, m ) :: a = reshape ( &
  ...
  real ( kind = rk ), save, dimension ( m ) :: c = &
...
!
  ...
  real ( kind = rk ) df1dx1
  real ( kind = rk ) df1dx11
  real ( kind = rk ) df2dx2
  real ( kind = rk ) df2dx22
  real ( kind = rk ) factor1
  real ( kind = rk ) factor2
  real ( kind = rk ) h(n,n)
  integer i
...
!  Parameters:
!
!    Output, real ( kind = rk ) B, M1, M2, R11, R12, R21, R22, 
!    problem parameters.
!
...
!
  ...
  m1  = r8_uniform_ab ( 0.0D+00, 100.0D+00 )
  m2  = r8_uniform_ab ( 0.0D+00, 100.0D+00 )
  b   = r8_uniform_ab ( 0.0D+00, 10.0D+00 )
  r11 = r8_uniform_ab ( 0.0D+00, b )
  r12 = r8_uniform_ab ( b,       10.0D+00 )
  r21 = r8_uniform_ab ( 0.0D+00, 10.0D+00 )
  r22 = r8_uniform_ab ( 0.0D+00, 10.0D+00 )
...
!  Parameters:
!
!    Input, real ( kind = rk ) B, M1, M2, R11, R12, R21, R22, problem parameters.
!
...
!
!    Input/output, real ( kind = rk ) B, M1, M2, R11, R12, R21, R22, 
!    problem parameters.
!
!    Input/output, integer SEED, a seed for the random 
!    number generator.
!
  ...
  character ( len = * ) action
  real ( kind = rk ) b
  real ( kind = rk ), save :: b_save = 0.0D+00
  real ( kind = rk ) m1
  real ( kind = rk ), save :: m1_save = 0.0D+00
  real ( kind = rk ) m2
  real ( kind = rk ), save :: m2_save = 0.0D+00
  real ( kind = rk ) r11
  real ( kind = rk ), save :: r11_save = 0.0D+00
  real ( kind = rk ) r12
  real ( kind = rk ), save :: r12_save = 0.0D+00
  real ( kind = rk ) r21
  real ( kind = rk ), save :: r21_save = 0.0D+00
  real ( kind = rk ) r22
  real ( kind = rk ), save :: r22_save = 0.0D+00
  real ( kind = rk ) r8_uniform_ab
  integer seed
  integer :: seed_save = 0.0
...
!
  ...
  real ( kind = rk ) arg
  real ( kind = rk ) dargdx1
  real ( kind = rk ) dargdx2
  real ( kind = rk ) dfdx1
  real ( kind = rk ) dfdx2
  real ( kind = rk ) factor
  real ( kind = rk ) h(n,n)
...
!
!    Output, real ( kind = rk ) VALUE, the truncated version of X.
!
  implicit none
