...
!
  ...
  integer n_llv
  integer n_mc
  real ( kind = rk ) result_llc
  real ( kind = rk ) result_llm
  real ( kind = rk ) result_llv
  real ( kind = rk ) result_mc
  ...
  do i = 0, 17
    ...
    do h_test = 1, 3
      ...
      call sphere01_quad_mc ( polyterm_value_3d, h, n_mc, result_mc )
...
!
!    Input, real ( kind = rk ) X(3,N), the points where the polynomial term 
!    is to be evaluated.
!
!    Output, real ( kind = rk ) F(N), the value of the polynomial term.
!
  implicit none
