...
!
  ...
  integer ihi
  integer ilo
  integer, parameter :: i_inc = 5
  integer, parameter :: j_inc = 4
  integer jhi
  integer jlo
  integer m
  integer n
  ...
  integer output_unit
  real ( kind = rk8 ), allocatable, dimension ( : ) :: table_hi
  real ( kind = rk8 ), allocatable, dimension ( : ) :: table_lo
  real ( kind = rk8 ), allocatable, dimension ( :, : ) :: table
...
!
!    Input, real ( kind = rk8 ) TABLE(M,N), the point coordinates.
!
!    Input, integer ILO, JLO, IHI, JHI, the minimum row and column,
!    and the maximum row and column to plot.
!
!    Input, integer I_INC, J_INC, the number of rows and columns
!    of subplots that can appear on one page.
!
!  Local:
!
!    integer MARKER_SIZE, controls the size of the dots used
...
!
  ...
  integer jlo
  integer k
  integer :: marker_size = 2
  real ( kind = rk8 ), dimension ( 2 ) :: plot_max = (/ 8.25D+00, 10.50D+00 /)
  real ( kind = rk8 ), dimension ( 2 ) :: plot_min = (/ 0.25D+00,  0.50D+00 /)
...
!
  ...
  do j = jlo, jhi
    ...
    xlo = ( real ( j_inc - j + jlo, kind = rk8 ) * ( plot_min(1) ) &
          + real (         j - jlo, kind = rk8 ) * ( plot_max(1) ) ) &
          / real ( j_inc,           kind = rk8 )

    xhi = ( real ( j_inc - j + jlo - 1, kind = rk8 ) * ( plot_min(1) ) &
          + real (         j - jlo + 1, kind = rk8 ) * ( plot_max(1) ) ) &
          / real ( j_inc,               kind = rk8 )
  ...
  do i = ilo, ihi
    ...
    yhi = ( real ( i_inc - i + ilo, kind = rk8 ) * ( plot_max(2) - 0.25D+00 ) &
          + real (         i - ilo, kind = rk8 ) * ( plot_min(2) ) ) &
          / real ( i_inc,           kind = rk8 )

    ylo = ( real ( i_inc - i + ilo - 1, kind = rk8 ) &
        * ( plot_max(2) - 0.25D+00 ) &
          + real (         i - ilo + 1, kind = rk8 ) * ( plot_min(2) ) ) &
          / real ( i_inc,               kind = rk8 )
    ...
    do j = jlo, jhi
      ...
      xlo = ( real ( j_inc - j + jlo, kind = rk8 ) &
            * ( plot_min(1) + 0.25D+00 ) &
            + real (         j - jlo, kind = rk8 ) * ( plot_max(1) ) ) &
            / real ( j_inc,           kind = rk8 )

      xhi = ( real ( j_inc - j + jlo - 1, kind = rk8 ) &
            * ( plot_min(1) + 0.25D+00 ) &
            + real (         j - jlo + 1, kind = rk8 ) * ( plot_max(1) ) ) &
            / real ( j_inc,               kind = rk8 )
!
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X1, Y1, X2, Y2, the two corners of the box.
!
!    Input, real ( kind = rk8 ) X, Y, the point to be checked.
!
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) FONT_SIZE, the font size, in inches.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, Y, the X and Y components of the new point.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, Y, the coordinates of the point to mark.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) X, Y, the X and Y components of the current point.
!
  implicit none
