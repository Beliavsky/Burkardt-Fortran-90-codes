...
!*****************************************************************************80
!
!! hermite_product_polynomial_test() tests hermite_product_polynomial().
!
!  Licensing:
!
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'HERMITE_PRODUCT_POLYNOMIAL_TEST:'
...
!
  ...
  do test = 1, 5
    call comp_random_grlex ( k, rank1, rank2, x, rank )
    x_sum = sum ( x(1:k) )
    write ( *, '(2x,i4,2x,i4,a)', advance = 'no' ) rank, x_sum, ':'
    do i = 1, k
      write ( *, '(i4)', advance = 'no' ) x(i)
...
!
  ...
  xlo = -1.0D+00
  xhi = +1.0D+00
  call r8vec_uniform_ab ( m, xlo, xhi, x )
