...
!
  ...
  do
    ...
    alpha_frac = alpha - real ( int ( alpha ), kind = rk )
...
!
  ...
  mean = mean / real ( sample_num, kind = rk )

  variance = 0.0D+00
  do i = 1, sample_num
    variance = variance + ( x(i) - mean ) ** 2
  end do
  variance = variance / real ( sample_num - 1, kind = rk )
