...
!
!    Input, integer M, the number of edges.
!
!    Input/output, integer LEVEL, ?
!
!    Input/output, logical MIDDLE, ?
!
!    Output, logical FAIL, is TRUE if the algorithm failed.
!
!    Input/output, integer PARM1(M), ?
!
!    Input/output, integer PAINT(M+2-N), ?
!
!    Input/output, integer ARCSEC(7*M+2-5*N), ?
!
!    Input, integer ARCTOP(7*M+2-5*N), ?
!
...
!
  ...
  integer arcsec(7*m+2-5*n)
  integer arctop(7*m+2-5*n)
...
!
  ...
  integer arctop(7*m+2-5*n)
  logical arctyp(7*m+2-5*n)
  integer auxpf1(m+m+2)
  integer auxpf2(m+m+2)
  integer auxpf3(m+m+m+3)
  integer auxpf4(m+m+m+3)
  integer descp(n)
  integer finish(m+2-n)
  integer first(n+m+m)
  logical ind
  integer initp
  integer level
...
!    Input, integer NEDGE, the number of edges.
!
!    Input, integer I1, J1, the nodes defining the ends of the
!    desired path.
!
!    Input, integer FWDARC(NEDGE); FWDARC(I) is the ending node
!    of the I-th edge in the forward star representation of the digraph.
!
!    Input, integer ARCFIR(NNODE+1); ARCFIR(I) is the number of
...
!    Input, integer NEDGE, the number of edges.
!
!    Input, integer K, the number of the path to be generated.
!
!    Input, integer KPATHS, the number of shortest paths that were
!    generated.
!
!    Input, integer MAXQUE, the size of the auxilliary storage.
!
!    Input, integer INODE(NEDGE), JNODE(NEDGE); the I-th edge
!    is directed from INODE(I) to JNODE(I).
!
!    Input, integer PTHLEN(KPATHS+3); the shortest path lengths
!    are stored in PTHLEN(2), PTHLEN(3), ..., PTHLEN(KPATHS+1).
!
!    Input, integer ARCDIR(NNODE); array of shortest path tree, 
!    as computed by KSHOT2.
!
!    Input, integer QUFIRP(KPATHS+3); description of the first 
!    path in a queue, as computed by KSHOT2.
!
!    Input, integer QUNXTP(KPATHS+3); next entry in a queue, 
!    as computed by KSHOT2.
!
!    Input, integer CROSAR(MAXQUE); cross edge of the path, as 
!    computed by KSHOT2.
!
!    Input, integer NEXTRD(MAXQUE); next entry of the pool storage,
!    as computed by KSHOT2.
!
!    Output, integer NUMP, the number of edges in the path.
!
!    Output, integer LENGTH, the length of the path.
!
!    Output, integer ARCNOD(NNODE); the edges of the K-th shortest
...
!
  ...
  integer arcdir(nnode)
  integer arcnod(nnode)
  integer crosar(maxque)
  integer index1
  integer index2
  integer index3
  integer inode(nedge)
  ...
  integer k
  integer length
  integer nextrd(maxque)
  integer number
  integer nump
  integer pathlen(kpaths+3)
  integer qufirp(kpaths+3)
  integer qunxtp(kpaths+3)
...
!    and FALSE otherwise.
!
!    Output, integer HCYCLE(NNODE); the nodes of the hamiltonian
...
!    Input, integer NEDGE, the number of edges.
!
!    Input, integer K, number of shortest paths desired.
!
!    Input, integer ISORCE, the starting point for the shortest
!    paths.
!
!    Input/output, integer ITER; if ITER is 0 on input, it is
...
!
!    Input, real ( kind = rk ) ARCLEN(NEDGE); ARCLEN(I) is the length of the
!    I-th edge.
!
!    Output, real ( kind = rk ) DIST(NNODE,K); DIST(I,J) is the length of the
...
!
  ...
  real ( kind = rk ) arclen(nedge)
  logical best
  real ( kind = rk ) dist(nnode,k)
  real ( kind = rk ) dist1
  real ( kind = rk ) dist2
  real ( kind = rk ) dist3
  integer i
  ...
  integer ids1
  integer ids2
  integer iedge1(nnode)
  integer iedge2(nnode)
  real ( kind = rk ) ilen1(nedge)
  real ( kind = rk ) ilen2(nedge)
  integer index
  integer index1
  integer index2
  integer init
...
!    is directed from INODE(I) to JNODE(I).
!
!    Input, integer ARCLEN(NEDGE); ARCLEN(I) is the length of
!    the I-th edge.
!
!    Input, integer KPATHS, the requested number of shortest paths.
!    KPATHS must be greater than 0.
!
!    Input, integer MAXQUE, the size of the auxilliary storage.
!    In most cases, MAXQUE = 2 * KPATHS is sufficient.
!
!    Input, integer ISORCE, ISINK; the paths to be found must
!    go from node ISORCE to node ISINK.
!
!    Output, integer IFLAG, completion flag.
...
!
!    Output, integer IPATHS, the actual number of shortest paths
!    found by KSHOT2; IPATHS <= KPATHS.
!
!    Output, integer PTHLEN(KPATHS+3); the shortest path lengths
!    are stored in PTHLEN(2), PTHLEN(3), ..., PTHLEN(KPATHS+1).
!
...
!
  ...
  integer arcbwd(nnode)
  integer arcdir(nnode)
  integer arcfwd(nnode)
  integer arclen(nedge)
  integer arcnod(nnode)
  integer auxdis(nnode)
  integer auxlnk(nnode)
  integer auxstg(nnode)
  integer auxtre(nnode)
  integer crosar(maxque)
  logical finem1
  logical finem2
  logical forwrd
  ...
  integer nqp2
  integer nqsize
  integer nqtab(kpaths+3)
  integer nrdsiz
  integer nxtbwd(nedge)
  integer nxtfwd(nedge)
  integer pathlen(kpaths+3)
  integer qufirp(kpaths+3)
  integer qunxtp(kpaths+3)
  logical rdfull
...
!
  ...
  integer i
  integer i1
  integer iances(nnode)
  integer idescn(nnode)
  integer iedges
  integer ierase
  integer ihigh
  integer inode(nedge)
...
!    Input, integer NNODE, the number of nodes.
!
!    Input, integer NEDGE, the number of edges in the network.
!
!    Input, integer INODE(NEDGE), JNODE(NEDGE); these arrays
...
!      INODE(J) = V, JNODE(I) = U.
!
!    Input, integer CAPAC(NEDGE); since a given edge is represented
...
!
!    Input, integer ISORCE, ISINK, the indices of the source and
!    sink nodes, which must be distinct.
!
!    Output, integer MINCUT(NNODE); MINCUT(I) = 1 if node I is in
!    the minimal cut set; otherwise, it is 0.
!
!    Output, integer FLOW(NEDGE); FLOW(I) is the flow on edge I.
!
!    Output, integer NODE_FLOW(NNODE); NODE_FLOW(I) is the flow
!    through node I.
!
...
!    Input, integer NEDGE, the number of edges.
!
!    Input, integer NPMAX, the maximum number of nodes in a 
!    possible path which allows repeated nodes.
!
!    Input, integer K, the number of shortest paths.
!
!    Input, integer MAXPTH, the maximum number of paths to 
!    be generated.
!
!    Input, integer ISORCE, ISINK; the paths to be generated will 
...
!    is already sorted in nondecreasing order.
!
!    Input, real ( kind = rk ) ARCLEN(NEDGE); ARCLEN(I) is the length of
!    the I-th edge.
!
!    Input, real ( kind = rk ) DIST(NNODE,K), the output from KSHOT1.
!
...
!
  ...
  real ( kind = rk ) large
  real ( kind = rk ) len
  real ( kind = rk ) leninc(nedge)
  real ( kind = rk ) lt
  integer maxpath
  integer nd
  integer nodeu
  integer nodev
  integer nodinc(nedge)
  integer nt
  integer numpath
  integer pathnd(npmax)
  integer point(npmax)
  real ( kind = rk ) plen(npmax)
...
!    Input, integer NEDGE, the number of edges.
!
!    Input, integer IROOT, the base node, from which distances to
!    all other nodes will be calculated.
!
!    Input, integer INODE(NEDGE), JNODE(NEDGE); INODE(I), JNODE(I)
!    are the beginning and ending nodes of edge I.
!
!    Input, real ( kind = rk ) ARCLEN(NEDGE), the length of each edge.
!
!    Output, real ( kind = rk ) DIST(NNODE).  DIST(I) is the length of
!    the shortest path from node IROOT to node I.
!
!    Output, integer ITREE(NNODE-1), JTREE(NNODE-1), the arcs in 
...
!
  ...
  integer nodev
  integer nodey
  integer origin(nedge)
  integer queue(nnode)
  integer tree_dad(nnode)
!
!  Set up the forward star representation of the digraph.
!
  k = 0
...
!
!    Output, integer NUMCOMP, the number of strongly connected 
!    components of the digraph
!
!    Output, integer COMP(NNODE), contains the component of 
!    each node.
!
  implicit none
  ...
  integer jnode(nedge)
  integer k
  integer lowlnk(nnode)
  integer nodei
  integer nodesc(nnode)
  integer nodeu
  integer nodev
  integer number(nnode)
  integer numcomp
  integer stack1(nnode)
  integer stack2(nnode)
!
!  Set up the forward star representation of the digraph.
!
  arcfir(1) = 0
...
!    Input, integer NNODE, the number of nodes.
!
!    Input/output, real ( kind = rk ) DIST(NDIM,NNODE);
!
...
!    to node J.
!
!    Input, integer NDIM, the row dimension of DIST as specified
!    in the calling program.  NDIM must be at least NNODE.
!
!    Output, integer NEXT(NNODE,NNODE); NEXT(I,J) is the 
...
!
  ...
  real ( kind = rk ) d
  real ( kind = rk ) dist(ndim,nnode)
  integer i
  integer j
  integer k
  integer next(nnode,nnode)
...
!  Parameters:
!
!    Input, real ( kind = rk ) DIST(LDA,NNODE), the distance matrix.
!    DIST(I,J) is the distance from node I to node J.
!
!    Input, integer LDA, the leading dimension of DIST, which
...
!    Input, integer NNODE, the number of nodes.
!
!    Input, real ( kind = rk ) DIST(NDIM,NNODE);  DIST(I,J) is the nonnegative
!    length of the edge from node I to node J.  DIST(I,I) should be 0.0
!    for all I.
!
!    Input, integer NDIM, the row dunensdon of matrix DIST exactly
!    as specifled in the dimension statement of the calling program.
!
!    Input, integer IROOT, the node from which all distances will
!    be measured.
!
!    Output, real ( kind = rk ) SHDIST(NNODE).  SHDIST(I) is the length of
...
!
  ...
  integer j
  integer k
  integer locate(nnode)
  integer minj
  real ( kind = rk ) minlen
  integer minv
  integer nodeu
  integer nodev
  real ( kind = rk ) shdist(nnode)
  real ( kind = rk ) temp
!
...
!    Input, integer NNODE, the number of nodes.
!
!    Input, real ( kind = rk ) DIST(NDIM,NNODE); DIST(I,J) is the non-negative
!    length of the edge directed from node I to node J, with DIST(I,I) = 0.0
!    for all I.
!
!    Input, integer NDIM, the row dimension of matrix DIST exactly
!    as specified in the dimension statement of the calling program.
!
!    Input, integer ISORCE, ISINK, the specified source and 
!    destination node.
!
!    Output, integer NUMNOD, the number of nodes in the shortest
!    path from ISORCE to ISINK.
!
!    Output, integer PATH(NNODE); the nodes of the shortest path
!    are stored in entries 1 through NUMNOD of PATH.
!
!    Output, real ( kind = rk ) LNPATH, is the length of the shortest path from
!    ISORCE to ISINK.
!
...
!
!    Workspace, real ( kind = rk ) LEN1(NNODE), LEN2(NNODE), the lengths of
...
!
  ...
  real ( kind = rk ) key1
  real ( kind = rk ) key2
  real ( kind = rk ) len1(nnode)
  real ( kind = rk ) len2(nnode)
  real ( kind = rk ) lensum
  real ( kind = rk ) lnpath
  real ( kind = rk ) minln1
  real ( kind = rk ) minln2
  ...
  integer numnod
  integer path(nnode)
  integer point1(nnode)
  integer point2(nnode)
  integer stack1(nnode)
  integer stack2(nnode)
...
!
!    Output, integer CLIQ(NNODE); each time the output WRITE 
...
!
  ...
  integer arcfir(nnode+1)
  integer cand1(nnode+1)
  integer cand2(nnode+1)
  integer cliq(nnode)
  integer cliq_num
...
!
!    Output, integer NCOLOR, the number of colors used.
!
!    Output, integer COLOR(NNODE), the colors assigned to 
!    each node.
!
  implicit none
  ...
  integer arcfir(nnode+1)
  integer availc(nnode,nnode)
  integer cmax1(nnode)
  integer cmax2(nnode)
  integer color(nnode)
  integer color1(nnode)
  integer color2(nnode)
...
!    starts at node INODE(I) and ends at node JNODE(I).
!
!    Output, integer CPOLY1(NNODE), CPOLY2(NNODE), CPOLY3(NNODE),
...
!
  ...
  integer cpoly1(nnode)
  integer cpoly2(nnode)
  integer cpoly3(nnode)
  integer i
  integer ilast
  integer incr
  integer indx
  integer inode(nedge)
  integer istack((nnode*(nedge+nedge-nnode+1))/2)
  integer isub1
  integer isub2
  integer itop
  integer ivertx
  ...
  integer jlast
  integer jnode(nedge)
  integer jstack((nnode*(nedge+nedge-nnode+1))/2)
  integer jsub1
  integer jsub2
  integer jvertx
  integer loop
...
!    Input, integer NEDGE, the number of edges.
!
!    Input, integer INODE(NEDGE), JNODE(NEDGE); INODE(I), 
!    JNODE(I) are the end nodes of the I-th edge in the input graph.
!
!    Input, integer IROOT, a specified node for the root of 
!    the tree.
!
!    Output, integer NCUT, the number of cut nodes.
!
!    Output, integer NBRIDG, the number of bridges.
!
!    Output, integer CUTNOD(NNODE); during execution of the 
!    routine, CUTNOD(I) is the unique predecessor of node I in the tree.  
!    On output, the cut nodes are stored in
!      CUTNOD(I), I = 1, 2, ..., NCUT.
!
!    Output, integer BRIDGE(NEDGE); during execution of the 
...
!
!    Input/output, integer NEXT(NNODE);
...
!
  ...
  integer arcfir(nnode)
  integer bridge(nedge)
  integer cutnod(nnode)
  integer i
  integer iedge
  integer ii
  integer index
...
!
!    Output, integer EDGE_CON, the edge-connectivity of the graph.
!
...
!
  ...
  integer capac(4*nedge)
  integer edge_con
  integer flow(4*nedge)
  integer i
  integer iedge(4*nedge)
  integer inode(nedge)
  integer isink
  integer isorce
  integer j
  integer jedge(4*nedge)
  integer jnode(nedge)
  integer m4
  integer mincut(nnode)
  integer node_flow(nnode)
...
!
!    integer INODE(NEDGE), JNODE(NEDGE); INODE(I) and
!    JNODE(I) are the end nodes of the I-th edge in the input graph.
!
!  Output:
!
!    integer NUMCYC, the number of independent cycles 
!    in the graph.
!
!    integer NUMCMP, the number of components of the graph.
!
!    integer NCYC(NNODE); each time the output WRITE 
...
!    Input, integer NNODE, the number of nodes.
!
!    Input, integer K, the desired connectivity of the graph.  K
!    must be at least 2.
!
!    Output, integer INODE((NNODE*K)/2), JNODE((NNODE*K)/2); 
...
!
  ...
  integer i
  integer iedge
  integer inode((nnode*k)/2)
  integer j
  integer jnode((nnode*k)/2)
  logical join
  integer l
  integer nmm
  integer npp
...
!    the graph extends from node INODE(I) to JNODE(I).
!
!    Output, integer PAIR(NNODE), describes the matching.  Node I 
!    is matched with node PAIR(I).  If PAIR(I) is 0, then node I is
!    unmatched.
!
!    Output, integer NOTMAT, the number of unmatched nodes.
!
...
!
!    Input, real ( kind = rk ) ARCLEN(NEDGE), contains the length of each edge.
!
!    Output, integer NTREE.  If the input graph is connected, then
...
!    forest.
!
!    Output, integer ITREE(NNODE), JTREE(NNODE).  The edges in the
...
!    Input, integer M, the number of edges.
!
!    Input, integer INODE(M), JNODE(M), the pairs of nodes that
...
!
  ...
  integer snum
  integer sortn(n+n+m)
  integer sortp1(n+n+m)
  integer sortp2(n+n+m)
  integer stakc1(m+m+2)
  integer stakc2(m+m+2)
  integer stakc3(m+m+2)
  integer stakc4(m+m+2)
  integer stacke(m+m)
  integer start(m+2-n)
  integer tnode
  integer tnum
...
!    Input, integer NNODE, the number of nodes.
!
!    Input, real ( kind = rk ) DIST(NDIM,NNODE), the distance matrix.
...
!    HUGE(1.0).
!
!    Input, integer NDIM, the first dimension of DIST, which must
!    be at least NNODE.
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) A(LDA,NCOL), an NROW by NCOL matrix to be printed.
!
