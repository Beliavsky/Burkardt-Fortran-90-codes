...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument of ERF.
!
!    Output, real ( kind = rk ) ERROR_F, the value of ERF(X).
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ), save, dimension ( 5 ) :: a = (/ &
  ...
  real ( kind = rk ), save, dimension ( 9 ) :: c = (/ &
  ...
  real ( kind = rk ), save, dimension ( 8 ) :: d = (/ &
  ...
  integer i
  real ( kind = rk ), save, dimension ( 6 ) :: p = (/ &
  ...
  real ( kind = rk ), save, dimension ( 5 ) :: q = (/ &
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: y0
  real ( kind = rk ), allocatable, dimension ( : ) :: y1
  real ( kind = rk ) y_ave
  real ( kind = rk ) y_max
  real ( kind = rk ) y_min
  real ( kind = rk ) y_norm
  real ( kind = rk ), allocatable, dimension ( : ) :: y_start
  real ( kind = rk ), allocatable, dimension ( : ) :: y_stop
...
!
  write ( *, '(a,g14.6)' ) '  Stepsize H = ', &
    ( t_stop - t_start) / real ( step_num, kind = rk )
  ...
  if ( neqn <= 4 ) then
    ...
    y_ave = sum ( y_start(1:neqn) ) / real ( neqn, kind = rk )
  ...
  do step = 1, step_num
    ...
    t0 = ( real ( step_num - step + 1, kind = rk ) * t_start &
         + real (            step - 1, kind = rk ) * t_stop ) &
         / real ( step_num,            kind = rk )

    t1 = ( real ( step_num - step, kind = rk ) * t_start &
    ...
    if ( mod ( 10 * step, step_num ) == 0 .or. step == step_num ) then
      if ( neqn <= 4 ) then
        ...
        y_ave = sum ( y1(1:neqn) ) / real ( neqn, kind = rk )
  ...
  if ( neqn <= 4 ) then
    ...
    y_ave = sum ( y_stop(1:neqn) ) / real ( neqn, kind = rk )
...
!
!    Output, real ( kind = rk ) Y(NEQN), the "next" equilibrium solution, if any.
!
!    Input/output, integer NEXT, on input the index of the previous
...
!    Input, integer NEQN, the number of equations.
!
!    Input, real ( kind = rk ) T0, Y0(NEQN), the arguments of the derivative
!    function.
!
!    Input, real ( kind = rk ) T1, the point at which an estimate of the solution
!    is desired.
!
!    Output, real ( kind = rk ) Y1(NEQN), the estimated solution at T1.
!
  implicit none
  ...
  real ( kind = rk ) t0
  real ( kind = rk ) t1
  real ( kind = rk ) y0(neqn)
  real ( kind = rk ) y1(neqn)
  real ( kind = rk ) yp0(neqn)
...
!    Input, integer NEQN, the number of equations.
!
!    Input, real ( kind = rk ) T, Y(NEQN), the arguments of the derivative
!    function.
!
!    Output, real ( kind = rk ) YP(NEQN), the value of the derivative function.
!
  implicit none
...
!    Input, integer NEQN, the number of equations.
!
!    Input, real ( kind = rk ) T, Y(NEQN), the arguments of the jacobian.
!
!    Output, real ( kind = rk ) JAC(NEQN,NEQN), the jacobian matrix.
!
  implicit none
...
!    Input, integer TEST, the problem number.
!
!    Output, integer NEQN, the number of variables.
!
  implicit none
...
!    Input, integer NEQN, the number of equations.
!
!    Input, integer ORDER, the order of the Runge-Kutta method to
...
!
  ...
  real ( kind = rk ) y0(neqn)
  real ( kind = rk ) y1(neqn)
  real ( kind = rk ) yk1(neqn)
  real ( kind = rk ) yk2(neqn)
  real ( kind = rk ) yk3(neqn)
  real ( kind = rk ) yk4(neqn)
  real ( kind = rk ) yk5(neqn)
  real ( kind = rk ) yp0(neqn)
  real ( kind = rk ) ypk1(neqn)
  real ( kind = rk ) ypk2(neqn)
  real ( kind = rk ) ypk3(neqn)
  real ( kind = rk ) ypk4(neqn)
  real ( kind = rk ) ypk5(neqn)
...
!    Input, integer NEQN, the number of equations.
!
!    Output, real ( kind = rk ) SCALE(NEQN), the scaling factors.
!
  implicit none
...
!    Input, integer NEQN, the number of equations.
!
!    Output, real ( kind = rk ) T_START, Y_START(NEQN), the initial data.
!
  implicit none
...
!    Input, integer NEQN, the number of equations.
!
!    Output, real ( kind = rk ) T_STOP, Y_STOP(NEQN), the final data.
!
  implicit none
...
!
  ...
  yp(1)        =             - y(1)
  do i = 2, neqn-1
    yp(i) = real ( i - 1, kind = rk ) * y(i-1) - real ( i, kind = rk ) * y(i)
  end do
  yp(neqn) = real ( neqn - 1, kind = rk ) * y(neqn-1)
...
!
  ...
  do i = 1, neqn-1
    jac(i,i) = - real ( i, kind = rk )
  end do

  do i = 2, neqn
    jac(i,i-1) = real ( i - 1, kind = rk )
  end do
...
!
!    Input/output, real ( kind = rk ) VALUE, the value to assign to the
...
!
  ...
  real ( kind = rk ), save :: k2 = 2.95912208286D+00
  real ( kind = rk ), save :: m0 = 1.00000597682D+00
  real ( kind = rk ), save :: m1 = 0.954786104043D-03
  real ( kind = rk ), save :: m2 = 0.285583733151D-03
  real ( kind = rk ), save :: m3 = 0.437273164546D-04
  real ( kind = rk ), save :: m4 = 0.517759138449D-04
  real ( kind = rk ), save :: m5 = 0.277777777778D-05
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: delta = 0.1D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: delta = 0.3D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: delta = 0.5D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: delta = 0.7D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: delta = 0.9D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  csum = 0.0D+00
  do i = 1, 19
    csum = csum + real ( i, kind = rk )**(4.0D+00/3.0D+00)
  end do

  pprime = 0.0D+00
  do i = 1, 19
    pprime = pprime + ( 4.0D+00 / 3.0D+00 ) &
      * r8_cube_root ( t - real ( i, kind = rk ) )
  end do
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: a = 5.0D+00
  real ( kind = rk ), save :: b = 1.0D+00
  real ( kind = rk ), save :: c = 0.5D+00
  real ( kind = rk ), save :: d = 2.0D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: beta = 8.0D+00 / 3.0D+00
  character ( len = * ) name
  real ( kind = rk ), save :: rho = 28.0D+00
  logical s_eqi
  real ( kind = rk ), save :: sigma = 10.0D+00
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: delta = 1.0D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: g = 32.0D+00
  real ( kind = rk ), save :: k = 1.0D+00
  real ( kind = rk ), save :: l = 1.0D+00
  real ( kind = rk ), save :: m = 1.0D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: a = 0.3D+00
  real ( kind = rk ), save :: k = 0.2D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
  real ( kind = rk ), save :: w = 1.0D+00
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: delta = 0.01D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: a = 1.00D+00
  real ( kind = rk ), save :: b = 0.00D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the number whose cube root is desired.
!
!    Output, real ( kind = rk ) R8_CUBE_ROOT, the cube root of X.
!
  implicit none
...
!
!    Input, real ( kind = rk ) A(N), B(N), the lower and upper limits
!    for each dimension.
!
...
!
  ...
  do i = 1, n
    ...
    r(i) = a(i) + ( b(i) - a(i) ) * real ( seed, kind = rk ) * 4.656612875D-10
