...
!    generator.
!
!    Input, integer N, the number of steps.
!
!    Output, real ( kind = rk ) W(0:N), the Brownian path.
!
  implicit none
...
!    Input, integer N, the number of steps.
!
!    Input, real ( kind = rk ) W(0:N), the Brownian path.
!
  implicit none
...
!
  ...
  do i = 0, n
    t = real ( i, kind = rk ) / real ( n, kind = rk )
...
!    number generator.
!
!    Input, integer M, the number of simulations to compute 
!    and average.  A typical value is 1000.
!
!    Input, integer N, the number of steps.  A typical value
!    is 500.
!
!    Output, real ( kind = rk ) U(M,0:N), the M paths.
!
!    Output, real ( kind = rk ) UMEAN(0:N), the averaged path.
!
!    Output, real ( kind = rk ) ERROR, the maximum difference between the
...
!
  ...
  dt = tmax / real ( n, kind = rk )
  do j = 0, n
    t(j) = real ( j, kind = rk ) * tmax / real ( n, kind = rk )
  end do
...
!
  umean(0:n) = sum ( u(1:m,0:n), 1 ) / real ( m, kind = rk )
...
!    Input, integer N, the number of steps. 
!
!    Input, real ( kind = rk ) U(M,0:N), the M paths.
!
!    Input, real ( kind = rk ) UMEAN(0:N), the averaged path.
!
  implicit none
...
!
!    Output, real ( kind = rk ) XEM(0:N), the computed value of X.
!
!    Output, real ( kind = rk ) VEM(0:N), the computed value of V.
!
!    Output, real ( kind = rk ) DIFF, the maximum value of |sqrt(XEM)-V|.
!
  implicit none
...
!    Input, integer N, the number of steps.
!
!    Input, real ( kind = rk ) X(0:N), the value of X.
!
!    Input, real ( kind = rk ) V(0:N), the value of V.
!
  implicit none
...
!    number generator.
!
!    Input, integer N, the number of time steps.  A typical
!    value is 2^8.  N should be a multiple of 4.
!
!    Output, real ( kind = rk ) T(0:N), the time values for the exact solution.
!
!    Output, real ( kind = rk ) XTRUE(0:N), the exact solution.
!
!    Output, real ( kind = rk ) T2(0:N/4), the time values for the 
!    Euler-Maruyama solution.
!
!    Output, real ( kind = rk ) XEM(0:N/4), the Euler-Maruyama solution.
!
!    Output, real ( kind = rk ) ERROR, the value of | XEM(T) - XTRUE(T) |.
!
  implicit none
...
!
  r = 4
  dt2 = real ( r, kind = rk ) * dt
  l = n / r

  do j = 0, l
    t2(j) = real ( j, kind = rk ) * tmax / real ( l, kind = rk )
  end do
!
...
!    Input, integer N, the number of steps.
!
!    Input, real ( kind = rk ) T(0:N), the time values for the exact solution.
!
!    Input, real ( kind = rk ) XTRUE(0:N), the exact solution.
!
!    Input, real ( kind = rk ) T2(0:N/4), the time values for the 
!    Euler-Maruyama solution.
!
!    Input, real ( kind = rk ) XEM(0:N/4), the Euler-Maruyama solution.
!
  implicit none
...
!    number generator.
!
!    Input, integer M, the number of simulations to perform.
!    A typical value is M = 1000.
!
!    Input, integer N, the number of time steps to take.
!    A typical value is N = 512.
!
!    Input, integer P_MAX, the number of time step sizes to use.
!    A typical value is 5.
!
!    Output, real ( kind = rk ) DTVALS(P_MAX), the time steps used.
!
!    Output, real ( kind = rk ) XERR(P_MAX), the averaged absolute error in the
...
!
  ...
  xerr(1:p_max) = xerr(1:p_max) / real ( m, kind = rk )
!
...
!    Input, integer P_MAX, the number of time step sizes to use.
!
!    Input, real ( kind = rk ) DTVALS(P_MAX), the time steps used.
!
!    Input, real ( kind = rk ) XERR(P_MAX), the averaged absolute error in the
!    solution estimate at the final time.
!
  implicit none
...
!
  ...
  real ( kind = rk ) tmax
  real ( kind = rk ) winc(m)
  real ( kind = rk ) xem(p_max)
  real ( kind = rk ) xerr(p_max)
  real ( kind = rk ) xtemp(m)
  real ( kind = rk ) xzero
!
!  Problem parameters;
!
...
!
  ...
  real ( kind = rk ) dt
  real ( kind = rk ) dtp
  real ( kind = rk ) dtvals(1:p_max)
  real ( kind = rk ), allocatable :: dw(:,:)
  real ( kind = rk ) e
  integer i
  integer j
  real ( kind = rk ) k
  ...
  real ( kind = rk ) sol(2)
  real ( kind = rk ) tmax
  real ( kind = rk ), allocatable :: winc(:)
  real ( kind = rk ) xerr(p_max)
  real ( kind = rk ), allocatable :: xref(:)
  real ( kind = rk ), allocatable :: xtemp(:)
  real ( kind = rk ) xzero
!
!  Set problem parameters.
!
...
!
  ...
  do p = 1, p_max
    ...
    xerr(p) = sum ( abs ( xtemp(1:m) - xref(1:m) ) ) / real ( m, kind = rk )
...
!    number generator.
!
!    Input, integer N, the number of time steps for the
!    first solution.
!
!    Input, integer P_MAX, the number of time step sizes.
!
  implicit none
  ...
  real ( kind = rk ) u(1000)
  real ( kind = rk ) winc
  real ( kind = rk ), allocatable :: xemabs(:)
  real ( kind = rk ) xmin
  real ( kind = rk ) xtemp
  real ( kind = rk ) xzero
...
!
    xmin = exp ( -200.0D+00 )
    do i = 0, nval
      t = tmax * real ( i, kind = rk ) / real ( nval, kind = rk )
...
! 
    ...
    do j = 1, n
      ...
      xms(j) = sum ( xtemp(1:m) ** 2 ) / real ( m, kind = rk )
    end do
!
!  Write this data to a file.
!
    ...
    do j = 0, n
      t = tmax * real ( j, kind = rk ) / real ( n, kind = rk )
...
!    Input, integer SEED, a seed for the random number generator.
!
!    Output, real ( kind = rk ) ESTIMATE, the estimate of the integral.
!
!    Output, real ( kind = rk ) EXACT, the exact value of the integral.
!
!    Output, real ( kind = rk ) ERROR, the error in the integral estimate.
!
  implicit none
