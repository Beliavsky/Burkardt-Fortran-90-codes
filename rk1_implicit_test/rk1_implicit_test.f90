...
!*****************************************************************************80
!
!! rk1_implicit_test() tests rk1_implicit().
!
!  Licensing:
!
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'rk1_implicit_test():'
  write ( *, '(a)' ) '  Fortran90 version'
  write ( *, '(a)' ) '  Test rk1_implicit().'
...
!*****************************************************************************80
!
!! predator_prey_test(): predator-prey using rk1_implicit().
!
!  Discussion:
!
...
!  Input:
!
!    real ( kind = rk8 ) tspan: contains [ T0, TMAX ], the initial and final 
!    times.  A reasonable value might be [ 0, 5 ].
!
!    real ( kind = rk8 ) y0 = [ PREY, PRED ], the initial number of prey and
...
!
  ...
  write ( *, '(a)' ) '  of predators and prey are solved using rk1_implicit().'
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) '  The exact solution shows periodic behavior, with a fixed'
  write ( *, '(a)' ) '  period and amplitude.'

  call rk1_implicit ( predator_prey_dydt, tspan, y0, n, m, t, y )
!
...
!
!    real ( kind = rk8 ) y(2): the current solution variables, rabbits and foxes.
!
!  Output:
!
!    real ( kind = rk8 ) dydt(2): the right hand side of the 2 ODE's.
!
  implicit none
...
!*****************************************************************************80
!
!! stiff_test() uses the rk1_implicit method on the stiff ODE.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!  Input:
!
!    real ( kind = rk8 ) TSPAN(2): the first and last times.
!
!    real ( kind = rk8 ) Y0(1): the initial condition.
!
...
!
  ...
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'stiff_test():'
  write ( *, '(a)' ) '  Solve stiff ODE using rk1_implicit()'

  call rk1_implicit ( stiff_dydt, tspan, y0, n, m, t1, y1 )
...
!  Input:
!
!    real ( kind = rk8 ) T, Y(1): the time and solution value.
!
!  Output:
!
!    real ( kind = rk8 ) DYDT(1): the derivative value.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ) dydt(1)
  real ( kind = rk8 ) t
  real ( kind = rk8 ) y(1)
