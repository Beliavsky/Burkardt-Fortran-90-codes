...
!
  ...
  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ), allocatable, dimension (:,:) :: coord
  real ( kind = rk ), dimension ( 2 ) :: coord_max = (/ 1.0D+00, 1.0D+00 /)
  real ( kind = rk ), dimension ( 2 ) :: coord_min = (/ 0.0D+00, 0.0D+00 /)
  character ( len = 255 ) :: filedot_name = 'ranmap.eps'
  character ( len = 255 ) :: filepoint_name = 'ranmap.txt'
  integer i
  integer i4_uniform
  integer iunit
  integer j
  integer k
  integer map
  real ( kind = rk ), allocatable, dimension ( : ) :: map_bin
  real ( kind = rk ), allocatable, dimension (:,:,:) :: map_matrix
  real ( kind = rk ), allocatable, dimension ( : ) :: map_weight
  integer n
  integer map_num
...
!
  ...
  if ( map == 1 ) then
    ...
    call random_number ( harvest = map_weight(1:map_num) )
...
!
  ...
  if ( map == 1 ) then
    ...
    call random_number ( harvest = map_matrix(1:2,1:3,1:map_num) )
...
!
  ...
  call random_map ( iunit, n, map_num, map_bin, map_matrix, coord )
...
!  Parameters:
!
!    Output, real ( kind = rk ) MAP_MATRIX(2,3,MAP_NUM), the set of MAP_NUM
...
!
!    Input, real ( kind = rk ) COORD(2,NUM_PTS), the point coordinates.
!
!    Input, real ( kind = rk ) COORD_MIN(2), COORD_MAX(2), the data ranges.
!
  implicit none
  ...
  real ( kind = rk ) coord(2,num_pts)
  real ( kind = rk ) coord_max(2)
  real ( kind = rk ) coord_min(2)
  ...
  integer, parameter :: y_ps_max = 576
...
!  Parameters:
!
!    Input, integer BIN_NUM, the number of bins.
!
!    Input, real ( kind = rk ) XBIN(BIN_NUM-1), the dividing values for the bins.
!
!    Input, real ( kind = rk ) X, a value to be placed in a bin.
!
...
!
!    Input, real ( kind = rk ) X(N), the array to be examined.
!
...
subroutine random_map ( iunit, n, map_num, map_bin, map_matrix, coord )
...
!  Parameters:
!
!    Input, integer IUNIT, the FORTRAN unit to which the points 
!    should be written.
!
!    Input, integer N, the number of times to apply 
!    the transformation.
!
!    Input, integer MAP_NUM, the number of affine maps.
!
!    Input, real ( kind = rk ) MAP_BIN(MAP_NUM-1), the cumulative probabilities
...
!
!    Input, real ( kind = rk ) MAP_MATRIX(2,3,MAP_NUM), the set of affine maps.
!    Each map has the form of a 2 by 3 matrix, so that
!      Xnew = A11 * X + A12 * Y + A13
!      Ynew = A21 * X + A22 * Y + A23
!
!    Output, real ( kind = rk ) COORD(2,0:N), the coordinates of the points.
!
  implicit none
  ...
  real ( kind = rk ) coord(2,0:n)
  integer i
  integer iunit
  integer map
  real ( kind = rk ) map_matrix(2,3,map_num)
  real ( kind = rk ) map_bin(map_num-1)
  real ( kind = rk ) p
  real ( kind = rk ) x(3)
!
