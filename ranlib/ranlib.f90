...
!  Parameters:
!
!    Input, real ( kind = rk ) X(N), Y(N), the two vectors.
!
!    Input, integer N, the dimension of the two vectors.
!
!    Output, real ( kind = rk ) R8VEC_COVAR, the covariance of the vectors.
!
  implicit none
  ...
  r8vec_covar = value / real ( n - 1, kind = rk )
