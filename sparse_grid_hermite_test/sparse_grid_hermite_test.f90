...
!
  ...
  write ( *, '(a)' ) '  Test sparse_grid_hermite().'
!
...
!    to consider.
!
!    Input, integer LEVEL_MAX_MIN, the minimum value of
!    LEVEL_MAX to consider.
!
!    Input, integer LEVEL_MAX_MAX, the maximum value of
!    LEVEL_MAX to consider.
!
  implicit none
  ...
  integer level_max_min
  integer point_num
  integer point_total_num(dim_min:dim_max)
...
!
  ...
  integer point_num
  integer point_total_num
  real ( kind = rk ), allocatable, dimension ( :, : ) :: sparse_point
  real ( kind = rk ), allocatable, dimension ( : ) :: sparse_weight
...
!
  ...
  real ( kind = rk ) weight_sum_error
  real ( kind = rk ) weight_sum_exact
