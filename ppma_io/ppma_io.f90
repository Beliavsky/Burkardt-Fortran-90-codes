...
!  Parameters:
!
!    Input, integer COL_NUM, ROW_NUM, the number of rows
!    and columns of data.
!
!    Input, integer RGB_MAX, the maximum RGB value.
!
...
!    600 for COL_NUM.
!
!    Output, integer R(ROW_NUM,COL_NUM), G(ROW_NUM,COL_NUM),
...
!
  ...
  do i = 1, row_num
    ...
    y = real ( row_num - i, kind = rk ) / real ( row_num - 1, kind = rk )

    do j = 1, col_num

      x = real ( j - 1, kind = rk ) / real ( col_num - 1, kind = rk )
...
!    Input, integer FILE_IN_UNIT, the unit number of the file.
!
!    Output, integer ROW_NUM, COL_NUM, the number of rows
!    and columns of data.
!
!    Output, integer RGB_MAX, the maximum RGB value.
!
!    Output, integer IERROR, is nonzero if an error occurred.
!
  implicit none
