...
!  Author:
!
!    Original Fortran77 version by Helmuth Spaeth
...
!  Parameters:
!
!    Input, integer X(M,S), the M by S data matrix.
!
!    Input, integer M, the number of rows of data.
!
!    Input, integer S, the spatial dimension of the data.
!
!    Input/output, integer Z(M), the cluster to which each data
!    item is assigned.
!
!    Output, integer MJ(N), the number of data items in 
!    each cluster.
!
!    Input, integer N, the number of clusters.
!
!    Output, integer E(N), the per-cluster value of the
!    L1 criterion.
!
!    Output, integer D, the total value of the L1 criterion.
!
!    Output, integer IT, the number of iterations that were taken.
!
!    Output, integer IFLAG, is nonzero if an error occurred.
!
  implicit none
...
!    Input, integer N, the number of clusters.
!
!    Input/output, integer Z(M), the cluster to which each 
!    data item is assigned.
!
!    Input, integer M0, the minimum number of items per cluster.
!
!    Output, integer MJ(N), the number of data items in 
!    each cluster.
!
...
!
!    Input, integer J1, J2, the columns of X to associate with
!    the X and Y directions.
!
!    Input, integer Z(M), the clusters associated with the data.
!
...
!    Input, integer M, the number of data items.
!
!    Input, integer N, the number of components in a data item.
!
...
!    Input, integer N, the dimension of each data item.
!
!    Output, integer X(M,N), the data values.
!
  implicit none
...
!
  ...
  do i = 1, m
    ...
    i2 = 1 + nint ( columns * real ( x(i,j1) - x1_min ) &
      / real ( x1_max - x1_min ) )
    i2 = max ( i2, 1 )
    i2 = min ( i2, columns )
...
!    item is assigned.
!
!    Input, integer M0, the minimum number of data items in each
...
!    Input, integer S, the spatial dimension of the data.
!
!    Input, integer J, the cluster to which the point XI belongs.
!
...
!
!    Output, integer NNMAX, the number of values of RJ that 
!    were set.
!
!    Output, integer KMAX, the number of times the loop was 
...
!    Input, integer M, the number of rows of data.
!
!    Input, integer S, the number of columns of data.
!
!    Input, integer Z(M), the cluster to which each data item
!    belongs.
!
!    Input, integer M0, the minimum number of data items in each
!    cluster.  This is usually set to 1.
...
!  Parameters:
!
!    Input, integer T, the number of entries in B.
!
!    Input, integer B(T), ?
!
...
!    Input, integer N, the number of clusters.
!
!    Output, integer Z(M), the cluster to which each item 
!    is assigned.
!
...
!    each cluster.
!
!    Input, integer METHOD, specifies the cluster objective
!    function used.
...
!    Input, real ( kind = 8 ) X(M,S), the M by S data matrix.
!
!    Input, integer M, the number of items of data.
!
!    Input, integer S, the spatial dimension of the data.
!
!    Input/output, integer Z(M), the cluster to which each
...
!  Parameters:
!
!    Input, integer S, the order of the matrices.
!
...
!    Input, integer N, the number of clusters.
!
!    Input, integer J, the cluster whose scatter matrix is desired.
!
