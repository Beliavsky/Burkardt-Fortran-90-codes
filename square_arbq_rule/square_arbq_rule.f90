...
!
!    Input, real ( kind = rk ) Z(2), the evaluation point.
!
!    Output, real ( kind = rk ) POLS((DEGREE+1)*(DEGREE+2)/2), the orthogonal
...
!
  ...
  integer m
  integer n
  real ( kind = rk ) pols(((degree+1)*(degree+2))/2)
  real ( kind = rk ) scale
  real ( kind = rk ) z(2)
  ...
  do m = 0, degree
    do n = 0, m
      ...
      scale = real ( ( 1 + 2 * n ) * ( 1 + 2 * ( m - n ) ), kind = rk )
...
!
!    Output, real ( kind = rk ) X(2,N), the coordinates of the nodes.
!
...
!
!    Input, real ( kind = rk ) X(2,N), the coordinates of the nodes.
!
