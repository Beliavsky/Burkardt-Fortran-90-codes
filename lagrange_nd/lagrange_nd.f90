...
!
!    Input, real ( kind = rk ) PC(PN,R), the coefficients of the polynomial.
!
!    Input, integer ( kind = 4 ) PE(PN,R), the indices of the exponents of 
!    the polynomial.
!
!    Input, real ( kind = rk ) PD(PN), the coefficient of each polynomial.  
...
!    evaluation points.
!
!    Input, real ( kind = rk ) XI(D,NI), the coordinates of the interpolation 
!    evaluation points.
!
!    Output, real ( kind = rk ) YI(NI), the value of the interpolant at XI.
!
  implicit none
  ...
  integer ( kind = 4 ) j
  integer ( kind = 4 ) oj
  real ( kind = rk ) pc(pn,r)
  real ( kind = rk ) pd(pn)
  integer ( kind = 4 ) pe(pn,r)
  integer ( kind = 4 ) po(pn)
  real ( kind = rk ) value(ni)
  real ( kind = rk ) xi(d,ni)
  real ( kind = rk ) yi(ni)
...
!
!    Input, real ( kind = rk ) XD(D,ND), the data points.
...
!
!    Output, real ( kind = rk ) PC(ND,R), the coefficients for the 
...
!
!    Input, real ( kind = rk ) XD(D,ND), the data points, which must be distinct.
!
!    Output, integer ( kind = 4 ) PO(ND), the order (number of nonzero
!    coefficients), for the Lagrange basis polynomials.
!
...
!    coefficients) for the Lagrange basis polynomials.
!
!    Output, real ( kind = rk ), allocatable :: PC(:,:), the ND by R 
...
!
  ...
  integer ( kind = 4 ) po(nd)
  integer ( kind = 4 ) r
  real ( kind = rk ), parameter :: r8_epsilon = 2.220446049250313D-016
  logical success
  real ( kind = rk ) tol
  real ( kind = rk ) xd(d,nd)
!
...
!    1, Legendre products, 1, y, x, (3y^2-1)/2, xy, (3^x^2-1), (5y^3-3)/2,...
!
!    Input, real ( kind = rk ) TOL, a tolerance for the pivoting operation.
...
!
!    Output, real ( kind = rk ) C((N+2)/2), the coefficients of the Legendre
...
!
  ...
  real ( kind = rk ) c((n+2)/2)
  real ( kind = rk ) ctable(0:n,0:n)
  ...
  if ( 0 < n ) then
    ...
    do i = 2, n
      ...
      ctable(i,0:i-2) = &
         real (   - i + 1, kind = rk ) * ctable(i-2,0:i-2) &
       / real (     i,     kind = rk )

      ctable(i,1:i) = ctable(i,1:i) &
        + real ( i + i - 1, kind = rk ) * ctable(i-1,0:i-1) &
        / real (     i,     kind = rk )
...
!
!    Output, real ( kind = rk ) C(O), the coefficients of the polynomial product.
!
...
!
  ...
  real ( kind = rk ) c(o_max)
  real ( kind = rk ) c1(o_max)
  real ( kind = rk ) c2(o_max)
...
!
!    Input, real ( kind = rk ) X(D,NX), the coordinates of the evaluation points.
!
!    Output, real ( kind = rk ) V(NX), the value of the monomial at X.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) S, the multiplier for the first polynomial.
!
!    Input, integer ( kind = 4 ) O1, the "order" of polynomial 1.
!
!    Input, real ( kind = rk ) C1(O1), the coefficients of polynomial 1.
!
...
!
!    Input, real ( kind = rk ) C2(O2), the coefficients of polynomial 2.
!
...
!
!    Output, real ( kind = rk ) C(O), the coefficients of the polynomial sum.
!
...
!
!    Input, real ( kind = rk ) C1(O1), the coefficients of the polynomial.
!
!    Input, integer ( kind = 4 ) E1(O1), the indices of the exponents of 
!    the polynomial.
!
...
!
!    Output, real ( kind = rk ) C2(O2), the coefficients of the polynomial.
!
...
!
  ...
  integer ( kind = 4 ) get
  integer ( kind = 4 ) put
  real ( kind = rk ), parameter :: r8_epsilon_sqrt = 0.1490116119384766D-07
...
!
!    Input, real ( kind = rk ) C(O), the coefficients.
!
...
!
!    Input/output, real ( kind = rk ) C(O), the coefficients of the polynomial.
!
...
!    Input, integer ( kind = 4 ) O, the "order" of the polynomial.
!
!    Input, real ( kind = rk ) C(O), the coefficients of the polynomial.
!
!    Input, integer ( kind = 4 ) E(O), the indices of the exponents 
!    of the polynomial.
!
...
!    Input, real ( kind = rk ) X(D,NX), the coordinates of the evaluation points.
!
!    Output, real ( kind = rk ) P(NX), the value of the polynomial at X.
!
  implicit none
...
!
!    Output, real ( kind = rk ) D_MIN, D_MAX, the minimum and maximum distances.
!
  implicit none
...
!    Input, integer ( kind = 4 ) N1, the number of entries in the first vector.
!
!    Input, real ( kind = rk ) A(N1), the first vector.
!
!    Input, integer ( kind = 4 ) N2, the number of entries in the second vector.
!
!    Input, real ( kind = rk ) B(N2), the second vector.
!
!    Output, real ( kind = rk ) C(N1+N2), the concatenation of A and B.
!
  implicit none
