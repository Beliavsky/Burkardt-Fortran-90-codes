...
!
!    Output, real ( kind = rk ) X(2,ORDER), the points.
!
  implicit none
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: sub_w
  real ( kind = rk ), allocatable, dimension ( :, : ) :: sub_xyz
  real ( kind = rk ) w(order)
  real ( kind = rk ) x(2,order)
!
...
!    Input, integer RULE, the index of the rule.
!
!    Input, integer SUB_ORDER_NUM, the number of suborders 
!    of the rule.
!
...
!
  ...
  integer rule
  real ( kind = rk ) sub_w(suborder_num)
  real ( kind = rk ) sub_xyz(3,suborder_num)
