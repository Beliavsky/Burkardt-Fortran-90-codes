...
!  Parameters:
!
!    Input, integer ELEMENT_NUM, the number of faces.
!
...
!
!    Output, integer EDGE_NODES(2,3*ELEMENT_NUM), contains in
...
!
  ...
  integer edge_nodes(2,3*element_num)
  integer edge_num
...
!
!    Input, real ( kind = rk ) NODE_X(NODE_NUM), NODE_Y(NODE_NUM),
...
!
!    Input, integer ELEMENT_NODE(3,ELEMENT_NUM), the
!    element->node data.
!
...
!    element->node data.
!
!    Output, integer NODE_ADJ(NODE_NUM,NODE_NUM), the node 
!    adjacency matrix.
!
  implicit none
  ...
  integer n1
  integer n2
  integer node_adj(node_num,node_num)
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Input, real ( kind = rk ) X(NODE_NUM), Y(NODE_NUM), the XY
...
!
!    Input, integer EDGE_NODES(2,EDGE_NUM), the nodes that
!    form each edge.
!
!    Input, integer HOLE_NUM, the number of holes in the mesh.
!
!    Input, real ( kind = rk ) HOLE_X(NODE_NUM), HOLE_Y(NODE_NUM),
...
!
  ...
  character ( len = * ) file_name
  integer hole
  real ( kind = rk ) hole_x(hole_num)
  real ( kind = rk ) hole_y(hole_num)
  integer ios
  integer node
  integer poly_unit
  integer, parameter :: region_num = 0
  real ( kind = rk ) x(node_num)
  real ( kind = rk ) y(node_num)
...
!  Parameters:
!
!    Input, integer NUMBER, the number of the mesh.
!
!    Input, integer ELEMENT_NUM, the number of elements.
!
!    Output, integer ELEMENT_NODE(3,ELEMENT_NUM), the
!    element->node data.
!
  implicit none
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: node_y
  integer number
...
!    Input, integer NUMBER, the number of the mesh.
!
!    Output, integer HOLE_NUM, the number of holes in the mesh.
!
  implicit none
...
!
!    Output, real ( kind = rk ) HOLE_X(HOLE_NUM), HOLE_Y(HOLE_NUM),
...
!    Input, integer NUMBER, the number of the mesh.
!
!    Output, integer NODE_NUM, the number of elements in the mesh.
!
  implicit none
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Output, real ( kind = rk ) X(NODE_NUM), Y(NODE_NUM), the coordinates
!    of the nodes.
!
  implicit none
...
!  Parameters:
!
!    Output, integer MESH_NUM, the number of meshes.
!
  implicit none
...
!
  ...
  character ( len = * ) file_name
  integer hole_num
  real ( kind = rk ), allocatable, dimension ( : ) :: hole_x
  real ( kind = rk ), allocatable, dimension ( : ) :: hole_y
  character ( len = 80 ) name
  integer node_num
  integer number
...
!
  ...
  integer element_node(3,2)
  integer, dimension(3,2) :: element_node_save &
    = reshape ( (/ &
    1, 2, 3, &
    4, 3, 2 /), (/ 3, 2 /) )
...
!  Parameters:
!
!    Input, integer HOLE_NUM, the number of nodes.
!
!    Output, real ( kind = rk ) HOLE_X(HOLE_NUM), HOLE_Y(HOLE_NUM),
...
!*****************************************************************************80
!
!! MESH01_NODE_ELEMENT_NUM returns the number of node-element items of mesh #01.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!  Parameters:
!
!    Output, integer NODE_ELEMENT_NUM, the number of node-element
!    data items.
!
  implicit none
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Input, integer NODE_ELEMENT_NUM, the number of node-element
!    data items.
!
!    Output, integer NODE_ELEMENT_INDEX(NODE_NUM+1), for each 
!    node I, entry I points to the first entry in NODE_ELEMENT, and entry I+1
!    to the entry after the least entry in NODE_ELEMENT.
!
!    Output, integer NODE_ELEMENT(NODE_ELEMENT_NUM), for node I,
...
!
  ...
  integer node_element(node_element_num)
  integer node_element_index(node_num+1)
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Output, real ( kind = rk ) X(NODE_NUM), Y(NODE_NUM), the XY
!    coordinates of the nodes.
!
  implicit none
...
!
  ...
  integer element_node(3,30)
  integer, dimension(3,30) :: element_node_save = reshape ( (/ &
     4,  1,  2, &
     6,  2,  3, &
     4, 10,  1, &
     5,  4,  2, &
...
!*****************************************************************************80
!
!! MESH02_NODE_ELEMENT_NUM returns the number of node-element items of mesh #02.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
  ...
  integer element_node(3,26)
  integer, dimension ( 3, 26 ) :: element_node_save = reshape ( &
    (/ &
     3,  1,  4, &
     2,  4,  1, &
     5,  3,  6, &
