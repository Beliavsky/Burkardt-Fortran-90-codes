...
!*****************************************************************************80
!
!! stripack_voronoi() uses stripack() to compute a Voronoi diagram on a sphere.
!
!  Licensing:
!
...
!
  ...
  real ( kind = rk8 ), allocatable :: xc(:)
  real ( kind = rk8 ), allocatable :: xyz(:,:)
  real ( kind = rk8 ), allocatable :: xyzv(:,:)
  character ( len = 255 ) :: xyz_filename = ' '
  real ( kind = rk8 ), allocatable :: yc(:)
  real ( kind = rk8 ), allocatable :: zc(:)
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Call STRIPACK(), and determine a Voronoi diagram.'
!
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk8 ) TABLE(M,N), the data.
!
  implicit none
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk8 ) X(N), Y(N), Z(N), the coordinates of points 
!    on the sphere.
!
!    Output, integer NT, the number of Delaunay triangles
!    and Voronoi vertices.
!
!    Output, real ( kind = rk8 ) XC(6*(N-2)), YC(6*(N-2)), ZC(6*(N-2)), the 
!    coordinates of the vertices.
!
!    Output, integer LEND(N), points to the "first" vertex in the
!    Voronoi polygon around a particular node.
!
!    Output, integer LISTC(6*(N-2)), the Voronoi vertex indices.
!
!    Output, integer LPTR(6*(N-2)), given a vertex, returns the 
...
!
  ...
  integer nb
  integer nt
  real ( kind = rk8 ) rc(2*(n-2))
  real ( kind = rk8 ) x(n)
  real ( kind = rk8 ) xc(2*(n-2))
  real ( kind = rk8 ) y(n)
  real ( kind = rk8 ) yc(2*(n-2))
  real ( kind = rk8 ) z(n)
  real ( kind = rk8 ) zc(2*(n-2))
!
...
!    one?" because the vertex index also tells us what polygon we are in.)
!
!    Output, integer ORDER(N), the order of each polygon.
!
  implicit none
...
!    Input, integer N, the number of nodes, and Voronoi polygons.
!
!    Input, integer LIST_NUM, the number of entries to be stored
!    in LIST.
!
!    Input, integer LEND(N), points to the "first" vertex in the
!    Voronoi polygon around a particular node.
!
!    Input, integer LISTC(6*(N-2)), the Voronoi vertex indices.
!
!    Input, integer LPTR(6*(N-2)), given a vertex, returns the 
!    next vertex in the Voronoi polygon.  (The vertex numbering is done
!    in such a way that the physical vertex has three distince indices,
!    depending on which polygon we are considering.  Thus, it is always
!    possible to answer the question "which is the next vertex from this
...
!    Input, integer N, the number of nodes, and Voronoi polygons.
!
!    Input, real ( kind = rk8 ) X(N), Y(N), Z(N), the coordinates of the nodes.
!
!    Input, real ( kind = rk8 ) XC(6*(N-2)), YC(6*(N-2)), ZC(6*(N-2)), the 
!    coordinates of the vertices.
!
!    Input, integer LEND(N), points to the "first" vertex in the
!    Voronoi polygon around a particular node.
...
!
  ...
  real ( kind = rk8 ) area_polygon
  real ( kind = rk8 ) area_triangle
  real ( kind = rk8 ) areas
  integer index_polygon
  integer index_triangle
  integer lend(n)
  integer listc(6*(n-2))
  integer lptr(6*(n-2))
  integer node
  ...
  integer vertex_new
  real ( kind = rk8 ) x(n)
  real ( kind = rk8 ) xc(6*(n-2))
  real ( kind = rk8 ) y(n)
  real ( kind = rk8 ) yc(6*(n-2))
  real ( kind = rk8 ) z(n)
  real ( kind = rk8 ) zc(6*(n-2))
