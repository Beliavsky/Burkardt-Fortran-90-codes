...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument at which the series is 
!    to be evaluated.
...
subroutine p00_c ( prob, m, c )

!*****************************************************************************80
!
!! P00_C computes a random C parameter vector for any problem.
!
!  Licensing:
!
!    This code is distributed under the MIT license.
...
!
!    Output, real ( kind = rk ) C(M), the parameter vector.
!
  implicit none
...
!    Input, integer M, the spatial dimension.
!
!    Input, integer ID, the spatial coordinate to differentiate.
!
!    Input, real ( kind = rk ) C(M), W(M), the problem parameters.
!
!    Input, integer N, the number of evaluation points.
!
!    Input, real ( kind = rk ) X(M,N), the evaluation points.
!
!    Output, real ( kind = rk ) D(N), the ID-th derivative component.
!
  implicit none
...
!  Parameters:
!
!   Output, integer PROB_NUM, the number of test functions.
!
  implicit none
...
!    Input, real ( kind = rk ) C(M), W(M), the problem parameters.
!
!    Output, real ( kind = rk ) Q, the integral.
!
  implicit none
...
!    Input, integer M, the spatial dimension.
!
!    Output, real ( kind = rk ) W(M), the parameter vector.
!
  implicit none
...
!
  ...
  d(1:n) = - c(id) * real ( m + 1, kind = rk ) / d(1:n) ** ( m + 2 )
