...
!
!    Output, real ( kind = rk ) F(M), the value of the functions evaluated at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X(N), the evaluation point.
!
!    Output, real ( kind = rk ) G(N), the least squares gradient vector.
!
  implicit none
...
!    Input, real ( kind = rk ) X(N), the evaluation point.
!
!    Output, real ( kind = rk ) FJAC(M,N), the jacobian matrix.
!
  implicit none
...
!    Input, integer NPROB, the problem number.
!
!    Output, integer M, information defining the number of 
...
!
!    Output, integer N, information defining the number of 
...
!  Parameters:
!
!    Output, integer MPROB, the number of problems available.
!
  implicit none
...
!
!    Output, real ( kind = rk ) X(N), a starting point for the problem.
!
  implicit none
...
!
  ...
  x_sum = sum ( x(1:n) )
  f(1:m) = - 1.0D+00 - 2.0D+00 * x_sum / real ( m, kind = rk )
  mn = min ( m, n )
...
!
  ...
  fjac(1:m,1:n) = - 2.0D+00 / real ( m, kind = rk )
  do j = 1, n
...
!
  ...
  x_sum = 0.0D+00
  do j = 1, n
    x_sum = x_sum + real ( j, kind = rk ) * x(j)
  end do

  do i = 1, m
    f(i) = real ( i, kind = rk ) * x_sum - 1.0D+00
  end do
...
!
  ...
  known = 1
  x(1:n) = 6.0D+00 / real ( ( 2 * m + 1 ) * ( n + 1 ) * n, kind = rk )
...
!
  ...
  do i = 1, m-1
    f(i) = real ( i - 1, kind = rk ) * x_sum - 1.0D+00
  end do
...
!
  ...
  known = 1
  x(1:n) = 6.0D+00 / &
    real ( ( 2 * m - 3 ) * ( n * ( n + 1 ) - 2 * n - 2 ), kind = rk )
...
!
  ...
  do i = 1, 16
    f(i) = x(1) * exp ( x(2) &
      / ( 5.0D+00 * real ( i, kind = rk ) + 45.0D+00 + x(3) ) ) &
      - y(i)
  end do
...
!
  ...
  do i = 1, 16
    temp = 5.0D+00 * real ( i, kind = rk ) + 45.0D+00 + x(3)
...
!
  ...
  do i = 1, 29
    ...
    div = real ( i, kind = rk ) / 29.0D+00
    s1 = 0.0D+00
    dx = 1.0D+00
    do j = 2, n
      s1 = s1 + real ( j - 1, kind = rk ) * dx * x(j)
      dx = div * dx
    end do
...
!
  ...
  do i = 1, 29
    ...
    dx = 1.0D+00 / div
    do j = 1, n
      fjac(i,j) = dx * ( real ( j - 1, kind = rk ) - temp )
      dx = div * dx
    end do
  end do
...
!
  ...
  do i = 1, m
    ...
    f(i) = exp ( - i * x(1) / 10.0D+00 ) - exp ( - i * x(2) / 10.0D+00 ) &
      + ( exp ( - real ( i, kind = rk ) ) - exp ( - i / 10.0D+00 ) ) * x(3)
...
!
  ...
  do i = 1, m
    f(i) = 2.0D+00 + 2.0D+00 * real ( i, kind = rk ) &
      - exp ( real ( i, kind = rk ) * x(1) ) &
      - exp ( real ( i, kind = rk ) * x(2) )
  end do
...
!
  ...
  do i = 1, m
    fjac(i,1) = - real ( i, kind = rk ) * exp ( real ( i, kind = rk ) * x(1) )
    fjac(i,2) = - real ( i, kind = rk ) * exp ( real ( i, kind = rk ) * x(2) )
  end do
...
!
  ...
  do i = 1, m
    f(i) = &
        ( x(1) + real ( i, kind = rk ) / 5.0D+00 * x(2) &
      - exp ( real ( i, kind = rk ) / 5.0D+00 ) )**2 &
      + ( x(3) + sin ( real ( i, kind = rk ) / 5.0D+00 ) * x(4) &
      - cos ( real ( i, kind = rk ) / 5.0D+00 ) )**2
  end do
...
!
  ...
  dx = 1.0D+00 / real ( n, kind = rk )

  iev = -1
  do i = 1, m
    f(i) = dx * f(i)
    if ( 0 < iev ) then
      f(i) = f(i) + 1.0D+00 / ( real ( i * i, kind = rk ) - 1.0D+00 )
    end if
    iev = -iev
  end do
...
!
  ...
  do j = 1, n
    x(j) = real ( j, kind = rk ) / real ( n + 1, kind = rk )
  end do
...
!
  ...
  do i = 1, 33
    ...
    f(i) = y(i) - ( x(1) &
      + x(2) * exp ( - x(4) * 10.0D+00 * real ( i - 1, kind = rk ) ) &
      + x(3) * exp ( - x(5) * 10.0D+00 * real ( i - 1, kind = rk ) ) )
  end do
...
!
  ...
  do i = 1, 33
    ...
    temp = 10.0D+00 * real ( i - 1, kind = rk )
    tmp1 = exp ( -x(4) * temp )
    tmp2 = exp ( -x(5) * temp )
...
!
  ...
  do i = 1, 65
    ...
    temp = real ( i - 1, kind = rk ) / 10.0D+00
...
!
  ...
  real ( kind = rk ), dimension ( 4 ) :: d = (/ 1.75168D+00, -1.35195D+00, &
...
!
  ...
  real ( kind = rk ), dimension ( 5, 5 ) :: b = reshape ( (/ &
    ...
    /), (/ 5, 5 /) )
  real ( kind = rk ), dimension ( 10, 5 ) :: c = reshape ( (/ &
