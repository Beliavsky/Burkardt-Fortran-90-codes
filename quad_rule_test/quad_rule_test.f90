...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'quad_rule_test():'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Test quad_rule().'
...
!
  ...
  integer nk
  integer nl
  integer, dimension ( 4 ) :: nl_test = (/ 7, 10, 15, 20 /)
  integer test
  real ( kind = rk ), allocatable :: wk(:)
  real ( kind = rk ), allocatable :: wl(:)
...
!
  ...
  integer j
  integer n
  integer, dimension ( 4 ) :: n_test = (/ 1, 3, 7, 15 /)
  real ( kind = rk ), allocatable :: w(:)
  real ( kind = rk ), allocatable :: x(:)
