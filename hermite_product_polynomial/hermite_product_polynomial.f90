...
!
!    Input, integer RANK1, RANK2, the lowest and highest ranks 
...
!  Parameters:
!
!    Input, integer N, the degree of the polynomial to compute.
!
...
!
!    Output, integer F((N+2)/2), the exponents.
!
  implicit none
  ...
  do i = 2, n
    ct(i,0)     =               - real ( i - 1, kind = rk ) * ct(i-2,0)
    ct(i,1:i-2) = ct(i-1,0:i-3) - real ( i - 1, kind = rk ) * ct(i-2,1:i-2)
    ct(i,  i-1) = ct(i-1,  i-2)
    ct(i,  i  ) = ct(i-1,  i-1)
  end do
!
...
!
!    Output, real ( kind = rk ) V(N), the values of the polynomials 
  ...
  if ( 1 <= o ) then
    ...
    do j = 2, o
      vtable(1:n,j) =                   x(1:n) * vtable(1:n,j-1) &
                    - real ( j - 1, kind = rk ) * vtable(1:n,j-2)
    end do
...
!
!    Input, integer L(M), the index of each polynomial factor.  
!    0 <= L(*).
!
!    Input, integer O_MAX, an upper limit on the size of the 
!    output arrays.
!      O_MAX = product ( 1 <= I <= M ) (L(I)+2)/2.
!
...
!    Input, integer N, the number of evaluation points.
!
!    Input, integer O(M), the degree of the polynomial factors.
!    0 <= O(*).
!
!    Input, real ( kind = rk ) X(M,N), the evaluation points.
!
!    Output, real ( kind = rk ) VALUE(N), the value of the Product 
...
!  Parameters:
!
!    Input, integer M, spatial dimension.
!    1 <= M.
!
!    Input, integer XC(M), the composition.
...
!    1 <= RANK.
!
!    Output, integer XC(M), the composition of the given rank.
!    For each I, 0 <= XC(I) <= NC, and 
!    sum ( 1 <= I <= M ) XC(I) = NC.
!
  implicit none
...
!
!    Input, real ( kind = rk ) X(M,N), the coordinates of the evaluation points.
!
!    Output, real ( kind = rk ) V(N), the value of the monomial at X.
!
  implicit none
...
!    the polynomial.
!
!    Output, integer O2, the "order" of the polynomial.
!
!    Output, real ( kind = rk ) C2(O2), the coefficients of the polynomial.
!
!    Output, integer E2(O2), the indices of the exponents of 
!    the polynomial.
!
  implicit none
...
!
!    Input/output, integer E(O), the indices of the exponents of 
!    the polynomial.
!
  implicit none
...
!    Input, real ( kind = rk ) X(M,N), the coordinates of the evaluation points.
!
!    Output, real ( kind = rk ) P(N), the value of the polynomial at X.
!
  implicit none
