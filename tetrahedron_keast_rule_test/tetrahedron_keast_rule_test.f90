...
!
  ...
  write ( *, '(a)' ) '  Test TETRAHEDRON_KEAST_RULE().'
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: suborder_xyzz
  real ( kind = rk ) xyzz_sum
...
!
  ...
  real ( kind = rk ), dimension ( 3, node_num ) :: node_xyz2 = reshape ( (/ &
...
!
  ...
  integer degree
  integer, parameter:: degree_max = 3
