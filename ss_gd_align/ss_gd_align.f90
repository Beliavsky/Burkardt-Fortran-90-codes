...
!
!    Output, real PAM120_SCORE, the score for matching 
!    the two characters.
!  
  implicit none
...
!    C1 is from sequence A, C2 from sequence B.
!
!    Output, real SIMPLE_SCORE, the score for matching the 
!    two characters.
!
  implicit none
...
!
!    Input, real GAP_EXTEND, the penalty for extending a gap.  A 
!    gap of length 7 incurs a penalty 7 * GAP_EXTEND.
!
...
!    LDS must be at least M.
!
!    Output, real S(0:LDS,0:N), the M1:M by 0:N reverse score
!    table.  The maximum possible alignment score is in S(M1,0).
!
...
!    first NPATH entries, the indices of the matched items from sequences A 
!    and B.
!
  implicit none
