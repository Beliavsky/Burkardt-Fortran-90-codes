...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the value to which function is to be
!    integrated.  X must be in the range [0,1] inclusive.
!
!    Input, real ( kind = rk ) P, the first parameter.  P must be greater
!    than 0.0.
!
!    Input, real ( kind = rk ) Q, the second parameter.  Q must be greater
!    than 0.0.
!
!    Output, real ( kind = rk ) PROB.  The probability that a random variable
...
!  Local parameters:
!
!    Local, real ( kind = rk ) ALEPS, the logarithm of EPS1.
!
!    Local, real ( kind = rk ) EPS, the machine precision.
!
!    Local, real ( kind = rk ) EPS1, the smallest representable number.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ), parameter :: aleps = - 179.6016D+00
  real ( kind = rk ) alogam
  real ( kind = rk ) c
  real ( kind = rk ) cnt
  real ( kind = rk ) d4
  real ( kind = rk ) dp
  real ( kind = rk ) dq
  real ( kind = rk ), parameter :: eps = 2.2D-16
  real ( kind = rk ), parameter :: eps1 = 1.0D-78
  real ( kind = rk ) finsum
  integer ib
  integer ier
  integer ifault
...
!
  ...
  c = 1.0D+00 / ( 1.0D+00 - y )
  cnt = exp ( xb - real ( ib, kind = rk ) * aleps )
  ps = dq
  wh = dq
