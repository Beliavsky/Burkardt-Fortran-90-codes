...
!    0 < N.
!
!    Input, real ( kind = rk ) P, the probability of a success in one trial.
!
!    Input, integer K, the number of successes.
!
!    Output, real ( kind = rk ) I4_BINOMIAL_PDF, the probability of K successes
...
!    0 < N.
!
!    Input, real ( kind = rk ) PP, the probability of an event in each trial of
!    the binomial distribution from which a random deviate is to be generated.
!    0.0 < PP < 1.0.
!
!    Output, integer I4_BINOMIAL_SAMPLE, a random deviate from the
!    distribution.
!
  implicit none
  ...
  p = min ( pp, 1.0D+00 - pp )
  q = 1.0D+00 - p
  xnp = real ( n, kind = rk ) * p

  if ( xnp < 30.0D+00 ) then

    qn = q ** n
    r = p / q
    g = r * real ( n + 1, kind = rk )
...
!
    ...
    if ( k <= 20 .or. xnpq / 2.0D+00 - 1.0D+00 <= k ) then
      ...
      ynorm = - real ( k * k, kind = rk ) / ( 2.0D+00 * xnpq )
      alv = log ( v )
      ...
      x1 = real ( ix + 1, kind = rk )
      f1 = fm + 1.0D+00
      z = real ( n + 1, kind = rk ) - fm
      w = real ( n - ix + 1, kind = rk )
      z2 = z * z
      x2 = x1 * x1
      f2 = f1 * f1
      w2 = w * w
      ...
      t = xm * log ( f1 / x1 ) + ( n - m + 0.5D+00 ) * log ( z / w ) &
        + real ( ix - m, kind = rk ) * log ( w * p / ( x1 * q )) &
...
!
  u = ( 1.0D+00 - u ) * ( real ( a2, kind = rk ) - 0.5D+00 ) &
    +             u   * ( real ( b2, kind = rk ) + 0.5D+00 )
!
!  Round.
!
  value = nint ( u )
...
!
!    Input, real ( kind = rk ) P(M), the probability of each outcome
!    on any single trial.
!
...
!
!    Output, real ( kind = rk ) I4VEC_MULTINOMIAL_PDF, the probability
...
!
  ...
  integer c
  integer i
  real ( kind = rk ) i4vec_multinomial_pdf
  integer j
  real ( kind = rk ) p(m)
  real ( kind = rk ) pdf
  integer top
...
!    Input, integer N, the number of trials.
!
!    Input, real ( kind = rk ) P(NCAT).  P(I) is the probability that an event
!    will be classified into category I.  Thus, each P(I) must be between 
!    0.0 and 1.0, and the P's must sum to 1.
!
!    Input, integer NCAT, the number of possible outcomes
!    of a single trial.
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) ALPHA, BETA, shape parameters.
!    0.0 < ALPHA, BETA.
!
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_BETA_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) AA, the first parameter of the beta distribution.
!    0.0 < AA.
!
!    Input, real ( kind = rk ) BB, the second parameter of the beta distribution.
!    0.0 < BB.
!
!    Output, real ( kind = rk ) R8_BETA_SAMPLE, a beta random variate.
!
  implicit none
  ...
  real ( kind = rk ) k1
  real ( kind = rk ) k2
  real ( kind = rk ), parameter :: log4 = 1.3862943611198906188D+00
  real ( kind = rk ), parameter :: log5 = 1.6094379124341003746D+00
  real ( kind = rk ) r
  real ( kind = rk ) r8_beta_sample
  real ( kind = rk ) s
  real ( kind = rk ) t
  real ( kind = rk ) u1
  real ( kind = rk ) u2
...
!  Parameters:
!
!    Input, real ( kind = rk ) DF, the degrees of freedom.
!    0.0 < DF.
!
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_CHI_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!    0.0 < DF.
!
!    Output, real ( kind = rk ) R8_CHI_SAMPLE, a random deviate 
...
!
  ...
  real ( kind = rk ) arg2
  real ( kind = rk ) df
  real ( kind = rk ) r8_chi_sample
  real ( kind = rk ) r8_gamma_sample
...
!  Parameters:
!
!    Input, real ( kind = rk ) BETA, the scale value.
!    0.0 < BETA.
!
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_EXPONENTIAL_PDF, the value of the PDF at RVAL.
!
  implicit none
  ...
  real ( kind = rk ) beta
  real ( kind = rk ) r8_exponential_pdf
  real ( kind = rk ) rval
...
!  Parameters:
!
!    Input, real ( kind = rk ) LAMBDA, the parameter of the PDF.
!
!    Output, real ( kind = rk ) R8_EXPONENTIAL_SAMPLE, a sample of the PDF.
!
  implicit none
  ...
  real ( kind = rk ) lambda
  real ( kind = rk ) r
  real ( kind = rk ) r8_exponential_sample
...
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_EXPONENTIAL_01_PDF, the value of the PDF.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) r8_exponential_01_pdf
  real ( kind = rk ) rval
...
!  Parameters:
!
!    Output, real ( kind = rk ) R8_EXPONENTIAL_01_SAMPLE, a sample of the PDF.
!
  implicit none
  ...
  real ( kind = rk ) r
  real ( kind = rk ) r8_exponential_01_sample
...
!  Parameters:
!
!    Input, real ( kind = rk ) BETA, the rate parameter.
!    0.0 < BETA.
!
!    Input, real ( kind = rk ) ALPHA, the shape parameter.
!    0.0 < ALPHA.
!
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_GAMMA_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) A, the rate parameter.
!    A nonzero.
!
!    Input, real ( kind = rk ) R, the shape parameter.
!    0.0 < R.
!
!    Output, real ( kind = rk ) R8_GAMMA_SAMPLE, a random deviate 
!    from the distribution.
!
  implicit none
  ...
  real ( kind = rk ) r
  real ( kind = rk ) r8_gamma_sample
  real ( kind = rk ) r8_gamma_01_sample
...
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_GAMMA_01_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) A, the shape parameter. 
!    0.0 < A.
!
!    Output, real ( kind = rk ) R8_GAMMA_01_SAMPLE, a random deviate 
!    from the distribution.
!
  implicit none
  ...
  real ( kind = rk ) s2
  real ( kind = rk ) si
  real ( kind = rk ), parameter :: sqrt32 = 5.656854D+00
  real ( kind = rk ) t
  real ( kind = rk ) u
  real ( kind = rk ) v
  real ( kind = rk ) w
...
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_INVCHI_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!    0.0 < DF.
!
!    Output, real ( kind = rk ) R8_INVCHI_SAMPLE, a sample value.
!
  implicit none
...
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_INVGAM_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!    0.0 < ALPHA.
!
!    Output, real ( kind = rk ) R8_INVGAM_SAMPLE, a sample value.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) AV, the mean value.
!
!    Input, real ( kind = rk ) SD, the standard deviation.
!    0.0 < SD.
!
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_NORMAL_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) AV, the mean.
!
!    Input, real ( kind = rk ) SD, the standard deviation.
!
!    Output, real ( kind = rk ) R8_NORMAL_SAMPLE, a random deviate 
!    from the distribution.
!
  implicit none
...
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_NORMAL_01_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) R8_NORMAL_01_SAMPLE, a sample of the standard
!    normal PDF.
!
  implicit none
...
!    0.0 < DF.
!
!    Input, real ( kind = rk ) S, the scale factor.
!    0.0 < S.
!
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_SCINVCHI_PDF, the value of the PDF at RVAL.
...
!    0.0 < S.
!
!    Input, real ( kind = rk ) R8_SCINVCHI_SAMPLE, a sample value.
!
  implicit none
  ...
  real ( kind = rk ) df
  real ( kind = rk ) r8_gamma_sample
  real ( kind = rk ) r8_scinvchi_sample
  real ( kind = rk ) s
  real ( kind = rk ) value
...
!  Parameters:
!
!    Input, real ( kind = rk ) LOWER, UPPER, the lower and upper range limits.
!    LOWER < UPPER.
!
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_UNIFORM_PDF, the value of the PDF at RVAL.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) LOW, HIGH, the lower and upper bounds.
!
!    Output, real ( kind = rk ) R8_UNIFORM_SAMPLE, a random deviate 
!    from the distribution.
!
  implicit none
  ...
  real ( kind = rk ) high
  real ( kind = rk ) low
  real ( kind = rk ) r8_uniform_01_sample
...
!    Input, real ( kind = rk ) RVAL, the point where the PDF is evaluated.
!
!    Output, real ( kind = rk ) R8_UNIFORM_01_PDF, the value of the PDF at RVAL.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) r8_uniform_01_pdf
  real ( kind = rk ) rval
...
!  Parameters:
!
!    Output, real ( kind = rk ) R8_UNIFORM_01_SAMPLE, a random deviate 
!    from the distribution.
!
  implicit none
...
!
!    Input, real ( kind = rk ) A(N,N), the R8PO matrix.
!
...
!    Input, integer N, the order of the matrix.
!
!    Input, real ( kind = rk ) A_LU(N,N), the LU factors from R8PO_FA.
!
...
!
!    Output, real ( kind = rk ) R(N,N), the Cholesky factor R.
!
  implicit none
...
!    Input, integer N, the order of the matrix.
!
!    Input, real ( kind = rk ) R(N,N), the Cholesky factor..
!
!    Output, real ( kind = rk ) B(N,N), the inverse matrix, in R8PO storage.
!
  implicit none
...
!
!    Input, real ( kind = rk ) MU(N), the mean vector.
!
!    Input, real ( kind = rk ) R(N,N), the upper triangular Cholesky
!    factor of the covariance matrix C.
!
!    Input, real ( kind = rk ) C_DET, the determinant of the
!    covariance matrix C.
!
!    Input, real ( kind = rk ) X(N), a sample of the distribution.
!
!    Output, real ( kind = rk ) R8VEC_MULTINORMAL_PDF, the PDF evaluated
!    at X.
!
  implicit none
...
!    factor of the covariance matrix C.
!
!    Output, real ( kind = rk ) X(N), a sample of the distribution.
!
  implicit none
