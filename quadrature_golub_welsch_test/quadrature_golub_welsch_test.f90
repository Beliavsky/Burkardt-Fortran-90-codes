...
!*****************************************************************************80
!
!! quadrature_golub_welsch_test() tests quadrature_golub_welsch().
!
!  Licensing:
!
...
!
  ...
  do j = 1, n - 1
    if ( mod ( j, 2 ) == 1 ) then
      bj(j) = ( real ( j, kind = rk ) + alpha ) / 2.0D+00
    else
      bj(j) = real ( j, kind = rk ) / 2.0D+00
    end if
  end do
  bj(n) = 0.0D+00
...
!
  ...
  do j = 1, n
    aj(j) = alpha + real ( 2 * j - 1, kind = rk )
  end do

  do j = 1, n - 1
    bj(j) = real ( j, kind = rk ) * ( alpha + real ( j, kind = rk ) )
  end do
  bj(n) = 0.0D+00
...
!
  ...
  do j = 1, n
    aj(j) = real ( 2 * j - 1, kind = rk )
  end do

  do j = 1, n - 1
    bj(j) = real ( j**2, kind = rk )
  end do
  bj(n) = 0.0D+00
...
!
  ...
  do j = 1, n - 1
    bj(j) = real ( j**2, kind = rk ) / real ( 4 * j**2 - 1, kind = rk )
  end do
  bj(n) = 0.0D+00
