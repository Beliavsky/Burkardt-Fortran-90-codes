...
!    Input, integer M, the number of equations.
!
!    Output, real ( kind = rk ) B(M), the right hand side.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) PROB_NUM, the number of problems.
!
  implicit none
...
!
  ...
  do i = 1, m
    a(i,n) = 1.0D+00
    do j = n - 1, 1, -1
      a(i,j) = a(i,j+1) * real ( i - 1, kind = rk ) / 5.0D+00
    end do
  end do
...
!
  ...
  real ( kind = rk ) a(m,n)
  real ( kind = rk ), dimension ( 5, 3 ) :: a_save = reshape ( (/ &
...
!
  ...
  do i = 1, m
    do j = 1, n
      angle = real ( i * j, kind = rk ) * pi / real ( n + 1, kind = rk )
      a(i,j) = sin ( angle )
    end do
  end do

  a(1:m,1:n) = a(1:m,1:n) * sqrt ( 2.0D+00 / real ( n + 1, kind = rk ) )
...
!
  ...
  x(1:n) = x(1:n) * sqrt ( 2.0D+00 / real ( n + 1, kind = rk ) )
