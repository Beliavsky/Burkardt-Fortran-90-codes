...
!
!    Output, real ( kind = rk ) LP(M,N), the derivative of the completed Lobatto
...
!
  ...
  integer j
  real ( kind = rk ) lp(m,n)
  real ( kind = rk ) p(m,n+1)
  real ( kind = rk ) pp(m,n+1)
  real ( kind = rk ) x(m)
  ...
  if ( 1 <= n ) then
    ...
    if ( 2 <= n ) then
      ...
      do j = 2, n
        p(1:m,j+1) = &
          ( real ( 2 * j - 1, kind = rk ) * x(1:m) * p(1:m,j)     &
          - real (     j - 1, kind = rk ) *          p(1:m,j-1) ) &
          / real (     j,     kind = rk )
      end do
      ...
      do j = 2, n
        pp(1:m,j+1) = &
          ( real ( 2 * j - 1, kind = rk ) * ( p(1:m,j) + x(1:m) * pp(1:m,j) )   &
          - real (     j - 1, kind = rk ) *                       pp(1:m,j-1) ) &
          / real (     j,     kind = rk )
      end do
...
!  Parameters:
!
!    Input, integer NDX_NUM, the number of polynomials to plot.
!
!    Input, integer NDX(NDX_NUM), the orders of 1 or more 
...
!
  ...
  real ( kind = rk ) x_hi
  real ( kind = rk ) x_lo
  real ( kind = rk ) y(x_num,ndx_num)
  real ( kind = rk ) yp(x_num,ndx_num)
...
!    Input, real ( kind = rk ) X(M), the evaluation points.
!
!    Output, real ( kind = rk ) L(M,N), the values of the completed Lobatto
!    polynomials of order 1 through N at the point X.
!
  implicit none
  ...
  if ( 1 <= n ) then
    ...
    if ( 2 <= n ) then
      ...
      do j = 2, n
        l(1:m,j) = real ( j, kind = rk ) * ( p(1:m,j) - x(1:m) * p(1:m,j+1) )
      end do
