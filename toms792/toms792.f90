...
!  Parameters:
!
!    Input, integer K, chooses the data set:
...
!
!    Output, integer N, the size of the data vectors.
!
!    Output, real ( kind = rk ), X(N), Y(N), the coordinates of the nodes.
!
  implicit none
...
!  Parameters:
!
!    Input, integer K, chooses the function:
!     1, Exponential;
!     2, Cliff;
!     3, Saddle;
!     4, Gentle;
...
!    10, Cosine Peak.
!
!    Input, real ( kind = rk ) X, Y, the coordinates of the point where the
!    function is to be evaluated.
!
!    Input, integer FLAG, indicates whether derivatives 
!    are desired.
!    0, only a function value is required;
!    1, both the function and its first partial derivatives are required.
!
!    Output, real ( kind = rk ) F, the value of the function at (X,Y).
!
!    Output, real ( kind = rk ) FX, FY, the first partial derivatives of the
...
!    function at (X,Y) if 1 <= FLAG.
!
!    Output, real ( kind = rk ) FXX, FXY, FYY, the second partial derivatives
