...
!
  ...
  real ( kind = rk ) rc(2*nmax-4)
  real ( kind = rk ) rlat(nmax)
  real ( kind = rk ) rlon(nmax)
  real ( kind = rk ) sc
  ...
  real ( kind = rk ) x(nmax)
  real ( kind = rk ) xc(2*nmax-4)
  real ( kind = rk ) y(nmax)
  real ( kind = rk ) yc(2*nmax-4)
  real ( kind = rk ) z(nmax)
  real ( kind = rk ) zc(2*nmax-4)
...
!
  ...
  trplot_file_name = 'stripack_test_del.eps'
...
!
  ...
  vrplot_file_name = 'stripack_test_vor.eps'
...
!
  ...
  rlon(1) = 0.0D+00
  do k = 2, n
    rlon(k) = real ( k - 2, kind = rk ) * 360.0D+00 / real ( n - 1, kind = rk )
  end do
