...
!*****************************************************************************80
!
!! ZIGGURAT_OPENMP runs the ZIGGURAT code with OpenMP.
!
!  Licensing:
!
...
!
  ...
  integer jsr
  integer jsr_value
  real ( kind = rk ) mega_rate_par
  real ( kind = rk ) mega_rate_seq
  integer r
  integer, parameter :: r_num = 1000
  integer, allocatable, dimension ( : ) :: result_par
  integer, allocatable, dimension ( : ) :: result_seq
  integer s
  integer, parameter :: s_num = 10000
  integer, allocatable, dimension ( : ) :: seed
  integer shr3
  integer thread
  integer thread_num
  real ( kind = rk ) wtime_par
  real ( kind = rk ) wtime_seq
...
!
  ...
  mega_rate_seq = real ( r_num, kind = rk ) * real ( s_num, kind = rk ) &
...
!$omp end parallel
  ...
  mega_rate_par = real ( r_num, kind = rk ) * real ( s_num, kind = rk ) &
...
!
  ...
  real r4_uni
  real r4_value
  real, allocatable, dimension ( : ) :: result_par
  real, allocatable, dimension ( : ) :: result_seq
  integer s
  integer, parameter :: s_num = 10000
  integer, allocatable, dimension ( : ) :: seed
  integer shr3
