...
!*****************************************************************************80
!
!! vandermonde_test() tests vandermonde().
!
!  Licensing:
!
...
!
  ...
  integer, parameter :: n2 = ( n * ( n + 1 ) ) / 2

  real ( kind = rk ) a(n2,n2)
  real ( kind = rk ), dimension ( n ) :: alpha = (/ &
    1.0, 2.0, 3.0 /)
  real ( kind = rk ), dimension ( n ) :: beta = (/ &
    10.0, 20.0, 30.0 /)
...
!
  ...
  real ( kind = rk ), dimension ( n ) :: alpha1 = (/ &
    0.0, 1.0, 2.0, 3.0, 4.0 /)
  real ( kind = rk ) b(n)
  integer test
  real ( kind = rk ) x(n)
