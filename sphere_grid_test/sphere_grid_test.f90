...
!
  ...
  call sphere_icos_point_num ( factor, node_num )
...
!
  ...
  integer, parameter :: lat_num = 3
  integer, parameter :: long_num = 4

  real ( kind = rk ), dimension ( 3 ) :: pc = (/ &
    0.0D+00, 0.0D+00, 0.0D+00 /)
  integer i
  integer j
  integer k
  integer node_num
  real ( kind = rk ) node_xyz(3,2+lat_num*long_num)
  real ( kind = rk ) :: r = 10.0D+00
...
!
  ...
  integer, parameter :: node_num = 500

  character ( len = 80 ) filename
  real ( kind = rk ), dimension ( 3 ) :: center_xyz = (/ &
    0.0D+00, 0.0D+00, 0.0D+00 /)
...
!
  ...
  integer, parameter :: line_max = 1000
...
!
  ...
  integer, parameter :: rectangle_num = lat_num * long_num
...
!
  ...
  character ( len = 80 ) filename
  integer node_num
  real ( kind = rk ), allocatable :: node_xyz(:,: )
  ...
  call sphere_unit_sample ( node_num, node_xyz )
...
!
  ...
  real ( kind = rk ) x_max
  real ( kind = rk ) x_min
  real ( kind = rk ) xyz_range(3)
  real ( kind = rk ) xyz_max(3)
  real ( kind = rk ) xyz_min(3)
  real ( kind = rk ) y_max
  real ( kind = rk ) y_min
  real ( kind = rk ) z_max
  real ( kind = rk ) z_min
