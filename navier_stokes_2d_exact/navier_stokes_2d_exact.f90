...
!  Input:
!
!    real ( kind = rk ) NU, the kinematic viscosity.
!
!    real ( kind = rk ) RHO, the fluid density.
!
!    integer( kind = 4 ) N, the number of nodes.
!
!    real ( kind = rk ) X(N), Y(N), the coordinates of nodes.
!
!    real ( kind = rk ) T, the current time.
!
!  Output:
!
!    real ( kind = rk ) U(N), DUDT(N), DUDX(N), DUDXX(N), DUDY(N), DUDYY(N), US(N),
!    the horizontal velocity values.
!
!    real ( kind = rk ) V(N), DVDT(N), DVDX(N), DVDXX(N), DVDY(N), DVDYY(N), VS(N),
!    the vertical velocity values.
!
!    real ( kind = rk ) P(N), DPDT(N), DPDX(N), DPDXX(N), DPDY(N), DPDYY(N), PS(N),
!    the pressure values.
!
  implicit none
  ...
  real ( kind = rk ) dudt(n)
  real ( kind = rk ) dudx(n)
  real ( kind = rk ) dudxx(n)
  real ( kind = rk ) dudy(n)
  real ( kind = rk ) dudyy(n)
  real ( kind = rk ) us(n)
  real ( kind = rk ) v(n)
  real ( kind = rk ) dvdt(n)
  real ( kind = rk ) dvdx(n)
  real ( kind = rk ) dvdxx(n)
  real ( kind = rk ) dvdy(n)
  real ( kind = rk ) dvdyy(n)
  real ( kind = rk ) vs(n)
  real ( kind = rk ) p(n)
  real ( kind = rk ) dpdt(n)
  real ( kind = rk ) dpdx(n)
  real ( kind = rk ) dpdxx(n)
  real ( kind = rk ) dpdy(n)
  real ( kind = rk ) dpdyy(n)
  real ( kind = rk ) ps(n)
...
!
!    real ( kind = rk ) X_LO, X_HI, the range of X values.
!
!    integer Y_NUM, the number of Y values to use.
!
!    real ( kind = rk ) Y_LO, Y_HI, the range of Y values.
!
!  Output:
!
!    real ( kind = rk ) X(X_NUM,Y_NUM), Y(X_NUM,Y_NUM), 
...
!
!    real ( kind = rk ) X(N), Y(N), the evaluation points.
!
!    real ( kind = rk ) U(N), V(N), P(N), the velocity and pressure.
!
!    real ( kind = rk ) S, a scale factor for the velocity vectors.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) UR(N), VR(N), PR(N), the residuals.
!
  implicit none
...
!    real ( kind = rk ) NU, the kinematic viscosity.
!
!    real ( kind = rk ) RHO, the density.
!
!    integer N, the number of evaluation points.
!
!    real ( kind = rk ) X(N), Y(N), the coordinates of the points.
!
!    real ( kind = rk ) T, the time coordinate or coordinates.
!
!  Output:
!
!    real ( kind = rk ) UR(N), VR(N), PR(N), the residuals in the U, 
!    V and P equations.
!
  implicit none
...
!    real ( kind = rk ) T, the current time.
!
!    real ( kind = rk ) U(N), V(N), the X and Y velocity.
!
!    real ( kind = rk ) P(N), the pressure.
!
!  Output:
!
!    real ( kind = rk ) UR(N), the U-equation residual.
!
!    real ( kind = rk ) VR(N), the V-equation residual.
!
!    real ( kind = rk ) PR(N), the P-equation residual.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) US(N), VS(N), PS(N), the source terms.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) F(N), G(N), H(N), the right hand sides.
!
  implicit none
...
!  Output:
!
!    real ( kind = rk ) F(N), G(N), H(N), the right hand sides of 
...
!  Output:
!
!    real ( kind = rk ) U(N), V(N), P(N), the velocity components and
