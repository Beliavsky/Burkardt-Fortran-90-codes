...
!
!    Input, real ( kind = rk ) A, B, the interval.
!
!    Input, integer N, the number of values in X, and the number
!    of columns in the matrix.
!
!    Input, real ( kind = rk ) X(N), the vector that defines A.
!
...
!
!    Input, real ( kind = rk ) X(N), the vector that defines the matrix.
!
!    Output, real ( kind = rk ) V(M,N), the matrix.
!
  implicit none
  ...
  do j = 1, n
    ...
    do i = 1, m
      ...
      if ( i == 1 ) then
        ...
        v(i,j) = ( real ( 2 * i - 1, kind = rk ) * xi * v(i-1,j) + &
                   real (   - i + 1, kind = rk ) *      v(i-2,j) ) &
                 / real (     i, kind = rk )
      end if
...
!
!    Output, real ( kind = rk ) XF(NF), the coordinates of the Fekete points.
!
!    Output, real ( kind = rk ) WF(NF), the weights of the Fekete points.
!
  implicit none
...
!    Input, integer M, the number of basis polynomials.
!
!    Output, real ( kind = rk ) MOM(M), the moments.
!
  implicit none
  ...
  do i = 1, m
    mom(i) = ( b - a ) * real ( mod ( i, 2 ), kind = rk ) / real ( i, kind = rk )
  end do
