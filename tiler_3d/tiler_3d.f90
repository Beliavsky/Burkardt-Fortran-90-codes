...
!
  ...
  real ( kind = rk8 ) u0
  real ( kind = rk8 ) u1
  real ( kind = rk8 ), parameter :: umax =   30.0D+00
  real ( kind = rk8 ), parameter :: umin =  150.0D+00
  real ( kind = rk8 ) v0
  real ( kind = rk8 ) v1
  real ( kind = rk8 ), parameter :: vmax =    5.0D+00
  real ( kind = rk8 ), parameter :: vmin =    1.0D+00
  real ( kind = rk8 ) w0
  real ( kind = rk8 ) w1
  real ( kind = rk8 ), parameter :: wmax = 30.0D+00
  real ( kind = rk8 ), parameter :: wmin = -30.0D+00
...
!
  do i = 1, ni
    ...
    u0 = ( real ( ni - i + 1, kind = rk8 ) * umin    &
         + real (      i - 1, kind = rk8 ) * umax )  &
         / real ( ni,         kind = rk8 )

    u1 = ( real ( ni - i, kind = rk8 ) * umin   &
         + real (      i, kind = rk8 ) * umax ) &
         / real ( ni,     kind = rk8 )
!
...
!
    do j = 1, nj
      ...
      v0 = ( real ( nj - j + 1, kind = rk8 ) * vmin   &
           + real (      j - 1, kind = rk8 ) * vmax ) &
           / real ( nj,         kind = rk8 )

      v1 = ( real ( nj - j, kind = rk8 ) * vmin   &
           + real (      j, kind = rk8 ) * vmax ) &
           / real ( nj,     kind = rk8 )
!
...
!
      do k = 1, nk
        ...
        w0 = ( real ( nk - k + 1, kind = rk8 ) * wmin   &
             + real (      k - 1, kind = rk8 ) * wmax ) &
             / real ( nk,         kind = rk8 )

        w1 = ( real ( nk - k, kind = rk8 ) * wmin   &
             + real (      k, kind = rk8 ) * wmax ) &
             / real ( nk,     kind = rk8 )
!
...
!
  ...
  write ( *, '(a)' ) '  Graphics saved as "tiler_3d.tri"'
!
!  Terminate.
!
  write ( *, '(a)' ) ' '
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) UMIN, VMIN, WMIN, UMAX, VMAX, WMAX, the (U,V,W) 
!    coordinates of the two opposite corners of the big box.
!
!    Input, real ( kind = rk8 ) U, V, W, the (U,V,W) coordinates of a point in 
!    the big box.
!
!    Output, real ( kind = rk8 ) X, Y, Z, the (X,Y,Z) coordinates of the point.
!
  implicit none

  integer, parameter :: rk8 = kind ( 1.0D+00 )

  real ( kind = rk8 ), parameter :: deg2rad = 3.14159265D+00 / 180.0D+00
  real ( kind = rk8 ) psi
  real ( kind = rk8 ) theta
  real ( kind = rk8 ) u
  real ( kind = rk8 ) umax
...
!    coordinates of the two opposite corners of the big box.
!
!    Input, real ( kind = rk8 ) U0, V0, W0, U1, V1, W1, the (U,V,W) coordinates 
...
!
  ...
  integer i
  real ( kind = rk8 ) r
  real ( kind = rk8 ) r_tab(npoint)
  real ( kind = rk8 ), parameter :: r0 = 0.0D+00
  real ( kind = rk8 ), parameter :: r1 = 1.0D+00
  real ( kind = rk8 ) s
  real ( kind = rk8 ) s_tab(npoint)
  real ( kind = rk8 ), parameter :: s0 = 0.0D+00
  real ( kind = rk8 ), parameter :: s1 = 1.0D+00
  real ( kind = rk8 ) t
  real ( kind = rk8 ) t_tab(npoint)
  real ( kind = rk8 ), parameter :: t0 = 0.0D+00
  real ( kind = rk8 ), parameter :: t1 = 1.0D+00
  real ( kind = rk8 ) u
  real ( kind = rk8 ) u0
  real ( kind = rk8 ) u1
  real ( kind = rk8 ) umax
