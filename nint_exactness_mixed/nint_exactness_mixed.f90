...
!*****************************************************************************80
!
!! nint_exactness_mixed() tests exactness of multidimensional mixed rule quadrature.
!
!  Licensing:
!
...
!
  ...
  real ( kind = rk8 ), allocatable, dimension ( : ) :: alpha
  integer arg_num
  real ( kind = rk8 ), allocatable, dimension ( : ) :: beta
  integer degree
  integer degree_max
  integer dim
  integer dim_num
  ...
  integer t
  real ( kind = rk8 ), allocatable, dimension ( : ) :: weight
  real ( kind = rk8 ), allocatable, dimension ( :, : ) :: x
  real ( kind = rk8 ), allocatable, dimension ( :, : ) :: x_range
...
!    0 <= EXPON.
!
!    Input, real ( kind = rk8 ) ALPHA, the exponent of |x| in the weight.
!    -1.0 < ALPHA.
!
!    Output, real ( kind = rk8 ) MONOMIAL_INTEGRAL_GENERALIZED_HERMITE, 
...
!
  ...
  real ( kind = rk8 ) arg
  integer expon
  real ( kind = rk8 ) monomial_integral_generalized_hermite
...
!    -1.0 < ALPHA.
!
!    Output, real ( kind = rk8 ) MONOMIAL_INTEGRAL_GENERALIZED_LAGUERRE, 
!    the value of the integral.
!
  implicit none
  ...
  real ( kind = rk8 ) arg
  integer expon
  real ( kind = rk8 ) monomial_integral_generalized_laguerre
  real ( kind = rk8 ) r8_gamma
  real ( kind = rk8 ) value

  arg = alpha + real ( expon + 1, kind = rk8 )
...
!    0 <= EXPON.
!
!    Output, real ( kind = rk8 ) MONOMIAL_INTEGRAL_HERMITE, 
!    the value of the integral.
!
  implicit none
  ...
  integer expon
  real ( kind = rk8 ) monomial_integral_hermite
...
!    Input, integer EXPON, the exponent.
!
!    Input, real ( kind = rk8 ) ALPHA, the exponent of (1-X) in the weight factor.
!
!    Input, real ( kind = rk8 ) BETA, the exponent of (1+X) in the weight factor.
!
!    Output, real ( kind = rk8 ) MONOMIAL_INTEGRAL_JACOBI, 
!    the value of the integral.
!
  implicit none
  ...
  real ( kind = rk8 ) c
  integer expon
  real ( kind = rk8 ) monomial_integral_jacobi
  real ( kind = rk8 ) r8_gamma
  real ( kind = rk8 ) s
  real ( kind = rk8 ) value
  real ( kind = rk8 ) value1
  real ( kind = rk8 ) value2

  c = real ( expon, kind = rk8 )
...
!    0 <= EXPON.
!
!    Output, real ( kind = rk8 ) MONOMIAL_INTEGRAL_LAGUERRE, 
!    the value of the integral.
!
  implicit none
  ...
  integer expon
  real ( kind = rk8 ) monomial_integral_laguerre
...
!    0 <= EXPON.
!
!    Output, real ( kind = rk8 ) MONOMIAL_INTEGRAL_LEGENDRE, 
!    the value of the integral.
!
  implicit none
  ...
  integer expon
  real ( kind = rk8 ) monomial_integral_legendre
  real ( kind = rk8 ) value

  if ( mod ( expon, 2 ) == 1 ) then
    value = 0.0D+00
  else if ( mod ( expon, 2 ) == 0 ) then
    value = 2.0D+00 / real ( expon + 1, kind = rk8 )
  end if
...
!
  ...
  real ( kind = rk8 ) alpha(dim_num)
  real ( kind = rk8 ) beta(dim_num)
  integer dim
  ...
  real ( kind = rk8 ) monomial_integral_legendre
  real ( kind = rk8 ) monomial_integral_laguerre
  real ( kind = rk8 ) monomial_integral_mixed
...
!    Input, integer EXPON(DIM_NUM), the exponents.
!
!    Input, real ( kind = rk8 ) WEIGHT(POINT_NUM), the quadrature weights.
!
!    Input, real ( kind = rk8 ) X(DIM_NUM,POINT_NUM), the quadrature points.
!
!    Output, real ( kind = rk8 ) QUAD_ERROR, the quadrature error.
!
  implicit none
  ...
  real ( kind = rk8 ) quad_error
  integer rule(dim_num)
  real ( kind = rk8 ) value(point_num)
  real ( kind = rk8 ) weight(point_num)
...
!    Input, integer EXPON(DIM_NUM), the exponents.
!
!    Input, real ( kind = rk8 ) X(DIM_NUM,POINT_NUM), the point coordinates.
!
!    Output, real ( kind = rk8 ) VALUE(POINT_NUM), the value of the monomial.
!
  implicit none
...
!
!    Output, real ( kind = rk8 ) R8_FACTORIAL, the factorial of N.
!
  implicit none
  ...
  do i = 1, n
    r8_factorial = r8_factorial * real ( i, kind = rk8 )
  end do
...
!
!    Output, real ( kind = rk8 ) R8_FACTORIAL2, the value of the 
...
!
!    Output, real ( kind = rk8 ) R8_GAMMA, the value of the function.
!
  implicit none
...
!
  real ( kind = rk8 ), dimension ( 7 ) :: c = (/ &
   ...
   -2.777777777777681622553D-03, &
    ...
    5.7083835261D-03 /)
  real ( kind = rk8 ), parameter :: eps = 2.22D-16
  real ( kind = rk8 ) fact
  ...
  logical parity
  real ( kind = rk8 ), parameter :: pi = 3.1415926535897932384626434D+00
  ...
  real ( kind = rk8 ) sum
  real ( kind = rk8 ), parameter :: twelve = 12.0D+00
  real ( kind = rk8 ), parameter :: two = 2.0D+00
  real ( kind = rk8 ) x
  real ( kind = rk8 ), parameter :: xbig = 171.624D+00
  real ( kind = rk8 ) xden
  real ( kind = rk8 ), parameter :: xinf = 1.0D+30
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) A, B, C, X, the arguments of the function.
!    C must not be equal to a nonpositive integer.
!    X < 1.
!
!    Output, real ( kind = rk8 ) HF, the value of the function.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) XX, the argument of the function.
!
!    Output, real ( kind = rk8 ) R8_PSI, the value of the function.
!
  implicit none
  ...
  real ( kind = rk8 ), parameter :: fourth = 0.25D+00
  real ( kind = rk8 ), parameter :: half = 0.5D+00
  integer i
  integer n
  integer nq
  real ( kind = rk8 ), parameter :: one = 1.0D+00
  real ( kind = rk8 ), dimension ( 9 ) :: p1 = (/ &
  ...
  real ( kind = rk8 ), parameter :: piov4 = 0.78539816339744830962D+00
  ...
  real ( kind = rk8 ) w
  real ( kind = rk8 ) x
  real ( kind = rk8 ), parameter :: x01 = 187.0D+00
  real ( kind = rk8 ), parameter :: x01d = 128.0D+00
  real ( kind = rk8 ), parameter :: x02 = 6.9464496836234126266D-04
  real ( kind = rk8 ), parameter :: xinf = 1.70D+38
  real ( kind = rk8 ), parameter :: xlarge = 2.04D+15
  real ( kind = rk8 ), parameter :: xmax1 = 3.60D+16
  real ( kind = rk8 ), parameter :: xmin1 = 5.89D-39
  real ( kind = rk8 ), parameter :: xsmall = 2.05D-09
  real ( kind = rk8 ) xx
  real ( kind = rk8 ) z
  real ( kind = rk8 ), parameter :: zero = 0.0D+00
...
!
      ...
      w = w - real ( int ( w ), kind = rk8 )
      nq = int ( w * four )
      w = four * ( w - real ( nq, kind = rk8 ) * fourth )
!
...
!
!    Output, real ( kind = rk8 ) TABLE(M,N), the table data.
!
  implicit none
