...
!
!    Output, real ( kind = rk ) X(DATA_NUM), the abscissa data.
!
!    Output, real ( kind = rk ) Y(DATA_NUM), the ordinate data.
!
  implicit none
  ...
  integer prob
  real ( kind = rk ) x(data_num)
  real ( kind = rk ) y(data_num)
...
!  Parameters:
!
!    Input, integer DATA_NUM, the dimension of the data vector.
!
...
!  Parameters:
!
!    Output, integer PROB_NUM, the number of test problems.
!
  implicit none
...
!
  ...
  real ( kind = rk ), parameter :: radius = 0.01D+00
  real ( kind = rk ) x(data_num)
  real ( kind = rk ) y(data_num)
...
!
  ...
  do i = 1, num_int
    ...
    do j = 1, i
      n = n + 1
      x(n) = real ( i - 1, kind = rk ) + 0.5D+00 * real ( j, kind = rk ) &
        / real ( i, kind = rk )
      y(n) = real ( j, kind = rk ) / real ( i, kind = rk )
    end do

    do j = 1, i
      n = n + 1
      x(n) = real ( i - 1, kind = rk ) + 0.5D+00 &
        + 0.5D+00 * real ( j, kind = rk ) / real ( i, kind = rk )
      y(n) = 1.0D+00 - real ( j, kind = rk ) / real ( i, kind = rk )
    end do
