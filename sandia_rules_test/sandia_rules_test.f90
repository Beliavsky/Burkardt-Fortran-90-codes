...
!
  ...
  real ( kind = rk ) error1
  real ( kind = rk ) error2
  real ( kind = rk ) estimate1
  real ( kind = rk ) estimate2
  real ( kind = rk ) exact
  real ( kind = rk ), allocatable, dimension ( : ) :: f
  integer n
  integer order
  integer :: order_max = 10
  real ( kind = rk ), allocatable, dimension ( : ) :: w1
  real ( kind = rk ), allocatable, dimension ( : ) :: w2
  real ( kind = rk ), allocatable, dimension ( : ) :: x1
  real ( kind = rk ), allocatable, dimension ( : ) :: x2
...
!
  ...
  real ( kind = rk ), allocatable :: r3(:)
  real ( kind = rk ) r8_epsilon
  integer seed
  real ( kind = rk ) tol
  integer undx_value
  integer, allocatable :: undx1(:)
  integer, allocatable :: undx2(:)
  integer, allocatable :: undx3(:)
  integer unique_num1
  integer unique_num2
  integer unique_num3
  ...
  integer, allocatable :: xdnu1(:)
  integer, allocatable :: xdnu2(:)
  integer, allocatable :: xdnu3(:)
...
!
  ...
  real ( kind = rk ) :: a(m,n) = reshape ( (/ &
    ...
    1.0,  5.0,  9.1, &
    1.0,  5.1,  9.0 /), (/ m, n /) )
  real ( kind = rk ), allocatable :: au(:,:)
  integer i
  integer j
  integer n_unique
  real ( kind = rk ) tol
  integer, allocatable :: undx(:)
  integer, allocatable :: xdnu(:)
...
!
  ...
  real ( kind = rk ), allocatable :: ydp(:)
  real ( kind = rk ), allocatable :: yp(:)
  real ( kind = rk ), allocatable :: ys(:)
  real ( kind = rk ), allocatable :: ysp(:)
  real ( kind = rk ) yt
...
!
  ...
  do n = 5, 21, 4
    ...
    do e = 0, 15
      ...
      exact = ( b**(e+1) - a**(e+1) ) / real ( e + 1, kind = rk )
...
!
  ...
  integer o
  integer, dimension ( 8 ) :: order = (/ 1, 3, 9, 19, 35, 37, 41, 43 /)
  real ( kind = rk ), parameter :: sqrtpi = 1.7724538509055159D+00
  integer rule;
  real ( kind = rk ) s
...
!
!    Output, real ( kind = rk ) CUBIC_ANTIDERIVATIVE, the value.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) cubic_antiderivative
  real ( kind = rk ) x
...
!  Parameters:
!
!    Input, real ( kind = rk ) A, B, the integration interval.
!
!    Output, real ( kind = rk ) Q, the integral from A to B.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the argument.
!
!    Output, real ( kind = rk ) F, D, S, T, the value and first three
