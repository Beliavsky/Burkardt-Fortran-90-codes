...
!
!    Input, integer B, the base of the sequence.  The standard 
!    sequence uses B = 2, and this function expects 2 <= B.
!
!    Output, real ( kind = rk ) VDC_BASE, the I-th element of the van der 
!    Corput sequence.
!
  implicit none
...
!
  base_inv = 1.0D+00 / real ( b, kind = rk )
...
!  Parameters:
!
!    Input, real ( kind = rk ) R, the I-th element of the van der Corput 
!    sequence.  |R| < 1.0
!
!    Output, integer VDC_INVERSE, the index of the element of 
!    the sequence.
!
  implicit none
...
!    I = 0 is allowed, and returns R = 0.
!
!    Output, real ( kind = rk ) VDC, the I-th element of the van der Corput 
!    sequence.
!
  implicit none
...
!  Parameters:
!
!    Input, integer I1, I2, the indices of the first and
!    last elements.
!
!    Output, real ( kind = rk ) R(|I2+1-I1|), elements I1 through I2 of 
...
!
    ...
    do while ( t /= 0 )
      d = mod ( t, 2 )
      r(j) = r(j) + real ( d, kind = rk ) * base_inv
      base_inv = base_inv / 2.0D+00
      t = ( t / 2 )
    end do
!
