...
!
!    Input, integer ENERGY_IT, the number of energy iterations.
!
...
!
  ...
  real ( kind = rk ) energy_it
  real ( kind = rk ) energy_max
  real ( kind = rk ) energy_min
  integer energy_it_max
  integer i
  integer j
  real ( kind = rk ) norm
...
!
  ...
  do cluster_num = cluster_lo, cluster_hi
    t = energy(cluster_num) / real ( point_num, kind = rk )
...
!    Input, real ( kind = rk ) POINT(DIM_NUM,POINT_NUM), the data points.
!
!    Input, real ( kind = rk ) R_MIN(DIM_NUM), R_MAX(DIM_NUM), the
!    minimum and maximum corners of the region.
!
!    Input/output, integer SEED, a seed for the random 
!    number generator.
!
!    Output, real ( kind = rk ) CENTER(DIM_NUM,CLUSTER_NUM), the centers
!    associated with the minimal energy clustering.
!
!    Output, real ( kind = rk ) ENERGY, the total energy associated with the
...
!
  ...
  real ( kind = rk ), dimension ( dim_num ) :: r_min
  integer seed
  integer, dimension ( cluster_num ) :: subs
!
...
!    Input, real ( kind = rk ) POINT(DIM_NUM,POINT_NUM), the data points.
!
!    Input, real ( kind = rk ) CENTER(DIM_NUM,CLUSTER_NUM), the center points.
!
...
!    Input, character ( len = * ) STRING, the prompt string.
!
!    Output, integer VALUE1, VALUE2, the values entered by 
!    the user.
!
!    Output, integer IERROR, an error flag, which is zero 
!    if no error occurred.
...
!
!    Output, real ( kind = rk ) POINT(DIM_NUM,POINT_NUM), the points.
!
  implicit none
