...
!*****************************************************************************80
!
!! pce_burgers() performs a polynomial chaos expansion on the Burgers equation.
!
!  Discussion:
!
...
!  Local parameters:
!
!    Local, real ( kind = rk ) DT, the timestep.
!
!    Local, real ( kind = rk ) DX, the spacing between grid points.
!
!    Local, integer N, the number of intervals in the spatial
!    domain.
!
!    Local, real ( kind = rk ) NUMEAN, the mean of viscosity.
!
!    Local, real ( kind = rk ) NUVARIANCE, the variance of viscosity.
!
!    Local, integer P, the order of the PC expansion.
!
!    Local, real ( kind = rk ) T, the current time.
!
!    Local, real ( kind = rk ) TF, the final integration time.
!
!    Local, real ( kind = rk ) U1(N+1,P+1), the PCE representation at the 
!    current time.
!
!    Local, real ( kind = rk ) U2(N+1,P+1), the PCE representation for the
!    next time.
!
!    Local, real ( kind = rk ) X(N+1,1), the grid points.
!
  implicit none
  ...
  real ( kind = rk ), allocatable :: umean(:)
  real ( kind = rk ), allocatable :: uvariance(:)
  real ( kind = rk ) :: visc(2)
...
!
  do i = 1, n + 1
    x(i) = ( real ( n - i + 1, kind = rk ) * ( -3.0D+00 )   &
...
!
!    Output, real ( kind = rk ) HE_TRIPLE_PRODUCT_INTEGRAL, the value of 
!    the integral.
!
  implicit none
