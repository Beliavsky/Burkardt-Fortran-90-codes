...
!
  ...
  logical goodx
  integer i, nfc
  real ( kind = rk ) emax, emaxs, gts, rfac1, xmax
  real ( kind = rk ) half, one, onep2, two
...
!
  ...
  real ( kind = rk ) dig(n), nwtstp(n), step(n), v(lv)
  external dotprd, v2norm
  real ( kind = rk ) dotprd, v2norm
  real ( kind = rk ) cfact, cnorm, ctrnwt, ghinvg, femnsq, gnorm
  real ( kind = rk ) nwtnrm, relax, rlambd, t, t1, t2
  real ( kind = rk ) half, two
...
!
  ...
  integer i
  real ( kind = rk ) rmdcon
  real ( kind = rk ), save :: sqteta = 0.0D+00
  real ( kind = rk ) t
  real ( kind = rk ) x(p)
  real ( kind = rk ) y(p)
...
!
  ...
  integer ka, p
  real ( kind = rk ) d(p), dig(p), dihdi(*), l(*), v(21), step(p), w(*)
...
!
  ...
  real ( kind = rk ) alphak, aki, akk, delta, dst, eps, gtsta, lk
  real ( kind = rk ) oldphi
  real ( kind = rk ) phi, phimax, phimin, psifac, rad, radsq
  real ( kind = rk ) root, si, sk, sw, t, twopsi, t1, t2, uk, wi
  real ( kind = rk ) big, dgxfac, epsfac, four, half, kappa, negone
  real ( kind = rk ) one, p001, six, three, two, zero
  real ( kind = rk ) dotprd, lsvmin, rmdcon, v2norm
...
!
  ...
  integer iv(liv)
  real ( kind = rk ) d(n), fx, g(n), h(lh), v(lv), x(n)
  integer dg1, i, j, k, l, lstgst, nn1o2, step1
  integer temp1, w1, x01
  real ( kind = rk ) t
  real ( kind = rk ) one, onep2
  logical stopx
  real ( kind = rk ) dotprd, reldst, v2norm
...
!
  ...
  integer iv(liv)
  integer uiparm(*)
  real ( kind = rk ) d(n), x(n), v(lv), urparm(*)
...
!
  ...
  real ( kind = rk ) nreldf, oldf, preldf, reldf
...
!
  ...
  real ( kind = rk ) lsvmin
  real ( kind = rk ) l(p*(p+1)/2)
  real ( kind = rk ) x(p)
  real ( kind = rk ) y(p)

  integer i, ii, ix, j, ji, jj, jjj, jm1, j0, pm1
  real ( kind = rk ) b, sminus, splus, t, xminus, xplus
  real ( kind = rk ) half, one, r9973
  real ( kind = rk ) dotprd, v2norm
...
!
  ...
  integer n
  real ( kind = rk ) beta(n), gamma(n), l(*), lambda(n), lplus(*), w(n), z(n)
!     dimension l(n*(n+1)/2), lplus(n*(n+1)/2)
!
  integer i, ij, j, jj, jp1, k, nm1
  integer np1
  real ( kind = rk ) a, b, bj, eta, gj, lj, lij, ljj, nu, s, theta, wj, zj
  real ( kind = rk ) one
...
!
  ...
  real ( kind = rk ) big, machep, tiny, vk, vm(34), vx(34)
...
!
  ...
  real ( kind = rk ) reldst
  real ( kind = rk ) d(p), x(p), x0(p)
  integer i
  real ( kind = rk ) emax, t, xmax
...
!
  ...
  integer irc, n
  real ( kind = rk ) alpha(n), d(n), eta0, fx, g(n), w(6), x(n)
  external rmdcon
  real ( kind = rk ) rmdcon
  integer fh, fx0, hsave, i, xisave
  real ( kind = rk ) aai, afx, afxeta, agi, alphai, axi, axibar
  real ( kind = rk ) discon, eta, gi, h, hmin
  real ( kind = rk ) c2000, four, hmax0, hmin0, h0, machep, one, p002
  real ( kind = rk ) three, two
...
!
  ...
  real ( kind = rk ) t
  real ( kind = rk ) half, negone, one, onep2, zero
  logical stopx
  real ( kind = rk ) dotprd, reldst, v2norm
...
!
  ...
  real ( kind = rk ) v(lv)
  real ( kind = rk ) rmdcon
  real ( kind = rk ) machep, mepcrt, one, sqteps, three
...
!
  ...
  real ( kind = rk ) cs
  real ( kind = rk ) cy
  real ( kind = rk ), parameter :: eps = 0.1D+00
  real ( kind = rk ) epsrt
  real ( kind = rk ) l(n*(n+1)/2)
  real ( kind = rk ) s(n)
  real ( kind = rk ) shs
  real ( kind = rk ) theta
  real ( kind = rk ) w(n)
