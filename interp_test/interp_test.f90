...
!
  ...
  real ( kind = rk ), allocatable, dimension ( :, : ) :: p_interp
  real ( kind = rk ), allocatable, dimension ( : ) :: p_value
  real ( kind = rk ) t_data(data_num)
  real ( kind = rk ), allocatable, dimension ( : ) :: t_interp
  real ( kind = rk ) t_max
  real ( kind = rk ) t_min
...
!  Parameters:
!
!    Input, integer DATA_NUM, the number of data values.
!
  implicit none
