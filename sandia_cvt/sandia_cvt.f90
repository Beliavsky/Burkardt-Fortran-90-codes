...
!
!    Input, real ( kind = rk ) BOX_MIN(NDIM), BOX_MAX(NDIM), the coordinates
!    of the two extreme corners of the bounding box.
!
!    Input, integer N, the number of Voronoi cells.
!
!    Input, real ( kind = rk ) CELL_GENERATOR(NDIM,N), the Voronoi
!    cell generators.
!
!    Input, integer NBIN(3) is the number of bins to use in 
...
!
!    Output, integer BIN_START(NBIN(1),NBIN(2),NBIN(3)), the index 
!    of the first cell generator in the bin, or -1 if none.
!
!    Output, integer BIN_LAST(NBIN(1),NBIN(2),NBIN(3)), the index 
!    of the last cell generator in the bin, or -1 if none.
!
!    Output, integer BIN_NEXT(N), the index of the next cell 
...
!
  ...
  real ( kind = rk ), dimension ( ndim ) :: box_min
  real ( kind = rk ), dimension ( ndim ) :: box_max
  real ( kind = rk ), dimension ( ndim, n ) :: cell_generator
  integer empty
...
!    the user deserves.
!
!    Input, real ( kind = rk ) A, B, the lower and upper limits of the bin
!    interval.  While A is expected to be less than B, the code should
!    return useful results if A is actually greater than B.
!
!    Output, real ( kind = rk ) CMIN, CMAX, the minimum and maximum limits
!    on the bin.
!
  implicit none
...
!
  if ( bin < 1 ) then
    ...
    cmin = ( real ( nbin - bin + 1, kind = rk ) * a &
           + real (        bin - 1, kind = rk ) * b ) &
           / real ( nbin,           kind = rk )
    cmax = ( real ( nbin - bin, kind = rk ) * a &
           + real (        bin, kind = rk ) * b ) &
           / real ( nbin,       kind = rk )
...
!    considered.
!
!    Input, real ( kind = rk ) A(NDIM), B(NDIM), the lower and upper limits
...
!
!    Output, real ( kind = rk ) CMIN(NDIM), CMAX(NDIM), the minimum and maximum
!    limits on the bin.
!
  implicit none
  ...
  real ( kind = rk ) b(ndim)
  integer bin(ndim)
  real ( kind = rk ) cmax(ndim)
  real ( kind = rk ) cmin(ndim)
  integer i
  integer nbin(ndim)
...
subroutine cvt_iteration ( ndim, box_min, box_max, n, cell_generator, ns_cvt, &
  use_bins, dr, updates, nbin, bin_start, bin_last, bin_next )
...
!    Input, integer N, the number of Voronoi cells.
!
!    Input/output, real ( kind = rk ) CELL_GENERATOR(NDIM,N), the Voronoi
!    cell generators.  On output, these have been modified
!
!    Input, integer NS_CVT, the number of sample points 
!    per generator.
!
...
!
!    Input, real ( kind = rk ) DR, a tolerance used by DIATOM when testing
!    whether a point is within, outside of, or on the boundary of the
!    physical region.
!
!    Input/output, integer UPDATES(N), counts the number of times 
...
!
!    Input, integer NBIN(3) is the number of bins to use in each 
!    direction.  For 3D problems, set NBIN(3) = 1.
!    For efficiency, these values should be set in such a way that the bins
!    are nearly square or cubical.
!
!    Input, integer BIN_START(NBIN(1),NBIN(2),NBIN(3)), the index
!    of the first cell generator in the bin, or -1 if none.
!
!    Input, integer BIN_LAST(NBIN(1),NBIN(2),NBIN(3)), the index 
!    of the last cell generator in the bin, or -1 if none.
!
!    Input, integer BIN_NEXT(N), the index of the next cell
!    generator in the bin containing this cell generator.
!
  implicit none
  ...
  integer, dimension ( n ) :: bin_next
  integer, dimension ( nbin(1),nbin(2),nbin(3) ) :: bin_start
  real ( kind = rk ) box_max(1:ndim)
  real ( kind = rk ) box_min(1:ndim)
  real ( kind = rk ) cell_generator(ndim,n)
  real ( kind = rk ) cell_generator2(ndim,n)
  integer count(n)
...
!    Input, integer NDIM, the spatial dimension.
!
!    Input, real ( kind = rk ) X(NDIM), the point to be checked.
!
!    Input, integer N, the number of cell generators.
!
!    Input, real ( kind = rk ) CELL_GENERATOR(NDIM,N), the cell generators.
!
...
!    Input, integer N, the number of Voronoi cells.
!
!    Output, real ( kind = rk ) CELL_GENERATOR(NDIM,N), the Voronoi cell
!    generators.
!
!    Input, real ( kind = rk ) DR, a tolerance used by DIATOM when testing
!    whether a point is within, outside of, or on the boundary of the
...
!
  ...
  real ( kind = rk ) box_max(ndim)
  real ( kind = rk ) box_min(ndim)
  real ( kind = rk ) cell_generator(ndim,n)
  real ( kind = rk ) dr
  integer i
  integer ngen
...
!
!    Output, real ( kind = rk ) R(NDIM), the SEED-th element of the Halton
...
!
  ...
  base_inv(1:ndim) = 1.0D+00 / real ( base(1:ndim), kind = rk )

  do while ( any ( seed2(1:ndim) /= 0 ) )
    digit(1:ndim) = mod ( seed2(1:ndim), base(1:ndim) )
    r(1:ndim) = r(1:ndim) + real ( digit(1:ndim), kind = rk ) * base_inv(1:ndim)
    base_inv(1:ndim) = base_inv(1:ndim) / real ( base(1:ndim), kind = rk )
...
!
!    Input/output, integer I, J, K.  On input, the previous index
...
!
!    Input, real ( kind = rk ) PSET(2,NSET), the coordinates of the points
!    in the set.
!
!    Input, integer NBIN(2), the number of cells in the horizontal 
!    and vertical directions.
!
!    Input, real ( kind = rk ) BIN_MIN(2), BIN_MAX(2), the minimum and maximum
!    bin values.
!
...
!
!    Input, real ( kind = rk ) PTEST(2), the coordinates of the test point.
!
...
!
  ...
  real ( kind = rk ) bin_max(ndim)
  real ( kind = rk ) bin_min(ndim)
  integer bin_start(nbin(1),nbin(2))
  integer bin_next(nset)
  real ( kind = rk ) c_max(ndim)
  real ( kind = rk ) c_min(ndim)
  real ( kind = rk ) d_min
  real ( kind = rk ) d_min_sq
  real ( kind = rk ) d_sq
  integer i
  integer i_min
  integer ic
  ...
  integer jc
  integer layer
  real ( kind = rk ) layer_width
  logical more_bins
  integer node
  real ( kind = rk ) pset(ndim,nset)
  real ( kind = rk ) ptest(ndim)
  real ( kind = rk ) search_radius
!
!  Special cases.
!
  if ( nset <= 0 ) then
...
!
!    ( bin_max(1:ndim) - bin_min(1:ndim) ) / real ( nbin(1:ndim), kind = rk )
!
...
!    Input, integer NSET, the number of points in the set.
!
!    Input, real ( kind = rk ) PSET(3,NSET), the coordinates of the points
!    in the set.
!
!    Input, integer NBIN(3), the number of cells in the X, Y and 
!    Z directions.
!
!    Input, real ( kind = rk ) BIN_MIN(3), BIN_MAX(3), the minimum and
!    maximum bin values.
!
...
!    of the bin containing this element.
!
!    Input, real ( kind = rk ) PTEST(3), the coordinates of the test points.
!
...
!    Input, integer N, the number of cell generators.
!
!    Input, real ( kind = rk ) CELL_MOMENT(NDIM,NDIM,N), the second moment
!    matrix for each Voronoi cell.
!
!    Input, real ( kind = rk ) CELL_VOLUME(N), the Voronoi cell volumes.
!
!    Input, real ( kind = rk ) REGION_VOLUME, the volume of the region,
...
!
  ...
  real ( kind = rk ) cell_det(n)
  real ( kind = rk ) cell_moment(ndim,ndim,n)
  real ( kind = rk ) cell_trace(n)
  real ( kind = rk ) cell_volume(n)
  real ( kind = rk ) dd_l1
  real ( kind = rk ) dd_l2
  real ( kind = rk ) dd_linf
  real ( kind = rk ) r8mat_det_2d
  real ( kind = rk ) r8mat_det_3d
  real ( kind = rk ) ev
  real ( kind = rk ) ev_l1
  real ( kind = rk ) ev_l2
  real ( kind = rk ) ev_linf
  integer i
  integer j
  real ( kind = rk ) matrix2(2,2)
  real ( kind = rk ) matrix3(3,3)
  real ( kind = rk ) region_volume
  real ( kind = rk ) tr
  real ( kind = rk ) tr_l1
  real ( kind = rk ) tr_l2
  real ( kind = rk ) tr_linf
!
!  Measure 1: the deviation of the cell volumes from the expected cell volume.
!
  ev = region_volume / real ( n, kind = rk )
...
!
  ...
  tr = sum ( cell_trace(1:n) ) / real ( n, kind = rk )
...
!
  if ( ndim == 2 ) then
    ...
    do i = 1, n
      ...
      do j = 1, ndim
        matrix2(j,j) = matrix2(j,j) - cell_trace(i) / real ( ndim, kind = rk )
      end do
    ...
    do i = 1, n
      ...
      do j = 1, ndim
        matrix3(j,j) = matrix3(j,j) - cell_trace(i) / real ( ndim, kind = rk )
      end do
...
!    return useful results if A is actually greater than B.
!
!    Input, real ( kind = rk ) C, a value to be placed in a bin.
!
...
!
  if ( c <= a2 ) then
    ...
    bin = 1 + int ( real ( nbin, kind = rk ) * ( c - a2 ) / ( b2 - a2 ) )
    bin = max ( bin, 1 )
    bin = min ( bin, nbin )
  end if
!
...
!
!    Input, real ( kind = rk ) A(2,N), the D2 data to be binned.
!
!    Input, integer NBIN(2), the number of bins in each dimension.
!    NBIN must be at least 1.
!
!    Input, real ( kind = rk ) BIN_MIN(2), BIN_MAX(2), the bin limits.
!
...
!
!    Output, integer BIN_NEXT(N), contains the index of the next 
...
!
!    Input/output, real ( kind = rk ) A(2,N), the D2 data to be sorted.
!
...
!    in the bin.
!
!    Input/output, integer BIN_NEXT(N), contains the index of 
...
!    Input, integer N, the number of data points.
!
!    Input/output, real ( kind = rk ) A(2,N), the R2 data to be sorted.
!
!    Input, integer NBIN(2), the number of bins in each dimension.
!
!    Input, integer BIN_START(NBIN(1),NBIN(2)), 
...
!    Input, integer N, the number of points in the data set.
!
!    Input, real ( kind = rk ) A(3,N), the R3 data to be binned.
!
!    Input, integer NBIN(3), the number of bins in each dimension.
!    NBIN must be at least 1.
!
!    Input, real ( kind = rk ) BIN_MIN(3), BIN_MAX(3), the bin limits.
!
...
!    Input, integer N, the number of data points.
!
!    Input/output, real ( kind = rk ) A(3,N), the data to be sorted.
!
!    Input, integer NBIN(3), the number of bins in each dimension.
!
...
!    or -1 if there are no entries in the bin.
!
!    Input/output, integer BIN_NEXT(N), contains the index of the 
...
!  Parameters:
!
!    Input, integer M, the row dimension of A, and the length 
!    of a column.
!
!    Input, integer N, the column dimension of A.
!
!    Input/output, real ( kind = rk ) A(M,N).  On input, the array to be checked.
...
!  Parameters:
!
!    Input, integer M, the row order of A, and the length 
!    of a column.
!
...
!
!    Output, real ( kind = rk ) RMAT_DET_2D, the determinant of the matrix.
!
  implicit none
...
!
!    Output, real ( kind = rk ) RMAT_DET_3D, the determinant of the matrix.
!
  implicit none
...
!    each dimension.
!
!    Input, real ( kind = rk ) A(NDIM), B(NDIM), the lower and upper limits of
...
!
!    Input, real ( kind = rk ) C(NDIM), a value to be placed in a bin.
!
...
!    physical region.
!
!    Output, real ( kind = rk ) X(NDIM), the random point.
!
...
!
!    Output, integer NGEN, the number of points that were 
...
!    Input, real ( kind = rk ) CELL_GENERATOR(NDIM,N), the cell generators.
!
!    Input, integer NS_MOM, the number of sample points per 
!    cell generator.
!
!    Input, logical USE_BINS, is TRUE if the bounding box is to be divided
!    up into bins to speed up the nearest neighbor search;
...
!
!    Input/output, real ( kind = rk ) REGION_VOLUME, the volume of the region.
...
!    generator in the bin containing this cell generator.
!
!    Output, real ( kind = rk ) CELL_VOLUME(N), the Voronoi cell volumes.
!
!    Output, real ( kind = rk ) CELL_CENTROID(NDIM,N), the Voronoi cell
!    centroids.
!
!    Output, real ( kind = rk ) CELL_MOMENT(NDIM,NDIM,N), the second moment
!    matrix for each Voronoi cell.
!
  implicit none
  ...
  real ( kind = rk ) box_max(ndim)
  real ( kind = rk ) box_min(ndim)
  real ( kind = rk ) box_volume
  real ( kind = rk ) cell_centroid(ndim,n)
  real ( kind = rk ) cell_generator(ndim,n)
  integer cell_hit(n)
  real ( kind = rk ) cell_moment(ndim,ndim,n)
  real ( kind = rk ) cell_volume(n)
  ...
  real ( kind = rk ) region_volume
  real ( kind = rk ) region_volume_estimate
...
!
  ...
  region_volume_estimate = real ( n * ns_mom, kind = rk ) * box_volume &
    / real ( ntries, kind = rk )
...
!
  do k = 1, n
    ...
    if ( 0 < cell_hit(k) ) then
      ...
      cell_volume(k) = real ( cell_hit(k), kind = rk ) * region_volume &
        / real ( n * ns_mom, kind = rk )
      cell_centroid(1:ndim,k) = cell_centroid(1:ndim,k) &
        / real ( cell_hit(k), kind = rk )
