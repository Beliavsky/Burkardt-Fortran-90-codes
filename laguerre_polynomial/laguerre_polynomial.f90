...
!
  ...
  integer ( kind = 4 ) j
  integer ( kind = 4 ) k
  real ( kind = rk ), allocatable :: l_table(:)
  ...
  do k = 1, order
    ...
    x = x_table(k)
    call l_polynomial ( 1, p, x_table(k), l_table )
...
!
!    Output, real ( kind = rk ) V(M,0:N), the function values.
!
  implicit none
  ...
  do j = 2, n
    ...
    v(1:m,j) = ( ( real ( 2 * j - 1, kind = rk ) - x(1:m) ) * v(1:m,j-1)   &
                 - real (     j - 1, kind = rk )            * v(1:m,j-2) ) &
                 / real (     j,     kind = rk )
...
!
  ...
  do i = 2, n
    cx(1:m,i) = ( &
      ( real ( 2 * i - 1, kind = rk ) + alpha - x(1:m) ) * cx(1:m,i-1)   &
    + ( real (   - i + 1, kind = rk ) - alpha          ) * cx(1:m,i-2) ) &
      / real (     i,     kind = rk )
  end do
...
!
!    Output, real ( kind = rk ) A, the parameter.
!
...
!
  ...
  cx(1:mm,1) = real ( m + 1, kind = rk ) - x(1:mm)

  do i = 2, n
    cx(1:mm,i) = &
      ( ( real (   m + 2 * i - 1, kind = rk ) - x(1:mm) ) * cx(1:mm,i-1)   &
        + real ( - m     - i + 1, kind = rk )             * cx(1:mm,i-2) ) &
        / real (           i,     kind = rk )
  end do
...
!
  ...
  c(1,0) = real ( m + 1, kind = rk )
  c(1,1) = -1.0D+00
 
  do i = 2, n

    c(i,0:i) = ( real (   m + 2 * i - 1, kind = rk ) * c(i-1,0:i)   &
               + real ( - m     - i + 1, kind = rk ) * c(i-2,0:i) ) &
               / real (           i,     kind = rk )

    c(i,1:i) = c(i,1:i) - c(i-1,0:i-1) / real ( i, kind = rk )
...
!
  do i = 1, n
    bj(i) = real ( i * ( i + m ), kind = rk )
  end do
  bj(1:n) = sqrt ( bj(1:n) )

  do i = 1, n
    x(i) = real ( 2 * i - 1 + m, kind = rk )
  end do
