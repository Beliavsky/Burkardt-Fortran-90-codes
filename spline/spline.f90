...
!  Input:
!
!    real ( kind = rk ) TDATA(5), the nodes associated with the
!    basis function.  The entries of TDATA are assumed to be distinct
!    and increasing.
!
!    real ( kind = rk ) TVAL, a point at which the B spline basis
!    function is to be evaluated.
!
!  Output:
!
!    real ( kind = rk ) YVAL, the value of the function at TVAL.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) BETA1, the skew or bias parameter.
!    BETA1 = 1 for no skew or bias.
!
!    Input, real ( kind = rk ) BETA2, the tension parameter.
!    BETA2 = 0 for no tension.
!
!    Input, real ( kind = rk ) TDATA(5), the nodes associated with the
!    basis function.  The entries of TDATA are assumed to be distinct
!    and increasing.
!
!    Input, real ( kind = rk ) TVAL, a point at which the B spline
!    basis function is to be evaluated.
!
!    Output, real ( kind = rk ) YVAL, the value of the function at TVAL.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) MBASIS(4,4), the basis matrix.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) mbasis(4,4)
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) ALPHA.
!    ALPHA = || P2 - P1 || / ( || P3 - P2 || + || P2 - P1 || )
!
!    Output, real ( kind = rk ) MBASIS(3,3), the basis matrix.
!
  implicit none
...
!
!    Input, real ( kind = rk ) MBASIS(N,N), the basis matrix.
!
!    Input, integer NDATA, the dimension of the vectors TDATA
!    and YDATA.
!
!    Input, real ( kind = rk ) TDATA(NDATA), the abscissa values.  This routine
!    assumes that the TDATA values are uniformly spaced, with an
!    increment of 1.0.
!
!    Input, real ( kind = rk ) YDATA(NDATA), the data values to be
!    interpolated or approximated.
!
!    Input, real ( kind = rk ) TVAL, the value of T at which the spline is to be
!    evaluated.
!
!    Output, real ( kind = rk ) YVAL, the value of the spline at TVAL.
!
  implicit none
  ...
  integer j
  integer left
  real ( kind = rk ) mbasis(n,n)
  real ( kind = rk ) tdata(ndata)
  real ( kind = rk ) tval
  real ( kind = rk ) tvec(maxn)
  real ( kind = rk ) ydata(ndata)
  real ( kind = rk ) yval
...
!
!    Input, real ( kind = rk ) T, the point at which the Bezier curve should
!    be evaluated.  The best results are obtained within the interval
!    [0,1] but T may be anywhere.
!
!    Input, real ( kind = rk ) XCON(0:N), YCON(0:N), the X and Y coordinates
...
!
!    Output, real ( kind = rk ) XVAL, YVAL, the X and Y coordinates of the point
...
!
  ...
  real ( kind = rk ) bval(0:n)
  real ( kind = rk ) t
  real ( kind = rk ) xcon(0:n)
  real ( kind = rk ) xval
  real ( kind = rk ) ycon(0:n)
  real ( kind = rk ) yval
...
!    N must be at least 0.
!
!    Input, real ( kind = rk ) X, the point at which the Bezier function should
!    be evaluated.  The best results are obtained within the interval
!    [A,B] but X may be anywhere.
!
!    Input, real ( kind = rk ) A, B, the interval over which the Bezier function
...
!
!    Input, real ( kind = rk ) Y(0:N), a set of data defining the Y coordinates
!    of the control points.
!
!    Output, real ( kind = rk ) BEZ_VAL, the value of the Bezier function at X.
!
  implicit none
...
!
!    Output, real ( kind = rk ) BERN(0:N), the values of the N+1 Bernstein basis
!    polynomials at X.
!
  implicit none
...
!    Input, real ( kind = rk ) X, the point at which the polynomials are to be
...
!
!    Input, real ( kind = rk ) YDATA(0:N), the data values at N+1 equally
!    spaced points in [A,B].  If N = 0, then the evaluation point should
!    be 0.5 * ( A + B).  Otherwise, evaluation point I should be
!    ( (N-I)*A + I*B ) / N ).
!
!    Input, real ( kind = rk ) XVAL, the point at which the Bernstein polynomial
!    approximant is to be evaluated.  XVAL does not have to lie in the
!    interval [A,B].
!
!    Output, real ( kind = rk ) YVAL, the value of the Bernstein polynomial
...
!
!    Input, real ( kind = rk ) XVAL, the value where the polynomial
!    is to be evaluated.
!
...
!
!    Input, real ( kind = rk ) XTAB(NTAB), the X data.  The values in XTAB
!    should be distinct, and in increasing order.
!
!    Input, real ( kind = rk ) YTAB(NTAB), the Y data values corresponding
...
!    equal to NTAB-1.
!
!    Output, real ( kind = rk ) PTAB(NTAB), the value of the least
!    squares polynomial at the points XTAB(1:NTAB).
!
!    Output, real ( kind = rk ) B(1:NDEG), C(0:NDEG), D(2:NDEG), arrays
!    needed to evaluate the polynomial.
!
!    Output, real ( kind = rk ) EPS, the root-mean-square discrepancy of the
!    polynomial fit.
!
...
!
  ...
  real ( kind = rk ) b(1:ndeg)
  real ( kind = rk ) c(0:ndeg)
  real ( kind = rk ) d(2:ndeg)
  real ( kind = rk ) eps
  integer i
  integer i0l1
  integer i1l1
...
!
  ...
  rn0 = ntab
  c(0) = y_sum / real ( ntab, kind = rk )

  ptab(1:ntab) = y_sum / real ( ntab, kind = rk )

  if ( ndeg == 0 ) then
    eps = sum ( ( ptab(1:ntab) - ytab(1:ntab) )**2 )
    eps = sqrt ( eps / real ( ntab, kind = rk ) )
    return
  end if
!
!  The polynomial is of degree at least 1.
!
  b(1) = sum ( xtab(1:ntab) ) / real ( ntab, kind = rk )
...
!    polynomial.
!
!    Input, real ( kind = rk ) B(1:NDEG), C(0:NDEG), D(2:NDEG), arrays
!    defined by LEAST_SET_OLD, and needed to evaluate the polynomial.
!
!    Output, real ( kind = rk ) VALUE, the value of the polynomial at X.
!
  implicit none
...
!
!    Input, real ( kind = rk ) X(POINT_NUM), the abscissas of the data points.
!    At least NTERMS of the values in X must be distinct.
!
!    Input, real ( kind = rk ) F(POINT_NUM), the data values at the points X(*).
!
!    Input, real ( kind = rk ) W(POINT_NUM), the weights associated with
...
!
!    Output, real ( kind = rk ) B(NTERMS), C(NTERMS), D(NTERMS), are quantities
...
!
  ...
  real ( kind = rk ) b(nterms)
  real ( kind = rk ) c(nterms)
  real ( kind = rk ) d(nterms)
  real ( kind = rk ) f(point_num)
  integer i
  integer j
  real ( kind = rk ) p
  real ( kind = rk ) pj(point_num)
  real ( kind = rk ) pjm1(point_num)
  real ( kind = rk ) s(nterms)
  real ( kind = rk ), parameter :: tol = 0.0D+00
  integer unique_num
  real ( kind = rk ) w(point_num)
  real ( kind = rk ) x(point_num)
!
...
!
!    Input, real ( kind = rk ) B(NTERMS), C(NTERMS), D(NTERMS), the information
!    computed by LEAST_SET.
!
!    Input, real ( kind = rk ) X, the point at which the least squares polynomial
!    is to be evaluated.
!
!    Output, real ( kind = rk ) PX, the value of the least squares
!    polynomial at X.
!
  implicit none
...
!    is to be evaluated.
!
!    Output, real ( kind = rk ) PX, PXP, the value and derivative of the least
...
!
!    Input, real ( kind = rk ) TDATA(NDATA), the abscissas of the data
...
!
!    Input, real ( kind = rk ) A1(N), A2(N), A3(N), A4(N), A5(N), the nonzero
...
!    Input, integer N, the order of the linear system.
!
!    Input/output, real ( kind = rk ) A(5,N),
...
!
!    Input, real ( kind = rk ) T(N), an array sorted into ascending order.
!
...
!
!    Input, real ( kind = rk ) A(N), the array to be checked.
!
...
!    Input, integer N, the number of elements of A.
!
!    Input, real ( kind = rk ) A(N), the unsorted array to examine.
!
...
!    Input, real ( kind = rk ) TDATA(NDATA), the abscissas of the data.
!
!    Input, real ( kind = rk ) YDATA(NDATA), the data values.
!
!    Input, real ( kind = rk ) TVAL, a point at which the spline is
!    to be evaluated.
!
!    Output, real ( kind = rk ) YVAL, the value of the function at TVAL.
!
...
!
!    Input, real ( kind = rk ) DATA_VAL(DIM_NUM,3*INTERVAL_NUM+1), the control
!    values.
!
!    Input, integer POINT_NUM, the number of sample points at
!    which the Bezier cubic spline is to be evaluated.
!
!    Input, real ( kind = rk ) POINT_T(POINT_NUM), the "T" values associated
...
!
!    Output, real ( kind = rk ) POINT_VAL(DIM_NUM,POINT_NUM), the value
...
!
  ...
  real ( kind = rk ) bernstein_val(0:cubic)
  real ( kind = rk ) data_val(dim_num,cubic*interval_num+1)
  integer dim
  integer interval
  integer offset
  integer point
  real ( kind = rk ) point_t(point_num)
  real ( kind = rk ) point_val(dim_num,point_num)
  real ( kind = rk ) t
  real ( kind = rk ) t_01
  ...
  do point = 1, point_num
    ...
    t_01 = t - real ( interval - 1, kind = rk )
...
!
!    Input, real ( kind = rk ) TDATA(NDATA-1), the breakpoints.  The values
!    of TDATA should be distinct and increasing.
!
!    Input, real ( kind = rk ) YDATA(NDATA), the values of the spline in
!    the intervals defined by the breakpoints.
!
!    Input, real ( kind = rk ) TVAL, the point at which the spline is
!    to be evaluated.
!
!    Output, real ( kind = rk ) YVAL, the value of the spline at TVAL.
!
...
!    at least 2.
!
!    Input, real ( kind = rk ) T(N), the points where data is specified.
!    The values should be distinct, and increasing.
!
!    Input, real ( kind = rk ) Y(N), the data values to be interpolated.
!
...
!
!    Input, real ( kind = rk ) YBCBEG, the left boundary value, if needed.
!
...
!
!    Input, real ( kind = rk ) YBCEND, the right boundary value, if needed.
!
!    Output, real ( kind = rk ) YPP(N), the second derivatives of
!    the cubic spline.
!
  implicit none
...
!  Input:
!
!    integer N, the number of data points; N must be
!    at least 2.
!
!    real ( kind = rk ) T(N), the points where data is specified.
!    The values should be distinct, and increasing.
!
!    real ( kind = rk ) Y(N), the data values to be interpolated.
!
!    integer IBCBEG, the left boundary condition flag:
!    0: the spline should be a quadratic over the first interval;
!    1: the first derivative at the left endpoint should be YBCBEG;
!    2: the second derivative at the left endpoint should be YBCBEG;
!    3: Not-a-knot: the third derivative is continuous at T(2).
!
!    real ( kind = rk ) YBCBEG, the left boundary value, if needed.
!
!    integer IBCEND, the right boundary condition flag:
!    0: the spline should be a quadratic over the last interval;
!    1: the first derivative at the right endpoint should be YBCEND;
!    2: the second derivative at the right endpoint should be YBCEND;
!    3: Not-a-knot: the third derivative is continuous at T(N-1).
!
!    real ( kind = rk ) YBCEND, the right boundary value, if needed.
!
!  Output:
!
!    real ( kind = rk ) YPP(N), the second derivatives of
!    the cubic spline.
!
  implicit none
...
!
!    real ( kind = rk ) T(N), the knot values.
!
!    real ( kind = rk ) Y(N), the data values at the knots.
!
!    real ( kind = rk ) YPP(N), the second derivatives of the
!    spline at the knots.
!
!    real ( kind = rk ) TVAL, a point, typically between T(1) and
!    T(N), at which the spline is to be evalulated.  If TVAL lies outside
!    this range, extrapolation is used.
!
!  Output:
!
!    real ( kind = rk ) YVAL, YPVAL, YPPVAL, the value of the spline, and
...
!
!    Input, real ( kind = rk ) T(N), the knot values.
!
!    Input, real ( kind = rk ) Y(N), the data values at the knots.
!
!    Input, real ( kind = rk ) YPP(N), the second derivatives of the spline at
!    the knots.
!
...
!
!    Input, real ( kind = rk ) TVAL, a point, typically between T(1) and T(N), at
!    which the spline is to be evalulated.  If TVAL lies outside
!    this range, extrapolation is used.
!
!    Output, real ( kind = rk ) YVAL, YPVAL, YPPVAL, the value of the spline, and
!    its first two derivatives at TVAL.
!
  implicit none
...
!
!    Input, real ( kind = rk ) Y(NDATA), YP(NDATA), the value of the
!    function and its derivative at TDATA(1:NDATA).
!
!    Output, real ( kind = rk ) C(4,NDATA), the coefficients of the
!    Hermite polynomial.
...
!
  ...
  real ( kind = rk ) c(4,ndata)
  real ( kind = rk ) divdif1
  real ( kind = rk ) divdif3
  real ( kind = rk ) dt
  integer i
  real ( kind = rk ) tdata(ndata)
  real ( kind = rk ) ydata(ndata)
  real ( kind = rk ) ypdata(ndata)
!
!  Check NDATA.
!
  if ( ndata < 2 ) then
...
!    The entries of TDATA are assumed to be strictly increasing.
!
!    Input, real ( kind = rk ) C(4,NDATA), the coefficient data computed by
!    SPLINE_HERMITE_SET.
!
!    Input, real ( kind = rk ) TVAL, the point where the interpolant is to
!    be evaluated.
!
!    Output, real ( kind = rk ) SVAL, SPVAL, the value of the interpolant
...
!
!    Input, real ( kind = rk ) TDATA(NDATA), YDATA(NDATA), the values of
!    the independent and dependent variables at the data points.  The
!    values of TDATA should be distinct and increasing.
!
!    Input, real ( kind = rk ) A, B, the interval over which the
!    integral is desired.
!
!    Output, real ( kind = rk ) INT_VAL, the value of the integral.
!
  implicit none
...
!
!    Input, real ( kind = rk ) INT_X(N+1), the points that define the intervals.
!    Interval I lies between INT_X(I) and INT_X(I+1).
!
!    Input, real ( kind = rk ) INT_V(N), the desired value of the integral of the
...
!
  ...
  real ( kind = rk ) a(3,n)
  real ( kind = rk ) data_x(n)
  real ( kind = rk ) data_y(n)
  real ( kind = rk ) int_v(n)
  real ( kind = rk ) int_x(n+1)
!
...
!    to be evaluated.
!
!    Output, real ( kind = rk ) YVAL, YPVAL, the value of the spline and
...
!    Input, real ( kind = rk ) YDATA(NDATA), the data values.
!
!    Input, real ( kind = rk ) TVAL, the value where the spline is to
!    be evaluated.
!
!    Output, real ( kind = rk ) YVAL, the value of the spline at TVAL.
!
  ...
  integer left
  real ( kind = rk ) mbasis(4,4)
  real ( kind = rk ) mbasis_l(3,3)
  real ( kind = rk ) mbasis_r(3,3)
  integer right
  real ( kind = rk ) tdata(ndata)
  real ( kind = rk ) tval
  real ( kind = rk ) ydata(ndata)
...
!    corresponding to the abscissas.
!
!    Input, real ( kind = rk ) TVAL, the abscissa value at which the spline
!    is to be evaluated.  Normally, TDATA(1) <= TVAL <= T(NDATA), and
!    the data will be interpolated.  For TVAL outside this range,
!    extrapolation will be used.
!
!    Output, real ( kind = rk ) YVAL(DIM_NUM), the value of the spline at TVAL.
!
  implicit none
  ...
  real ( kind = rk ) tval
  real ( kind = rk ) ydata(dim_num,ndata)
  real ( kind = rk ) yl(dim_num)
  real ( kind = rk ) yr(dim_num)
  real ( kind = rk ) yval(dim_num)
!
!  Check.
!
...
!    variable values.
!
!    Input, real ( kind = rk ) F(N), dependent variable values to be
...
!    direction.
!
!    Output, real ( kind = rk ) D(N), the derivative values at the
...
!    variable values.
!
!    Input, real ( kind = rk ) F(N), the function values.
!
!    Input, real ( kind = rk ) D(N), the derivative values.
!
!    Input, integer NE, the number of evaluation points.
!
...
!    values of TDATA should be distinct and increasing.
!
!    Input, real ( kind = rk ) TVAL, the point at which the spline is to
!    be evaluated.
!
!    Output, real ( kind = rk ) YVAL, YPVAL, the value of the spline and
!    its first derivative dYdT at TVAL.  YPVAL is not reliable if TVAL
