...
!*****************************************************************************80
!
!! ising_3d_simulation() simulations the Ising model in 3D.
!
!  Licensing:
!
...
!
  ...
  integer last
  integer n
  real ( kind = rk ) , dimension ( 0:6 ) :: prob
...
! prob = (/ &
!    0.97D+00, 0.95D+00, 0.85D+00, 0.50D+00, 0.15D+00, 0.05D+00, 0.03D+00 /)
  prob = (/ &
    0.99D+00, 0.99D+00, 0.99D+00, 0.80D+00, 0.30D+00, 0.20D+00, 0.10D+00 /)
...
!  Parameters:
!
!    Input, integer N, the number of cells in each dimension.
!
...
!  Parameters:
!
!    Input, integer N, the number of cells in each spatial 
!    dimension.
!
...
!    to carry out.
!
!    Input, real ( kind = rk ) PROB(0:6).  PROB(I) represents the probability 
!    that the spin of a given cell will be reversed, given that it has I
!    immediate neighbors with spin the same as its own.
!
  implicit none
  ...
  integer ones(n,n,n)
  integer ones_count
  real ( kind = rk ) prob(0:6)
  real ( kind = rk ) r(n,n,n)
  real ( kind = rk ) threshhold(n,n,n)
  ...
  do i = 1, iterations
!
!  COUNT contains the number of immediate neighbors of (I,J,K) 
!  with a value of 1.
!
    count = cshift ( ones, -1,  1 ) &
          ...
          + cshift ( ones, +1,  3 )
!
!  Now COUNT contains the number of immediate neighbors of (I,J,K) that are 
!  the same as the (I,J,K) value.
!
