...
!
!    Input, integer FACE_NODE(3,FACE_NUM), the nodes that 
!    make up each face.
!
  implicit none
...
!
!    Output, real ( kind = rk ) FACE_NORMAL(3,FACE_NUM), the normal vector
!    at each face.
!
  implicit none
  ...
  integer face
  integer face_node(3,face_num)
  real ( kind = rk ) face_normal(3,face_num)
  integer n1
  integer n2
  integer n3
...
!    Input, integer FACE_NUM, the number of faces.
!
!    Input, real ( kind = rk ) FACE_NORMAL(3,FACE_NUM), the normal vector
!    at each face.
!
  implicit none
...
!
!    Output, real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the coordinates of points.
!
!    Output, integer FACE_NODE(3,FACE_NUM), the nodes that
!    make up each face.
!
!    Output, real ( kind = rk ) FACE_NORMAL(3,FACE_NUM), the normal vector
!    at each face.
