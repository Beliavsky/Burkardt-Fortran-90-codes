...
!  Parameters:
!
!    Input, real ( kind = rk8 ) BOX(2,2), the coordinates of the lower left
...
!
  ...
  real ( kind = rk8 ) h_goal
  real ( kind = rk8 ) h
  integer nodes_per_layer
  integer nodes_per_layer2
...
!
  if ( 2 < nodes_per_layer ) then
    ...
    h = 1.0D+00 / real ( nodes_per_layer2 - 1, kind = rk8 )
  ...
  h = 1.0D+00 / real ( nodes_per_layer - 1, kind = rk8 )
...
!
  ...
  integer n_low
  integer nodes_per_layer
  integer nodes_per_layer_high
  integer nodes_per_layer_low
  ...
  nodes_per_layer = nint ( sqrt ( real ( n_goal, kind = rk8 ) ) )
...
!  Parameters:
!
!    Input, integer NODES_PER_LAYER, the number of grid points
!    on the first horizontal layer of points.
!
!    Output, real ( kind = rk8 ) HX, the spacing between grid points
!    on a horizontal line.
!
!    Output, real ( kind = rk8 ) HY, the spacing between horizontal lines.
!
  implicit none
  ...
  if ( nodes_per_layer < 1 ) then
    ...
    hx = 1.0D+00 / real ( nodes_per_layer - 1, kind = rk8 )
...
!  Parameters:
!
!    Input, integer NODES_PER_LAYER, the number of grid points on 
...
!
!    Output, real (kind = rk8 ) P(2,N), the coordinates of the
  ...
  do j = 1, layers
    ...
    if ( jmod == 1 ) then
      ...
      do i = 1, nodes_per_layer
        x = real ( i - 1, kind = rk8 ) / real ( nodes_per_layer - 1, kind = rk8 )
        k = k + 1
        if ( k <= n ) then
          p(1,k) = x
          p(2,k) = y
      ...
      do i = 1, nodes_per_layer-1
        x = real ( 2 * i - 1, kind = rk8 ) &
          / real ( 2 * nodes_per_layer - 2, kind = rk8 )
        k = k + 1
        if ( k <= n ) then
          p(1,k) = x
          p(2,k) = y
...
!  Parameters:
!
!    Input, real ( kind = rk8 ) BOX(2,2), the lower and upper corners
...
!
  if ( 2 < nodes_per_layer ) then
    ...
    nodes_per_layer2 = nodes_per_layer - 1
    h = ( box(1,2) - box(1,1) ) / real ( nodes_per_layer2 - 1, kind = rk8 )
  ...
  h = ( box(1,2) - box(1,1) ) / real ( nodes_per_layer - 1, kind = rk8 )
...
!    on the first horizontal layer of points.
!
!    Input, real ( kind = rk8 ) BOX(2,2), the values of A, B, C and D
...
!
  ...
  if ( nodes_per_layer < 1 ) then
    ...
    hx = ( box(1,2) - box(1,1) ) / real ( nodes_per_layer - 1, kind = rk8 )
    hy = hx * sqrt ( 3.0D+00 ) / 2.0D+00
...
!
  ...
  do j = 1, layers
    ...
    y = box(2,1) + hy * real ( j - 1, kind = rk8 )
    ...
    if ( jmod == 1 ) then
      ...
      do i = 1, nodes_per_layer
        x = box(1,1) + ( box(1,2) - box(1,1) ) &
          * real ( i - 1, kind = rk8 ) &
          / real ( nodes_per_layer - 1, kind = rk8 )
        k = k + 1
        if ( k <= n ) then
          p(1,k) = x
          p(2,k) = y
...
!
!    Input, integer NODES_PER_LAYER, the number of nodes in the 
!    first layer.
!
!    Input, integer LAYERS, the number of layers.
!
!    Input, real ( kind = rk8 ) HX, HY, the row and column spacings.
!
!    Input, real ( kind = rk8 ) BOX(2,2), the values of A, B, C and D
!    that define the coordinate box.
!
!    Input, real ( kind = rk8 ) R(M,N), the points.
!
