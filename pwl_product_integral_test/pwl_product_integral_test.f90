...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'pwl_PRODUCT_INTEGRAL_TEST'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Test the pwl_PRODUCT_INTEGRAL library.'
...
!
  ...
  integer, parameter :: f_num = 2
  integer, parameter :: g_num = 2

  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ) exact
  real ( kind = rk ) :: f_v(f_num) = (/ 0.0D+00, 5.0D+00 /)
  real ( kind = rk ) :: f_x(f_num) = (/ 0.0D+00, 5.0D+00 /)
  real ( kind = rk ) :: g_v(g_num) = (/ 0.0D+00, 5.0D+00 /)
  real ( kind = rk ) :: g_x(g_num) = (/ 0.0D+00, 5.0D+00 /)
  integer i
  ...
  do i = 1, 5
    b = real ( i, kind = rk )
    call pwl_product_integral ( a, b, f_num, f_x, f_v, g_num, &
      g_x, g_v, integral )
...
!
  ...
  integer, parameter :: g_num = 4

  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ) exact
  real ( kind = rk ) :: f_v(f_num) = (/ 0.0D+00, 2.0D+00, 5.0D+00 /)
  real ( kind = rk ) :: f_x(f_num) = (/ 0.0D+00, 2.0D+00, 5.0D+00 /)
  real ( kind = rk ) :: g_v(g_num) = (/ 0.0D+00, 1.5D+00, 3.0D+00, 5.0D+00 /)
  real ( kind = rk ) :: g_x(g_num) = (/ 0.0D+00, 1.5D+00, 3.0D+00, 5.0D+00 /)
  integer i
  real ( kind = rk ) integral
...
!
  ...
  integer, parameter :: f_num = 11
  integer, parameter :: g_num = 31
  ...
  do i = 1, f_num
    f_x(i) = ( real ( f_num - i,     kind = rk     ) * 0.0D+00 &
             + real (         i - 1, kind = rk ) * pi ) &
             / real ( f_num     - 1, kind = rk )
    f_v(i) = sin ( f_x(i) )
  end do

  do i = 1, g_num
    g_x(i) = ( real ( g_num - i,     kind = rk     ) * 0.0D+00 &
             + real (         i - 1, kind = rk ) * pi ) &
             / real ( g_num     - 1, kind = rk )
    g_v(i) = 2.0D+00 * cos ( g_x(i) )
  end do

  a = 0.0D+00
  do i = 0, 6
    b = real ( i, kind = rk ) * pi / 6.0D+00
    call pwl_product_integral ( a, b, f_num, f_x, f_v, g_num, &
      g_x, g_v, integral )
    exact = - ( cos ( 2.0D+00 * b ) - cos ( 2.0D+00 * a ) ) / 2.0D+00
    quad_num = 2000
    call pwl_product_quad ( a, b, f_num, f_x, f_v, g_num, &
...
!
  ...
  integer, parameter :: f_num = 3
  integer, parameter :: g_num = 3

  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ) :: f_v(f_num) = (/ 0.0D+00, 1.0D+00, 0.0D+00 /)
  real ( kind = rk ) :: f_x(f_num) = (/ 0.0D+00, 1.0D+00, 2.0D+00 /)
  real ( kind = rk ) :: g_v(g_num) = (/ 1.0D+00, 0.0D+00, 0.0D+00 /)
  real ( kind = rk ) :: g_x(g_num) = (/ 0.0D+00, 1.0D+00, 2.0D+00 /)
  real ( kind = rk ) integral

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST04'
  write ( *, '(a)' ) '  Test pwl_PRODUCT_INTEGRAL.'
  ...
  call pwl_product_integral ( a, b, f_num, f_x, f_v, f_num, &
    f_x, f_v, integral )
  ...
  call pwl_product_integral ( a, b, g_num, g_x, g_v, g_num, &
    g_x, g_v, integral )
