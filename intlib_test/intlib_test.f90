...
!
  ...
  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ), external :: f1d1
  real ( kind = rk ), external :: fbd1
  real ( kind = rk ), external :: fed1
  real ( kind = rk ), external :: fqd1
  real ( kind = rk ), external :: fxd1
  real ( kind = rk ), external :: fx2d1
  real ( kind = rk ), external :: fx3d1
...
!
  ...
  integer, parameter :: ntab = 11

  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ) ftab(ntab)
  real ( kind = rk ), external :: func
  integer i
  real ( kind = rk ) result
...
!
  ...
  real ( kind = rk ) result
  real ( kind = rk ) xtab(ntab)
  real ( kind = rk ) work(ntab)
  real ( kind = rk ) y(3,ntab)
!
!  Note that, for accuracy, it is useful to have two data points
!  outside the interval (A,B).
!
  aleft = a - ( b - a ) / real ( ntab - 3, kind = rk )
  brite = b + ( b - a ) / real ( ntab - 3, kind = rk )
...
!
  ...
  integer, parameter :: nleast = 3
  integer, parameter :: nmost = nleast + 2
...
!
  ...
  integer, parameter :: ntab = 11
  integer, parameter :: nwork = 2 * ( ntab - 1 )
...
!
  ...
  integer, parameter :: ntab = 25

  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ), dimension (ntab) :: ftab = (/ &
    1.01D-03, &
    7.88D-04, &
    5.63D-04, &
    3.38D-04, &
  ...
  real ( kind = rk ), dimension (ntab) :: xtab = (/ &
    9.821221D+00, &
    64.28147D+00, &
    216.5835D+00, &
    730.615D+00, &
